...
CONFIG configuration_list[] = {
  ...
  {"background_compact", "configure background compaction",
    C_BOOL, 50, 0, 0, V_GLOBAL_BACKGROUND_COMPACT},

  {"background_compact.free_space_target", "free space target for background compaction (MB)",
    0x0, 1, 100, UINT_MAX, V_GLOBAL_BACKGROUND_COMPACT_FREE_SPACE_TARGET},

  {"backup", "configure backups",
    C_BOOL, 20, 0, 0, V_GLOBAL_BACKUP},

  {"backup.incremental", "backup type (off | block)",
  ...
  {"backup.live_restore", "configure backup live restore recovery",
    C_BOOL, 25, 0, 0, V_GLOBAL_BACKUP_LIVE_RESTORE},

  {"backup.live_restore_read_size", "live restore read size (KB power of 2)",
    C_POW2, 1, 16384, 16384, V_GLOBAL_BACKUP_LIVE_RESTORE_READ_SIZE},

  {"backup.live_restore_threads", "number of live restore worker threads",
    0x0, 0, 12, 12, V_GLOBAL_BACKUP_LIVE_RESTORE_THREADS},
  ...
  {"btree.reverse", "reverse order collation",
    C_BOOL | C_TABLE | C_TYPE_ROW, 20, 0, 0, V_TABLE_BTREE_REVERSE},
  ...
  {"cache.eviction_dirty_target", "dirty content target for eviction",
    C_IGNORE, 0, 0, 100, V_GLOBAL_CACHE_EVICTION_DIRTY_TARGET},

  {"cache.eviction_dirty_trigger", "dirty content trigger for eviction",
    C_IGNORE, 0, 0, 100, V_GLOBAL_CACHE_EVICTION_DIRTY_TRIGGER},

  {"cache.eviction_updates_target", "update content target for eviction",
    C_IGNORE, 0, 0, 100, V_GLOBAL_CACHE_EVICTION_UPDATES_TARGET},

  {"cache.eviction_updates_trigger", "update content trigger for eviction",
    C_IGNORE, 0, 0, 100, V_GLOBAL_CACHE_EVICTION_UPDATES_TRIGGER},

  {"cache.minimum", "minimum cache size (MB)",
    C_IGNORE, 0, 0, 100 * 1024, V_GLOBAL_CACHE_MINIMUM},

  {"cache.maximum", "maximum cache size (MB)",
    C_IGNORE, 0, 0, UINT_MAX, V_GLOBAL_CACHE_MAXIMUM},
  ...
  {"chunk_cache", "enable chunk cache",
    C_BOOL | C_IGNORE, 0, 0, 0, V_GLOBAL_CHUNK_CACHE},

  {"chunk_cache.capacity", "maximum memory or storage to use for the chunk cache (MB)",
    0x0, 100, 5120, 100 * 1024, V_GLOBAL_CHUNK_CACHE_CAPACITY},

  {"chunk_cache.chunk_size", "size of cached chunks (MB)",
    0x0, 1, 5, 100 * 1024, V_GLOBAL_CHUNK_CACHE_CHUNK_SIZE},

  {"chunk_cache.storage_path", "the on-disk storage path for the chunk cache.",
    C_STRING | C_IGNORE, 0, 0, 0, V_GLOBAL_CHUNK_CACHE_STORAGE_PATH},

  {"chunk_cache.type", "cache location (DRAM | FILE)",
    C_STRING | C_IGNORE, 0, 0, 0, V_GLOBAL_CHUNK_CACHE_TYPE},

  {"compact.free_space_target", "free space target for compaction (MB)",
    0x0, 1, 100, UINT_MAX, V_GLOBAL_COMPACT_FREE_SPACE_TARGET},

  {"debug.background_compact", "background compaction processes files more often",
    C_BOOL, 5, 0, 0, V_GLOBAL_DEBUG_BACKGROUND_COMPACT},
  ...
  {"disagg.enabled", "configure disaggregated storage",
    C_IGNORE | C_BOOL | C_TABLE | C_TYPE_ROW, 0, 0, 0, V_TABLE_DISAGG_ENABLED},

  {"disagg.layered", "use layered URI for any disaggregated tables",
    C_BOOL, 100, 1, 0, V_GLOBAL_DISAGG_LAYERED},
 ...
 */
  ...
  {"obsolete_cleanup.method", "obsolete cleanup strategy",
    C_IGNORE | C_STRING, 0, 0, 0, V_GLOBAL_OBSOLETE_CLEANUP_METHOD},

  {"obsolete_cleanup.wait", "obsolete cleanup interval in seconds",
    0x0, 1, 3600, 100000, V_GLOBAL_OBSOLETE_CLEANUP_WAIT},
  ...
  {"ops.pareto", "configure crud operations to be pareto distributed",
    C_BOOL | C_TABLE, 20, 0, 0, V_TABLE_OPS_PARETO},

  {"ops.pareto.skew", "adjusts the amount of skew used by the pareto distribution",
    C_TABLE, 1, 100, 100, V_TABLE_OPS_PARETO_SKEW},
  ...
  {"ops.throttle", "enable delay between ops",
    C_BOOL, 10, 0, 0, V_GLOBAL_OPS_THROTTLE},

  {"ops.throttle.sleep_us", "average duration of sleep between ops per table, us",
    0x0, 0, M(1), M(60), V_GLOBAL_OPS_THROTTLE_SLEEP_US},
  ...
  {"prefetch", "configure prefetch",
    C_BOOL, 50, 0, 0, V_GLOBAL_PREFETCH},

  {"precise_checkpoint", "Precise checkpoint",
    C_BOOL, 50, 0, 0, V_GLOBAL_PRECISE_CHECKPOINT},

  {"preserve_prepared", "Preserve prepared",
    C_BOOL, 50, 0, 0, V_GLOBAL_PRESERVE_PREPARED},
  ...
  {"runs.source", "data source type (file | layered | table)",
  ...
  {"stress.aggressive_stash_free", "stress freeing stashed memory aggressively",
    C_BOOL, 2, 0, 0, V_GLOBAL_STRESS_AGGRESSIVE_STASH_FREE},
  ...
  {"stress.compact_slow", "stress compact",
    C_BOOL, 2, 0, 0, V_GLOBAL_STRESS_COMPACT_SLOW},

  {"stress.evict_reposition", "stress evict reposition",
    C_BOOL, 2, 0, 0, V_GLOBAL_STRESS_EVICT_REPOSITION},

  {"stress.failpoint_eviction_split", "stress failpoint eviction split",
    C_BOOL, 30, 0, 0, V_GLOBAL_STRESS_FAILPOINT_EVICTION_SPLIT},
  ...
  {"stress.prefetch_delay", "stress prefetch delay",
    C_BOOL, 2, 0, 0, V_GLOBAL_STRESS_PREFETCH_DELAY},

  {"stress.prepare_resolution_1", "stress prepare resolution (#1)",
    C_BOOL, 2, 0, 0, V_GLOBAL_STRESS_PREPARE_RESOLUTION_1},
  ...
  {"tiered_storage.storage_source", "storage source used (azure_store | dir_store | gcp_store | none | off | s3_store)",
  ...
  {"transaction.operation_timeout_ms", "requested limit on the time taken to complete operations in this transaction",
    0, 0, 0, UINT_MAX, V_GLOBAL_TRANSACTION_OPERATION_TIMEOUT_MS},
