...
while test "$run" -le "$runmax"; do
	rm -rf $home
	mkdir $home
	LD_PRELOAD=/home/jenkins/jemalloc_custom/jemalloc-4.2.1/LOCAL_INSTALL/lib/libjemalloc.so LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/lib ./wtperf -O $wttest $wtarg $wtarg2
	if test "$?" -ne "0"; then
		exit 1
	fi
