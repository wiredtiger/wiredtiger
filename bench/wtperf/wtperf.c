...
start_all_runs(WTPERF *wtperf)
{
    ...
    for (i = 0; i < opts->database_count; i++) {
        ...
        if (opts->create != false && strcmp(next_wtperf->home, next_wtperf->monitor_dir) != 0)
...
start_run(WTPERF *wtperf)
{
    ...
    if (opts->create != false && (ret = create_tables(wtperf)) != 0)
        goto err;
    ...
    if (opts->create != false && execute_populate(wtperf) != 0)
        goto err;
    ...
    if (wtperf->workers_cnt != 0 && (opts->run_time != 0 || opts->run_ops != 0)) {
        ...
        if (opts->create == false && opts->random_range == 0 && find_table_count(wtperf) != 0)
            goto err;
