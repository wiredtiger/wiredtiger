...
#
# Read write balanced workload: ~50% reads, ~50% (inserts+updates) + checkpoints,
...
# Updates in snapshot transactions
...
tupdate = Thread(op_update * 1000)
tupdate.options.session_config = "isolation=snapshot"

# Inserts in snapshot transactions
...
tinsert = Thread(op_insert * 1000)
tinsert.options.session_config = "isolation=snapshot"

# ------------------------
# Lagged readers near stable_timestamp (~50% of ops overall)
# ------------------------
...
for lag in lags:
    ...
    # 2000 ops per reader thread: 10 lags * 5 threads * 2000 = 100,000 reads
    tread = Thread(read_txn * 2000)
...
# 600s / 10s = 60 checkpoints over the run
ops = checkpoint_ops * 60
...
print("disagg workload: writers + checkpoint:")
start_time = time.time()
