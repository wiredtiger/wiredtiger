...
conn_config = "cache_size=500MB,log=(enabled=false),statistics=[fast],statistics_log=(json,wait=1)"
conn = context.wiredtiger_open("create," + conn_config)
s = conn.open_session("")

# Table configuration.
table_config = "leaf_page_max=8k,internal_page_max=8k,leaf_key_max=1433,leaf_value_max=1433,type=file,memory_page_max=1KB,split_deepen_min_child=100"
tables = []
table_count = 1
for i in range(0, table_count):
    ...
    table.options.key_size = 50
    table.options.value_size = 20
...
workload = Workload(context, 16 * thread0)
