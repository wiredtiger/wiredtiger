...
def get_make_check_dirs():
    ...
    cmd = f"find . -not -path './releases/*' -not -path './.claude/*' {ignore_build_folders} -name CMakeLists.txt -exec grep -H -e '\\(add_test\\|define_c_test|define_test_variants\\)' {{}} \\; | cut -d: -f1 | cut -c3- | uniq"
