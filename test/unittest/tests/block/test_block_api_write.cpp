  ...
  addr_cookie cookie, wt_off_t offset, uint32_t size)
{
     ...
     */
    REQUIRE(bm->read(
              bm, session->get_wt_session_impl(), &read_buf, NULL, cookie.addr.data(), cookie.size) == 0);
  ...
  const std::vector<addr_cookie> &cookies, const std::vector<std::string> &expected_strings)
{
    for (int i = 0; i < cookies.size(); i++) {
        ...
        WT_ITEM read_buf;
        WT_CLEAR(read_buf);
        REQUIRE(bm->read(bm, session->get_wt_session_impl(), &read_buf, NULL,
                  cookie.addr.data(), cookie.size) == 0);
...
TEST_CASE("Block manager: file operation read, write and write_size functions", "[block_api]")
{
    ...
    SECTION("Test simple cases: writing a single string using the block manager")
    {
        ...
        addr_cookie cookie;
        // Perform a write.
        REQUIRE(bm.write(&bm, session->get_wt_session_impl(), &buf, NULL,
