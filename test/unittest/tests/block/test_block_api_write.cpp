...
void create_write_buffer(WT_BM* bm, std::shared_ptr<mock_session> session, std::string contents, WT_ITEM* buf) {
...
TEST_CASE("Block manager: file operation read, write and write_size functions", "[block_api]")
{
    ...
    {   
        ...
        WT_ITEM read_buf;
        REQUIRE(bm->read(bm, session->get_wt_session_impl(), &read_buf, addr, addr_size) == 0);
        CHECK(memcmp(buf.data, read_buf.data, buf.size) == 0);
        REQUIRE(bm->addr_invalid(bm, session->get_wt_session_impl(), addr, addr_size) == 0);
    }

    SECTION("Test complex write api")
    {   
        // Should the buffer size be 512 at this point? For alignment purposes.
        WT_RAND_STATE rnd;
        __wt_random_init_custom_seed(&rnd, 1);
        WT_ITEM buf;
        create_write_buffer(bm, session, "hello", &buf);
