...
class SessionWrapper {
    public:
        ~SessionWrapper();
        WT_SESSION_IMPL* getWtSessionImpl() { return _sessionImpl; };

        static std::shared_ptr<SessionWrapper> buildTestSessionWrapper();

    private:
        explicit SessionWrapper(WT_SESSION_IMPL* sessionImpl,
                                std::shared_ptr<ConnectionWrapper> connectionWrapper = nullptr);
