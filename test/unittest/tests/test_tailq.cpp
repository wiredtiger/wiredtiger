...
template <class T>
TestTailQWrapper<T>::TestTailQWrapper()
{
...
template <class T>
void TestTailQWrapper<T>::pushBack(const T& value)
{
...
template <class T>
void TestTailQWrapper<T>::pushBack(std::list< T > const& values)
{
    std::for_each(
      values.begin(),
      values.end(),
      [this] (const int n) { this->pushBack(n); }
    );
}


template <class T>
void TestTailQWrapper<T>::removeValue(const T& value)
{
    tailq_entry<T>* item = TAILQ_FIRST(&_tailq);
    bool searching = true;
...
template <class T>
std::list< T > TestTailQWrapper<T>::copyItemsFromTailQ()
{
    std::list<T> items;
