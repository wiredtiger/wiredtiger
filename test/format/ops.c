    ...
    *iso_configp = ISOLATION_SNAPSHOT;
    ...
    if (ts != 0) {
        ...
        if (ret == 0) {
            ...
            logop(tinfo, "begin snapshot read-ts=%" PRIu64 " (repeatable)", ts);
            return;
        }
        if (ret != EINVAL)
            testutil_check(ret);
    ...
    logop(tinfo, "begin snapshot read-ts=%" PRIu64 " (not repeatable)", ts);
}
...
rollback_transaction(TINFO *tinfo)
{
    ...
    logop(tinfo, "abort read-ts=%" PRIu64, tinfo->read_ts);
}
...
prepare_transaction(TINFO *tinfo)
{
    ...
    logop(tinfo, "prepare ts=%" PRIu64, ts);
  ...
  WT_CURSOR *cursor, TINFO *tinfo, uint64_t keyno, WT_ITEM *key, WT_ITEM *value, bool sn)
{
    ...
    if (ret == 0)
        switch (g.type) {
        case FIX:
            if (tinfo == NULL && g.log_all)
                logmsg("%-10s%" PRIu64 " {0x%02x}", "read", keyno, ((char *)value->data)[0]);
            if (tinfo != NULL)
                logop(tinfo, "%-10s%" PRIu64 " {0x%02x}", "read", keyno, ((char *)value->data)[0]);
        ...
        case VAR:
            ...
            if (tinfo != NULL)
                logop(tinfo, "%-10s%" PRIu64 " {%.*s}", "read", keyno, (int)value->size,
                  (char *)value->data);
            break;
        }
...
order_error_row:
    ...
    if (g.log_all && ret == 0)
        switch (g.type) {
        case FIX:
            logop(tinfo, "%-10s%" PRIu64 " {0x%02x}", which, keyno, ((char *)value.data)[0]);
            break;
        case ROW:
            logop(tinfo, "%-10s%" PRIu64 " {%.*s}, {%.*s}", which, keyno, (int)key.size,
...
row_reserve(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    logop(tinfo, "%-10s%" PRIu64 " {%.*s}", "reserve", tinfo->keyno, (int)tinfo->key->size,
      (char *)tinfo->key->data);
...
col_reserve(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    logop(tinfo, "%-10s%" PRIu64, "reserve", tinfo->keyno);
...
row_modify(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    logop(tinfo, "%-10s%" PRIu64 " {%.*s}, {%.*s}", "modify", tinfo->keyno, (int)tinfo->key->size,
...
col_modify(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    logop(tinfo, "%-10s%" PRIu64 ", {%.*s}", "modify", tinfo->keyno, (int)tinfo->value->size,
...
row_truncate(TINFO *tinfo, WT_CURSOR *cursor)
{
    ...
    logop(tinfo, "%-10s%" PRIu64 ", %" PRIu64, "truncate", tinfo->keyno, tinfo->last);
...
row_update(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    logop(tinfo, "%-10s%" PRIu64 " {%.*s}, {%.*s}", "update", tinfo->keyno, (int)tinfo->key->size,
      (char *)tinfo->key->data, (int)tinfo->value->size, (char *)tinfo->value->data);
...
col_update(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    if (g.type == FIX)
        logop(tinfo, "%-10s%" PRIu64 " {0x%02" PRIx8 "}", "update", tinfo->keyno,
          ((uint8_t *)tinfo->value->data)[0]);
    else
        logop(tinfo, "%-10s%" PRIu64 " {%.*s}", "update", tinfo->keyno, (int)tinfo->value->size,
          (char *)tinfo->value->data);
...
row_insert(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    /* Log the operation */
    logop(tinfo, "%-10s%" PRIu64 " {%.*s}, {%.*s}", "insert", tinfo->keyno, (int)tinfo->key->size,
      (char *)tinfo->key->data, (int)tinfo->value->size, (char *)tinfo->value->data);
...
col_insert(TINFO *tinfo, WT_CURSOR *cursor)
{
    ...
    if (g.type == FIX)
        logop(tinfo, "%-10s%" PRIu64 " {0x%02" PRIx8 "}", "insert", tinfo->keyno,
          ((uint8_t *)tinfo->value->data)[0]);
    else
        logop(tinfo, "%-10s%" PRIu64 " {%.*s}", "insert", tinfo->keyno, (int)tinfo->value->size,
          (char *)tinfo->value->data);
...
row_remove(TINFO *tinfo, WT_CURSOR *cursor, bool positioned)
{
    ...
    logop(tinfo, "%-10s%" PRIu64, "remove", tinfo->keyno);
