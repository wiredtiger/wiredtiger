...
CONFIG configuration_list[] = {{"assert.read_timestamp", "assert read_timestamp",
                                 C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_ASSERT_READ_TIMESTAMP},
  ...
  {"block_cache", "enable the block cache", C_BOOL | C_NEUTRAL, 10, 0, 0, V_GLOBAL_BLOCK_CACHE},

  {"block_cache.cache_on_checkpoint", "block cache: cache checkpoint writes", C_BOOL | C_NEUTRAL,
    30, 0, 0, V_GLOBAL_BLOCK_CACHE_CACHE_ON_CHECKPOINT},

  {"block_cache.cache_on_writes", "block cache: populate the cache on writes", C_BOOL | C_NEUTRAL,
    60, 0, 0, V_GLOBAL_BLOCK_CACHE_CACHE_ON_WRITES},

  {"block_cache.size", "block cache size (MB)", C_NEUTRAL, 1, 100, 100 * 1024,
    V_GLOBAL_BLOCK_CACHE_SIZE},
  ...
  {"btree.compression", "data compression (off | lz4 | snappy | zlib | zstd)",
    C_IGNORE | C_NEUTRAL | C_STRING | C_TABLE, 0, 0, 0, V_TABLE_BTREE_COMPRESSION},

  {"btree.dictionary", "configure dictionary compressed values",
    C_BOOL | C_NEUTRAL | C_TABLE | C_TYPE_ROW | C_TYPE_VAR, 20, 0, 0, V_TABLE_BTREE_DICTIONARY},

  {"btree.huffman_value", "configure huffman encoded values",
    C_BOOL | C_NEUTRAL | C_TABLE | C_TYPE_ROW | C_TYPE_VAR, 20, 0, 0, V_TABLE_BTREE_HUFFMAN_VALUE},

  {"btree.internal_key_truncation", "truncate internal keys", C_BOOL | C_NEUTRAL | C_TABLE, 95, 0,
    0, V_TABLE_BTREE_INTERNAL_KEY_TRUNCATION},
  ...
  {"btree.split_pct", "page split size as a percentage of the maximum page size",
    C_NEUTRAL | C_TABLE, 50, 100, 100, V_TABLE_BTREE_SPLIT_PCT},
  ...
  {"buffer_alignment", "buffer alignment (off | on), on configures to 512", C_BOOL | C_NEUTRAL, 5,
    0, 0, V_GLOBAL_BUFFER_ALIGNMENT},

  {"cache", "cache size (MB)", C_NEUTRAL, 1, 100, 100 * 1024, V_GLOBAL_CACHE},
  ...
  {"cache.minimum", "minimum cache size (MB)", C_NEUTRAL | C_IGNORE, 0, 0, 100 * 1024,
    V_GLOBAL_CACHE_MINIMUM},

  {"checkpoint", "checkpoint type (on | off | wiredtiger)", C_NEUTRAL | C_IGNORE | C_STRING, 0, 0,
    0, V_GLOBAL_CHECKPOINT},

  {"checkpoint.log_size", "MB of log to wait if wiredtiger checkpoints configured", C_NEUTRAL, 20,
    200, 1024, V_GLOBAL_CHECKPOINT_LOG_SIZE},

  {"checkpoint.wait", "seconds to wait if wiredtiger checkpoints configured", C_NEUTRAL, 5, 100,
    3600, V_GLOBAL_CHECKPOINT_WAIT},

  {"debug.checkpoint_retention", "adjust log removal to retain the log records", C_NEUTRAL, 0, 10,
    1024, V_GLOBAL_DEBUG_CHECKPOINT_RETENTION},

  {"debug.eviction",
    "modify internal algorithms to force history store eviction to happen more aggressively",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_DEBUG_EVICTION},

  {"debug.log_retention", "adjust log removal to retain at least this number of log files",
    C_NEUTRAL, 0, 10, 1024, V_GLOBAL_DEBUG_LOG_RETENTION},

  {"debug.realloc_exact", "reallocation of memory will only provide the exact amount requested",
    C_BOOL | C_NEUTRAL, 0, 0, 0, V_GLOBAL_DEBUG_REALLOC_EXACT},

  {"debug.realloc_malloc", "every realloc call will force a new memory allocation by using malloc",
    C_BOOL | C_NEUTRAL, 5, 0, 0, V_GLOBAL_DEBUG_REALLOC_MALLOC},

  {"debug.slow_checkpoint",
    "slow down checkpoint creation by slowing down internal page processing", C_BOOL | C_NEUTRAL, 2,
    0, 0, V_GLOBAL_DEBUG_SLOW_CHECKPOINT},
  ...
  {"debug.update_restore_evict",
    "control all dirty page evictions through forcing update restore eviction", C_BOOL | C_NEUTRAL,
    2, 0, 0, V_GLOBAL_DEBUG_UPDATE_RESTORE_EVICT},

  {"disk.checksum", "checksum type (on | off | uncompressed | unencrypted)",
    C_IGNORE | C_NEUTRAL | C_STRING | C_TABLE, 0, 0, 0, V_TABLE_DISK_CHECKSUM},

  {"disk.data_extend", "configure data file extension", C_BOOL | C_NEUTRAL, 5, 0, 0,
    V_GLOBAL_DISK_DATA_EXTEND},

  {"disk.direct_io", "configure direct I/O for data objects", C_BOOL | C_NEUTRAL | C_IGNORE, 0, 0,
    1, V_GLOBAL_DISK_DIRECT_IO},
  ...
  {"logging", "configure logging", C_BOOL | C_NEUTRAL, 50, 0, 0, V_GLOBAL_LOGGING},

  {"logging.compression", "logging compression (off | lz4 | snappy | zlib | zstd)",
    C_IGNORE | C_NEUTRAL | C_STRING, 0, 0, 0, V_GLOBAL_LOGGING_COMPRESSION},
  ...
  {"ops.compaction", "configure compaction", C_BOOL | C_NEUTRAL, 10, 0, 0, V_GLOBAL_OPS_COMPACTION},
  ...
  {"ops.verify", "configure verify", C_BOOL | C_NEUTRAL, 100, 1, 0, V_GLOBAL_OPS_VERIFY},

  {"quiet", "quiet run (same as -q)", C_BOOL | C_IGNORE | C_NEUTRAL, 0, 0, 1, V_GLOBAL_QUIET},
  ...
  {"runs.threads", "number of worker threads", C_NEUTRAL, 1, 32, 128, V_GLOBAL_RUNS_THREADS},
  ...
  {"stress.checkpoint", "stress checkpoints", C_BOOL | C_NEUTRAL, 2, 0, 0,
    V_GLOBAL_STRESS_CHECKPOINT},

  {"stress.checkpoint_evict_page", "stress force checkpoint to evict all reconciling pages",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_CHECKPOINT_EVICT_PAGE},
  ...
  {"stress.failpoint_eviction_fail_after_reconciliation",
    "stress failpoint eviction fail after reconciliation", C_BOOL | C_NEUTRAL, 30, 0, 0,
  ...
  {"stress.failpoint_hs_delete_key_from_ts", "stress failpoint history store delete key from ts",
    C_BOOL | C_NEUTRAL, 30, 0, 0, V_GLOBAL_STRESS_FAILPOINT_HS_DELETE_KEY_FROM_TS},

  {"stress.hs_checkpoint_delay", "stress history store checkpoint delay", C_BOOL | C_NEUTRAL, 2, 0,
    0, V_GLOBAL_STRESS_HS_CHECKPOINT_DELAY},

  {"stress.hs_search", "stress history store search", C_BOOL | C_NEUTRAL, 2, 0, 0,
    V_GLOBAL_STRESS_HS_SEARCH},

  {"stress.hs_sweep", "stress history store sweep", C_BOOL | C_NEUTRAL, 2, 0, 0,
    V_GLOBAL_STRESS_HS_SWEEP},

  {"stress.sleep_before_read_overflow_onpage", "stress onpage overflow read race with checkpoint",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SLEEP_BEFORE_READ_OVERFLOW_ONPAGE},

  {"stress.split_1", "stress splits (#1)", C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_1},

  {"stress.split_2", "stress splits (#2)", C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_2},

  {"stress.split_3", "stress splits (#3)", C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_3},

  {"stress.split_4", "stress splits (#4)", C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_4},

  {"stress.split_5", "stress splits (#5)", C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_5},

  {"stress.split_6", "stress splits (#6)", C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_6},

  {"stress.split_7", "stress splits (#7)", C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_7},
