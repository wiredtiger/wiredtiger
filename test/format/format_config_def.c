...
CONFIG configuration_list[] = {
  ...
  {"block_cache", "enable the block cache",
    C_BOOL | C_NEUTRAL, 10, 0, 0, V_GLOBAL_BLOCK_CACHE},

  {"block_cache.cache_on_checkpoint", "block cache: cache checkpoint writes",
    C_BOOL | C_NEUTRAL, 30, 0, 0, V_GLOBAL_BLOCK_CACHE_CACHE_ON_CHECKPOINT},

  {"block_cache.cache_on_writes", "block cache: populate the cache on writes",
    C_BOOL | C_NEUTRAL, 60, 0, 0, V_GLOBAL_BLOCK_CACHE_CACHE_ON_WRITES},

  {"block_cache.size", "block cache size (MB)",
     C_NEUTRAL, 1, 100, 100 * 1024, V_GLOBAL_BLOCK_CACHE_SIZE},
  ...
  {"btree.internal_key_truncation", "truncate internal keys",
    C_BOOL | C_NEUTRAL | C_TABLE, 95, 0, 0, V_TABLE_BTREE_INTERNAL_KEY_TRUNCATION},

  {"btree.internal_page_max", "btree internal node maximum size",
    C_TABLE | C_NEUTRAL, 9, 17, 27, V_TABLE_BTREE_INTERNAL_PAGE_MAX},
  ...
  {"btree.leaf_page_max", "btree leaf node maximum size",
    C_NEUTRAL | C_TABLE, 9, 17, 27, V_TABLE_BTREE_LEAF_PAGE_MAX},

  {"btree.memory_page_max", "maximum cache page size",
    C_NEUTRAL | C_TABLE, 1, 10, 128, V_TABLE_BTREE_MEMORY_PAGE_MAX},
  ...
  {"buffer_alignment", "buffer alignment (off | on), on configures to 512",
    C_BOOL | C_NEUTRAL, 5, 0, 0, V_GLOBAL_BUFFER_ALIGNMENT},

  {"cache", "cache size (MB)",
    C_NEUTRAL, 1, 100, 100 * 1024, V_GLOBAL_CACHE},

  {"cache.evict_max", "maximum number of eviction workers",
    C_NEUTRAL, 0, 5, 100, V_GLOBAL_CACHE_EVICT_MAX},

  {"cache.minimum", "minimum cache size (MB)",
    C_NEUTRAL | C_IGNORE, 0, 0, 100 * 1024, V_GLOBAL_CACHE_MINIMUM},

  {"checkpoint", "checkpoint type (on | off | wiredtiger)",
    C_NEUTRAL | C_IGNORE | C_STRING, 0, 0, 0, V_GLOBAL_CHECKPOINT},

  {"checkpoint.log_size", "MB of log to wait if wiredtiger checkpoints configured",
    C_NEUTRAL, 20, 200, 1024, V_GLOBAL_CHECKPOINT_LOG_SIZE},

  {"checkpoint.wait", "seconds to wait if wiredtiger checkpoints configured",
    C_NEUTRAL, 5, 100, 3600, V_GLOBAL_CHECKPOINT_WAIT},

  {"debug.checkpoint_retention", "adjust log removal to retain the log records",
    C_NEUTRAL, 0, 10, 1024, V_GLOBAL_DEBUG_CHECKPOINT_RETENTION},
  ...
  {"debug.slow_checkpoint", "slow down checkpoint creation by slowing down internal page processing",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_DEBUG_SLOW_CHECKPOINT},
  ...
  {"debug.update_restore_evict", "control all dirty page evictions through forcing update restore eviction",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_DEBUG_UPDATE_RESTORE_EVICT},
  ...
  {"disk.data_extend", "configure data file extension",
    C_BOOL | C_NEUTRAL, 5, 0, 0, V_GLOBAL_DISK_DATA_EXTEND},

  {"disk.direct_io", "configure direct I/O for data objects",
    C_BOOL | C_NEUTRAL | C_IGNORE, 0, 0, 1, V_GLOBAL_DISK_DIRECT_IO},

  {"disk.encryption", "encryption type (off | rotn-7)",
    C_IGNORE | C_NEUTRAL | C_STRING, 0, 0, 0, V_GLOBAL_DISK_ENCRYPTION},

  {"disk.firstfit", "configure first-fit allocation",
    C_BOOL | C_NEUTRAL | C_TABLE, 10, 0, 0, V_TABLE_DISK_FIRSTFIT},

  {"disk.mmap", "configure mmap operations (reads only)",
    C_BOOL | C_NEUTRAL, 90, 0, 0, V_GLOBAL_DISK_MMAP},

  {"disk.mmap_all", "configure mmap operations (read and write)",
    C_BOOL | C_NEUTRAL, 5, 0, 0, V_GLOBAL_DISK_MMAP_ALL},

  {"format.abort", "drop core during timed run",
    C_BOOL | C_NEUTRAL, 0, 0, 0, V_GLOBAL_FORMAT_ABORT},
 ...
 */
  ...
  {"logging", "configure logging",
    C_BOOL | C_NEUTRAL, 50, 0, 0, V_GLOBAL_LOGGING},
  ...
  {"logging.file_max", "maximum log file size (KB)",
    C_NEUTRAL, 100, 512 * WT_THOUSAND, 2097152, V_GLOBAL_LOGGING_FILE_MAX},

  {"logging.prealloc", "configure log file pre-allocation",
    C_BOOL | C_NEUTRAL, 50, 0, 0, V_GLOBAL_LOGGING_PREALLOC},

  {"logging.remove", "configure log file removal",
    C_BOOL | C_NEUTRAL, 50, 0, 0, V_GLOBAL_LOGGING_REMOVE},
  ...
  {"ops.compaction", "configure compaction",
    C_BOOL | C_NEUTRAL, 10, 0, 0, V_GLOBAL_OPS_COMPACTION},

  {"ops.hs_cursor", "configure history store cursor reads",
    C_BOOL | C_NEUTRAL, 50, 0, 0, V_GLOBAL_OPS_HS_CURSOR},
  ...
  {"ops.bound_cursor", "configure bound cursor reads",
    C_BOOL | C_NEUTRAL, 5, 0, 0, V_GLOBAL_OPS_BOUND_CURSOR},

  {"ops.prepare", "configure transaction prepare",
    C_BOOL | C_NEUTRAL, 5, 0, 0, V_GLOBAL_OPS_PREPARE},

  {"ops.random_cursor", "configure random cursor reads",
    C_BOOL | C_NEUTRAL, 10, 0, 0, V_GLOBAL_OPS_RANDOM_CURSOR},
  ...
  {"ops.verify", "configure verify",
    C_BOOL | C_NEUTRAL, 100, 1, 0, V_GLOBAL_OPS_VERIFY},

  {"quiet", "quiet run (same as -q)",
    C_BOOL | C_IGNORE | C_NEUTRAL, 0, 0, 1, V_GLOBAL_QUIET},
  ...
  {"runs.threads", "number of worker threads",
    C_NEUTRAL, 1, 32, 128, V_GLOBAL_RUNS_THREADS},
  ...
  {"stress.aggressive_sweep", "stress aggressive sweep",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_AGGRESSIVE_SWEEP},

  {"stress.checkpoint", "stress checkpoints",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_CHECKPOINT},
  ...
  {"stress.checkpoint_prepare", "stress checkpoint prepare",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_CHECKPOINT_PREPARE},

  {"stress.evict_reposition", "stress evict reposition",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_EVICT_REPOSITION},

  {"stress.failpoint_eviction_fail_after_reconciliation", "stress failpoint eviction fail after reconciliation",
    C_BOOL | C_NEUTRAL, 30, 0, 0, V_GLOBAL_STRESS_FAILPOINT_EVICTION_FAIL_AFTER_RECONCILIATION},
  ...
  {"stress.hs_checkpoint_delay", "stress history store checkpoint delay",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_HS_CHECKPOINT_DELAY},

  {"stress.hs_search", "stress history store search",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_HS_SEARCH},

  {"stress.hs_sweep", "stress history store sweep",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_HS_SWEEP},
  ...
  {"stress.split_1", "stress splits (#1)",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_1},

  {"stress.split_2", "stress splits (#2)",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_2},

  {"stress.split_3", "stress splits (#3)",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_3},

  {"stress.split_4", "stress splits (#4)",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_4},

  {"stress.split_5", "stress splits (#5)",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_5},

  {"stress.split_6", "stress splits (#6)",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_6},

  {"stress.split_7", "stress splits (#7)",
    C_BOOL | C_NEUTRAL, 2, 0, 0, V_GLOBAL_STRESS_SPLIT_7},
  ...
  {"wiredtiger.rwlock", "configure wiredtiger read/write mutexes",
    C_BOOL | C_NEUTRAL, 80, 0, 0, V_GLOBAL_WIREDTIGER_RWLOCK},

  {"wiredtiger.leak_memory", "leak memory on wiredtiger shutdown",
    C_BOOL | C_NEUTRAL, 0, 0, 0, V_GLOBAL_WIREDTIGER_LEAK_MEMORY},
