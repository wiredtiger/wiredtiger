...
timestamp_sync_threads_commit_ts(void)
{
     ...
     * used value). timestamp_minimum_committed() returns g.timestamp-1, which would leave the
     * final committed timestamp uncovered by stable. Bump by one so stable lands exactly at the
     * last used timestamp.
     */
