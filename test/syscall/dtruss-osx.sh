...
#          -t syscall,...  # examine these syscalls only
#          -a              # print all details
...
# 12-Sep-2016	Don Anderson	Added multiple syscalls with -t.
#
...
c=''
for part in `echo "$trace" | sed -e 's/,/ /g'`; do
    if [ "$c" != '' ]; then
        c="$c && "
    fi

    ### Option translation
    ## if [ "$part" = "exec" ]; then part="exece"; fi
    if [ "$part" = "exec" ]; then part="execve"; fi
