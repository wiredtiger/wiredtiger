...
#   enabled. Note: the prepare timestamp here is above the last checkpoint, so the drain takes the
#   standard value path and the on-disk bytes are produced by __wt_clayered_ingest_to_stable_value,
#   not the prepared-fix strip (__wt_clayered_ingest_to_stable_update); that helper is guarded by
#   the s_tombstone_encoding.py dist check rather than this test's assertion.
...
class test_layered_tombstone_prepared_drain(wttest.WiredTigerTestCase):
    ...
    # Every node in the cluster runs the same mode; the drain behavior is what differs.
    modes = [
    ...
    def role_config(self, role):
        return f'disaggregated=(stable_tombstone_encoding={self.encoding},role="{role}")'
