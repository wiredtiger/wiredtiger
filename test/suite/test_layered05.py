...
class test_layered05(wttest.WiredTigerTestCase):
    ...
    # -----------------------------------------------------------------------
    # Test: search() on a tombstoned key returns NOTFOUND. A subsequent next()
    # must continue in order and must not return the tombstoned key.
    #
    # Stable: all keys 0-999. Ingest: tombstone at 500.
    # 1. next() to key 499.
    # 2. search(500) -> NOTFOUND (tombstoned).
    # 3. Scan forward with next(). Key 500 must NOT appear.
    # -----------------------------------------------------------------------
    ...
    def test_search_tombstone_past_position_then_next(self):
        ...
        # A forward scan after the failed search must not include key 500.
        seen = []
        while cursor.next() == 0:
            seen.append(cursor.get_key())
    ...
    def test_search_tombstone_desync_merge(self):
        ...
        # search() on the deleted key returns NOTFOUND.
        cursor.set_key(self.fmt_key(500))
        self.assertEqual(cursor.search(), wiredtiger.WT_NOTFOUND)
