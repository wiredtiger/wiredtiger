...
class test_disagg_checkpoint_size11(DisaggSizeTestMixin, wttest.WiredTigerTestCase):
    ...
    #      (which fires during eviction reconciliation after the block write,
    #      before the commit path). Keep delta_pct=90 so the write stays a delta.
    ...
    def test_rec_write_err_delta_post_plh_put(self):
        ...
        # chance. Keep delta_pct=90 (inherited from conn_config) so the eviction below
        # produces a delta write, staying well inside the max_consecutive_delta=32
        # window before a full-image rollover would occur.
