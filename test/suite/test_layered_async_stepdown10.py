...
#    Stress the async step-down: one thread walks the connection through the step-down phases (pin
#    stable, final checkpoint, writes above it, quiesce and demote, follower) while a workload
#    thread hammers creates, drops, inserts, removes and reads. The demoted node must serve every
#    table's exact contents, and a fresh follower must see exactly the final checkpoint, before the
#    node steps back up; after the step-up every surviving table must serve its exact contents,
#    current and at the final checkpoint, to the leader and to a fresh follower. Runs in both the
...
class test_layered_async_stepdown10(
  LayeredStepdownMixin, wttest.WiredTigerTestCase, DisaggSchemaEpochMixin):
    ...
    def setup_stress_state(self):
        # Commit timestamps and the stable timestamp come from one counter and every timestamp call
        # happens under one lock, so a commit can never trail the stable timestamp, and once stable
        # is pinned every later commit lands above it.
        ...
        # (None in the epoch-less world), and whether the final checkpoint can hold the table:
        # True, False, or None when the create raced the checkpoint.
        self.tables = {}
        self.seed_uris = set()
    ...
    def drop_removal_is_guaranteed(self):
        ...
        if not self.use_epochs and self.final_ts is not None:
            return False
        return True
    ...
    # final checkpoint's timestamp, because a checkpoint refuses stable data on an uncovered table.
    def setup_seed_tables(self):
        ...
        for _ in range(3):
            ...
            self.tables[uri] = {'history': [(ts, rows)], 'publish_epoch': epoch,
                                'in_checkpoint': True}
    ...
    def workload_create(self, wsession, rng):
        # Raise the cap once stable is pinned, so creates above the final checkpoint still happen
        # when the table population filled up before it.
        cap = self.table_cap + (20 if self.final_ts is not None else 0)
        if len(self.tables) >= cap:
            return
        uri = self.uri(f'w{next(self.name_counter)}')
        after_checkpoint = self.checkpoint_done
        # Creates and publishes tolerate no errors: a failure fails the test.
        wsession.create(uri, self.table_config)
        before_checkpoint = not self.checkpoint_started
        epoch = self.publish_if_epochs(uri, session=wsession)
        in_checkpoint = True if before_checkpoint else (False if after_checkpoint else None)
        self.tables[uri] = {'history': [], 'publish_epoch': epoch, 'in_checkpoint': in_checkpoint}
        self.op_counts['window_creates' if after_checkpoint else 'creates'] += 1
    ...
    # Choose the table for the next write. In the epoch world, until stable is pinned any commit
    # may become stable, which a checkpoint refuses on an uncovered table, so only the covered
    # seed tables are safe. Once it is pinned, commits land above it and any table works.
    def choose_write_table(self, rng):
        with self.ts_lock:
            window_open = self.final_ts is not None
    ...
    def write_txn(self, wsession, rng, uri, kvs):
        ...
        try:
            ...
            if not resolved:
                wsession.rollback_transaction()
            # A write may only be rolled back by the demotion: a snapshot begun under the leader
            # role cannot bind stable content under the follower role.
            ...
            self.assertTrue(self.demotion_started,
                f'write to {uri} was refused before the demotion')
            self.op_counts['demotion_rollbacks'] += 1
        cursor.close()
        if ts is not None:
    ...
    def workload_read(self, wsession, rng):
        ...
        try:
            ...
            if not self.is_rollback(e):
                raise
            # An untimestamped snapshot taken before the demotion may be refused at its first
            # stable bind after it, and verifies nothing. No other reason may roll a read back.
            self.assertIn('A newer checkpoint was adopted', wsession.get_last_error()[2],
                f'read of {uri} rolled back for an unexpected reason')
            self.assertTrue(self.demotion_started,
                f'read of {uri} was refused before the demotion')
            self.op_counts['demotion_rollbacks'] += 1
        wsession.rollback_transaction()
        cursor.close()
    ...
    def workload_move_stable(self, wsession, rng):
        # Keep stable moving like a live system, but only until it is pinned for the final
        # checkpoint: after that it must stay there until the demotion.
        with self.ts_lock:
            if self.final_ts is None:
    ...
    def step_down_in_phases(self):
        ...
        # Phase 1: pin stable for the final checkpoint.
        with self.ts_lock:
            self.final_ts = next(self.ts_counter)
            self.conn.set_timestamp('stable_timestamp=' + self.timestamp_str(self.final_ts))
        time.sleep(self.phase_sleep)

        # Phase 2: take the final checkpoint under load; writes keep landing above it.
        ...
        # Phase 3: stop writing and demote.
        ...
        # Phase 4: the workload continues against the demoted node.
        time.sleep(self.phase_sleep)
    ...
    # The demoted node, before any pickup, serves every surviving table's exact rows: its own
    # commits above the final checkpoint from the frozen tree, and its follower writes from ingest.
    def verify_demoted_node(self):
        for uri, info in self.tables.items():
            self.assertEqual(self.read_kvs(uri), self.rows_at(info),
                f'{uri} served the wrong rows on the demoted node')
            if info['history']:
                newest = max(ts for ts, _ in info['history'])
                self.assertEqual(self.read_kvs_at(uri, newest), self.rows_at(info, newest),
                    f'{uri} served the wrong rows at {newest} on the demoted node')

    # Verify the final checkpoint itself, before this node steps back up and re-covers everything:
    # a fresh follower must serve exactly the rows at the final checkpoint's timestamp, and must not
    # see the tables the checkpoint could not include, that is tables created after it and, in the
    # epoch world, everything but the covered seed tables.
    def verify_step_down_checkpoint(self):
        conn_follow, session_follow = self.open_follower()
        for uri, info in self.tables.items():
            if info['in_checkpoint'] is None:
                continue
            in_checkpoint = info['in_checkpoint'] and (not self.use_epochs or uri in self.seed_uris)
            if in_checkpoint:
                self.assertEqual(
                    self.read_kvs_at(uri, self.final_ts, session=session_follow),
                    self.rows_at(info, self.final_ts),
                    f'{uri} read wrong from the final checkpoint')
            else:
    ...
    # state and the state at the final checkpoint's timestamp.
    def assert_rows_served(self, final_ts, session, where):
        for uri, info in self.tables.items():
            ...
            self.assertEqual(self.read_kvs_at(uri, self.final_ts, session=session),
                self.rows_at(info, self.final_ts),
                f'{uri} served the wrong rows at the final checkpoint on the {where}')
