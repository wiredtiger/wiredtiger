...
@disagg_test_class
class test_layered_follower07(wttest.WiredTigerTestCase):
    conn_config = 'disaggregated=(role="leader")'
    conn_config_follower = 'disaggregated=(role="follower")'

    uri = "layered:test_layered_follower07"
    ...
    def test_standby_uses_table_id_high_water_mark(self):
        ...
        for i in range(0, 100):
            self.session.create(f"layered:test_layered_follower07_{i}", 'key_format=S,value_format=S')
        ...
        for i in range(0, 100):
            self.session.drop(f"layered:test_layered_follower07_{i}")
        self.session.checkpoint()
        ...
        self.session_follow.create(f"layered:test_layered_follower07_101", 'key_format=S,value_format=S')
