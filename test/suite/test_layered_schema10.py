...
class test_layered_schema10(wttest.WiredTigerTestCase, suite_subprocess, DisaggSchemaEpochMixin):
    ...
    def test_stepup_after_second_no_epoch_leader_checkpoint(self):
        """A published table survives a second no-epoch leader checkpoint pickup before step-up."""
        # Leader checkpoint carries schema_epoch == 0 (no epoch set).
        self.leader_checkpoint(1)
        ...
        # Follower queue holds CREATE uri at epoch 20. Leader takes a second no-epoch checkpoint
        # and the follower picks it up. Without the fix, prune(NONE) wipes CREATE@epoch20.
        self.leader_checkpoint(2)
        ...
        self.checkpoint_and_advance(15, 3, conn_follow)
        # Stable epoch 15 has not reached the publish epoch so CREATE is still pending.
        self.assertFalse(self.uri_in_local_metadata(self.conn, self.uri))

        self.checkpoint_and_advance(20, 4, conn_follow)
        # Stable epoch 20 matches the publish epoch so CREATE is flushed and uri is visible.
        self.assertTrue(self.uri_in_local_metadata(self.conn, self.uri))

        conn_follow.close('debug=(skip_checkpoint=true)')

    def test_stepup_no_epoch_checkpoint_between_create_and_publish(self):
        """A table created before a no-epoch checkpoint pickup survives a later publish and step-up."""
        # Leader checkpoint carries schema_epoch == 0 (no epoch set).
        self.leader_checkpoint(1)
        ...
        # Create before publishing. The queue holds CREATE uri at the unpublished epoch.
        session_follow.create(self.uri, self.table_config)

        # Leader takes a second no-epoch checkpoint and the follower picks it up before the publish.
        # Without the fix, prune(NONE) drops the unpublished CREATE, then publish silently matches
        # no entry and the epoch is lost.
        self.leader_checkpoint(2)
        self.disagg_advance_checkpoint(conn_follow)
