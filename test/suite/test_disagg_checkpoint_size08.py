...
#     without restoring cumulative_size_aggregated.  The copy-site reset leaves
...
#     WT_ASSERT(session, page->disagg_info->block_meta.cumulative_size_aggregated)
#   and aborted the process.
#
#   The fix restores cumulative_size_aggregated = (cumulative_size > 0) after the
...
#       line 3205 incorrectly set cumulative_size_aggregated=false before the fix.
...
class test_disagg_checkpoint_size08(wttest.WiredTigerTestCase):
    ...
    # (which had cumulative_size_aggregated=false from the copy-site reset) into
    ...
    def test_supd_restore_wrapup_aggregated_flag(self):
        ...
        # reconciliation result; a subsequent skip-write turns it to REPLACE with
        # (before the fix) aggregated=false.  The failpoint then fires during the
        # next full-image write and __rec_write_err triggers the assert.
        ...
        # left cumulative_size_aggregated=false; skip-write then set REPLACE with the
    ...
    # r->multi->block_meta (cumulative_size_aggregated=false, set by
    ...
    #
    # The fix adds cumulative_size_aggregated = (cumulative_size > 0) after the
    ...
    #      __rec_copy_prev_addr reset cumulative_size_aggregated to false) into
    ...
    #      __rec_write_err finds REPLACE + cumulative_size>0 + aggregated=false.
    #      Before the fix this aborts.
    def test_skip_write_wrapup_aggregated_flag(self):
        ...
        #      correctly restores cumulative_size_aggregated = (cumulative_size > 0).
        #      Before the fix, it propagated cumulative_size_aggregated=false from
        ...
        #      Before fix: REPLACE + cumulative_size>0 + aggregated=false  assert.
        #      After fix: aggregated=true  no assert, stat incremented.
        max_iters = 500
        for i in range(max_iters):
