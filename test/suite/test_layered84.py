...
class test_layered84(wttest.WiredTigerTestCase):
    ...
    def test_standby_uses_table_id_high_water_mark(self):
        ...
        for i in range(0, 100):
            self.session.create(f"layered:test_layered84_{i}", 'key_format=S,value_format=S')
        ...
        for i in range(0, 100):
            self.session.drop(f"layered:test_layered84_{i}")
        self.session.checkpoint()
        ...
        self.session_follow.create(f"layered:test_layered84_101", 'key_format=S,value_format=S')
