...
#   Verify that debug_mode.disagg_slow_truncate_follower actually selects the
#   slow per-record truncate path on a layered-table follower. The slow path
#   walks the range and calls cursor->remove() once per key (incrementing the
#   layered_curs_remove stat); the fast path uses the truncate-list and, when
#   the ingest table is empty, performs no per-key cursor work at all.

@disagg_test_class
class test_layered_fast_truncate20(wttest.WiredTigerTestCase):
