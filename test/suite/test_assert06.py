...
class test_assert06(wttest.WiredTigerTestCase, suite_subprocess):
    ...
    def test_timestamp_alter(self):
        base = 'assert06'
        uri = 'file:' + base
        cfg_on = 'write_timestamp_usage=key_consistent,assert=(write_timestamp=all)'
        cfg_off = 'write_timestamp_usage=never,assert=(write_timestamp=none)'
    ...
    def test_timestamp_usage(self):
        ...
        self.session.create(uri, 'key_format=S,value_format=S,write_timestamp_usage=key_consistent,assert=(write_timestamp=all)')
