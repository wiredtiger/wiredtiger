...
class test_assert06(wttest.WiredTigerTestCase, suite_subprocess):
    ...
    def test_timestamp_alter(self):
        ...
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(4))
...
"""     def test_timestamp_usage(self):
        ...
        self.session.begin_transaction()
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(2))
        c['key_ts1'] = 'value2'
        self.session.commit_transaction()
        c.close()
        ...
        self.session.begin_transaction()
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(1))
        c['key_ts1'] = 'value1'
        self.assertRaisesWithMessage(wiredtiger.WiredTigerError,
            lambda: self.session.commit_transaction(), msg_ooo)
        c.close()
        ...
        self.session.begin_transaction()
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(13))
        c['key_ts3'] = 'value13'
        c['key_ts4'] = 'value13'
        self.assertRaisesWithMessage(wiredtiger.WiredTigerError,
            lambda: self.session.commit_transaction(), msg_ooo)
        ...
        self.session.begin_transaction()
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(14))
        c['key_ts4'] = 'value14'
        self.assertRaisesWithMessage(wiredtiger.WiredTigerError,
            lambda: self.session.commit_transaction(), msg_ooo)
        c.close()
        ...
        self.session.begin_transaction()
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(16))
        c['key_ts4'] = 'value16'
        self.session.commit_transaction()
        c.close()
        c = self.session.open_cursor(uri)
        ...
        self.session.begin_transaction()
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(17))
        c['key_nots'] = 'value17'
        self.assertRaisesWithMessage(wiredtiger.WiredTigerError,
            lambda: self.session.commit_transaction(), msg_usage)
        c.close()
        ...
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(20))
        c['key_ts5'] = 'value20'
        self.session.commit_transaction()
        c.close()
        ...
        self.session.timestamp_transaction(
            'durable_timestamp=' + timestamp_str(21))
        self.session.commit_transaction()
        c.close()
