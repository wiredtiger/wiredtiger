...
# A table created on a node that has just stepped down has no stable constituent until a later
...
class test_layered_cursor26(
  DisaggSizeTestMixin, LayeredStepdownMixin, wttest.WiredTigerTestCase, DisaggSchemaEpochMixin):
    ...
    # The final checkpoint's timestamp; every test creates its table above it.
    cutoff = 5

    table_uri = f'table:{__qualname__}'
    layered_uri = f'layered:{__qualname__}'

    def create_after_step_down(self):
        """Step down, then create and populate a table on the demoted node."""
    ...
    def test_size_reported_once_step_up_creates_the_constituent(self):
        """A table created after the step-down reports its real size once it has a constituent."""
        self.create_after_step_down()
