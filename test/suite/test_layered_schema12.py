...
class test_layered_schema12(wttest.WiredTigerTestCase, DisaggSchemaEpochMixin):
    ...
    def assertOnFollower(self, present):
        ...
        try:
            for uri, expected in present.items():
                if expected:
                    self.assertTrue(self.uri_has_stable_constituent(conn, uri))
                    self.assertTrue(self.uri_in_shared_metadata(conn, uri))
                else:
                    self.assertFalse(self.uri_has_stable_constituent(conn, uri))
    ...
    def test_create_drop_unpublished_not_resurrected_on_restart(self):
        ...
        self.assertTrue(self.uri_has_stable_constituent(self.conn, self.uri))
        self.assertFalse(self.uri_has_stable_constituent(self.conn, self.uri2))
