...
# test_eviction06.py
#
# Test that eviction max page size stats for clean, dirty, and updates are set properly.
...
class test_eviction06(wttest.WiredTigerTestCase):
    ...
    def test_eviction_page_size_stats(self):
        ...
        self.assertGreater(self.get_stat(stat.conn.eviction_maximum_updates_page_size), 0)
        
        # Check that dirty page stat was incremented
        self.assertGreater(self.get_stat(stat.conn.eviction_maximum_dirty_page_size), 0)

        # Check that clean page stat was not incremented
        self.assertEqual(self.get_stat(stat.conn.eviction_maximum_clean_page_size), 0)
        ...
        self.assertGreater(self.get_stat(stat.conn.eviction_maximum_clean_page_size), 0)
