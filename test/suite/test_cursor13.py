...
class test_cursor13_base(wttest.WiredTigerTestCase):
    conn_config = 'cache_cursors=true'
...
class test_cursor13(test_cursor13_base):
    uri = 'table:test_cursor13'
    ...
    def test_open_and_drop(self):
        ...
        # Cursor.close should cache the cursor, not really close it.
        c.close()
        # But the drop should trigger the close
        session.drop(uri)
        confirm_does_not_exist(self, uri)

        # It should still work with different sessions
        session2 = self.conn.open_session(None)
        session2.create(uri, None)
        c = session2.open_cursor(uri)
        c.close()
        session.drop(uri)
        confirm_does_not_exist(self, uri)
        session2.close()
    ...
    def test_reopen(self):
        ...
        c.close()
        count = 0
        for x,y in session.open_cursor(uri):
            if count == 0:
