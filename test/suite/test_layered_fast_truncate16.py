...
class test_layered_fast_truncate_stepup(LayeredFastTruncateConfigMixin, wttest.WiredTigerTestCase):
    ...
    def setup_follower(self):
        self.setup_dual_conn_follower(value=lambda i: 'v' + str(i))

    # Convenience wrappers binding session=self.session_follow.
    def truncate_f(self, start, stop, ts):
        self.truncate(start, stop, commit_timestamp=ts, session=self.session_follow)

    def write(self, key, value, ts):
        self.write_kv(self.session_follow, key, value, ts)

    def remove(self, key, ts):
        self.remove_kv(self.session_follow, key, ts)

    def visible(self, keys, value=None, ts=None):
        self.assert_visible(self.session_follow, keys, value, ts)

    def deleted(self, keys, ts):
        self.assert_deleted(self.session_follow, keys, ts)

    def keys_gone(self, ranges):
        self.assert_ranges_deleted(self.session_follow, ranges)

    # Single truncate, stable-only keys: the simplest case.
    def test_stable_only_keys(self):
        self.setup_follower()
        self.truncate_f(100, 700, 20)
        self.step_up()
        self.keys_gone([(100, 700)])

    # Truncated range mixes follower-updated keys and stable-only keys.
    def test_mixed_keys(self):
        self.setup_follower()
        for i in [200, 300, 400, 500, 600]:
            self.write(i, "follower-update", 15)
        self.truncate_f(100, 700, 20)
        self.step_up()
        self.keys_gone([(100, 700)])
    ...
    def test_truncate_then_reinsert(self):
        self.setup_follower()
        self.truncate_f(100, 700, 20)
        self.write(300, "reinserted", 25)
        self.step_up()
        self.visible([300], "reinserted", ts=100)
        self.deleted([100, 150, 250, 400, 500, 700], ts=100)

    # --- Range boundaries ---

    def test_single_key_truncate(self):
        self.setup_follower()
        self.truncate_f(500, 500, 20)
        self.step_up()
        self.keys_gone([(500, 500)])

    def test_truncate_at_table_start(self):
        self.setup_follower()
        self.truncate_f(0, 50, 20)
        self.step_up()
        self.keys_gone([(0, 50)])

    def test_truncate_at_table_end(self):
        self.setup_follower()
        self.truncate_f(950, 999, 20)
        self.step_up()
        self.keys_gone([(950, 999)])

    def test_truncate_full_table(self):
        self.setup_follower()
        self.truncate_f(0, 999, 20)
        self.step_up()
        self.keys_gone([(0, 999)])

    # Range fully outside the populated key space.
    def test_truncate_empty_range(self):
        self.setup_follower()
        self.truncate_f(2000, 3000, 20)
        self.step_up()
        self.visible([0, 100, 500, 999], lambda k: f"v{k}", ts=100)
    ...
    def test_multiple_truncates(self):
        self.setup_follower()
        self.truncate_f(100, 200, 20)
        self.truncate_f(400, 500, 25)
        self.truncate_f(800, 900, 30)
        self.step_up()
        self.keys_gone([(100, 200), (400, 500), (800, 900)])

    # Same range truncated twice.
    def test_duplicate_truncates(self):
        self.setup_follower()
        self.truncate_f(200, 500, 20)
        self.truncate_f(200, 500, 25)
        self.step_up()
        self.keys_gone([(200, 500)])
    ...
    def test_truncate_reinsert_truncate(self):
        ...
        self.truncate_f(100, 700, 20)
        self.write(300, "reinserted", 25)
        self.truncate_f(100, 700, 30)
        self.step_up()
        self.deleted([100, 300, 500, 700], ts=100)
        self.deleted([300], ts=22)
        self.visible([300], "reinserted", ts=27)
    ...
    def test_snapshot_read_around_truncate(self):
        ...
        self.truncate_f(100, 700, 20)
        self.step_up()
        self.visible([100, 250, 500, 700], lambda k: f"v{k}", ts=15)
        self.deleted([100, 250, 500, 700], ts=30)
        self.visible([50, 800], ts=30)
    ...
    def test_read_at_truncate_timestamp(self):
        ...
        self.truncate_f(100, 700, 20)
        self.step_up()
        self.deleted([100, 250, 500, 700], ts=20)
    ...
    def test_intermediate_read_overlapping_truncates(self):
        self.setup_follower()
        self.truncate_f(100, 400, 20)
        self.truncate_f(300, 600, 30)
        self.step_up()
        self.deleted([100, 200, 350, 400], ts=25)
        self.visible([450, 500, 600], ts=25)
        self.deleted([100, 250, 350, 450, 600], ts=35)
    ...
    def test_intermediate_read_mixed_per_key_history(self):
        self.setup_follower()
        for i in [200, 400]:
            self.write(i, "follower-pre", 15)
        self.truncate_f(100, 700, 20)
        self.write(300, "reinserted-stable-only", 25)
        self.write(400, "reinserted-follower-updated", 25)
        self.step_up()
        self.deleted([100, 200, 300, 400, 500, 700], ts=22)
        self.visible([300], "reinserted-stable-only", ts=30)
        self.visible([400], "reinserted-follower-updated", ts=30)
        self.deleted([100, 200, 500, 700], ts=30)
    ...
    def test_post_stepup_writes_to_truncated_range(self):
        ...
        self.truncate_f(100, 700, 20)
        self.step_up()
        self.write(300, "after-stepup", 30)
        self.visible([300], "after-stepup", ts=100)
    ...
    def test_ingest_key_at_start_bound(self):
        self.setup_follower()
        self.write(100, "ingest-at-start", 15)
        self.truncate_f(100, 700, 20)
        self.step_up()
        self.keys_gone([(100, 700)])
    ...
    def test_ingest_key_at_stop_bound(self):
        self.setup_follower()
        self.write(700, "ingest-at-stop", 15)
        self.truncate_f(100, 700, 20)
        self.step_up()
        self.keys_gone([(100, 700)])
    ...
    def test_drain_truncate_below_stable_timestamp(self):
        ...
        self.deleted([100, 250, 500, 700], ts=35)
        self.visible([50, 800], lambda k: f"v{k}", ts=35)
    ...
    def test_layered_remove_only(self):
        ...
        self.remove(300, 20)
        self.step_up()
        self.deleted([300], ts=100)
        self.visible([300], "v300", ts=15)

    # Staggered remove then truncate.
    def test_layered_remove_then_truncate(self):
        self.setup_follower()
        self.remove(300, 20)
        self.truncate_f(100, 700, 25)
        self.step_up()
        for ts in [20, 22, 25, 30]:
            self.deleted([300], ts=ts)
        self.visible([300], "v300", ts=15)
        self.deleted([100, 250, 500, 700], ts=100)
