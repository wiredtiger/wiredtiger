...
#   Turning stable tombstone encoding off raises the checkpoint's minimum reader version: an
#   unescaped checkpoint stamps compatible_version=2 (a node that still strips the escape byte cannot
#   read it), while an escaped checkpoint stays compatible with every reader at compatible_version=1.
#   A reader whose maximum version is below the checkpoint's compatible_version must refuse the
#   pickup. This build's maximum reader version is WT_DISAGG_CHECKPOINT_META_VERSION (2), so to stand
#   in for an older reader we rewrite the fetched metadata to demand version 3 and confirm the pickup
#   is rejected with a not-supported error.
...
class test_layered_tombstone_version_gate(wttest.WiredTigerTestCase):
    ...
    # The stable tombstone encoding mode determines the checkpoint's compatible_version stamp.
    encodings = [
        ('escaped',   dict(encoding='true',  compatible_version=1)),
        ('unescaped', dict(encoding='false', compatible_version=2)),
    ]
    ...
    def conn_config(self):
        return self.extensionsConfig() + ',create,' + self.conn_base_config + \
            f'disaggregated=(stable_tombstone_encoding={self.encoding},role="leader")'

    def setUp(self):
        super().setUp()
        self.ignoreStdoutPattern('stable table value in the tombstone namespace')

    # Write a colliding value and checkpoint, stamping the encoding mode and version into the
    ...
    def test_reader_version_gate(self):
        ...
        # The encoding mode is stamped, and the compatible_version reflects it: unescaped raises the
        # minimum reader version to 2, escaped stays at 1.
        meta = self.disagg_get_complete_checkpoint_meta()
        self.assertIn(f'stable_tombstone_encoding={self.encoding}', meta)
        ...
        # A follower configured for the checkpoint's own encoding mode would otherwise pick it up
        ...
        conn_follow = self.wiredtiger_open('follower',
            ...
            f'disaggregated=(stable_tombstone_encoding={self.encoding},role="follower")')
        ...
        # only the forged version stamp was rejected.
