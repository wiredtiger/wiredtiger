...
class test_error_info02(error_info_util):
    ...
    def test_wt_rollback_oldest_for_eviction(self):
        ...
        # Two independent mechanisms can produce this rollback: the stuck-cache detector, which
        # needs the aggressive score to escalate over real time, and a check that fires as soon as
        # this session's own dirty content exceeds the updates trigger, with no escalation delay.
        # Whichever wins depends on how fast the cache's background eviction is progressing in this
        # environment, so either is a valid outcome here.
