...
class test_checkpoint(wttest.WiredTigerTestCase):
    ...
    if wttest.isfast():
        # Keep a small representative set in fast runs.
        _overlap = [('overlap', dict(do_overlap=True))]
        _stable = [('25', dict(stable_ts=25))]
        _advance = [('no-advance', dict(do_advance=False))]
        _name = [('named', dict(second_checkpoint='second_checkpoint', do_reopen=False))]
        scenarios = make_scenarios(format_values, _overlap, _stable, _advance, _name)
    else:
