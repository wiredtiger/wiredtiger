...
class burst_inserts : public test {
    ...
    insert_operation(thread_worker *tc) override final
    {
        ...
        struct collection_cursor {
            collection_cursor(collection &coll, cursor &&write_cursor, cursor &&read_cursor)
        ...
        for (int i = thread_offset; i < thread_offset + collections_per_thread && tc->running();
             ...
             */
            ccv.push_back({coll, std::move(tc->scoped_session.open_cursor(coll.name)),
              std::move(tc->scoped_session.open_cursor(coll.name, "next_random=true"))});
        }
