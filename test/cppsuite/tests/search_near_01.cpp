...
class search_near_01 : public test {
      ...
      operation_tracker *op_tracker) override final
    {
        ...
        for (uint64_t count = 0; count < collection_count; ++count) {
            ...
            wiredtiger_cursor evict_cursor =
              session.open_wiredtiger_cursor(coll.name.c_str(), "debug=(release_evict=true)");
      ...
      std::atomic<int64_t> &z_key_searches)
    {
        ...
        wiredtiger_cursor cursor = tc->session.open_wiredtiger_cursor(collection_name);
    ...
    read_operation(thread_worker *tc) override final
    {
        ...
        tc->stat_cursor = tc->session.open_wiredtiger_cursor(STATISTICS_URI);
