...
prepare_session_and_error(
  connection_wrapper *conn_wrapper, WT_SESSION **session, WT_ERROR_INFO **err_info, std::string cfg)
{
    WT_CONNECTION *conn = conn_wrapper->get_wt_connection();
    REQUIRE(conn->open_session(conn, NULL, NULL, session) == 0);
    REQUIRE((*session)->create(*session, URI, ("key_format=S,value_format=S," + cfg).c_str()) == 0);
...
TEST_CASE("Test WT_CONFLICT_BACKUP and WT_CONFLICT_DHANDLE", "[drop_conflict]")
{
    ...
    {
        connection_wrapper conn_wrapper = connection_wrapper(".", "create");
        prepare_session_and_error(&conn_wrapper, &session, &err_info, "columns=(col1,col2)");
