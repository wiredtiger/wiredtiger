  ...
  "[session_set_last_error]")
{
    ...
    {
        err_msg_content = "";
        WT_IGNORE_RET(__wt_session_set_last_error(session, 0, WT_NONE, err_msg_content));
    ...
    SECTION("Test with EINVAL error")
    {
        ...
        WT_IGNORE_RET(__wt_session_set_last_error(session, EINVAL, WT_NONE, err_msg_content));
