...
int mock_plh_open_handle(WT_PAGE_LOG *page_log, WT_SESSION *session,
  uint64_t table_id, WT_PAGE_LOG_HANDLE **plh)
{
...
int mock_terminate(WT_PAGE_LOG *page_log, WT_SESSION *session) {
    __wt_free((WT_SESSION_IMPL*)session, page_log);
    return (0);
}

int mock_plh_close(WT_PAGE_LOG_HANDLE *plh, WT_SESSION *session)
{
    WT_UNUSED(plh);
    WT_UNUSED(session);
    return (0);
...
void setup_page_log_queue(WT_SESSION_IMPL *session) {
...
TEST_CASE("Test Disaggregate configuration logic", "[disagg_config]")
{ 
    ...
    const char *disagg_cfg[] = {
        "disaggregated=(role=follower,page_log=mock)",
        NULL
    };
