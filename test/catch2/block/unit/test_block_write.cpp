  ...
  uint8_t *cookie, size_t cookie_size, const std::string &expected_str, bool data_checksum)
{
    ...
    REQUIRE(__wt_block_addr_unpack(session->get_wt_session_impl(), bm->block, cookie, cookie_size,
    ...
    // Test the read, was correct through __wt_read()
    WT_ITEM read_buf;
    WT_CLEAR(read_buf);
    ...
    // TODO: Test extent list avail bytes
    REQUIRE(bm->block->live.avail.track_size != 0);
...
TEST_CASE("Block manager: file operation write and write_size functions", "[block_write]")
{
    ...
    SECTION("Test generic write api")
    {
        ...
        validate_write_block(bm, session, &buf, addr, addr_size, test_string, false);

        // Validate data checksum.
        REQUIRE(
          bm->write(bm, session->get_wt_session_impl(), &buf, addr, &addr_size, true, false) == 0);
        validate_write_block(bm, session, &buf, addr, addr_size, test_string, true);
    ...
    SECTION("Test complex write api with same write buffer size")
    {
        ...
        for (const auto &str : test_strings) {
            ...
            validate_write_block(bm, session, &buf, addr.data(), addr_size, str, false);
    ...
    SECTION("Test os_cache_dirty_max and fh->written") {}

    SECTION("Test block->extend_size") {}

    SECTION("Test stats") {}

    SECTION("Test block->final_ckpt != NULL)") {}

    SECTION("Test spin lock")
    {
        std::string test_string("hello");
        REQUIRE(__wt_spin_trylock(session->get_wt_session_impl(), &bm->block->live_lock) == 0);
        uint8_t addr[WT_BTREE_MAX_ADDR_COOKIE];
        size_t addr_size;
        WT_ITEM buf;
        WT_CLEAR(buf);
        ...
        validate_write_block(bm, session, &buf, addr, addr_size, test_string, false);
        REQUIRE(__wt_spin_trylock(session->get_wt_session_impl(), &bm->block->live_lock) == 0);
        __wt_spin_unlock(session->get_wt_session_impl(), &bm->block->live_lock);
    }
