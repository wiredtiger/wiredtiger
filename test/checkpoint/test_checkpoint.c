...
main(int argc, char *argv[])
{
    ...
    g.failpoint_rec_split_write = false;
    ...
    while ((ch = __wt_getopt(
        ...
        case 's':
            ...
            case '8':
                ...
                break;
            case '9':
                g.failpoint_rec_split_write = true;
                break;
            default:
                return (usage());
            }
...
wt_connect(const char *config_open)
{
    ...
    if (g.evict_reposition_timing_stress || g.sweep_stress || g.failpoint_eviction_split ||
      ...
      g.failpoint_rec_split_write || g.hs_checkpoint_timing_stress ||
      g.checkpoint_slow_timing_stress) {
        testutil_snprintf(buf, sizeof(buf), ",timing_stress_for_test=[%s%s%s%s%s%s%s%s]",
          ...
          g.failpoint_rec_split_write ? "failpoint_rec_split_write" : "",
