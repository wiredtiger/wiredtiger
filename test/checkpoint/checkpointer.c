...
static int compare_cursors(WT_CURSOR *, WT_CURSOR *);
...
verify_consistency(WT_SESSION *session, wt_timestamp_t verify_ts, bool use_checkpoint)
{
    ...
    while (ret == 0) {
        ...
        for (i = 0; i < g.ntables; i++) {
            ...
            if ((ret = compare_cursors(cursors[reference_table], cursors[i])) != 0) {
 ...
 */
static int
compare_cursors(WT_CURSOR *cursor1, WT_CURSOR *cursor2)
{
    uint64_t key1, key2;
    int ret;
