...
/*
 * modify_run --
 *     Run some tests: 1. Create an initial value, a copy and a fake cursor to use with the
 *     WiredTiger routines. Generate a set of modify vectors and apply them to the item stored in
 *     the cursor using the modify apply API. Also apply the same modify vector to one of the copies
 *     using a helper routine written to test the modify API. The final value generated with the
 *     modify API and the helper routine should match.
 *
