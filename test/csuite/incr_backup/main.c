  ...
  ACTIVE_FILES *primary_active)
{
    ...
    active_files_print(primary_active, "primary list before incremental backup");
    WT_CLEAR(rbuf);
    WT_CLEAR(wbuf);
    rfd = wfd = -1;
    ...
    VERBOSE(2, " finished incremental backup: %d files, %d range copy, %d file copy\n", nfiles,
      nrange, ncopy);
    active_files_sort(&active);
    active_files_remove_missing(primary_active, &active, backup_home);

    /* Move the current active list to the primary list */
    active_files_move(primary_active, &active);
}
