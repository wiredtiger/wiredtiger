...
run_workload(uint32_t workload_iteration)
{
     ...
     * source of truth for a leader; the local turtle file can lag behind after a SIGKILL between
     * the metadata btree checkpoint and the page-log checkpoint commit (see WT-16711).
     */
    if (opts->disagg.is_enabled)
