 ...
 *     is at or below the last checkpoint timestamp, confirm all DATA_NROWS rows are present with
 *     the correct value. Slots whose data was committed past the last checkpoint are skipped
 *     because that data was never made durable.
 */
static void
  ...
  uint32_t pool_size, uint64_t last_ckpt_ts, bool *fatal)
{
    ...
    for (s = 0; s < pool_size; s++) {
        ...
        if (last_ckpt_ts > 0 && states[s].commit_ts > last_ckpt_ts)
            continue; /* Data committed after last checkpoint; not durable. */
