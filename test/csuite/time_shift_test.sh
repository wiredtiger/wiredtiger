...
if [ "$RUN_OS" = "Darwin" ]
then
    $WT_HOME_DIR/build_posix/test/csuite/test_rwlock
elif [ "$RUN_OS" = "Linux" ]
then
    if [ -z $2 ]
    then
    ...
    taskset -c $CPU_SET $WT_HOME_DIR/build_posix/test/csuite/test_rwlock
else
...
then
    ...
    LD_PRELOAD=$1 taskset -c $CPU_SET $WT_HOME_DIR/build_posix/test/csuite/test_rwlock &
fi
