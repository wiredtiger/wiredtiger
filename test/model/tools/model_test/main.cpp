...
reduce_counterexample(std::shared_ptr<model::kv_workload> workload, const std::string &home,
  const std::string &conn_config, const std::string &table_config)
{
    /* Separate the workload into sequences. */
    ...
    /*
     * Find the minimal subset that causes a failure by using an algorithm similar too binary
     * search: Remove the first half of the workload and then see if the failure happens again. If
     * it does, then that part of the workload can be eliminated. If the failure does not happen, it
     * means that the eliminated part of the workload includes something that contributes to the
     * failure. In the next iteration, try removing only half of the removed range - first the first
     * half and then the second half.
     */
...
usage(const char *progname)
{
    ...
    fprintf(stderr, "  -R         reduce the counterexample on failure\n");
