 ...
 *     Parse a comma-separated pair of numbers from a -P sub-option.
 */
static void
 ...
 *     Parse -Pe for simulated error frequency and delay.
 */
static void
 ...
 *     Parse -Pd for simulated delay frequency and duration.
 */
static void
...
/*
 * parse_random_seeds --
 *     Parse -PS for data and extra random seeds.
 */
static void
...
parse_and_set_disagg_opt(TEST_OPTS *opts)
{
    TESTUTIL_DISAGG_INIT(opts,
      /* is_enabled           */ true,
      /* key_provider         */ DISAGG_KEY_PROVIDER_PULL,
...
/*
 * parse_p_opt --
 *     Parse a -P sub-option (seeds, simulated delays, simulated errors).
 */
static int
...
testutil_parse_single_opt(TEST_OPTS *opts, int ch)
{
    ...
    case 'o': /* Number of operations */
        ...
        break;
    case 'P': /* Seed and simulated delay/error options follow */
