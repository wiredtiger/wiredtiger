...
DeflateJobHWPool & DeflateJobHWPool::instance(WT_COMPRESSOR *compressor, WT_SESSION *session)
{
...
DeflateJobHWPool::DeflateJobHWPool(WT_COMPRESSOR *compressor, WT_SESSION *session)
{
    ...
    if (initJobPool() < 0)
    {
        ...
        iaa_message(compressor, session, "DeflateJobHWPool", "initializing unsucessfully! Please check if IAA hardware support! Here run QPL software instead of hardware.", -2);
    }
    else
    {
        jobPoolEnabled = true;
    }
    printf("QPL Library Version:%s HW job Pool Enabled:%d\n", qpl_get_library_version(), jobPoolEnabled);
...
uint32_t HardwareCodecDeflate::doCompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t dest_size) const
{
    ...
    job_ptr->flags = QPL_FLAG_FIRST | QPL_FLAG_DYNAMIC_HUFFMAN | QPL_FLAG_GZIP_MODE | QPL_FLAG_LAST | QPL_FLAG_OMIT_VERIFY;
...
uint32_t HardwareCodecDeflate::doDecompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t uncompressed_size) const
{
    uint32_t job_id = 0;
    qpl_job * job_ptr = DeflateJobHWPool::instance().acquireJob(&job_id);
    if (job_ptr == nullptr)
...
uint32_t SoftwareCodecDeflate::doCompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t dest_size)
{
    qpl_status status;
...
uint32_t SoftwareCodecDeflate::doDecompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t uncompressed_size)
{
    qpl_status status;
    qpl_job * job_ptr = getJobCodecPtr();
...
uint32_t CompressionCodecDeflate::doCompressData(uint8_t * source, uint32_t source_size, uint8_t * dest,  uint32_t dest_size) const
{
    uint32_t res = 0;
...
void CompressionCodecDeflate::doDecompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t uncompressed_size, uint32_t* result_lenp) const
{
    uint32_t res = 0;
    if(hwCodec->hwEnabled)
