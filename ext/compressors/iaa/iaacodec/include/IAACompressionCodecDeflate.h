...
public:
    ...
    ~DeflateJobHWPool();
    static DeflateJobHWPool & instance(WT_COMPRESSOR *compressor = nullptr, WT_SESSION *session = nullptr);
    ...
    inline bool jobPoolReady()
    {
        return jobPoolEnabled;
    }
    inline qpl_job * acquireJob(uint32_t * job_id)
    {
        if(jobPoolEnabled)
        {
            uint32_t retry = 0;
...
private:
    ...
    inline int32_t get_job_size_helper()
    {
    ...
    inline int32_t init_job_helper(qpl_job * qpl_job_ptr)
    {
    ...
    inline bool tryLockJob(size_t index)
    {
        bool expected = false;
        return jobLock[index].compare_exchange_strong(expected, true);
    }

    inline void destroyJobPool()
    {
...
public:
    ...
    uint32_t doCompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t dest_size) const ;
    uint32_t doDecompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t uncompressed_size) const ;
};
...
public:
    ...
    uint32_t doCompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t dest_size) const ;
    void doDecompressData(uint8_t * source, uint32_t source_size, uint8_t * dest, uint32_t uncompressed_size, uint32_t* result_lenp=NULL) const ;
