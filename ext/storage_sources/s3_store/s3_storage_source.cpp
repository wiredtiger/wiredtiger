...
static int S3GetDirectory(const S3_STORAGE *, const std::string &, const std::string &, bool, std::string &);
 ...
 */
static int
S3GetDirectory(const S3_STORAGE *s3, const std::string &home, const std::string &name, bool create, std::string &copy)
{
    copy = "";
...
S3FileClose(WT_FILE_HANDLE *fileHandle, WT_SESSION *session)
{
    ...
    if (wtFileHandle != NULL) {
        ...
        if ((ret = wtFileHandle->close(wtFileHandle, session)) != 0)
            s3->log->LogVerboseMessage(-3, "S3FileClose: close file handle failed.");
    }
  ...
  WT_FS_OPEN_FILE_TYPE fileType, uint32_t flags, WT_FILE_HANDLE **fileHandlePtr)
{
    ...
    *fileHandlePtr = NULL;
    ...
    if (fileType != WT_FS_OPEN_FILE_TYPE_DATA && fileType != WT_FS_OPEN_FILE_TYPE_REGULAR) {
        s3->log->LogVerboseMessage(-3, "S3Open: only data file and regular types supported.");
        return (EINVAL);
    }

    if ((s3FileHandle = (S3_FILE_HANDLE *)calloc(1, sizeof(S3_FILE_HANDLE))) == NULL) {
        s3->log->LogVerboseMessage(-3, "S3Open: unable to allocate memory for file handle.");
        return (ENOMEM);
    }
    ...
    if (!LocalFileExists(cachePath)) {
        ...
        if ((ret = fs->connection->GetObject(name, cachePath)) != 0) {
            s3->log->LogVerboseMessage(-3, "S3Open: GetObject request to S3 failed.");
            return (ret);
        }
    }
    ...
    if (ret != 0) {
        s3->log->LogVerboseMessage(-3, "S3Open: fs_open_file failed.");
        return (ret);
    }
    ...
    if (fileHandle->name == NULL) {
        s3->log->LogVerboseMessage(-3, "S3Open: unable to allocate memory for object name.");
        return (ENOMEM);
    }
  ...
  const char *authToken, const char *config, WT_FILE_SYSTEM **fileSystem)
{
    ...
    if (bucketName == NULL || strlen(bucketName) == 0) {
        s3->log->LogVerboseMessage(-3, "S3CustomizeFileSystem: bucket not specified.");
        return (EINVAL);
    }
    ...
    if ((ret = s3->wtApi->config_get_string(
        ...
        s3->log->LogVerboseMessage(-3, "S3CustomizeFileSystem: error parsing config for object prefix.");
        return (1);
    }
    ...
    if ((fs = (S3_FILE_SYSTEM *)calloc(1, sizeof(S3_FILE_SYSTEM))) == NULL) {
        s3->log->LogVerboseMessage(
          -3, "S3CustomizeFileSystem: unable to allocate memory for file system.");
        return (ENOMEM);
    }
    fs->storage = s3;
  ...
  const char *prefix, char ***objectList, uint32_t *count)
{
    ...
    if ((ret = fs->connection->ListObjects(completePrefix, objects)) != 0) {
        s3->log->LogVerboseMessage(-3, "S3ObjectList: ListObjects request to S3 failed.");
        return (ret);
    }
    *count = objects.size();
  ...
  const char *prefix, char ***objectList, uint32_t *count)
{
    ...
    if ((ret = fs->connection->ListObjects(completePrefix, objects, 1, true)) != 0) {\
        s3->log->LogVerboseMessage(-3, "S3ObjectListSingle: ListObjects request to S3 failed.");
        return (ret);
    }
...
static int
S3ObjectListAdd(
  const S3_STORAGE *s3, char ***objectList, const std::vector<std::string> &objects, const uint32_t count)
{
    char **entries;
    ...
    for (int i = 0; i < count; i++) {
        if ((entries[i] = strdup(objects[i].c_str())) == NULL) {
            s3->log->LogVerboseMessage(-3, "S3ObjectListAdd: unable to allocate memory for object.");
            return (ENOMEM);
        }
  ...
  const char *source, const char *object, const char *config)
{
    ...
    if (ret = (fs->connection->PutObject(object, source)) != 0)
        s3->log->LogVerboseMessage(-3, "S3Flush: PutObject request to S3 failed.");
...
{
    s3->log->LogVerboseMessage(1, "S3 list objects count: " + std::to_string(s3->statistics.listObjectsCount));
    s3->log->LogVerboseMessage(1, "S3 put object count: " + std::to_string(s3->statistics.putObjectCount));
    s3->log->LogVerboseMessage(1, "S3 get object count: " + std::to_string(s3->statistics.getObjectCount));
    s3->log->LogVerboseMessage(1, "S3 object exists count: " + std::to_string(s3->statistics.objectExistsCount));

    s3->log->LogVerboseMessage(1, "Non read/write file handle operations: " + std::to_string(s3->statistics.fhOps));
    s3->log->LogVerboseMessage(1, "File handle read operations: " + std::to_string(s3->statistics.fhReadOps));
}
