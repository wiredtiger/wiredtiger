...
public:
    ...
    int
    put(uint64_t tableId, uint64_t pageId, uint64_t checkpointId,
      const WT_PAGE_LOG_PUT_ARGS &putArgs, const WT_ITEM &buf)
    {
        if (!available())
            return 0;
        ...
        auto cacheItem =
          std::make_unique<CacheItemContent>(CacheItemContent{putArgs.lsn, putArgs.backlink_lsn,
            putArgs.base_lsn, checkpointId, checkpointId, putArgs.delta_count, {}});
