...
namespace {
    ...
    char* write_ascii_chars(char* buf_ptr, std::span<const uint8_t> line_data) {
    ...
    bool check_buffer_space(char*& buf_ptr, char* buffer_start, size_t current_offset, size_t total_size) {
        ...
        if (used_space + LINE_BUFFER_SIZE > BUFFER_SIZE) {
            ...
            auto result = std::format_to_n(buf_ptr, remaining_space,
...
struct std::formatter<Config> {
    ...
    auto format(const Config& cfg, format_context& ctx) const {
        return std::format_to(ctx.out(),
            ...
            cfg.cache_size_mb, cfg.delay_ms, cfg.error_ms, cfg.force_delay,
            cfg.force_error, cfg.materialization_delay_ms, cfg.last_materialized_lsn,
            cfg.verbose, cfg.verbose_msg, cfg.sql_trace);
    }
};
...
static int safe_call(WT_SESSION* sess, S* api, MemberFunc func, Args&&... args) {
    if (!api) {
        return EINVAL;
    }

    session(sess);
    std::unique_ptr<WT_SESSION, std::function<decltype(session)>> reset_session{
        nullptr, &session };
...
class SQLiteCall {
    ...
    static
    consteval auto make_log_format(const char (&prefix)[K]) {
    ...
    std::string trace_sqlite3_call(R ret, Args&&... args) {
        ...
        auto fargs = std::make_format_args(func_name, ret, err_cstr,
            ext_err, err_msg, args...);
...
public:
    ...
    decltype(auto) exec(Func&& c_func, Args&&... args) {
    ...
    void enforce(auto ret, const std::string& error_msg) {
        ...
        static_assert(std::ranges::none_of(sqlite2errno,
            [](std::errc e) { return static_cast<int>(e) == 0; }),
...
enum class GlobalKey : uint64_t {
    LSN = 0,
    CHECKPOINT_COMPLETED = 1,
    CHECKPOINT_STARTED = 2
};
...
private:
    ...
    static uint64_t jitter_ms(uint32_t avg_ms) {
...
public:
    ...
    void simulate_unstable_network() {
        ...
        auto simulate = [this, ops](uint32_t every, uint32_t avg_ms, bool is_error) {
            ...
            LOG_TRACE("Artificial {} {} ms after {} object reads, {} object writes",
                is_error ? "error (sleep)" : "delay", msec,
                object_gets.load(), object_puts.load());
...
#define SQ_CHECK(func, ...) SQL_CALL_CHECK(conn.db_instance(), func, __VA_ARGS__)
    ...
    int get_page_ids(Connection& conn, uint64_t checkpoint_lsn, uint64_t table_id, WT_ITEM* page_ids, size_t* page_count) {
        ...
        SQ_CHECK(sqlite3_bind_int64, stmt.get(), 4, static_cast<sqlite3_int64>(WT_PAGE_LOG_DISCARDED));
...
public:
    ...
    int put(uint64_t page_id, uint64_t checkpoint_id,
        WT_PAGE_LOG_PUT_ARGS* args, const WT_ITEM* buf) {
        ...
        LOG_DIAG("Put page_id={} at lsn={}, backlink_lsn={}, base_lsn={}, "
    ...
    int get(uint64_t page_id, uint64_t checkpoint_id,
        ...
        LOG_DIAG("Get page_id={} at lsn={}, returned {} entries, "
            ...
            page_id, args->lsn, *results_count,
            args->backlink_lsn, args->base_lsn, flags);
        return 0;
    }
    ...
    int discard(uint64_t page_id, uint64_t checkpoint_id,
        WT_PAGE_LOG_DISCARD_ARGS* args) {
        storage.simulate_unstable_network();
        Storage::Transaction txn = storage.begin_transaction();
        uint64_t lsn = storage.get_global(txn.conn, GlobalKey::LSN);
        ...
        LOG_DEBUG("Discard page_id={} at lsn={}, backlink_lsn={}, base_lsn={}",
            page_id, lsn, args->backlink_lsn, args->base_lsn);
        return 0;
    }
...
public:
    ...
    int begin_checkpoint(uint64_t checkpoint_id) {
    ...
    int complete_checkpoint_ext(uint64_t checkpoint_id,
        ...
        LOG_DEBUG("checkpoint_id={}, timestamp={}, lsn={}",
            checkpoint_id, checkpoint_timestamp, lsn);
    ...
    int get_complete_checkpoint_ext(uint64_t* checkpoint_lsn,
        ...
        LOG_DEBUG("checkpoint_lsn={}, timestamp={}",
            checkpoint_lsn ? *checkpoint_lsn : 0,
    ...
    int open_handle(uint64_t table_id, WT_PAGE_LOG_HANDLE **plh) {
        if (!plh) {
 ...
 * otherwise it will conflict with other builtin libraries.
 */
