...
| `scenarios` | `_build_ops` / `Weights` | Add injected-scenario ops: `forced_evict`, `bulk_remove`, `massive_prepare`, `truncate`. Each = one `op_<name>` method + one `Op` row + one `Weights` field. | open |
| `workload-tuning` | `Weights` | The weights are a rough first pass. Revisit once the long run lands; in particular decide (D1) whether to reintroduce a derived "break fraction" knob (the old `P_BREAK`), now that break frequency is implicit in the weights. | open |
