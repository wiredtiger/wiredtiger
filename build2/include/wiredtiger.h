 ...
 * Version information
 *******************************************/
#define WIREDTIGER_VERSION_MAJOR    12
#define WIREDTIGER_VERSION_MINOR    0
#define WIREDTIGER_VERSION_PATCH    0
#define WIREDTIGER_VERSION_STRING   "WiredTiger 12.0.0: (November 15, 2024)"
...
struct __wt_page_log_complete_checkpoint_args;
typedef struct __wt_page_log_complete_checkpoint_args WT_PAGE_LOG_COMPLETE_CHECKPOINT_ARGS;
struct __wt_page_log_get_complete_checkpoint_args;
typedef struct __wt_page_log_get_complete_checkpoint_args WT_PAGE_LOG_GET_COMPLETE_CHECKPOINT_ARGS;
...
struct __wt_crypt_keys;
typedef struct __wt_crypt_keys WT_CRYPT_KEYS;
...
typedef enum {
    ...
    WT_TS_TXN_TYPE_ROLLBACK, /*!< Rollback timestamp. */
} WT_TS_TXN_TYPE;
...
struct __wt_session {
     ...
     * Publish a table.
     *
     * Make the table visible for connections that support publishing schema operations, which is
     * currently just disaggregated storage.  The effects of schema operations performed on a table
     ...
     *
     * @param session the session handle
     * @param name the URI of the object to publish, such as \c "table:stock"
     * @configstart{WT_SESSION.publish, see dist/api_data.py}
     * @config{disaggregated = (, configure disaggregated storage options for this object., a set of
     * related configuration options defined as follows.}
     * @config{&nbsp;&nbsp;&nbsp;&nbsp;
     * schema_epoch, set the schema epoch for publishing schema operations for this object., a
     * string; default empty.}
     * @config{ ),,}
     * @configend
     * @ebusy_errors
    ...
    /*!
     * Set a prepared_id on a transaction.
     *
     ...
     * @errors
     */
    int __F(prepared_id_transaction)(WT_SESSION *session, const char *config);

    /*!
     * Set a prepared id on a transaction numerically.  Prefer this method over
     ...
     * @errors
     */
    int __F(prepared_id_transaction_uint)(WT_SESSION *session, uint64_t prepared_id);
    /*! @} */
...
struct __wt_connection {
     ...
     * @config{debug = (, configure debug specific behavior on connection close.  Generally only
     * used for internal testing purposes., a set of related configuration options defined as
     * follows.}
     * @config{&nbsp;&nbsp;&nbsp;&nbsp;skip_checkpoint, Skips the checkpoint during
     * shutdown., a boolean flag; default \c false.}
     * @config{ ),,}
     ...
     * "compact_progress"\, \c "configuration"\, \c "cross_checkpoint_cache"\, \c
     * "disaggregated_storage"\, \c "error_returns"\, \c "eviction"\, \c "extension"\, \c
     * "fileops"\, \c "generation"\, \c "handleops"\, \c "history_store"\, \c
     ...
     * "prefetch"\, \c "read"\, \c "reconcile"\, \c "recovery"\, \c "recovery_progress"\, \c "rts"\,
     * \c "salvage"\, \c "shared_cache"\, \c "split"\, \c "sweep"\, \c "temporary"\, \c
     ...
     * timestamp of the most recent stable checkpoint; \c last_disaggregated_schema_epoch returns
     * the schema epoch from the most recent disaggregated storage checkpoint; \c oldest_timestamp
     ...
     * returns the minimum of the \c oldest_timestamp and the read timestamps of all active readers;
     * \c recovery returns the timestamp of the most recent stable checkpoint taken prior to a
     * shutdown; \c stable_disaggregated_schema_epoch returns the most recent \c
     * stable_disaggregated_schema_epoch set with WT_CONNECTION::set_timestamp; \c stable_timestamp
     ...
     * "last_disaggregated_schema_epoch"\, \c "oldest"\, \c "oldest_reader"\, \c
     * "oldest_timestamp"\, \c "pinned"\, \c "recovery"\, \c "stable"\, \c
     * "stable_disaggregated_schema_epoch"\, \c "stable_timestamp"; default \c all_durable.}
     * @configend
     *
     * A timestamp of 0 is returned if the timestamp is not available or has not been set.
     * @errors
     ...
     * @config{stable_disaggregated_schema_epoch, set the stable schema epoch for disaggregated
     * storage; the shared metadata included in the disaggregated storage checkpoint will not
     * include the effects of schema operations with higher epochs.  Values must be monotonically
     * increasing.  See @ref timestamp_global_api., a string; default empty.}
     ...
     * @errors
     */
    int __F(dump_error_log)(WT_CONNECTION *connection);
    ...
    int __F(set_key_provider)(
        WT_CONNECTION *connection, WT_KEY_PROVIDER *km, const char *config);

    /*!
     * Return the key provider registered with WT_CONNECTION::set_key_provider, or \c EINVAL if
     * none has been configured.
     *
     * @param connection the connection handle
     * @param key_providerp the returned key provider structure
     * @errors
     */
    int __F(get_key_provider)(WT_CONNECTION *connection, WT_KEY_PROVIDER **key_providerp);
#endif
 ...
 * boolean flag; default \c true.}
 * @config{checkpoint_threads, the number of checkpoint threads for syncing tables in parallel
 * during checkpoints., an integer greater than or equal to \c 1; default \c 1.}
 * @config{compatibility = (, set compatibility version of database.  Changing the compatibility
 * version requires that there are no active operations for the duration of the call., a set of
 * related configuration options defined as follows.}
 * @config{&nbsp;&nbsp;&nbsp;&nbsp;release,
 ...
 * @config{extensions_strict, if true\, fail ::wiredtiger_open with \c EINVAL when an early-loaded
 * extension recorded in \c WiredTiger.basecfg is not passed in the open configuration.  The default
 * is to log a warning and continue with the extension absent.  See @ref extensions_loadable., a
 * boolean flag; default \c false.}
 * @config{extra_diagnostics, enable additional diagnostics in WiredTiger.  These additional
 * diagnostics include diagnostic assertions that can cause WiredTiger to abort when an invalid
 * state is detected.  Options are given as a list\, such as
 ...
 * "configuration"\, \c "cross_checkpoint_cache"\, \c "disaggregated_storage"\, \c "error_returns"\,
 * \c "eviction"\, \c "extension"\, \c "fileops"\, \c "generation"\, \c "handleops"\, \c
 ...
 * This sub-level error indicates that an operation or configuration conflicts
...
struct __wt_file_system {
    ...
    /*!
     * Return the free disk space for the filesystem.
     *
     * @errors
     *
     * @param file_system the WT_FILE_SYSTEM
     ...
     */
    int (*fs_free_space)(WT_FILE_SYSTEM *file_system,
        WT_SESSION *session, const char *path, wt_off_t *freep);
...
/*!
 * The maximum page log LSN, which can be used by some calls to identify the most recent log entry.
 */
#define WT_PAGE_LOG_LSN_MAX UINT64_MAX

/*!
 * LSN used by Server during node step up process to avoid evicting pages.
 */
#define WT_DISAGG_START_LSN ((uint64_t)1 << 32)
...
struct __wt_page_log {
     ...
     * @param session the current WiredTiger session
     */
    int (*pl_abandon_checkpoint)(WT_PAGE_LOG *page_log, WT_SESSION *session);
    ...
    int (*pl_trim_table)(WT_PAGE_LOG *page_log, WT_SESSION *session, uint64_t table_id,
        uint64_t start_lsn, uint64_t* lsnp);
...
struct __wt_crypt_keys {
    ...
    uint64_t timestamp; /* Timestamp associated with the keys. */
};
 ...
 */
struct __wt_key_provider {
    /*!
     * Loads the current persisted key during checkpoint load.
     *
    ...
    /*!
     * Set the key to be used for subsequent checkpoint writes.
     *
     * The module calls this to hand WiredTiger the next key, along with the
     * timestamp the key is associated with. A checkpoint will include the most
     * recent key with a timestamp less than or equal to the checkpoint's
     * timestamp.
     *
     * WiredTiger copies the key data; the module may free its buffer on return.
     *
     * Returns an error if the timestamp is not greater than the global stable
     * timestamp, or not greater than the timestamp of any previous set_key call.
     *
     * This call is not transactional: it completes independently of any
     * surrounding transaction on the session.
     *
     * @param kp the WT_KEY_PROVIDER instance.
     * @param session a session from the associated WT_CONNECTION; consecutive
     *        calls need not use the same session.
     * @param crypt the WT_CRYPT_KEYS holding the new key.
     */
    int (*set_key)(WT_KEY_PROVIDER *kp, WT_SESSION *session, const WT_CRYPT_KEYS *crypt);
 ...
 */
typedef enum {
    WT_BTREE_STORAGE_TIER_NONE = 0, /*!< No storage tier specified (default). */
    WT_BTREE_STORAGE_TIER_COLD = 1  /*!< Cold storage tier. */
} WT_BTREE_STORAGE_TIER;

/*! @name Page log flags */
/*
* NOTE: These flags are passed through the API and may be persisted or consumed
* by external components. Their numeric values must remain
* stable for compatibility. Do not reorder or renumber these flags.
*
* The range 0x08-0x20 is reserved for storage tiers. Start next flag from the value 0x40.
*/
/*! page is compressed */
#define WT_PAGE_LOG_COMPRESSED 0x1u
/*! page is a delta page */
...
struct __wt_page_log_complete_checkpoint_args {
        ...
        uint64_t checkpoint_oldest_timestamp;
...
/*!
 * Values given to, or returned by the WT_PAGE_LOG_HANDLE::plh_discard interface.
 */
struct __wt_page_log_discard_args {
       /*
        * Input arguments
       ...
       /*
        * Output arguments, returned by the call. Not used for now.
        */
       uint64_t lsn;
};
...
struct __wt_page_log_handle {
    ...
    /*!
     * Get all live page IDs from the paging/logging service for a given checkpoint.
     *
     * @errors
     *
     * @param plh the WT_PAGE_LOG_HANDLE
     ...
     * @param args additional arguments used or returned by the call, memory owned by caller
     */
    int (*plh_discard)(WT_PAGE_LOG_HANDLE *plh, WT_SESSION *session,
        uint64_t page_id, uint64_t checkpoint_id, WT_PAGE_LOG_DISCARD_ARGS *args);
...
/*!
 * background-compact: background compact skipped file, it is part of the
 * exclude list
 */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SKIPPED_EXCLUDE	1008
/*!
 * background-compact: background compact skipped file, it is smaller
 * than 1MB in size
 */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SKIPPED_SMALL_FILE	1009
/*!
 * background-compact: background compact skipped file, not meeting
 * requirements for compaction
 */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SKIPPED		1010
/*!
 * background-compact: background compact skipped, last compact was
 * unsuccessful/less successful than average
 */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SKIPPED_UNSUCCESSFUL	1011
/*! background-compact: background compact skipped, no such file exists */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SKIPPED_NO_SUCH_FILE	1012
/*!
 * background-compact: background compact skipped, there is a permissions
 * issue
 */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SKIPPED_MISSING_PERMISSIONS	1013
/*! background-compact: background compact sleeps due to cache pressure */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SLEEP_CACHE_PRESSURE	1014
/*! background-compact: background compact successful calls */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_SUCCESS		1015
/*! background-compact: background compact timeout */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_TIMEOUT		1016
/*! background-compact: number of files tracked by background compaction */
#define	WT_STAT_CONN_BACKGROUND_COMPACT_FILES_TRACKED	1017
/*! backup: backup cursor open */
#define	WT_STAT_CONN_BACKUP_CURSOR_OPEN			1018
/*! backup: backup duplicate cursor open */
#define	WT_STAT_CONN_BACKUP_DUP_OPEN			1019
/*! backup: backup granularity size */
#define	WT_STAT_CONN_BACKUP_GRANULARITY			1020
/*! backup: backup total bits cleared */
#define	WT_STAT_CONN_BACKUP_BITS_CLR			1021
/*! backup: incremental backup enabled */
#define	WT_STAT_CONN_BACKUP_INCREMENTAL			1022
/*! backup: opening the backup cursor in progress */
#define	WT_STAT_CONN_BACKUP_START			1023
/*! backup: total modified incremental blocks */
#define	WT_STAT_CONN_BACKUP_BLOCKS			1024
/*! backup: total modified incremental blocks with compressed data */
#define	WT_STAT_CONN_BACKUP_BLOCKS_COMPRESSED		1025
/*! backup: total modified incremental blocks without compressed data */
#define	WT_STAT_CONN_BACKUP_BLOCKS_UNCOMPRESSED		1026
/*! block-cache: cached blocks updated */
#define	WT_STAT_CONN_BLOCK_CACHE_BLOCKS_UPDATE		1027
/*! block-cache: cached bytes updated */
#define	WT_STAT_CONN_BLOCK_CACHE_BYTES_UPDATE		1028
/*!
 * block-cache: cold collection pages not added to the disaggregated
 * victim cache during eviction
 */
#define	WT_STAT_CONN_BLOCK_CACHE_COLD_NOT_CACHED	1029
/*! block-cache: evicted blocks */
#define	WT_STAT_CONN_BLOCK_CACHE_BLOCKS_EVICTED		1030
...
#define	WT_STAT_CONN_DISAGG_BLOCK_PLH_PUT_FAILED	1060
/*! block-disagg: Disaggregated block manager page discard calls */
#define	WT_STAT_CONN_DISAGG_BLOCK_PAGE_DISCARD		1061
/*! block-disagg: Disaggregated block manager put */
#define	WT_STAT_CONN_DISAGG_BLOCK_PUT			1062
/*! block-disagg: Disaggregated block manager put cold page */
#define	WT_STAT_CONN_DISAGG_BLOCK_PUT_COLD		1063
/*!
 * block-disagg: Disaggregated block manager put to the shared history
 * store in SLS
 */
#define	WT_STAT_CONN_DISAGG_BLOCK_HS_PUT		1064
/*!
 * block-disagg: Disaggregated block manager read ahead of
 * materialization frontier
 */
#define	WT_STAT_CONN_DISAGG_BLOCK_READ_AHEAD_FRONTIER	1065
/*! block-manager: blocks pre-loaded */
#define	WT_STAT_CONN_BLOCK_PRELOAD			1066
/*! block-manager: blocks read */
#define	WT_STAT_CONN_BLOCK_READ				1067
/*! block-manager: blocks written */
#define	WT_STAT_CONN_BLOCK_WRITE			1068
/*! block-manager: bytes read */
#define	WT_STAT_CONN_BLOCK_BYTE_READ			1069
/*! block-manager: bytes read for internal pages */
#define	WT_STAT_CONN_BLOCK_BYTE_READ_INTL		1070
/*!
 * block-manager: bytes read for internal pages before decompression and
 * decryption
 */
#define	WT_STAT_CONN_BLOCK_BYTE_READ_INTL_DISK		1071
/*! block-manager: bytes read for leaf pages */
#define	WT_STAT_CONN_BLOCK_BYTE_READ_LEAF		1072
/*!
 * block-manager: bytes read for leaf pages before decompression and
 * decryption
 */
#define	WT_STAT_CONN_BLOCK_BYTE_READ_LEAF_DISK		1073
/*! block-manager: bytes read via memory map API */
#define	WT_STAT_CONN_BLOCK_BYTE_READ_MMAP		1074
/*! block-manager: bytes read via system call API */
#define	WT_STAT_CONN_BLOCK_BYTE_READ_SYSCALL		1075
/*!
 * block-manager: bytes saved from being written when using internal page
 * deltas
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_SAVED_DELTA_INTL	1076
/*!
 * block-manager: bytes saved from being written when using leaf page
 * deltas
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_SAVED_DELTA_LEAF	1077
/*! block-manager: bytes written */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE			1078
/*! block-manager: bytes written by compaction */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_COMPACT		1079
/*! block-manager: bytes written for checkpoint */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_CHECKPOINT	1080
/*!
 * block-manager: bytes written for internal pages after compression and
 * encryption
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL_DISK		1081
/*!
 * block-manager: bytes written for internal pages before compression and
 * encryption
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL		1082
/*!
 * block-manager: bytes written for leaf pages after compression and
 * encryption
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF_DISK		1083
/*!
 * block-manager: bytes written for leaf pages before compression and
 * encryption
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF		1084
/*! block-manager: bytes written via memory map API */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_MMAP		1085
/*! block-manager: bytes written via system call API */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_SYSCALL		1086
/*! block-manager: mapped blocks read */
#define	WT_STAT_CONN_BLOCK_MAP_READ			1087
/*! block-manager: mapped bytes read */
#define	WT_STAT_CONN_BLOCK_BYTE_MAP_READ		1088
/*!
 * block-manager: number of internal page deltas written that were
 * between 0-20 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL_DELTA_LT20	1089
/*!
 * block-manager: number of internal page deltas written that were
 * between 20-40 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL_DELTA_LT40	1090
/*!
 * block-manager: number of internal page deltas written that were
 * between 40-60 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL_DELTA_LT60	1091
/*!
 * block-manager: number of internal page deltas written that were
 * between 60-80 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL_DELTA_LT80	1092
/*!
 * block-manager: number of internal page deltas written that were
 * between 80-100 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL_DELTA_LT100	1093
/*!
 * block-manager: number of internal page deltas written that were
 * greater than the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_INTL_DELTA_GT100	1094
/*!
 * block-manager: number of leaf page deltas written that were between
 * 0-20 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF_DELTA_LT20	1095
/*!
 * block-manager: number of leaf page deltas written that were between
 * 20-40 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF_DELTA_LT40	1096
/*!
 * block-manager: number of leaf page deltas written that were between
 * 40-60 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF_DELTA_LT60	1097
/*!
 * block-manager: number of leaf page deltas written that were between
 * 60-80 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF_DELTA_LT80	1098
/*!
 * block-manager: number of leaf page deltas written that were between
 * 80-100 percent the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF_DELTA_LT100	1099
/*!
 * block-manager: number of leaf page deltas written that were greater
 * than the size of the full image
 */
#define	WT_STAT_CONN_BLOCK_BYTE_WRITE_LEAF_DELTA_GT100	1100
/*!
 * block-manager: number of times the file was remapped because it
 * changed size via fallocate or truncate
 */
#define	WT_STAT_CONN_BLOCK_REMAP_FILE_RESIZE		1101
/*! block-manager: number of times the region was remapped via write */
#define	WT_STAT_CONN_BLOCK_REMAP_FILE_WRITE		1102
/*!
 * block-manager: time spent(usecs) on the most recent linear walk of
 * extents during first-fit allocation
 */
#define	WT_STAT_CONN_BLOCK_FIRST_SRCH_WALK_TIME		1103
/*! cache: application requested eviction interrupt */
#define	WT_STAT_CONN_EVICTION_INTERUPTED_BY_APP		1104
/*! cache: application thread time evicting (usecs) */
#define	WT_STAT_CONN_EVICTION_APP_TIME			1105
/*!
 * cache: application threads eviction requested with cache fill ratio <
 * 25%
 */
#define	WT_STAT_CONN_CACHE_EVICTION_APP_THREADS_FILL_RATIO_LT_25	1106
/*!
 * cache: application threads eviction requested with cache fill ratio >=
 * 25% and < 50%
 */
#define	WT_STAT_CONN_CACHE_EVICTION_APP_THREADS_FILL_RATIO_25_50	1107
/*!
 * cache: application threads eviction requested with cache fill ratio >=
 * 50% and < 75%
 */
#define	WT_STAT_CONN_CACHE_EVICTION_APP_THREADS_FILL_RATIO_50_75	1108
/*!
 * cache: application threads eviction requested with cache fill ratio >=
 * 75%
 */
#define	WT_STAT_CONN_CACHE_EVICTION_APP_THREADS_FILL_RATIO_GT_75	1109
/*!
 * cache: application threads eviction skip page with updates or dirty
 * page
 */
#define	WT_STAT_CONN_CACHE_EVICTION_APP_THREADS_SKIP_UPDATES_DIRTY_PAGE	1110
/*! cache: application threads page read from disk to cache count */
#define	WT_STAT_CONN_CACHE_READ_APP_COUNT		1111
/*! cache: application threads page read from disk to cache time (usecs) */
#define	WT_STAT_CONN_CACHE_READ_APP_TIME		1112
/*! cache: application threads page write from cache to disk count */
#define	WT_STAT_CONN_CACHE_WRITE_APP_COUNT		1113
/*! cache: application threads page write from cache to disk time (usecs) */
#define	WT_STAT_CONN_CACHE_WRITE_APP_TIME		1114
/*! cache: bytes allocated for updates */
#define	WT_STAT_CONN_CACHE_BYTES_UPDATES		1115
/*! cache: bytes allocated for updates from the ingest btrees */
#define	WT_STAT_CONN_CACHE_BYTES_UPDATES_INGEST		1116
/*! cache: bytes allocated for updates from the stable btrees */
#define	WT_STAT_CONN_CACHE_BYTES_UPDATES_STABLE		1117
/*! cache: bytes belonging to page images in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_IMAGE			1118
/*!
 * cache: bytes belonging to page images in the cache from the ingest
 * btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_IMAGE_INGEST		1119
/*!
 * cache: bytes belonging to page images in the cache from the stable
 * btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_IMAGE_STABLE		1120
/*! cache: bytes belonging to the history store table in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_HS			1121
/*! cache: bytes currently in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_INUSE			1122
/*! cache: bytes dirty in the cache cumulative */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_TOTAL		1123
/*! cache: bytes not belonging to page images in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_OTHER			1124
/*! cache: bytes read into cache */
#define	WT_STAT_CONN_CACHE_BYTES_READ			1125
/*! cache: bytes written from cache */
#define	WT_STAT_CONN_CACHE_BYTES_WRITE			1126
/*! cache: cache tolerance configured */
#define	WT_STAT_CONN_CACHE_TOLERANCE_LEVEL		1127
/*! cache: checkpoint blocked page eviction */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_CHECKPOINT	1128
/*!
 * cache: checkpoint of history store file blocked non-history store page
 * eviction
 */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_CHECKPOINT_HS	1129
/*! cache: dirty bytes belonging to the history store table in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_HS_DIRTY		1130
/*! cache: dirty internal page cannot be evicted in disaggregated storage */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_DISAGG_DIRTY_INTERNAL_PAGE	1131
/*! cache: evict page attempts by eviction server */
#define	WT_STAT_CONN_EVICTION_SERVER_EVICT_ATTEMPT	1132
/*! cache: evict page attempts by eviction worker threads */
#define	WT_STAT_CONN_EVICTION_WORKER_EVICT_ATTEMPT	1133
/*! cache: evict page failures by eviction server */
#define	WT_STAT_CONN_EVICTION_SERVER_EVICT_FAIL		1134
/*! cache: evict page failures by eviction worker threads */
#define	WT_STAT_CONN_EVICTION_WORKER_EVICT_FAIL		1135
/*! cache: eviction calls to get a page found queue empty */
#define	WT_STAT_CONN_EVICTION_GET_REF_EMPTY		1136
/*! cache: eviction calls to get a page found queue empty after locking */
#define	WT_STAT_CONN_EVICTION_GET_REF_EMPTY2		1137
/*! cache: eviction currently operating in aggressive mode */
#define	WT_STAT_CONN_EVICTION_AGGRESSIVE_SET		1138
/*! cache: eviction empty score */
#define	WT_STAT_CONN_EVICTION_EMPTY_SCORE		1139
/*!
 * cache: eviction gave up due to detecting a disk value without a
 * timestamp behind the last update on the chain
 */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_1	1140
/*!
 * cache: eviction gave up due to detecting a tombstone without a
 * timestamp ahead of the selected on disk update
 */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_2	1141
/*!
 * cache: eviction gave up due to detecting a tombstone without a
 * timestamp ahead of the selected on disk update after validating the
 * update chain
 */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_3	1142
/*!
 * cache: eviction gave up due to detecting update chain entries without
 * timestamps after the selected on disk update
 */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_4	1143
/*!
 * cache: eviction gave up due to needing to remove a record from the
 * history store but checkpoint is running
 */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_REMOVE_HS_RACE_WITH_CHECKPOINT	1144
/*! cache: eviction gave up due to no progress being made */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_NO_PROGRESS	1145
/*! cache: eviction passes of a file */
#define	WT_STAT_CONN_EVICTION_WALK_PASSES		1146
/*! cache: eviction server candidate queue empty when topping up */
#define	WT_STAT_CONN_EVICTION_QUEUE_EMPTY		1147
/*! cache: eviction server candidate queue not empty when topping up */
#define	WT_STAT_CONN_EVICTION_QUEUE_NOT_EMPTY		1148
/*! cache: eviction server completed walks of all dhandles */
#define	WT_STAT_CONN_EVICTION_DHANDLE_COMPLETE_WALK	1149
/*!
 * cache: eviction server push pages to queue failed because of racing
 * with setting flag
 */
#define	WT_STAT_CONN_EVICTION_SERVER_PUSH_PAGES_FAILED_WHEN_FLAGING	1150
/*!
 * cache: eviction server skipped the internal pages if eviction is not
 * in aggressive mode
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_INTL_PAGE_NON_AGGRESSIVE	1151
/*! cache: eviction server skipped the pages already in the urgent queue */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_PAGES_ALREADY_IN_URGENT_QUEUE	1152
/*! cache: eviction server skipped the pages when prefetching */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_PREFETCHED	1153
/*! cache: eviction server skipped the root pages */
#define	WT_STAT_CONN_EVICTION_ROOT_PAGES_SKIPPED	1154
/*!
 * cache: eviction server skips clean history store pages with updates
 * when a precise checkpoint is in progress
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_HISTORY_STORE_PAGES_WITH_UPDATES_DURING_CHECKPOINT	1155
/*! cache: eviction server skips dirty pages during a running checkpoint */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_DIRTY_PAGES_DURING_CHECKPOINT	1156
/*!
 * cache: eviction server skips disaggregated trees already visited by
 * the ongoing checkpoint
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_DISAGG_TREES_CHECKPOINTED	1157
/*! cache: eviction server skips ingest btrees in disagg */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_INGEST_TREES	1158
/*! cache: eviction server skips internal pages as it has an active child */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_INTL_PAGE_WITH_ACTIVE_CHILD	1159
/*! cache: eviction server skips metadata pages with history */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_METATDATA_WITH_HISTORY	1160
/*!
 * cache: eviction server skips pages that are written with transactions
 * greater than the checkpoint timestamp
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_PAGES_CHECKPOINT_TIMESTAMP	1161
/*!
 * cache: eviction server skips pages that are written with transactions
 * greater than the last running
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_PAGES_LAST_RUNNING	1162
/*!
 * cache: eviction server skips pages that are written with transactions
 * greater than the prune timestamp
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_PAGES_PRUNE_TIMESTAMP	1163
/*!
 * cache: eviction server skips pages that have been reconciled
 * previously at the same prune timestamp
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_PAGES_PRUNE_TIMESTAMP_NOT_MOVE	1164
/*!
 * cache: eviction server skips pages that previously failed eviction and
 * likely will again
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_PAGES_RETRY	1165
/*! cache: eviction server skips pages that we do not want to evict */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_UNWANTED_PAGES	1166
/*! cache: eviction server skips stable btrees in disagg */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_STABLE_TREES	1167
/*! cache: eviction server skips tree that we do not want to evict */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_UNWANTED_TREE	1168
/*!
 * cache: eviction server skips trees because there are too many active
 * walks
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_TREES_TOO_MANY_ACTIVE_WALKS	1169
/*! cache: eviction server skips trees that are being checkpointed */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_CHECKPOINTING_TREES	1170
/*!
 * cache: eviction server skips trees that are configured to stick in
 * cache
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_TREES_STICK_IN_CACHE	1171
/*!
 * cache: eviction server skips trees that are read-only if it is not
 * looking for clean pages
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_TREES_READ_ONLY	1172
/*! cache: eviction server skips trees that disable eviction */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_TREES_EVICTION_DISABLED	1173
/*! cache: eviction server skips trees that were not useful before */
#define	WT_STAT_CONN_EVICTION_SERVER_SKIP_TREES_NOT_USEFUL_BEFORE	1174
/*!
 * cache: eviction server slept, because we did not make progress with
 * eviction
 */
#define	WT_STAT_CONN_EVICTION_SERVER_SLEPT		1175
...
#define	WT_STAT_CONN_CACHE_READ_DELETED			1317
/*! cache: pages read into cache after truncate in prepare state */
#define	WT_STAT_CONN_CACHE_READ_DELETED_PREPARED	1318
/*! cache: pages read into cache by checkpoint */
#define	WT_STAT_CONN_CACHE_READ_CHECKPOINT		1319
/*!
 * cache: pages removed from the ordinary queue to be queued for urgent
 * eviction
 */
#define	WT_STAT_CONN_EVICTION_CLEAR_ORDINARY		1320
/*! cache: pages requested from the cache due to pre-fetch */
#define	WT_STAT_CONN_CACHE_PAGES_PREFETCH		1321
/*! cache: pages requested from the cache internal */
#define	WT_STAT_CONN_CACHE_PAGES_REQUESTED_INTERNAL	1322
/*! cache: pages requested from the cache leaf */
#define	WT_STAT_CONN_CACHE_PAGES_REQUESTED_LEAF		1323
/*! cache: pages requested from the history store */
#define	WT_STAT_CONN_CACHE_PAGES_REQUESTED_HS		1324
/*! cache: pages seen by eviction walk */
#define	WT_STAT_CONN_CACHE_EVICTION_PAGES_SEEN		1325
/*! cache: pages seen by eviction walk that are already queued */
#define	WT_STAT_CONN_EVICTION_PAGES_ALREADY_QUEUED	1326
/*! cache: pages selected for eviction unable to be evicted */
#define	WT_STAT_CONN_EVICTION_FAIL			1327
/*!
 * cache: pages selected for eviction unable to be evicted because of
 * active children on an internal page
 */
#define	WT_STAT_CONN_EVICTION_FAIL_ACTIVE_CHILDREN_ON_AN_INTERNAL_PAGE	1328
/*!
 * cache: pages selected for eviction unable to be evicted because of
 * failure in reconciliation
 */
#define	WT_STAT_CONN_EVICTION_FAIL_IN_RECONCILIATION	1329
/*!
 * cache: pages selected for eviction unable to be evicted because of
 * race between checkpoint and updates without timestamps
 */
#define	WT_STAT_CONN_EVICTION_FAIL_CHECKPOINT_NO_TS	1330
/*!
 * cache: pages selected for eviction unable to be evicted belonging to
 * ingest btrees
 */
#define	WT_STAT_CONN_EVICTION_FAIL_INGEST		1331
/*! cache: pages walked for eviction */
#define	WT_STAT_CONN_EVICTION_WALK			1332
/*!
 * cache: pages with an unresolved multiblock split flagged by checkpoint
 * to be evicted soon
 */
#define	WT_STAT_CONN_CACHE_EVICTION_MULTIBLOCK_CHECKPOINT_FLAGGED	1333
/*!
 * cache: pages with an unresolved multiblock split re-reconciled by
 * checkpoint
 */
#define	WT_STAT_CONN_CACHE_EVICTION_MULTIBLOCK_SPLIT_RE_RECONCILED	1334
/*! cache: pages written from cache */
#define	WT_STAT_CONN_CACHE_WRITE			1335
/*!
 * cache: pages written requiring in-memory restoration due to invisible
 * updates
 */
#define	WT_STAT_CONN_CACHE_WRITE_RESTORE_INVISIBLE	1336
/*!
 * cache: pages written requiring in-memory restoration due to scrub
 * eviction
 */
#define	WT_STAT_CONN_CACHE_WRITE_RESTORE_SCRUB		1337
/*! cache: percentage overhead */
#define	WT_STAT_CONN_CACHE_OVERHEAD			1338
/*!
 * cache: precise checkpoint caused an eviction to be skipped because any
 * dirty content needs to remain in cache
 */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_PRECISE_CHECKPOINT	1339
/*!
 * cache: realizing in-memory split after reconciliation failed due to
 * internal lock busy
 */
#define	WT_STAT_CONN_CACHE_EVICT_SPLIT_FAILED_LOCK	1340
/*! cache: recent modification of a page blocked its eviction */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_RECENTLY_MODIFIED	1341
/*! cache: reconciled pages scrubbed and added back to the cache clean */
#define	WT_STAT_CONN_CACHE_SCRUB_RESTORE		1342
/*! cache: reverse splits performed */
#define	WT_STAT_CONN_CACHE_REVERSE_SPLITS		1343
/*!
...
#define	WT_STAT_CONN_CACHE_SHARED_DSK_HASH_SIZE		1345
/*! cache: shared disk hit */
#define	WT_STAT_CONN_CACHE_SHARED_DSK_HIT		1346
/*! cache: shared disk miss */
#define	WT_STAT_CONN_CACHE_SHARED_DSK_MISS		1347
/*! cache: shared history store cursor not cached during eviction */
#define	WT_STAT_CONN_CACHE_EVICTION_HS_SHARED_CURSOR_NOT_CACHED	1348
/*! cache: size of delta updates reconstructed on the base page */
#define	WT_STAT_CONN_CACHE_READ_DELTA_UPDATES		1349
/*! cache: size of tombstones restored when reading a page */
#define	WT_STAT_CONN_CACHE_READ_RESTORED_TOMBSTONE_BYTES	1350
/*! cache: the number of times full update inserted to history store */
#define	WT_STAT_CONN_CACHE_HS_INSERT_FULL_UPDATE	1351
/*! cache: the number of times reverse modify inserted to history store */
#define	WT_STAT_CONN_CACHE_HS_INSERT_REVERSE_MODIFY	1352
/*! cache: time eviction worker threads spend waiting for locks (usecs) */
#define	WT_STAT_CONN_EVICTION_WORKER_LOCK_WAIT_TIME	1353
/*!
 * cache: total milliseconds spent inside reentrant history store
 * evictions in a reconciliation
 */
#define	WT_STAT_CONN_EVICTION_REENTRY_HS_EVICTION_MILLISECONDS	1354
/*! cache: tracked bytes belonging to internal pages in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_INTERNAL		1355
/*!
 * cache: tracked bytes belonging to internal pages in the cache from the
 * ingest btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_INTERNAL_INGEST	1356
/*!
 * cache: tracked bytes belonging to internal pages in the cache from the
 * stable btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_INTERNAL_STABLE	1357
/*! cache: tracked bytes belonging to leaf pages in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_LEAF			1358
/*!
 * cache: tracked bytes belonging to leaf pages in the cache from the
 * ingest btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_LEAF_INGEST		1359
/*!
 * cache: tracked bytes belonging to leaf pages in the cache from the
 * stable btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_LEAF_STABLE		1360
/*! cache: tracked dirty bytes in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY			1361
/*! cache: tracked dirty bytes in the cache from the ingest btrees */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_INGEST		1362
/*! cache: tracked dirty bytes in the cache from the stable btrees */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_STABLE		1363
/*! cache: tracked dirty internal page bytes in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_INTERNAL		1364
/*!
 * cache: tracked dirty internal page bytes in the cache from the ingest
 * btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_INTERNAL_INGEST	1365
/*!
 * cache: tracked dirty internal page bytes in the cache from the stable
 * btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_INTERNAL_STABLE	1366
/*! cache: tracked dirty leaf page bytes in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_LEAF		1367
/*!
 * cache: tracked dirty leaf page bytes in the cache from the ingest
 * btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_LEAF_INGEST	1368
/*!
 * cache: tracked dirty leaf page bytes in the cache from the stable
 * btrees
 */
#define	WT_STAT_CONN_CACHE_BYTES_DIRTY_LEAF_STABLE	1369
/*! cache: tracked dirty pages in the cache */
#define	WT_STAT_CONN_CACHE_PAGES_DIRTY			1370
/*! cache: tracked dirty pages in the cache from the ingest btrees */
#define	WT_STAT_CONN_CACHE_PAGES_DIRTY_INGEST		1371
/*! cache: tracked dirty pages in the cache from the stable btrees */
#define	WT_STAT_CONN_CACHE_PAGES_DIRTY_STABLE		1372
/*! cache: uncommitted truncate blocked page eviction */
#define	WT_STAT_CONN_CACHE_EVICTION_BLOCKED_UNCOMMITTED_TRUNCATE	1373
/*! cache: unmodified pages evicted */
#define	WT_STAT_CONN_CACHE_EVICTION_CLEAN		1374
/*! cache: update bytes belonging to the history store table in the cache */
#define	WT_STAT_CONN_CACHE_BYTES_HS_UPDATES		1375
/*! cache: updates in uncommitted txn - bytes */
#define	WT_STAT_CONN_CACHE_UPDATES_TXN_UNCOMMITTED_BYTES	1376
/*! cache: updates in uncommitted txn - count */
#define	WT_STAT_CONN_CACHE_UPDATES_TXN_UNCOMMITTED_COUNT	1377
/*! capacity: background fsync file handles considered */
#define	WT_STAT_CONN_FSYNC_ALL_FH_TOTAL			1378
/*! capacity: background fsync file handles synced */
#define	WT_STAT_CONN_FSYNC_ALL_FH			1379
/*! capacity: background fsync time (msecs) */
#define	WT_STAT_CONN_FSYNC_ALL_TIME			1380
/*! capacity: bytes read */
#define	WT_STAT_CONN_CAPACITY_BYTES_READ		1381
/*! capacity: bytes written for checkpoint */
#define	WT_STAT_CONN_CAPACITY_BYTES_CKPT		1382
/*! capacity: bytes written for eviction */
#define	WT_STAT_CONN_CAPACITY_BYTES_EVICT		1383
/*! capacity: bytes written for log */
#define	WT_STAT_CONN_CAPACITY_BYTES_LOG			1384
/*! capacity: bytes written total */
#define	WT_STAT_CONN_CAPACITY_BYTES_WRITTEN		1385
/*! capacity: threshold to call fsync */
#define	WT_STAT_CONN_CAPACITY_THRESHOLD			1386
/*! capacity: time waiting due to total capacity (usecs) */
#define	WT_STAT_CONN_CAPACITY_TIME_TOTAL		1387
/*! capacity: time waiting during checkpoint (usecs) */
#define	WT_STAT_CONN_CAPACITY_TIME_CKPT			1388
/*! capacity: time waiting during eviction (usecs) */
#define	WT_STAT_CONN_CAPACITY_TIME_EVICT		1389
/*! capacity: time waiting during logging (usecs) */
#define	WT_STAT_CONN_CAPACITY_TIME_LOG			1390
/*! capacity: time waiting during read (usecs) */
#define	WT_STAT_CONN_CAPACITY_TIME_READ			1391
/*! checkpoint-cleanup: most recent duration on all eligible files (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_DURATION	1392
/*! checkpoint-cleanup: most recent handles processed */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_HANDLE_PROCESSED	1393
/*! checkpoint-cleanup: most recent in-memory pages visited */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_INMEM_PAGES_VISITED	1394
/*! checkpoint-cleanup: pages added for eviction */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_PAGES_EVICT	1395
/*! checkpoint-cleanup: pages dirtied due to obsolete time window */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_PAGES_OBSOLETE_TW	1396
/*! checkpoint-cleanup: pages read into cache (reclaim_space) */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_PAGES_READ_RECLAIM_SPACE	1397
/*! checkpoint-cleanup: pages read into cache due to obsolete time window */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_PAGES_READ_OBSOLETE_TW	1398
/*! checkpoint-cleanup: pages removed */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_PAGES_REMOVED	1399
/*! checkpoint-cleanup: pages skipped during tree walk */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_PAGES_WALK_SKIPPED	1400
/*! checkpoint-cleanup: pages visited */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_PAGES_VISITED	1401
/*! checkpoint-cleanup: successful calls */
#define	WT_STAT_CONN_CHECKPOINT_CLEANUP_SUCCESS		1402
/*! checkpoint: checkpoint has acquired a snapshot for its transaction */
#define	WT_STAT_CONN_CHECKPOINT_SNAPSHOT_ACQUIRED	1403
/*! checkpoint: checkpoints skipped because database was clean */
#define	WT_STAT_CONN_CHECKPOINT_SKIPPED			1404
/*! checkpoint: fsync calls after allocating the transaction ID */
#define	WT_STAT_CONN_CHECKPOINT_FSYNC_POST		1405
/*! checkpoint: fsync duration after allocating the transaction ID (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_FSYNC_POST_DURATION	1406
/*! checkpoint: generation */
#define	WT_STAT_CONN_CHECKPOINT_GENERATION		1407
/*! checkpoint: max time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_TIME_MAX		1408
/*!
 * checkpoint: metadata operations applied during disaggregated
 * checkpoint
 */
#define	WT_STAT_CONN_CHECKPOINT_DISAGG_METADATA_APPLY	1409
/*!
 * checkpoint: metadata operations skipped during disaggregated
 * checkpoint because they were not stable
 */
#define	WT_STAT_CONN_CHECKPOINT_DISAGG_METADATA_UNSTABLE	1410
/*! checkpoint: min time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_TIME_MIN		1411
/*!
 * checkpoint: most recent duration for checkpoint dropping all handles
 * (usecs)
 */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_DROP_DURATION	1412
/*! checkpoint: most recent duration for gathering all handles (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_DURATION		1413
/*! checkpoint: most recent duration for gathering applied handles (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_APPLY_DURATION	1414
/*! checkpoint: most recent duration for gathering skipped handles (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_SKIP_DURATION	1415
/*! checkpoint: most recent duration for handles metadata checked (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_META_CHECK_DURATION	1416
/*! checkpoint: most recent duration for locking the handles (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_LOCK_DURATION	1417
/*! checkpoint: most recent handles applied */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_APPLIED		1418
/*! checkpoint: most recent handles checkpoint dropped */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_DROPPED		1419
/*! checkpoint: most recent handles metadata checked */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_META_CHECKED	1420
/*! checkpoint: most recent handles metadata locked */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_LOCKED		1421
/*! checkpoint: most recent handles skipped */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_SKIPPED		1422
/*! checkpoint: most recent handles walked */
#define	WT_STAT_CONN_CHECKPOINT_HANDLE_WALKED		1423
/*! checkpoint: most recent time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_TIME_RECENT		1424
/*! checkpoint: number of bytes reconciled */
#define	WT_STAT_CONN_CHECKPOINT_PAGES_RECONCILED_BYTES	1425
/*! checkpoint: number of checkpoints started by api */
#define	WT_STAT_CONN_CHECKPOINTS_API			1426
/*! checkpoint: number of checkpoints started by compaction */
#define	WT_STAT_CONN_CHECKPOINTS_COMPACT		1427
/*! checkpoint: number of files synced */
#define	WT_STAT_CONN_CHECKPOINT_SYNC			1428
/*! checkpoint: number of handles visited after writes complete */
#define	WT_STAT_CONN_CHECKPOINT_PRESYNC			1429
/*! checkpoint: number of history store pages reconciled */
#define	WT_STAT_CONN_CHECKPOINT_HS_PAGES_RECONCILED	1430
/*! checkpoint: number of internal pages visited */
#define	WT_STAT_CONN_CHECKPOINT_PAGES_VISITED_INTERNAL	1431
/*! checkpoint: number of leaf pages visited */
#define	WT_STAT_CONN_CHECKPOINT_PAGES_VISITED_LEAF	1432
/*! checkpoint: number of pages reconciled */
#define	WT_STAT_CONN_CHECKPOINT_PAGES_RECONCILED	1433
/*!
 * checkpoint: number of pages reconciled by checkpoint parallel worker
 * threads
 */
#define	WT_STAT_CONN_CHECKPOINT_PARALLEL_PAGES_RECONCILED	1434
/*! checkpoint: prepare currently running */
#define	WT_STAT_CONN_CHECKPOINT_PREP_RUNNING		1435
/*! checkpoint: prepare max time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_PREP_MAX		1436
/*! checkpoint: prepare min time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_PREP_MIN		1437
/*! checkpoint: prepare most recent time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_PREP_RECENT		1438
/*! checkpoint: prepare total time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_PREP_TOTAL		1439
/*! checkpoint: progress state */
#define	WT_STAT_CONN_CHECKPOINT_STATE			1440
/*! checkpoint: scrub dirty target */
#define	WT_STAT_CONN_CHECKPOINT_SCRUB_TARGET		1441
/*! checkpoint: scrub max time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_SCRUB_MAX		1442
/*! checkpoint: scrub min time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_SCRUB_MIN		1443
/*! checkpoint: scrub most recent time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_SCRUB_RECENT		1444
/*! checkpoint: scrub total time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_SCRUB_TOTAL		1445
/*! checkpoint: stop timing stress active */
#define	WT_STAT_CONN_CHECKPOINT_STOP_STRESS_ACTIVE	1446
/*!
 * checkpoint: the percentage of checkpoint sync time spent in
 * reconciliation across all threads
 */
#define	WT_STAT_CONN_CHECKPOINT_SYNC_REC_PCT		1447
/*! checkpoint: time spent on per-tree checkpoint work (usecs) */
#define	WT_STAT_CONN_CHECKPOINT_TREE_DURATION		1448
/*! checkpoint: total failed number of checkpoints */
#define	WT_STAT_CONN_CHECKPOINTS_TOTAL_FAILED		1449
/*! checkpoint: total succeed number of checkpoints */
#define	WT_STAT_CONN_CHECKPOINTS_TOTAL_SUCCEED		1450
/*! checkpoint: total time (msecs) */
#define	WT_STAT_CONN_CHECKPOINT_TIME_TOTAL		1451
/*!
 * checkpoint: total time (msecs) writing pages to stable storage during
 * checkpoint reconciliation
 */
#define	WT_STAT_CONN_CHECKPOINT_REC_BLKCACHE_WRITE	1452
/*! checkpoint: wait cycles while cache dirty level is decreasing */
#define	WT_STAT_CONN_CHECKPOINT_WAIT_REDUCE_DIRTY	1453
/*! connection: auto adjusting condition resets */
#define	WT_STAT_CONN_COND_AUTO_WAIT_RESET		1454
/*! connection: auto adjusting condition wait calls */
#define	WT_STAT_CONN_COND_AUTO_WAIT			1455
/*!
 * connection: auto adjusting condition wait raced to update timeout and
 * skipped updating
 */
#define	WT_STAT_CONN_COND_AUTO_WAIT_SKIPPED		1456
/*! connection: btrees currently open */
#define	WT_STAT_CONN_BTREE_OPEN				1457
/*! connection: detected system time went backwards */
#define	WT_STAT_CONN_TIME_TRAVEL			1458
/*! connection: files currently open */
#define	WT_STAT_CONN_FILE_OPEN				1459
/*! connection: hash bucket array size for data handles */
#define	WT_STAT_CONN_BUCKETS_DH				1460
/*! connection: hash bucket array size general */
#define	WT_STAT_CONN_BUCKETS				1461
/*! connection: memory allocations */
#define	WT_STAT_CONN_MEMORY_ALLOCATION			1462
/*! connection: memory frees */
#define	WT_STAT_CONN_MEMORY_FREE			1463
/*! connection: memory re-allocations */
#define	WT_STAT_CONN_MEMORY_GROW			1464
/*! connection: number of sessions without a sweep for 5+ minutes */
#define	WT_STAT_CONN_NO_SESSION_SWEEP_5MIN		1465
/*! connection: number of sessions without a sweep for 60+ minutes */
#define	WT_STAT_CONN_NO_SESSION_SWEEP_60MIN		1466
/*! connection: pthread mutex condition wait calls */
#define	WT_STAT_CONN_COND_WAIT				1467
/*! connection: pthread mutex shared lock read-lock calls */
#define	WT_STAT_CONN_RWLOCK_READ			1468
/*! connection: pthread mutex shared lock write-lock calls */
#define	WT_STAT_CONN_RWLOCK_WRITE			1469
/*! connection: total fsync I/Os */
#define	WT_STAT_CONN_FSYNC_IO				1470
/*! connection: total read I/Os */
#define	WT_STAT_CONN_READ_IO				1471
/*! connection: total write I/Os */
#define	WT_STAT_CONN_WRITE_IO				1472
/*! cursor: Total number of deleted pages skipped during tree walk */
#define	WT_STAT_CONN_CURSOR_TREE_WALK_DEL_PAGE_SKIP	1473
/*! cursor: Total number of entries skipped by cursor next calls */
#define	WT_STAT_CONN_CURSOR_NEXT_SKIP_TOTAL		1474
/*! cursor: Total number of entries skipped by cursor prev calls */
#define	WT_STAT_CONN_CURSOR_PREV_SKIP_TOTAL		1475
/*!
 * cursor: Total number of entries skipped to position the history store
 * cursor
 */
#define	WT_STAT_CONN_CURSOR_SKIP_HS_CUR_POSITION	1476
/*!
 * cursor: Total number of in-memory deleted pages skipped during tree
 * walk
 */
#define	WT_STAT_CONN_CURSOR_TREE_WALK_INMEM_DEL_PAGE_SKIP	1477
/*! cursor: Total number of on-disk deleted pages skipped during tree walk */
#define	WT_STAT_CONN_CURSOR_TREE_WALK_ONDISK_DEL_PAGE_SKIP	1478
/*!
 * cursor: Total number of times a search near has exited due to prefix
 * config
 */
#define	WT_STAT_CONN_CURSOR_SEARCH_NEAR_PREFIX_FAST_PATHS	1479
/*!
 * cursor: Total number of times cursor fails to temporarily release
 * pinned page to encourage eviction of hot or large page
 */
#define	WT_STAT_CONN_CURSOR_REPOSITION_FAILED		1480
/*!
 * cursor: Total number of times cursor temporarily releases pinned page
 * to encourage eviction of hot or large page
 */
#define	WT_STAT_CONN_CURSOR_REPOSITION			1481
/*! cursor: bulk cursor count */
#define	WT_STAT_CONN_CURSOR_BULK_COUNT			1482
/*! cursor: cached cursor count */
#define	WT_STAT_CONN_CURSOR_CACHED_COUNT		1483
/*! cursor: cursor bound calls that return an error */
#define	WT_STAT_CONN_CURSOR_BOUND_ERROR			1484
/*! cursor: cursor bounds cleared from reset */
#define	WT_STAT_CONN_CURSOR_BOUNDS_RESET		1485
/*! cursor: cursor bounds comparisons performed */
#define	WT_STAT_CONN_CURSOR_BOUNDS_COMPARISONS		1486
/*! cursor: cursor bounds next called on an unpositioned cursor */
#define	WT_STAT_CONN_CURSOR_BOUNDS_NEXT_UNPOSITIONED	1487
/*! cursor: cursor bounds next early exit */
#define	WT_STAT_CONN_CURSOR_BOUNDS_NEXT_EARLY_EXIT	1488
/*! cursor: cursor bounds prev called on an unpositioned cursor */
#define	WT_STAT_CONN_CURSOR_BOUNDS_PREV_UNPOSITIONED	1489
/*! cursor: cursor bounds prev early exit */
#define	WT_STAT_CONN_CURSOR_BOUNDS_PREV_EARLY_EXIT	1490
/*! cursor: cursor bounds search early exit */
#define	WT_STAT_CONN_CURSOR_BOUNDS_SEARCH_EARLY_EXIT	1491
/*! cursor: cursor bounds search near call repositioned cursor */
#define	WT_STAT_CONN_CURSOR_BOUNDS_SEARCH_NEAR_REPOSITIONED_CURSOR	1492
/*! cursor: cursor bulk loaded cursor insert calls */
#define	WT_STAT_CONN_CURSOR_INSERT_BULK			1493
/*! cursor: cursor cache calls that return an error */
#define	WT_STAT_CONN_CURSOR_CACHE_ERROR			1494
/*! cursor: cursor close calls that result in cache */
#define	WT_STAT_CONN_CURSOR_CACHE			1495
/*! cursor: cursor close calls that return an error */
#define	WT_STAT_CONN_CURSOR_CLOSE_ERROR			1496
/*! cursor: cursor compare calls that return an error */
#define	WT_STAT_CONN_CURSOR_COMPARE_ERROR		1497
/*! cursor: cursor create calls */
#define	WT_STAT_CONN_CURSOR_CREATE			1498
/*! cursor: cursor equals calls that return an error */
#define	WT_STAT_CONN_CURSOR_EQUALS_ERROR		1499
/*! cursor: cursor get key calls that return an error */
#define	WT_STAT_CONN_CURSOR_GET_KEY_ERROR		1500
/*! cursor: cursor get value calls that return an error */
#define	WT_STAT_CONN_CURSOR_GET_VALUE_ERROR		1501
/*! cursor: cursor insert calls */
#define	WT_STAT_CONN_CURSOR_INSERT			1502
/*! cursor: cursor insert calls that return an error */
#define	WT_STAT_CONN_CURSOR_INSERT_ERROR		1503
/*! cursor: cursor insert check calls that return an error */
#define	WT_STAT_CONN_CURSOR_INSERT_CHECK_ERROR		1504
/*! cursor: cursor insert key and value bytes */
#define	WT_STAT_CONN_CURSOR_INSERT_BYTES		1505
/*! cursor: cursor largest key calls that return an error */
#define	WT_STAT_CONN_CURSOR_LARGEST_KEY_ERROR		1506
/*! cursor: cursor modify calls */
#define	WT_STAT_CONN_CURSOR_MODIFY			1507
/*! cursor: cursor modify calls that return an error */
#define	WT_STAT_CONN_CURSOR_MODIFY_ERROR		1508
/*! cursor: cursor modify key and value bytes affected */
#define	WT_STAT_CONN_CURSOR_MODIFY_BYTES		1509
/*! cursor: cursor modify value bytes modified */
#define	WT_STAT_CONN_CURSOR_MODIFY_BYTES_TOUCH		1510
/*! cursor: cursor next calls */
#define	WT_STAT_CONN_CURSOR_NEXT			1511
/*! cursor: cursor next calls that return an error */
#define	WT_STAT_CONN_CURSOR_NEXT_ERROR			1512
/*!
 * cursor: cursor next calls that skip due to a globally visible history
 * store tombstone
 */
#define	WT_STAT_CONN_CURSOR_NEXT_HS_TOMBSTONE		1513
/*!
 * cursor: cursor next calls that skip greater than 1 and fewer than 100
 * entries
 */
#define	WT_STAT_CONN_CURSOR_NEXT_SKIP_LT_100		1514
/*!
 * cursor: cursor next calls that skip greater than or equal to 100
 * entries
 */
#define	WT_STAT_CONN_CURSOR_NEXT_SKIP_GE_100		1515
/*! cursor: cursor next random calls that return an error */
#define	WT_STAT_CONN_CURSOR_NEXT_RANDOM_ERROR		1516
/*! cursor: cursor operation restarted */
#define	WT_STAT_CONN_CURSOR_RESTART			1517
/*! cursor: cursor prev calls */
#define	WT_STAT_CONN_CURSOR_PREV			1518
/*! cursor: cursor prev calls that return an error */
#define	WT_STAT_CONN_CURSOR_PREV_ERROR			1519
/*!
 * cursor: cursor prev calls that skip due to a globally visible history
 * store tombstone
 */
#define	WT_STAT_CONN_CURSOR_PREV_HS_TOMBSTONE		1520
/*!
 * cursor: cursor prev calls that skip greater than or equal to 100
 * entries
 */
#define	WT_STAT_CONN_CURSOR_PREV_SKIP_GE_100		1521
/*! cursor: cursor prev calls that skip less than 100 entries */
#define	WT_STAT_CONN_CURSOR_PREV_SKIP_LT_100		1522
/*! cursor: cursor reconfigure calls that return an error */
#define	WT_STAT_CONN_CURSOR_RECONFIGURE_ERROR		1523
/*! cursor: cursor remove calls */
#define	WT_STAT_CONN_CURSOR_REMOVE			1524
/*! cursor: cursor remove calls that return an error */
#define	WT_STAT_CONN_CURSOR_REMOVE_ERROR		1525
/*! cursor: cursor remove key bytes removed */
#define	WT_STAT_CONN_CURSOR_REMOVE_BYTES		1526
/*! cursor: cursor reopen calls that return an error */
#define	WT_STAT_CONN_CURSOR_REOPEN_ERROR		1527
/*! cursor: cursor reserve calls */
#define	WT_STAT_CONN_CURSOR_RESERVE			1528
/*! cursor: cursor reserve calls that return an error */
#define	WT_STAT_CONN_CURSOR_RESERVE_ERROR		1529
/*! cursor: cursor reset calls */
#define	WT_STAT_CONN_CURSOR_RESET			1530
/*! cursor: cursor reset calls that return an error */
#define	WT_STAT_CONN_CURSOR_RESET_ERROR			1531
/*! cursor: cursor search calls */
#define	WT_STAT_CONN_CURSOR_SEARCH			1532
/*! cursor: cursor search calls that return an error */
#define	WT_STAT_CONN_CURSOR_SEARCH_ERROR		1533
/*! cursor: cursor search history store calls */
#define	WT_STAT_CONN_CURSOR_SEARCH_HS			1534
/*! cursor: cursor search near calls */
#define	WT_STAT_CONN_CURSOR_SEARCH_NEAR			1535
/*! cursor: cursor search near calls that return an error */
#define	WT_STAT_CONN_CURSOR_SEARCH_NEAR_ERROR		1536
/*! cursor: cursor sweep buckets */
#define	WT_STAT_CONN_CURSOR_SWEEP_BUCKETS		1537
/*! cursor: cursor sweep cursors closed */
#define	WT_STAT_CONN_CURSOR_SWEEP_CLOSED		1538
/*! cursor: cursor sweep cursors examined */
#define	WT_STAT_CONN_CURSOR_SWEEP_EXAMINED		1539
/*! cursor: cursor sweeps */
#define	WT_STAT_CONN_CURSOR_SWEEP			1540
/*! cursor: cursor truncate calls */
#define	WT_STAT_CONN_CURSOR_TRUNCATE			1541
/*! cursor: cursor truncates performed on individual keys */
#define	WT_STAT_CONN_CURSOR_TRUNCATE_KEYS_DELETED	1542
/*! cursor: cursor update calls */
#define	WT_STAT_CONN_CURSOR_UPDATE			1543
/*! cursor: cursor update calls that return an error */
#define	WT_STAT_CONN_CURSOR_UPDATE_ERROR		1544
/*! cursor: cursor update key and value bytes */
#define	WT_STAT_CONN_CURSOR_UPDATE_BYTES		1545
/*! cursor: cursor update value size change */
#define	WT_STAT_CONN_CURSOR_UPDATE_BYTES_CHANGED	1546
/*! cursor: cursors reused from cache */
#define	WT_STAT_CONN_CURSOR_REOPEN			1547
/*! cursor: open cursor count */
#define	WT_STAT_CONN_CURSOR_OPEN_COUNT			1548
/*! cursor: open cursor time application (usecs) */
#define	WT_STAT_CONN_CURSOR_OPEN_TIME_USER_USECS	1549
/*! cursor: open cursor time internal (usecs) */
#define	WT_STAT_CONN_CURSOR_OPEN_TIME_INTERNAL_USECS	1550
/*! data-handle: Layered connection data handles currently active */
#define	WT_STAT_CONN_DH_CONN_HANDLE_LAYERED_COUNT	1551
/*! data-handle: Table connection data handles currently active */
#define	WT_STAT_CONN_DH_CONN_HANDLE_TABLE_COUNT		1552
/*! data-handle: Tiered connection data handles currently active */
#define	WT_STAT_CONN_DH_CONN_HANDLE_TIERED_COUNT	1553
/*! data-handle: Tiered_Tree connection data handles currently active */
#define	WT_STAT_CONN_DH_CONN_HANDLE_TIERED_TREE_COUNT	1554
/*! data-handle: btree connection data handles currently active */
#define	WT_STAT_CONN_DH_CONN_HANDLE_BTREE_COUNT		1555
/*! data-handle: checkpoint connection data handles currently active */
#define	WT_STAT_CONN_DH_CONN_HANDLE_CHECKPOINT_COUNT	1556
/*! data-handle: connection data handle size */
#define	WT_STAT_CONN_DH_CONN_HANDLE_SIZE		1557
/*! data-handle: connection data handles currently active */
#define	WT_STAT_CONN_DH_CONN_HANDLE_COUNT		1558
/*! data-handle: connection sweep candidate became referenced */
#define	WT_STAT_CONN_DH_SWEEP_REF			1559
/*! data-handle: connection sweep dead dhandles closed */
#define	WT_STAT_CONN_DH_SWEEP_DEAD_CLOSE		1560
/*! data-handle: connection sweep dhandles removed from hash list */
#define	WT_STAT_CONN_DH_SWEEP_REMOVE			1561
/*! data-handle: connection sweep expired dhandles closed */
#define	WT_STAT_CONN_DH_SWEEP_EXPIRED_CLOSE		1562
/*! data-handle: connection sweep time-of-death sets */
#define	WT_STAT_CONN_DH_SWEEP_TOD			1563
/*! data-handle: connection sweeps */
#define	WT_STAT_CONN_DH_SWEEPS				1564
/*!
 * data-handle: connection sweeps skipped due to checkpoint gathering
 * handles
 */
#define	WT_STAT_CONN_DH_SWEEP_SKIP_CKPT			1565
/*! data-handle: session dhandles swept */
#define	WT_STAT_CONN_DH_SESSION_HANDLES			1566
/*! data-handle: session sweep attempts */
#define	WT_STAT_CONN_DH_SESSION_SWEEPS			1567
/*! disagg: abandon checkpoints failed */
#define	WT_STAT_CONN_DISAGG_ABANDON_CHECKPOINT_FAILED	1568
/*! disagg: abandon checkpoints succeeded */
#define	WT_STAT_CONN_DISAGG_ABANDON_CHECKPOINT_SUCCEED	1569
/*! disagg: apply checkpoint metadata most recent time (msecs) */
#define	WT_STAT_CONN_DISAGG_APPLY_CHECKPOINT_META_TIME	1570
/*! disagg: connection reconfiguration */
#define	WT_STAT_CONN_DISAGG_CONN_RECONFIG		1571
/*! disagg: database size */
#define	WT_STAT_CONN_DISAGG_DATABASE_SIZE		1572
/*!
 * disagg: existing file metadata entries updated during checkpoint pick-
 * up
 */
#define	WT_STAT_CONN_DISAGG_PICK_UP_FILE_META_UPDATED	1573
/*! disagg: new file metadata entries inserted during checkpoint pick-up */
#define	WT_STAT_CONN_DISAGG_PICK_UP_FILE_META_INSERTED	1574
/*! disagg: pick up checkpoint most recent time (msecs) */
#define	WT_STAT_CONN_DISAGG_PICK_UP_CHECKPOINT_TIME	1575
/*! disagg: role leader */
#define	WT_STAT_CONN_DISAGG_ROLE_LEADER			1576
/*! disagg: step down most recent time (msecs) */
#define	WT_STAT_CONN_DISAGG_STEP_DOWN_TIME		1577
/*! disagg: step up most recent time (msecs) */
#define	WT_STAT_CONN_DISAGG_STEP_UP_TIME		1578
/*! layered: Layered table cursor insert operations */
#define	WT_STAT_CONN_LAYERED_CURS_INSERT		1579
/*! layered: Layered table cursor modify operations */
#define	WT_STAT_CONN_LAYERED_CURS_MODIFY		1580
/*! layered: Layered table cursor next operations */
#define	WT_STAT_CONN_LAYERED_CURS_NEXT			1581
/*! layered: Layered table cursor next operations from the ingest btrees */
#define	WT_STAT_CONN_LAYERED_CURS_NEXT_INGEST		1582
/*! layered: Layered table cursor next operations from the stable btrees */
#define	WT_STAT_CONN_LAYERED_CURS_NEXT_STABLE		1583
/*!
 * layered: Layered table cursor opens the stable btree for the first
 * time
 */
#define	WT_STAT_CONN_LAYERED_CURS_OPEN_STABLE		1584
/*! layered: Layered table cursor prev operations */
#define	WT_STAT_CONN_LAYERED_CURS_PREV			1585
/*! layered: Layered table cursor prev operations from the ingest btrees */
#define	WT_STAT_CONN_LAYERED_CURS_PREV_INGEST		1586
/*! layered: Layered table cursor prev operations from the stable btrees */
#define	WT_STAT_CONN_LAYERED_CURS_PREV_STABLE		1587
/*! layered: Layered table cursor remove operations */
#define	WT_STAT_CONN_LAYERED_CURS_REMOVE		1588
/*!
 * layered: Layered table cursor reopens the stable btree (role change or
 * checkpoint advance)
 */
#define	WT_STAT_CONN_LAYERED_CURS_REOPEN_STABLE		1589
/*! layered: Layered table cursor search near operations */
#define	WT_STAT_CONN_LAYERED_CURS_SEARCH_NEAR		1590
/*!
 * layered: Layered table cursor search near operations from the ingest
 * btrees
 */
#define	WT_STAT_CONN_LAYERED_CURS_SEARCH_NEAR_INGEST	1591
/*!
 * layered: Layered table cursor search near operations from the stable
 * btrees
 */
#define	WT_STAT_CONN_LAYERED_CURS_SEARCH_NEAR_STABLE	1592
/*! layered: Layered table cursor search operations */
#define	WT_STAT_CONN_LAYERED_CURS_SEARCH		1593
/*! layered: Layered table cursor search operations from the ingest btrees */
#define	WT_STAT_CONN_LAYERED_CURS_SEARCH_INGEST		1594
/*! layered: Layered table cursor search operations from the stable btrees */
#define	WT_STAT_CONN_LAYERED_CURS_SEARCH_STABLE		1595
/*! layered: Layered table cursor update operations */
#define	WT_STAT_CONN_LAYERED_CURS_UPDATE		1596
/*!
 * layered: checkpoints performed on this table by the layered table
 * manager
 */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_CHECKPOINTS	1597
/*! layered: disagg pick up checkpoints failed */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_CHECKPOINTS_DISAGG_PICK_UP_FAILED	1598
/*! layered: disagg pick up checkpoints succeeded */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_CHECKPOINTS_DISAGG_PICK_UP_SUCCEED	1599
/*!
 * layered: how many log applications the layered table manager applied
 * on this tree
 */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_LOGOPS_APPLIED	1600
/*!
 * layered: how many log applications the layered table manager skipped
 * on this tree
 */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_LOGOPS_SKIPPED	1601
/*!
 * layered: how many previously-applied LSNs the layered table manager
 * skipped on this tree
 */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_SKIP_LSN	1602
/*! layered: number of checkpoints picked up by a follower */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_CHECKPOINTS_DISAGG_PICK_UP_FOLLOWER	1603
/*! layered: the number of tables the layered table manager has open */
#define	WT_STAT_CONN_LAYERED_TABLE_MANAGER_TABLES	1604
/*! layered: the number of times the truncate list was searched */
#define	WT_STAT_CONN_LAYERED_TRUNCATE_LIST_SEARCH_CALLS	1605
/*!
 * layered: the number of times truncate list garbage collection ran with
 * a valid prune timestamp
 */
#define	WT_STAT_CONN_LAYERED_TRUNCATE_LIST_GC_RUNS	1606
/*!
 * layered: the number of truncate list entries removed by garbage
 * collection
 */
#define	WT_STAT_CONN_LAYERED_TRUNCATE_LIST_GC_ENTRIES_REMOVED	1607
/*! layered: the number of truncate list entries walked during search */
#define	WT_STAT_CONN_LAYERED_TRUNCATE_LIST_SEARCH_ENTRIES_WALKED	1608
/*!
 * live-restore: number of bytes copied from the source to the
 * destination
 */
#define	WT_STAT_CONN_LIVE_RESTORE_BYTES_COPIED		1609
/*! live-restore: number of files remaining for migration completion */
#define	WT_STAT_CONN_LIVE_RESTORE_WORK_REMAINING	1610
/*! live-restore: number of reads from the source database */
#define	WT_STAT_CONN_LIVE_RESTORE_SOURCE_READ_COUNT	1611
/*! live-restore: source read latency histogram (bucket 1) - 0-1ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT2	1612
/*! live-restore: source read latency histogram (bucket 2) - 2-4ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT5	1613
/*! live-restore: source read latency histogram (bucket 3) - 5-9ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT10	1614
/*! live-restore: source read latency histogram (bucket 4) - 10-49ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT50	1615
/*! live-restore: source read latency histogram (bucket 5) - 50-99ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT100	1616
/*! live-restore: source read latency histogram (bucket 6) - 100-249ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT250	1617
/*! live-restore: source read latency histogram (bucket 7) - 250-499ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT500	1618
/*! live-restore: source read latency histogram (bucket 8) - 500-999ms */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_LT1000	1619
/*! live-restore: source read latency histogram (bucket 9) - 1000ms+ */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_GT1000	1620
/*! live-restore: source read latency histogram total (msecs) */
#define	WT_STAT_CONN_LIVE_RESTORE_HIST_SOURCE_READ_LATENCY_TOTAL_MSECS	1621
/*! live-restore: state */
#define	WT_STAT_CONN_LIVE_RESTORE_STATE			1622
/*! load-control: number of read operations rejected due to load control */
#define	WT_STAT_CONN_READ_REJECT_COUNT			1623
/*! load-control: number of write operations rejected due to load control */
#define	WT_STAT_CONN_WRITE_REJECT_COUNT			1624
/*! load-control: read load at the system level */
#define	WT_STAT_CONN_READ_LOAD				1625
/*! load-control: write load at the system level */
#define	WT_STAT_CONN_WRITE_LOAD				1626
/*! lock: btree page lock acquisitions */
#define	WT_STAT_CONN_LOCK_BTREE_PAGE_COUNT		1627
/*! lock: btree page lock application thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_BTREE_PAGE_WAIT_APPLICATION	1628
/*! lock: btree page lock internal thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_BTREE_PAGE_WAIT_INTERNAL	1629
/*! lock: checkpoint lock acquisitions */
#define	WT_STAT_CONN_LOCK_CHECKPOINT_COUNT		1630
/*! lock: checkpoint lock application thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_CHECKPOINT_WAIT_APPLICATION	1631
/*! lock: checkpoint lock internal thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_CHECKPOINT_WAIT_INTERNAL	1632
/*! lock: dhandle lock application thread time waiting (usecs) */
#define	WT_STAT_CONN_LOCK_DHANDLE_WAIT_APPLICATION	1633
/*! lock: dhandle lock internal thread time waiting (usecs) */
#define	WT_STAT_CONN_LOCK_DHANDLE_WAIT_INTERNAL		1634
/*! lock: dhandle read lock acquisitions */
#define	WT_STAT_CONN_LOCK_DHANDLE_READ_COUNT		1635
/*! lock: dhandle write lock acquisitions */
#define	WT_STAT_CONN_LOCK_DHANDLE_WRITE_COUNT		1636
/*! lock: metadata lock acquisitions */
#define	WT_STAT_CONN_LOCK_METADATA_COUNT		1637
/*! lock: metadata lock application thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_METADATA_WAIT_APPLICATION	1638
/*! lock: metadata lock internal thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_METADATA_WAIT_INTERNAL	1639
/*! lock: schema lock acquisitions */
#define	WT_STAT_CONN_LOCK_SCHEMA_COUNT			1640
/*! lock: schema lock application thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_SCHEMA_WAIT_APPLICATION	1641
/*! lock: schema lock internal thread wait time (usecs) */
#define	WT_STAT_CONN_LOCK_SCHEMA_WAIT_INTERNAL		1642
/*!
 * lock: table lock application thread time waiting for the table lock
 * (usecs)
 */
#define	WT_STAT_CONN_LOCK_TABLE_WAIT_APPLICATION	1643
/*!
 * lock: table lock internal thread time waiting for the table lock
 * (usecs)
 */
#define	WT_STAT_CONN_LOCK_TABLE_WAIT_INTERNAL		1644
/*! lock: table read lock acquisitions */
#define	WT_STAT_CONN_LOCK_TABLE_READ_COUNT		1645
/*! lock: table write lock acquisitions */
#define	WT_STAT_CONN_LOCK_TABLE_WRITE_COUNT		1646
/*! lock: txn global lock application thread time waiting (usecs) */
#define	WT_STAT_CONN_LOCK_TXN_GLOBAL_WAIT_APPLICATION	1647
/*! lock: txn global lock internal thread time waiting (usecs) */
#define	WT_STAT_CONN_LOCK_TXN_GLOBAL_WAIT_INTERNAL	1648
/*! lock: txn global read lock acquisitions */
#define	WT_STAT_CONN_LOCK_TXN_GLOBAL_READ_COUNT		1649
/*! lock: txn global write lock acquisitions */
#define	WT_STAT_CONN_LOCK_TXN_GLOBAL_WRITE_COUNT	1650
/*! log: busy returns attempting to switch slots */
#define	WT_STAT_CONN_LOG_SLOT_SWITCH_BUSY		1651
/*! log: force log remove time sleeping (usecs) */
#define	WT_STAT_CONN_LOG_FORCE_REMOVE_SLEEP		1652
/*! log: log bytes of payload data */
#define	WT_STAT_CONN_LOG_BYTES_PAYLOAD			1653
/*! log: log bytes written */
#define	WT_STAT_CONN_LOG_BYTES_WRITTEN			1654
/*! log: log files manually zero-filled */
#define	WT_STAT_CONN_LOG_ZERO_FILLS			1655
/*! log: log flush operations */
#define	WT_STAT_CONN_LOG_FLUSH				1656
/*! log: log force write operations */
#define	WT_STAT_CONN_LOG_FORCE_WRITE			1657
/*! log: log force write operations skipped */
#define	WT_STAT_CONN_LOG_FORCE_WRITE_SKIP		1658
/*! log: log records compressed */
#define	WT_STAT_CONN_LOG_COMPRESS_WRITES		1659
/*! log: log records not compressed */
#define	WT_STAT_CONN_LOG_COMPRESS_WRITE_FAILS		1660
/*! log: log records too small to compress */
#define	WT_STAT_CONN_LOG_COMPRESS_SMALL			1661
/*! log: log release advances write LSN */
#define	WT_STAT_CONN_LOG_RELEASE_WRITE_LSN		1662
/*! log: log scan operations */
#define	WT_STAT_CONN_LOG_SCANS				1663
/*! log: log scan records requiring two reads */
#define	WT_STAT_CONN_LOG_SCAN_REREADS			1664
/*! log: log server thread advances write LSN */
#define	WT_STAT_CONN_LOG_WRITE_LSN			1665
/*! log: log server thread write LSN walk skipped */
#define	WT_STAT_CONN_LOG_WRITE_LSN_SKIP			1666
/*! log: log sync operations */
#define	WT_STAT_CONN_LOG_SYNC				1667
/*! log: log sync time duration (usecs) */
#define	WT_STAT_CONN_LOG_SYNC_DURATION			1668
/*! log: log sync_dir operations */
#define	WT_STAT_CONN_LOG_SYNC_DIR			1669
/*! log: log sync_dir time duration (usecs) */
#define	WT_STAT_CONN_LOG_SYNC_DIR_DURATION		1670
/*! log: log write operations */
#define	WT_STAT_CONN_LOG_WRITES				1671
/*! log: logging bytes consolidated */
#define	WT_STAT_CONN_LOG_SLOT_CONSOLIDATED		1672
/*! log: maximum log file size */
#define	WT_STAT_CONN_LOG_MAX_FILESIZE			1673
/*! log: number of pre-allocated log files to create */
#define	WT_STAT_CONN_LOG_PREALLOC_MAX			1674
/*! log: pre-allocated log files not ready and missed */
#define	WT_STAT_CONN_LOG_PREALLOC_MISSED		1675
/*! log: pre-allocated log files prepared */
#define	WT_STAT_CONN_LOG_PREALLOC_FILES			1676
/*! log: pre-allocated log files used */
#define	WT_STAT_CONN_LOG_PREALLOC_USED			1677
/*! log: records processed by log scan */
#define	WT_STAT_CONN_LOG_SCAN_RECORDS			1678
/*! log: slot close lost race */
#define	WT_STAT_CONN_LOG_SLOT_CLOSE_RACE		1679
/*! log: slot close unbuffered waits */
#define	WT_STAT_CONN_LOG_SLOT_CLOSE_UNBUF		1680
/*! log: slot closures */
#define	WT_STAT_CONN_LOG_SLOT_CLOSES			1681
/*! log: slot join atomic update races */
#define	WT_STAT_CONN_LOG_SLOT_RACES			1682
/*! log: slot join calls atomic updates raced */
#define	WT_STAT_CONN_LOG_SLOT_YIELD_RACE		1683
/*! log: slot join calls did not yield */
#define	WT_STAT_CONN_LOG_SLOT_IMMEDIATE			1684
/*! log: slot join calls found active slot closed */
#define	WT_STAT_CONN_LOG_SLOT_YIELD_CLOSE		1685
/*! log: slot join calls slept */
#define	WT_STAT_CONN_LOG_SLOT_YIELD_SLEEP		1686
/*! log: slot join calls yielded */
#define	WT_STAT_CONN_LOG_SLOT_YIELD			1687
/*! log: slot join found active slot closed */
#define	WT_STAT_CONN_LOG_SLOT_ACTIVE_CLOSED		1688
/*! log: slot joins yield time (usecs) */
#define	WT_STAT_CONN_LOG_SLOT_YIELD_DURATION		1689
/*! log: slot transitions unable to find free slot */
#define	WT_STAT_CONN_LOG_SLOT_NO_FREE_SLOTS		1690
/*! log: slot unbuffered writes */
#define	WT_STAT_CONN_LOG_SLOT_UNBUFFERED		1691
/*! log: total in-memory size of compressed records */
#define	WT_STAT_CONN_LOG_COMPRESS_MEM			1692
/*! log: total log buffer size */
#define	WT_STAT_CONN_LOG_BUFFER_SIZE			1693
/*! log: total size of compressed records */
#define	WT_STAT_CONN_LOG_COMPRESS_LEN			1694
/*! log: written slots coalesced */
#define	WT_STAT_CONN_LOG_SLOT_COALESCED			1695
/*! log: yields waiting for previous log file close */
#define	WT_STAT_CONN_LOG_CLOSE_YIELDS			1696
/*! perf: block manager read latency histogram (bucket 1) - 0-1ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT2	1697
/*! perf: block manager read latency histogram (bucket 2) - 2-4ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT5	1698
/*! perf: block manager read latency histogram (bucket 3) - 5-9ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT10	1699
/*! perf: block manager read latency histogram (bucket 4) - 10-49ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT50	1700
/*! perf: block manager read latency histogram (bucket 5) - 50-99ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT100	1701
/*! perf: block manager read latency histogram (bucket 6) - 100-249ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT250	1702
/*! perf: block manager read latency histogram (bucket 7) - 250-499ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT500	1703
/*! perf: block manager read latency histogram (bucket 8) - 500-999ms */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_LT1000	1704
/*! perf: block manager read latency histogram (bucket 9) - 1000ms+ */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_GT1000	1705
/*! perf: block manager read latency histogram total (msecs) */
#define	WT_STAT_CONN_PERF_HIST_BMREAD_LATENCY_TOTAL_MSECS	1706
/*! perf: block manager write latency histogram (bucket 1) - 0-1ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT2	1707
/*! perf: block manager write latency histogram (bucket 2) - 2-4ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT5	1708
/*! perf: block manager write latency histogram (bucket 3) - 5-9ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT10	1709
/*! perf: block manager write latency histogram (bucket 4) - 10-49ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT50	1710
/*! perf: block manager write latency histogram (bucket 5) - 50-99ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT100	1711
/*! perf: block manager write latency histogram (bucket 6) - 100-249ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT250	1712
/*! perf: block manager write latency histogram (bucket 7) - 250-499ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT500	1713
/*! perf: block manager write latency histogram (bucket 8) - 500-999ms */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_LT1000	1714
/*! perf: block manager write latency histogram (bucket 9) - 1000ms+ */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_GT1000	1715
/*! perf: block manager write latency histogram total (msecs) */
#define	WT_STAT_CONN_PERF_HIST_BMWRITE_LATENCY_TOTAL_MSECS	1716
/*! perf: disagg block manager read latency histogram (bucket 1) - 50-99us */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_LT100	1717
/*!
 * perf: disagg block manager read latency histogram (bucket 2) -
 * 100-249us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_LT250	1718
/*!
 * perf: disagg block manager read latency histogram (bucket 3) -
 * 250-499us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_LT500	1719
/*!
 * perf: disagg block manager read latency histogram (bucket 4) -
 * 500-999us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_LT1000	1720
/*!
 * perf: disagg block manager read latency histogram (bucket 5) -
 * 1000-2499us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_LT2500	1721
/*!
 * perf: disagg block manager read latency histogram (bucket 6) -
 * 2500-4999us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_LT5000	1722
/*!
 * perf: disagg block manager read latency histogram (bucket 7) -
 * 5000-9999us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_LT10000	1723
/*!
 * perf: disagg block manager read latency histogram (bucket 8) -
 * 10000us+
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_GT10000	1724
/*! perf: disagg block manager read latency histogram total (usecs) */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMREAD_LATENCY_TOTAL_USECS	1725
/*!
 * perf: disagg block manager write latency histogram (bucket 1) -
 * 50-99us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_LT100	1726
/*!
 * perf: disagg block manager write latency histogram (bucket 2) -
 * 100-249us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_LT250	1727
/*!
 * perf: disagg block manager write latency histogram (bucket 3) -
 * 250-499us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_LT500	1728
/*!
 * perf: disagg block manager write latency histogram (bucket 4) -
 * 500-999us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_LT1000	1729
/*!
 * perf: disagg block manager write latency histogram (bucket 5) -
 * 1000-2499us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_LT2500	1730
/*!
 * perf: disagg block manager write latency histogram (bucket 6) -
 * 2500-4999us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_LT5000	1731
/*!
 * perf: disagg block manager write latency histogram (bucket 7) -
 * 5000-9999us
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_LT10000	1732
/*!
 * perf: disagg block manager write latency histogram (bucket 8) -
 * 10000us+
 */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_GT10000	1733
/*! perf: disagg block manager write latency histogram total (usecs) */
#define	WT_STAT_CONN_PERF_HIST_DISAGGBMWRITE_LATENCY_TOTAL_USECS	1734
/*! perf: file system read latency histogram (bucket 1) - 0-1ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT2	1735
/*! perf: file system read latency histogram (bucket 2) - 2-4ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT5	1736
/*! perf: file system read latency histogram (bucket 3) - 5-9ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT10	1737
/*! perf: file system read latency histogram (bucket 4) - 10-49ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT50	1738
/*! perf: file system read latency histogram (bucket 5) - 50-99ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT100	1739
/*! perf: file system read latency histogram (bucket 6) - 100-249ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT250	1740
/*! perf: file system read latency histogram (bucket 7) - 250-499ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT500	1741
/*! perf: file system read latency histogram (bucket 8) - 500-999ms */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_LT1000	1742
/*! perf: file system read latency histogram (bucket 9) - 1000ms+ */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_GT1000	1743
/*! perf: file system read latency histogram total (msecs) */
#define	WT_STAT_CONN_PERF_HIST_FSREAD_LATENCY_TOTAL_MSECS	1744
/*! perf: file system write latency histogram (bucket 1) - 0-1ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT2	1745
/*! perf: file system write latency histogram (bucket 2) - 2-4ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT5	1746
/*! perf: file system write latency histogram (bucket 3) - 5-9ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT10	1747
/*! perf: file system write latency histogram (bucket 4) - 10-49ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT50	1748
/*! perf: file system write latency histogram (bucket 5) - 50-99ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT100	1749
/*! perf: file system write latency histogram (bucket 6) - 100-249ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT250	1750
/*! perf: file system write latency histogram (bucket 7) - 250-499ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT500	1751
/*! perf: file system write latency histogram (bucket 8) - 500-999ms */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_LT1000	1752
/*! perf: file system write latency histogram (bucket 9) - 1000ms+ */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_GT1000	1753
/*! perf: file system write latency histogram total (msecs) */
#define	WT_STAT_CONN_PERF_HIST_FSWRITE_LATENCY_TOTAL_MSECS	1754
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 1) -
 * 0-100us
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_LT100	1755
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 2) -
 * 100-249us
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_LT250	1756
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 3) -
 * 250-499us
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_LT500	1757
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 4) -
 * 500-999us
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_LT1000	1758
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 5) -
 * 1000-2499us
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_LT2500	1759
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 6) -
 * 2500-4999us
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_LT5000	1760
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 7) -
 * 5000-9999us
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_LT10000	1761
/*!
 * perf: internal page deltas reconstruct latency histogram (bucket 8) -
 * 10000us+
 */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_GT10000	1762
/*! perf: internal page deltas reconstruct latency histogram total (usecs) */
#define	WT_STAT_CONN_PERF_HIST_INTERNAL_RECONSTRUCT_LATENCY_TOTAL_USECS	1763
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 1) -
 * 0-100us
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_LT100	1764
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 2) -
 * 100-249us
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_LT250	1765
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 3) -
 * 250-499us
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_LT500	1766
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 4) -
 * 500-999us
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_LT1000	1767
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 5) -
 * 1000-2499us
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_LT2500	1768
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 6) -
 * 2500-4999us
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_LT5000	1769
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 7) -
 * 5000-9999us
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_LT10000	1770
/*!
 * perf: leaf page deltas reconstruct latency histogram (bucket 8) -
 * 10000us+
 */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_GT10000	1771
/*! perf: leaf page deltas reconstruct latency histogram total (usecs) */
#define	WT_STAT_CONN_PERF_HIST_LEAF_RECONSTRUCT_LATENCY_TOTAL_USECS	1772
/*! perf: operation read latency histogram (bucket 1) - 0-100us */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_LT100	1773
/*! perf: operation read latency histogram (bucket 2) - 100-249us */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_LT250	1774
/*! perf: operation read latency histogram (bucket 3) - 250-499us */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_LT500	1775
/*! perf: operation read latency histogram (bucket 4) - 500-999us */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_LT1000	1776
/*! perf: operation read latency histogram (bucket 5) - 1000-2499us */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_LT2500	1777
/*! perf: operation read latency histogram (bucket 6) - 2500-4999us */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_LT5000	1778
/*! perf: operation read latency histogram (bucket 7) - 5000-9999us */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_LT10000	1779
/*! perf: operation read latency histogram (bucket 8) - 10000us+ */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_GT10000	1780
/*! perf: operation read latency histogram total (usecs) */
#define	WT_STAT_CONN_PERF_HIST_OPREAD_LATENCY_TOTAL_USECS	1781
/*! perf: operation write latency histogram (bucket 1) - 0-100us */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_LT100	1782
/*! perf: operation write latency histogram (bucket 2) - 100-249us */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_LT250	1783
/*! perf: operation write latency histogram (bucket 3) - 250-499us */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_LT500	1784
/*! perf: operation write latency histogram (bucket 4) - 500-999us */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_LT1000	1785
/*! perf: operation write latency histogram (bucket 5) - 1000-2499us */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_LT2500	1786
/*! perf: operation write latency histogram (bucket 6) - 2500-4999us */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_LT5000	1787
/*! perf: operation write latency histogram (bucket 7) - 5000-9999us */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_LT10000	1788
/*! perf: operation write latency histogram (bucket 8) - 10000us+ */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_GT10000	1789
/*! perf: operation write latency histogram total (usecs) */
#define	WT_STAT_CONN_PERF_HIST_OPWRITE_LATENCY_TOTAL_USECS	1790
/*! prefetch: could not perform pre-fetch on internal page */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_INTERNAL_PAGE	1791
/*!
 * prefetch: could not perform pre-fetch on ref without the pre-fetch
 * flag set
 */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_NO_FLAG_SET	1792
/*! prefetch: pre-fetch attempted, session has pre-fetching enabled */
#define	WT_STAT_CONN_PREFETCH_ATTEMPTS			1793
/*! prefetch: pre-fetch not repeating for recently pre-fetched ref */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_SAME_REF		1794
/*! prefetch: pre-fetch not triggered after single disk read */
#define	WT_STAT_CONN_PREFETCH_DISK_ONE			1795
/*! prefetch: pre-fetch not triggered as there is no valid dhandle */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_NO_VALID_DHANDLE	1796
/*! prefetch: pre-fetch not triggered due to disk read count */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_DISK_READ_COUNT	1797
/*! prefetch: pre-fetch not triggered due to internal session */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_INTERNAL_SESSION	1798
/*! prefetch: pre-fetch not triggered due to special btree handle */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_SPECIAL_HANDLE	1799
/*! prefetch: pre-fetch not triggered, pre-fetch queue is full */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_QUEUE_FULL	1800
/*! prefetch: pre-fetch page not on disk when reading */
#define	WT_STAT_CONN_PREFETCH_PAGES_FAIL		1801
/*! prefetch: pre-fetch pages queued */
#define	WT_STAT_CONN_PREFETCH_PAGES_QUEUED		1802
/*! prefetch: pre-fetch pages read in background */
#define	WT_STAT_CONN_PREFETCH_PAGES_READ		1803
/*!
 * prefetch: pre-fetch session check passed, pre-fetch work issued to
 * btree
 */
#define	WT_STAT_CONN_PREFETCH_ATTEMPTS_SUCCEEDED	1804
/*! prefetch: pre-fetch skipped reading in a page due to harmless error */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_ERROR_OK		1805
/*!
 * prefetch: pre-fetch skipped traversal of internal page due to active
 * split generation
 */
#define	WT_STAT_CONN_PREFETCH_SKIPPED_INTERNAL_SPLIT_GEN	1806
/*! reconciliation: VLCS pages explicitly reconciled as empty */
#define	WT_STAT_CONN_REC_VLCS_EMPTIED_PAGES		1807
/*! reconciliation: approximate byte size of timestamps in pages written */
#define	WT_STAT_CONN_REC_TIME_WINDOW_BYTES_TS		1808
/*!
 * reconciliation: approximate byte size of transaction IDs in pages
 * written
 */
#define	WT_STAT_CONN_REC_TIME_WINDOW_BYTES_TXN		1809
/*!
 * reconciliation: average length of delta chain on internal page with
 * deltas
 */
#define	WT_STAT_CONN_REC_AVERAGE_INTERNAL_PAGE_DELTA_CHAIN_LENGTH	1810
/*! reconciliation: average length of delta chain on leaf page with deltas */
#define	WT_STAT_CONN_REC_AVERAGE_LEAF_PAGE_DELTA_CHAIN_LENGTH	1811
/*!
 * reconciliation: changes since prior reconciliation (bucket 1) between
 * 1 and 5
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_LE5			1812
/*!
 * reconciliation: changes since prior reconciliation (bucket 2) between
 * 6 and 10
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_LE10			1813
/*!
 * reconciliation: changes since prior reconciliation (bucket 3) between
 * 11 and 20
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_LE20			1814
/*!
 * reconciliation: changes since prior reconciliation (bucket 4) between
 * 21 and 50
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_LE50			1815
/*!
 * reconciliation: changes since prior reconciliation (bucket 5) between
 * 51 and 100
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_LE100		1816
/*!
 * reconciliation: changes since prior reconciliation (bucket 6) between
 * 101 and 200
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_LE200		1817
/*!
 * reconciliation: changes since prior reconciliation (bucket 7) between
 * 201 and 500
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_LE500		1818
/*!
 * reconciliation: changes since prior reconciliation (bucket 8) greater
 * than 500
 */
#define	WT_STAT_CONN_REC_PAGE_MODS_GT500		1819
/*! reconciliation: cursor next/prev calls during HS wrapup search_near */
#define	WT_STAT_CONN_REC_HS_WRAPUP_NEXT_PREV_CALLS	1820
/*! reconciliation: fast-path pages deleted */
#define	WT_STAT_CONN_REC_PAGE_DELETE_FAST		1821
/*!
 * reconciliation: free page ID due to failed page replacement
 * reconciliation in disagg
 */
#define	WT_STAT_CONN_REC_FREE_PAGE_ID_DUE_TO_FAILED_REPLACEMENT_RECONCILIATION	1822
/*! reconciliation: full internal pages written instead of a page delta */
#define	WT_STAT_CONN_REC_PAGE_FULL_IMAGE_INTERNAL	1823
/*! reconciliation: full leaf pages written instead of a page delta */
#define	WT_STAT_CONN_REC_PAGE_FULL_IMAGE_LEAF		1824
/*!
 * reconciliation: ingest btree reconciliation keeps the aborted prepare
 * updates
 */
#define	WT_STAT_CONN_REC_INGEST_KEEP_PREPARE_ROLLBACK	1825
/*! reconciliation: internal page delta keys deleted */
#define	WT_STAT_CONN_REC_PAGE_DELTA_INTERNAL_KEY_DELETED	1826
/*! reconciliation: internal page delta keys updated/inserted */
#define	WT_STAT_CONN_REC_PAGE_DELTA_INTERNAL_KEY_UPDATED	1827
/*! reconciliation: internal page deltas written */
#define	WT_STAT_CONN_REC_PAGE_DELTA_INTERNAL		1828
/*! reconciliation: internal page multi-block writes */
#define	WT_STAT_CONN_REC_MULTIBLOCK_INTERNAL		1829
/*! reconciliation: leaf page deltas written */
#define	WT_STAT_CONN_REC_PAGE_DELTA_LEAF		1830
/*! reconciliation: leaf page multi-block writes */
#define	WT_STAT_CONN_REC_MULTIBLOCK_LEAF		1831
/*! reconciliation: leaf-page overflow keys */
#define	WT_STAT_CONN_REC_OVERFLOW_KEY_LEAF		1832
/*! reconciliation: max deltas seen on internal page during reconciliation */
#define	WT_STAT_CONN_REC_MAX_INTERNAL_PAGE_DELTAS	1833
/*! reconciliation: max deltas seen on leaf page during reconciliation */
#define	WT_STAT_CONN_REC_MAX_LEAF_PAGE_DELTAS		1834
/*! reconciliation: maximum milliseconds spent in a reconciliation call */
#define	WT_STAT_CONN_REC_MAXIMUM_MILLISECONDS		1835
/*!
 * reconciliation: maximum milliseconds spent in building a disk image in
 * a reconciliation
 */
#define	WT_STAT_CONN_REC_MAXIMUM_IMAGE_BUILD_MILLISECONDS	1836
/*!
 * reconciliation: maximum milliseconds spent in moving updates to the
 * history store in a reconciliation
 */
#define	WT_STAT_CONN_REC_MAXIMUM_HS_WRAPUP_MILLISECONDS	1837
/*!
 ...
 */
#define	WT_STAT_CONN_REC_INGEST_GARBAGE_COLLECTION_KEYS_DISK_IMAGE	1838
/*!
 * reconciliation: number of keys that are garbage collected from the
 * update chains in the ingest btrees for disaggregated storage
 */
#define	WT_STAT_CONN_REC_INGEST_GARBAGE_COLLECTION_KEYS_UPDATE_CHAIN	1839
/*! reconciliation: overflow values written */
#define	WT_STAT_CONN_REC_OVERFLOW_VALUE			1840
/*! reconciliation: page deltas rejected due to invalid page ID */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_INVALID_PAGE_ID	1841
/*! reconciliation: page deltas rejected due to max consecutive limit */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_MAX_CONSECUTIVE_EXCEEDED	1842
/*! reconciliation: page deltas rejected due to multiblock reconciliation */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_MULTIBLOCK	1843
/*!
 * reconciliation: page deltas rejected due to non-single page in
 * previous reconciliation
 */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_NON_SINGLE_PAGE	1844
/*! reconciliation: page deltas rejected due to size threshold */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_SIZE_THRESHOLD	1845
/*!
 * reconciliation: page deltas rejected due to too many keys removed from
 * the disk image
 */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_DELETE_THRESHOLD	1846
/*! reconciliation: page deltas rejected due to zero entries */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_ZERO_ENTRIES	1847
/*!
 ...
 */
#define	WT_STAT_CONN_REC_PAGE_DELTA_REJECTED_BUILD_FAILED	1848
/*! reconciliation: page reconciliation calls */
#define	WT_STAT_CONN_REC_PAGES				1849
/*! reconciliation: page reconciliation calls for eviction */
#define	WT_STAT_CONN_REC_PAGES_EVICTION			1850
/*! reconciliation: page reconciliation calls for pages between 1 and 10MB */
#define	WT_STAT_CONN_REC_PAGES_SIZE_1MB_TO_10MB		1851
/*!
 * reconciliation: page reconciliation calls for pages between 10 and
 * 100MB
 */
#define	WT_STAT_CONN_REC_PAGES_SIZE_10MB_TO_100MB	1852
/*!
 * reconciliation: page reconciliation calls for pages between 100MB and
 * 1GB
 */
#define	WT_STAT_CONN_REC_PAGES_SIZE_100MB_TO_1GB	1853
/*! reconciliation: page reconciliation calls for pages over 1GB */
#define	WT_STAT_CONN_REC_PAGES_SIZE_1GB_PLUS		1854
/*!
 * reconciliation: page reconciliation calls that resulted in values with
 * prepared transaction metadata
 */
#define	WT_STAT_CONN_REC_PAGES_WITH_PREPARE		1855
/*!
 * reconciliation: page reconciliation calls that resulted in values with
 * timestamps
 */
#define	WT_STAT_CONN_REC_PAGES_WITH_TS			1856
/*!
 * reconciliation: page reconciliation calls that resulted in values with
 * transaction ids
 */
#define	WT_STAT_CONN_REC_PAGES_WITH_TXN			1857
/*! reconciliation: pages deleted */
#define	WT_STAT_CONN_REC_PAGE_DELETE			1858
/*! reconciliation: pages eligible for delta generation */
#define	WT_STAT_CONN_REC_PAGE_DELTA_ELIGIBLE		1859
/*!
 * reconciliation: pages written including an aggregated newest start
 * durable timestamp
 */
#define	WT_STAT_CONN_REC_TIME_AGGR_NEWEST_START_DURABLE_TS	1860
/*!
 * reconciliation: pages written including an aggregated newest stop
 * durable timestamp
 */
#define	WT_STAT_CONN_REC_TIME_AGGR_NEWEST_STOP_DURABLE_TS	1861
/*!
 * reconciliation: pages written including an aggregated newest stop
 * timestamp
 */
#define	WT_STAT_CONN_REC_TIME_AGGR_NEWEST_STOP_TS	1862
/*!
 * reconciliation: pages written including an aggregated newest stop
 * transaction ID
 */
#define	WT_STAT_CONN_REC_TIME_AGGR_NEWEST_STOP_TXN	1863
/*!
 * reconciliation: pages written including an aggregated newest
 * transaction ID
 */
#define	WT_STAT_CONN_REC_TIME_AGGR_NEWEST_TXN		1864
/*!
 * reconciliation: pages written including an aggregated oldest start
 * timestamp
 */
#define	WT_STAT_CONN_REC_TIME_AGGR_OLDEST_START_TS	1865
/*! reconciliation: pages written including an aggregated prepare */
#define	WT_STAT_CONN_REC_TIME_AGGR_PREPARED		1866
/*! reconciliation: pages written including at least one prepare state */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PAGES_PREPARED	1867
/*!
 * reconciliation: pages written including at least one start durable
 * timestamp
 */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PAGES_DURABLE_START_TS	1868
/*! reconciliation: pages written including at least one start timestamp */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PAGES_START_TS	1869
/*!
 * reconciliation: pages written including at least one start transaction
 * ID
 */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PAGES_START_TXN	1870
/*!
 * reconciliation: pages written including at least one stop durable
 * timestamp
 */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PAGES_DURABLE_STOP_TS	1871
/*! reconciliation: pages written including at least one stop timestamp */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PAGES_STOP_TS	1872
/*!
 * reconciliation: pages written including at least one stop transaction
 * ID
 */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PAGES_STOP_TXN	1873
/*! reconciliation: pages written with at least one internal page delta */
#define	WT_STAT_CONN_REC_PAGES_WITH_INTERNAL_DELTAS	1874
/*! reconciliation: pages written with at least one leaf page delta */
#define	WT_STAT_CONN_REC_PAGES_WITH_LEAF_DELTAS		1875
/*! reconciliation: records written including a prepare state */
#define	WT_STAT_CONN_REC_TIME_WINDOW_PREPARED		1876
/*! reconciliation: records written including a start durable timestamp */
#define	WT_STAT_CONN_REC_TIME_WINDOW_DURABLE_START_TS	1877
/*! reconciliation: records written including a start timestamp */
#define	WT_STAT_CONN_REC_TIME_WINDOW_START_TS		1878
/*! reconciliation: records written including a start transaction ID */
#define	WT_STAT_CONN_REC_TIME_WINDOW_START_TXN		1879
/*! reconciliation: records written including a stop durable timestamp */
#define	WT_STAT_CONN_REC_TIME_WINDOW_DURABLE_STOP_TS	1880
/*! reconciliation: records written including a stop timestamp */
#define	WT_STAT_CONN_REC_TIME_WINDOW_STOP_TS		1881
/*! reconciliation: records written including a stop transaction ID */
#define	WT_STAT_CONN_REC_TIME_WINDOW_STOP_TXN		1882
/*! reconciliation: split bytes currently awaiting free */
#define	WT_STAT_CONN_REC_SPLIT_STASHED_BYTES		1883
/*! reconciliation: split objects currently awaiting free */
#define	WT_STAT_CONN_REC_SPLIT_STASHED_OBJECTS		1884
/*! reconciliation: writes skipped in disaggregated storage */
#define	WT_STAT_CONN_REC_SKIP_WRITE			1885
/*! session: open session count */
#define	WT_STAT_CONN_SESSION_OPEN			1886
/*! session: session query timestamp calls */
#define	WT_STAT_CONN_SESSION_QUERY_TS			1887
/*! session: table alter failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_ALTER_FAIL		1888
/*! session: table alter successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_ALTER_SUCCESS	1889
/*! session: table alter triggering checkpoint calls */
#define	WT_STAT_CONN_SESSION_TABLE_ALTER_TRIGGER_CHECKPOINT	1890
/*! session: table alter unchanged and skipped */
#define	WT_STAT_CONN_SESSION_TABLE_ALTER_SKIP		1891
/*! session: table compact conflicted with checkpoint */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_CONFLICTING_CHECKPOINT	1892
/*! session: table compact dhandle successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_DHANDLE_SUCCESS	1893
/*! session: table compact failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_FAIL		1894
/*! session: table compact failed calls due to cache pressure */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_FAIL_CACHE_PRESSURE	1895
/*!
 * session: table compact in-memory page footprint rewritten by
 * compaction
 */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_BYTES_REWRITE_INMEM	1896
/*! session: table compact passes */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_PASSES	1897
/*! session: table compact pulled into eviction */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_EVICTION	1898
/*! session: table compact running */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_RUNNING	1899
/*! session: table compact skipped as process would not reduce file size */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_SKIPPED	1900
/*! session: table compact successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_SUCCESS	1901
/*! session: table compact timeout */
#define	WT_STAT_CONN_SESSION_TABLE_COMPACT_TIMEOUT	1902
/*! session: table create failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_CREATE_FAIL		1903
/*! session: table create successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_CREATE_SUCCESS	1904
/*! session: table create with import failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_CREATE_IMPORT_FAIL	1905
/*! session: table create with import repair calls */
#define	WT_STAT_CONN_SESSION_TABLE_CREATE_IMPORT_REPAIR	1906
/*! session: table create with import successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_CREATE_IMPORT_SUCCESS	1907
/*! session: table drop failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_DROP_FAIL		1908
/*! session: table drop successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_DROP_SUCCESS		1909
/*! session: table publish failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_PUBLISH_FAIL		1910
/*! session: table publish successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_PUBLISH_SUCCESS	1911
/*! session: table salvage failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_SALVAGE_FAIL		1912
/*! session: table salvage successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_SALVAGE_SUCCESS	1913
/*! session: table truncate failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_TRUNCATE_FAIL	1914
/*! session: table truncate successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_TRUNCATE_SUCCESS	1915
/*! session: table verify failed calls */
#define	WT_STAT_CONN_SESSION_TABLE_VERIFY_FAIL		1916
/*! session: table verify successful calls */
#define	WT_STAT_CONN_SESSION_TABLE_VERIFY_SUCCESS	1917
/*! thread-state: active filesystem fsync calls */
#define	WT_STAT_CONN_THREAD_FSYNC_ACTIVE		1918
/*! thread-state: active filesystem read calls */
#define	WT_STAT_CONN_THREAD_READ_ACTIVE			1919
/*! thread-state: active filesystem write calls */
#define	WT_STAT_CONN_THREAD_WRITE_ACTIVE		1920
/*! thread-yield: application thread operations waiting for cache */
#define	WT_STAT_CONN_APPLICATION_CACHE_OPS		1921
/*!
 * thread-yield: application thread operations waiting for interruptible
 * cache eviction
 */
#define	WT_STAT_CONN_APPLICATION_CACHE_INTERRUPTIBLE_OPS	1922
/*!
 * thread-yield: application thread operations waiting for mandatory
 * cache eviction
 */
#define	WT_STAT_CONN_APPLICATION_CACHE_UNINTERRUPTIBLE_OPS	1923
/*! thread-yield: application thread snapshot refreshed for eviction */
#define	WT_STAT_CONN_APPLICATION_EVICT_SNAPSHOT_REFRESHED	1924
/*! thread-yield: application thread time waiting for cache (usecs) */
#define	WT_STAT_CONN_APPLICATION_CACHE_TIME		1925
/*!
 * thread-yield: application thread time waiting for interruptible cache
 * eviction (usecs)
 */
#define	WT_STAT_CONN_APPLICATION_CACHE_INTERRUPTIBLE_TIME	1926
/*!
 * thread-yield: application thread time waiting for mandatory cache
 * eviction (usecs)
 */
#define	WT_STAT_CONN_APPLICATION_CACHE_UNINTERRUPTIBLE_TIME	1927
/*!
 * thread-yield: connection close blocked waiting for transaction state
 * stabilization
 */
#define	WT_STAT_CONN_TXN_RELEASE_BLOCKED		1928
/*! thread-yield: data handle lock yielded */
#define	WT_STAT_CONN_DHANDLE_LOCK_BLOCKED		1929
/*!
 * thread-yield: get reference for page index and slot time sleeping
 * (usecs)
 */
#define	WT_STAT_CONN_PAGE_INDEX_SLOT_REF_BLOCKED	1930
/*! thread-yield: page access yielded due to prepare state change */
#define	WT_STAT_CONN_PREPARED_TRANSITION_BLOCKED_PAGE	1931
/*! thread-yield: page acquire busy blocked */
#define	WT_STAT_CONN_PAGE_BUSY_BLOCKED			1932
/*! thread-yield: page acquire eviction blocked */
#define	WT_STAT_CONN_PAGE_FORCIBLE_EVICT_BLOCKED	1933
/*! thread-yield: page acquire locked blocked */
#define	WT_STAT_CONN_PAGE_LOCKED_BLOCKED		1934
/*! thread-yield: page acquire read blocked */
#define	WT_STAT_CONN_PAGE_READ_BLOCKED			1935
/*! thread-yield: page acquire time sleeping (usecs) */
#define	WT_STAT_CONN_PAGE_SLEEP				1936
/*!
 * thread-yield: page delete rollback time sleeping for state change
 * (usecs)
 */
#define	WT_STAT_CONN_PAGE_DEL_ROLLBACK_BLOCKED		1937
/*! thread-yield: page reconciliation yielded due to child modification */
#define	WT_STAT_CONN_CHILD_MODIFY_BLOCKED_PAGE		1938
/*! thread-yield: page split and restart read */
#define	WT_STAT_CONN_PAGE_SPLIT_RESTART			1939
/*! thread-yield: pages skipped during read due to deleted state */
#define	WT_STAT_CONN_PAGE_READ_SKIP_DELETED		1940
/*!
 * tiered-storage: attempts to remove a local object and the object is in
 * use
 */
#define	WT_STAT_CONN_LOCAL_OBJECTS_INUSE		1941
/*! tiered-storage: flush_tier failed calls */
#define	WT_STAT_CONN_FLUSH_TIER_FAIL			1942
/*! tiered-storage: flush_tier operation calls */
#define	WT_STAT_CONN_FLUSH_TIER				1943
/*! tiered-storage: flush_tier tables skipped due to no checkpoint */
#define	WT_STAT_CONN_FLUSH_TIER_SKIPPED			1944
/*! tiered-storage: flush_tier tables switched */
#define	WT_STAT_CONN_FLUSH_TIER_SWITCHED		1945
/*! tiered-storage: local objects removed */
#define	WT_STAT_CONN_LOCAL_OBJECTS_REMOVED		1946
/*! tiered-storage: tiered operations dequeued and processed */
#define	WT_STAT_CONN_TIERED_WORK_UNITS_DEQUEUED		1947
/*! tiered-storage: tiered operations removed without processing */
#define	WT_STAT_CONN_TIERED_WORK_UNITS_REMOVED		1948
/*! tiered-storage: tiered operations scheduled */
#define	WT_STAT_CONN_TIERED_WORK_UNITS_CREATED		1949
/*! tiered-storage: tiered storage local retention time (secs) */
#define	WT_STAT_CONN_TIERED_RETENTION			1950
/*! transaction: Number of prepared updates */
#define	WT_STAT_CONN_TXN_PREPARED_UPDATES		1951
/*! transaction: Number of prepared updates committed */
#define	WT_STAT_CONN_TXN_PREPARED_UPDATES_COMMITTED	1952
/*! transaction: Number of prepared updates repeated on the same key */
#define	WT_STAT_CONN_TXN_PREPARED_UPDATES_KEY_REPEATED	1953
/*! transaction: Number of prepared updates rolled back */
#define	WT_STAT_CONN_TXN_PREPARED_UPDATES_ROLLEDBACK	1954
/*!
 * transaction: a reader raced with a prepared transaction commit and
 * skipped an update or updates
 */
#define	WT_STAT_CONN_TXN_READ_RACE_PREPARE_COMMIT	1955
/*! transaction: number of times overflow removed value is read */
#define	WT_STAT_CONN_TXN_READ_OVERFLOW_REMOVE		1956
/*! transaction: oldest pinned transaction ID rolled back for eviction */
#define	WT_STAT_CONN_TXN_ROLLBACK_OLDEST_PINNED		1957
/*! transaction: oldest transaction ID rolled back for eviction */
#define	WT_STAT_CONN_TXN_ROLLBACK_OLDEST_ID		1958
/*! transaction: prepared transactions */
#define	WT_STAT_CONN_TXN_PREPARE			1959
/*! transaction: prepared transactions committed */
#define	WT_STAT_CONN_TXN_PREPARE_COMMIT			1960
/*! transaction: prepared transactions currently active */
#define	WT_STAT_CONN_TXN_PREPARE_ACTIVE			1961
/*! transaction: prepared transactions rolled back */
#define	WT_STAT_CONN_TXN_PREPARE_ROLLBACK		1962
/*! transaction: query timestamp calls */
#define	WT_STAT_CONN_TXN_QUERY_TS			1963
/*! transaction: race to read prepared update retry */
#define	WT_STAT_CONN_TXN_READ_RACE_PREPARE_UPDATE	1964
/*! transaction: rollback to stable applied btrees */
#define	WT_STAT_CONN_TXN_RTS_BTREES_APPLIED		1965
/*! transaction: rollback to stable calls */
#define	WT_STAT_CONN_TXN_RTS				1966
/*!
 * transaction: rollback to stable history store keys that would have
 * been swept in non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_SWEEP_HS_KEYS_DRYRUN	1967
/*!
 * transaction: rollback to stable history store records with stop
 * timestamps older than newer records
 */
#define	WT_STAT_CONN_TXN_RTS_HS_STOP_OLDER_THAN_NEWER_START	1968
/*! transaction: rollback to stable inconsistent checkpoint */
#define	WT_STAT_CONN_TXN_RTS_INCONSISTENT_CKPT		1969
/*! transaction: rollback to stable keys removed */
#define	WT_STAT_CONN_TXN_RTS_KEYS_REMOVED		1970
/*! transaction: rollback to stable keys restored */
#define	WT_STAT_CONN_TXN_RTS_KEYS_RESTORED		1971
/*!
 * transaction: rollback to stable keys that would have been removed in
 * non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_KEYS_REMOVED_DRYRUN	1972
/*!
 * transaction: rollback to stable keys that would have been restored in
 * non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_KEYS_RESTORED_DRYRUN	1973
/*! transaction: rollback to stable pages visited */
#define	WT_STAT_CONN_TXN_RTS_PAGES_VISITED		1974
/*!
 * transaction: rollback to stable prepared fast truncations that would
 * have been rolled back in non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_PREPARED_FAST_TRUNCATE_DRYRUN	1975
/*! transaction: rollback to stable restored tombstones from history store */
#define	WT_STAT_CONN_TXN_RTS_HS_RESTORE_TOMBSTONES	1976
/*! transaction: rollback to stable restored updates from history store */
#define	WT_STAT_CONN_TXN_RTS_HS_RESTORE_UPDATES		1977
/*! transaction: rollback to stable rolled back prepared fast truncations */
#define	WT_STAT_CONN_TXN_RTS_PREPARED_FAST_TRUNCATE	1978
/*! transaction: rollback to stable skipped btrees */
#define	WT_STAT_CONN_TXN_RTS_BTREES_SKIPPED		1979
/*! transaction: rollback to stable skipping delete rle */
#define	WT_STAT_CONN_TXN_RTS_DELETE_RLE_SKIPPED		1980
/*! transaction: rollback to stable skipping stable rle */
#define	WT_STAT_CONN_TXN_RTS_STABLE_RLE_SKIPPED		1981
/*! transaction: rollback to stable sweeping history store keys */
#define	WT_STAT_CONN_TXN_RTS_SWEEP_HS_KEYS		1982
/*!
 * transaction: rollback to stable tombstones from history store that
 * would have been restored in non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_HS_RESTORE_TOMBSTONES_DRYRUN	1983
/*! transaction: rollback to stable tree walk skipping pages */
#define	WT_STAT_CONN_TXN_RTS_TREE_WALK_SKIP_PAGES	1984
/*! transaction: rollback to stable updates aborted */
#define	WT_STAT_CONN_TXN_RTS_UPD_ABORTED		1985
/*!
 * transaction: rollback to stable updates from history store that would
 * have been restored in non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_HS_RESTORE_UPDATES_DRYRUN	1986
/*! transaction: rollback to stable updates removed from history store */
#define	WT_STAT_CONN_TXN_RTS_HS_REMOVED			1987
/*!
 * transaction: rollback to stable updates that would have been aborted
 * in non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_UPD_ABORTED_DRYRUN		1988
/*!
 * transaction: rollback to stable updates that would have been removed
 * from history store in non-dryrun mode
 */
#define	WT_STAT_CONN_TXN_RTS_HS_REMOVED_DRYRUN		1989
/*! transaction: sessions scanned in each walk of concurrent sessions */
#define	WT_STAT_CONN_TXN_SESSIONS_WALKED		1990
/*! transaction: set timestamp calls */
#define	WT_STAT_CONN_TXN_SET_TS				1991
/*! transaction: set timestamp durable calls */
#define	WT_STAT_CONN_TXN_SET_TS_DURABLE			1992
/*! transaction: set timestamp durable updates */
#define	WT_STAT_CONN_TXN_SET_TS_DURABLE_UPD		1993
/*! transaction: set timestamp force calls */
#define	WT_STAT_CONN_TXN_SET_TS_FORCE			1994
/*!
 * transaction: set timestamp global oldest timestamp set to be more
 * recent than the global stable timestamp
 */
#define	WT_STAT_CONN_TXN_SET_TS_OUT_OF_ORDER		1995
/*! transaction: set timestamp oldest calls */
#define	WT_STAT_CONN_TXN_SET_TS_OLDEST			1996
/*! transaction: set timestamp oldest updates */
#define	WT_STAT_CONN_TXN_SET_TS_OLDEST_UPD		1997
/*! transaction: set timestamp stable calls */
#define	WT_STAT_CONN_TXN_SET_TS_STABLE			1998
/*! transaction: set timestamp stable disaggregated schema epoch calls */
#define	WT_STAT_CONN_TXN_SET_TS_STABLE_DISAGG_EPOCH	1999
/*! transaction: set timestamp stable disaggregated schema epoch updates */
#define	WT_STAT_CONN_TXN_SET_TS_STABLE_DISAGG_EPOCH_UPD	2000
/*! transaction: set timestamp stable updates */
#define	WT_STAT_CONN_TXN_SET_TS_STABLE_UPD		2001
/*! transaction: set timestamp step down updates */
#define	WT_STAT_CONN_TXN_SET_TS_STEP_DOWN_UPD		2002
/*! transaction: transaction begins */
#define	WT_STAT_CONN_TXN_BEGIN				2003
/*!
 * transaction: transaction checkpoint history store file duration
 * (usecs)
 */
#define	WT_STAT_CONN_TXN_HS_CKPT_DURATION		2004
/*! transaction: transaction global checkpoint timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_CHECKPOINT_TIMESTAMP	2005
/*! transaction: transaction global durable timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_DURABLE_TIMESTAMP	2006
/*! transaction: transaction global last running timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_LAST_RUNNING_TIMESTAMP	2007
/*! transaction: transaction global newest timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_NEWEST_TIMESTAMP	2008
/*! transaction: transaction global oldest timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_OLDEST_TIMESTAMP	2009
/*! transaction: transaction global pinned timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_PINNED_TIMESTAMP	2010
/*! transaction: transaction global stable timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_STABLE_TIMESTAMP	2011
/*! transaction: transaction global version cursor timestamp */
#define	WT_STAT_CONN_TXN_GLOBAL_VERSION_CURSOR_TIMESTAMP	2012
/*!
 * transaction: transaction number of older readers older than oldest
 * timestamp
 */
#define	WT_STAT_CONN_TXN_PINNED_READERS			2013
/*! transaction: transaction range of IDs currently pinned */
#define	WT_STAT_CONN_TXN_PINNED_RANGE			2014
/*! transaction: transaction range of IDs currently pinned by a checkpoint */
#define	WT_STAT_CONN_TXN_PINNED_CHECKPOINT_RANGE	2015
/*! transaction: transaction range of timestamps currently pinned */
#define	WT_STAT_CONN_TXN_PINNED_TIMESTAMP_LAG		2016
/*! transaction: transaction range of timestamps pinned by a checkpoint */
#define	WT_STAT_CONN_TXN_PINNED_TIMESTAMP_CHECKPOINT_LAG	2017
/*!
 * transaction: transaction range of timestamps pinned by the oldest
 * active read timestamp
 */
#define	WT_STAT_CONN_TXN_PINNED_TIMESTAMP_READER_LAG	2018
/*!
 * transaction: transaction range of timestamps pinned by the oldest
 * timestamp
 */
#define	WT_STAT_CONN_TXN_PINNED_TIMESTAMP_OLDEST	2019
/*! transaction: transaction read timestamp of the oldest active reader */
#define	WT_STAT_CONN_TXN_TIMESTAMP_OLDEST_ACTIVE_READ	2020
/*! transaction: transaction rollback to stable currently running */
#define	WT_STAT_CONN_TXN_ROLLBACK_TO_STABLE_RUNNING	2021
/*! transaction: transaction walk of concurrent sessions */
#define	WT_STAT_CONN_TXN_WALK_SESSIONS			2022
/*! transaction: transactions committed */
#define	WT_STAT_CONN_TXN_COMMIT				2023
/*! transaction: transactions rolled back */
#define	WT_STAT_CONN_TXN_ROLLBACK			2024
/*! transaction: update conflicts */
#define	WT_STAT_CONN_TXN_UPDATE_CONFLICT		2025
...
/*! block-disagg: Disaggregated block manager get cold page */
#define	WT_STAT_DSRC_DISAGG_BLOCK_GET_COLD		2007
/*!
 * block-disagg: Disaggregated block manager get from the shared history
 * store in SLS
 */
#define	WT_STAT_DSRC_DISAGG_BLOCK_HS_GET		2008
/*! block-disagg: Disaggregated block manager log handle put failure */
#define	WT_STAT_DSRC_DISAGG_BLOCK_PLH_PUT_FAILED	2009
/*! block-disagg: Disaggregated block manager page discard calls */
#define	WT_STAT_DSRC_DISAGG_BLOCK_PAGE_DISCARD		2010
/*! block-disagg: Disaggregated block manager put */
#define	WT_STAT_DSRC_DISAGG_BLOCK_PUT			2011
/*! block-disagg: Disaggregated block manager put cold page */
#define	WT_STAT_DSRC_DISAGG_BLOCK_PUT_COLD		2012
/*!
 * block-disagg: Disaggregated block manager put to the shared history
 * store in SLS
 */
#define	WT_STAT_DSRC_DISAGG_BLOCK_HS_PUT		2013
/*!
 * block-disagg: Disaggregated block manager read ahead of
 * materialization frontier
 */
#define	WT_STAT_DSRC_DISAGG_BLOCK_READ_AHEAD_FRONTIER	2014
/*! block-manager: allocations requiring file extension */
#define	WT_STAT_DSRC_BLOCK_EXTENSION			2015
/*! block-manager: blocks allocated */
#define	WT_STAT_DSRC_BLOCK_ALLOC			2016
/*! block-manager: blocks freed */
#define	WT_STAT_DSRC_BLOCK_FREE				2017
...
#define	WT_STAT_DSRC_BTREE_COMPACT_PAGES_SELECTED_INMEM	2027
...
#define	WT_STAT_DSRC_BTREE_ROW_LEAF_AVG_ENTRIES		2049
/*!
...
#define	WT_STAT_DSRC_BTREE_ROW_LEAF_PAGES		2051
/*!
 * btree: time spent walking the tree for checkpoint including dirty page
 * reconciliation time (usecs)
 */
#define	WT_STAT_DSRC_BTREE_CHECKPOINT_RECONCILE_DURATION	2052
/*!
 * cache: application threads eviction requested with cache fill ratio <
 * 25%
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_APP_THREADS_FILL_RATIO_LT_25	2053
/*!
 * cache: application threads eviction requested with cache fill ratio >=
 * 25% and < 50%
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_APP_THREADS_FILL_RATIO_25_50	2054
/*!
 * cache: application threads eviction requested with cache fill ratio >=
 * 50% and < 75%
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_APP_THREADS_FILL_RATIO_50_75	2055
/*!
 * cache: application threads eviction requested with cache fill ratio >=
 * 75%
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_APP_THREADS_FILL_RATIO_GT_75	2056
/*!
 * cache: application threads eviction skip page with updates or dirty
 * page
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_APP_THREADS_SKIP_UPDATES_DIRTY_PAGE	2057
/*! cache: bytes currently in the cache */
...
/*! cache: dirty internal page cannot be evicted in disaggregated storage */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_DISAGG_DIRTY_INTERNAL_PAGE	2065
/*!
 * cache: eviction gave up due to detecting a disk value without a
 * timestamp behind the last update on the chain
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_1	2066
/*!
 * cache: eviction gave up due to detecting a tombstone without a
 * timestamp ahead of the selected on disk update
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_2	2067
/*!
 * cache: eviction gave up due to detecting a tombstone without a
 * timestamp ahead of the selected on disk update after validating the
 * update chain
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_3	2068
/*!
 * cache: eviction gave up due to detecting update chain entries without
 * timestamps after the selected on disk update
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_NO_TS_CHECKPOINT_RACE_4	2069
/*!
 * cache: eviction gave up due to needing to remove a record from the
 * history store but checkpoint is running
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_REMOVE_HS_RACE_WITH_CHECKPOINT	2070
/*! cache: eviction gave up due to no progress being made */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_NO_PROGRESS	2071
/*! cache: eviction server skipped the pages when prefetching */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_PREFETCHED	2072
/*! cache: eviction walk pages queued that had updates */
#define	WT_STAT_DSRC_CACHE_EVICTION_PAGES_QUEUED_UPDATES	2073
/*! cache: eviction walk pages queued that were clean */
#define	WT_STAT_DSRC_CACHE_EVICTION_PAGES_QUEUED_CLEAN	2074
/*! cache: eviction walk pages queued that were dirty */
#define	WT_STAT_DSRC_CACHE_EVICTION_PAGES_QUEUED_DIRTY	2075
/*! cache: eviction walk pages seen that had updates */
#define	WT_STAT_DSRC_CACHE_EVICTION_PAGES_SEEN_UPDATES	2076
/*! cache: eviction walk pages seen that were clean */
#define	WT_STAT_DSRC_CACHE_EVICTION_PAGES_SEEN_CLEAN	2077
/*! cache: eviction walk pages seen that were dirty */
#define	WT_STAT_DSRC_CACHE_EVICTION_PAGES_SEEN_DIRTY	2078
/*! cache: eviction walk passes of a file */
#define	WT_STAT_DSRC_EVICTION_WALK_PASSES		2079
/*! cache: eviction walk target pages histogram - 0-9 */
#define	WT_STAT_DSRC_CACHE_EVICTION_TARGET_PAGE_LT10	2080
/*! cache: eviction walk target pages histogram - 10-31 */
#define	WT_STAT_DSRC_CACHE_EVICTION_TARGET_PAGE_LT32	2081
/*! cache: eviction walk target pages histogram - 128 and higher */
#define	WT_STAT_DSRC_CACHE_EVICTION_TARGET_PAGE_GE128	2082
/*! cache: eviction walk target pages histogram - 32-63 */
#define	WT_STAT_DSRC_CACHE_EVICTION_TARGET_PAGE_LT64	2083
/*! cache: eviction walk target pages histogram - 64-128 */
#define	WT_STAT_DSRC_CACHE_EVICTION_TARGET_PAGE_LT128	2084
/*!
 * cache: eviction walk target pages reduced due to history store cache
 * pressure
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_TARGET_PAGE_REDUCED	2085
/*!
 * cache: garbage collection from the ingest btree page is skipped
 * because the prune timestamp has not moved
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_PRUNE_TIMESTAMP	2086
/*! cache: hazard pointer blocked page eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_HAZARD	2087
/*! cache: history store cursor not cached during eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_HS_CURSOR_NOT_CACHED	2088
/*! cache: history store table insert calls */
#define	WT_STAT_DSRC_CACHE_HS_INSERT			2089
/*! cache: history store table insert calls that returned restart */
#define	WT_STAT_DSRC_CACHE_HS_INSERT_RESTART		2090
/*! cache: history store table key to be processed */
#define	WT_STAT_DSRC_CACHE_HS_KEY_PROCESSED		2091
/*! cache: history store table reads */
#define	WT_STAT_DSRC_CACHE_HS_READ			2092
/*! cache: history store table reads missed */
#define	WT_STAT_DSRC_CACHE_HS_READ_MISS			2093
/*! cache: history store table reads requiring squashed modifies */
#define	WT_STAT_DSRC_CACHE_HS_READ_SQUASH		2094
/*!
 * cache: history store table resolved updates without timestamps that
 * lose their durable timestamp
 */
#define	WT_STAT_DSRC_CACHE_HS_ORDER_LOSE_DURABLE_TIMESTAMP	2095
/*!
 * cache: history store table truncation by rollback to stable to remove
 * an unstable update
 */
#define	WT_STAT_DSRC_CACHE_HS_KEY_TRUNCATE_RTS_UNSTABLE	2096
/*!
 * cache: history store table truncation by rollback to stable to remove
 * an update
 */
#define	WT_STAT_DSRC_CACHE_HS_KEY_TRUNCATE_RTS		2097
/*!
 * cache: history store table truncation to remove all the keys of a
 * btree
 */
#define	WT_STAT_DSRC_CACHE_HS_BTREE_TRUNCATE		2098
/*! cache: history store table truncation to remove an update */
#define	WT_STAT_DSRC_CACHE_HS_KEY_TRUNCATE		2099
/*!
 * cache: history store table truncation to remove range of updates due
 * to an update without a timestamp on data page
 */
#define	WT_STAT_DSRC_CACHE_HS_ORDER_REMOVE		2100
/*!
 * cache: history store table truncation to remove range of updates due
 * to key being removed from the data page during reconciliation
 */
#define	WT_STAT_DSRC_CACHE_HS_KEY_TRUNCATE_ONPAGE_REMOVAL	2101
/*!
 * cache: history store table truncations that would have happened in
 * non-dryrun mode
 */
#define	WT_STAT_DSRC_CACHE_HS_BTREE_TRUNCATE_DRYRUN	2102
/*!
 * cache: history store table truncations to remove an unstable update
 * that would have happened in non-dryrun mode
 */
#define	WT_STAT_DSRC_CACHE_HS_KEY_TRUNCATE_RTS_UNSTABLE_DRYRUN	2103
/*!
 * cache: history store table truncations to remove an update that would
 * have happened in non-dryrun mode
 */
#define	WT_STAT_DSRC_CACHE_HS_KEY_TRUNCATE_RTS_DRYRUN	2104
/*! cache: history store table update to be processed */
#define	WT_STAT_DSRC_CACHE_HS_UPDATE_PROCESSED		2105
/*!
 * cache: history store table updates without timestamps fixed up by
 * reinserting with the fixed timestamp
 */
#define	WT_STAT_DSRC_CACHE_HS_ORDER_REINSERT		2106
/*! cache: history store table writes requiring squashed modifies */
#define	WT_STAT_DSRC_CACHE_HS_WRITE_SQUASH		2107
/*! cache: in-memory page passed criteria to be split */
#define	WT_STAT_DSRC_CACHE_INMEM_SPLITTABLE		2108
/*! cache: in-memory page splits */
#define	WT_STAT_DSRC_CACHE_INMEM_SPLIT			2109
/*! cache: in-memory page splits performed on ingest btree pages */
#define	WT_STAT_DSRC_CACHE_INMEM_SPLIT_INGEST		2110
/*! cache: internal page split blocked its eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_INTERNAL_PAGE_SPLIT	2111
/*! cache: internal pages evicted */
#define	WT_STAT_DSRC_CACHE_EVICTION_INTERNAL		2112
/*! cache: internal pages read into cache */
#define	WT_STAT_DSRC_CACHE_READ_INTERNAL		2113
/*! cache: internal pages split during eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_SPLIT_INTERNAL	2114
/*! cache: leaf pages read into cache */
#define	WT_STAT_DSRC_CACHE_READ_LEAF			2115
/*! cache: leaf pages split during eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_SPLIT_LEAF		2116
/*!
 * cache: locate a random in-mem ref by examining all entries on the root
 * page
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_RANDOM_SAMPLE_INMEM_ROOT	2117
/*! cache: modified pages evicted */
#define	WT_STAT_DSRC_CACHE_EVICTION_DIRTY		2118
/*! cache: multi-block reconciliation blocked whilst checkpoint is running */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_MULTI_BLOCK_RECONCILIATION_DURING_CHECKPOINT	2119
/*! cache: number of internal pages read that had deltas attached */
#define	WT_STAT_DSRC_CACHE_READ_INTERNAL_DELTA		2120
/*! cache: number of leaf pages read that had deltas attached */
#define	WT_STAT_DSRC_CACHE_READ_LEAF_DELTA		2121
/*! cache: number of times dirty trigger was reached */
#define	WT_STAT_DSRC_CACHE_EVICTION_TRIGGER_DIRTY_REACHED	2122
/*! cache: number of times eviction trigger was reached */
#define	WT_STAT_DSRC_CACHE_EVICTION_TRIGGER_REACHED	2123
/*! cache: number of times updates trigger was reached */
#define	WT_STAT_DSRC_CACHE_EVICTION_TRIGGER_UPDATES_REACHED	2124
/*! cache: number of times when cas update the btree max_lsn failed */
#define	WT_STAT_DSRC_CACHE_CAS_BTREE_MAX_LSN_RACE	2125
/*! cache: obsolete updates removed */
#define	WT_STAT_DSRC_CACHE_OBSOLETE_UPDATES_REMOVED	2126
/*!
 * cache: overflow keys on a multiblock row-store page blocked its
 * eviction
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_OVERFLOW_KEYS	2127
/*! cache: overflow pages read into cache */
#define	WT_STAT_DSRC_CACHE_READ_OVERFLOW		2128
/*! cache: page eviction blocked due to materialization frontier */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_MATERIALIZATION	2129
/*!
 * cache: page eviction blocked in disaggregated storage as it can only
 * be written by the next checkpoint
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_DISAGG_NEXT_CHECKPOINT	2130
/*! cache: page split during eviction deepened the tree */
#define	WT_STAT_DSRC_CACHE_EVICTION_DEEPEN		2131
/*! cache: page written requiring history store records */
#define	WT_STAT_DSRC_CACHE_WRITE_HS			2132
/*! cache: pages dirtied due to obsolete time window by eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_DIRTY_OBSOLETE_TW	2133
/*! cache: pages evicted ahead of the page materialization frontier */
#define	WT_STAT_DSRC_CACHE_EVICTION_AHEAD_OF_LAST_MATERIALIZED_LSN	2134
/*! cache: pages read into cache after truncate */
#define	WT_STAT_DSRC_CACHE_READ_DELETED			2135
/*! cache: pages read into cache after truncate in prepare state */
#define	WT_STAT_DSRC_CACHE_READ_DELETED_PREPARED	2136
/*! cache: pages read into cache by checkpoint */
#define	WT_STAT_DSRC_CACHE_READ_CHECKPOINT		2137
/*! cache: pages requested from the cache due to pre-fetch */
#define	WT_STAT_DSRC_CACHE_PAGES_PREFETCH		2138
/*! cache: pages requested from the cache internal */
#define	WT_STAT_DSRC_CACHE_PAGES_REQUESTED_INTERNAL	2139
/*! cache: pages requested from the cache leaf */
#define	WT_STAT_DSRC_CACHE_PAGES_REQUESTED_LEAF		2140
/*! cache: pages requested from the history store */
#define	WT_STAT_DSRC_CACHE_PAGES_REQUESTED_HS		2141
/*! cache: pages seen by eviction walk */
#define	WT_STAT_DSRC_CACHE_EVICTION_PAGES_SEEN		2142
/*!
 * cache: pages with an unresolved multiblock split flagged by checkpoint
 * to be evicted soon
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_MULTIBLOCK_CHECKPOINT_FLAGGED	2143
/*!
 * cache: pages with an unresolved multiblock split re-reconciled by
 * checkpoint
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_MULTIBLOCK_SPLIT_RE_RECONCILED	2144
/*! cache: pages written from cache */
#define	WT_STAT_DSRC_CACHE_WRITE			2145
/*!
 * cache: pages written requiring in-memory restoration due to invisible
 * updates
 */
#define	WT_STAT_DSRC_CACHE_WRITE_RESTORE_INVISIBLE	2146
/*!
 * cache: pages written requiring in-memory restoration due to scrub
 * eviction
 */
#define	WT_STAT_DSRC_CACHE_WRITE_RESTORE_SCRUB		2147
/*!
 * cache: precise checkpoint caused an eviction to be skipped because any
 * dirty content needs to remain in cache
 */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_PRECISE_CHECKPOINT	2148
/*!
 * cache: realizing in-memory split after reconciliation failed due to
 * internal lock busy
 */
#define	WT_STAT_DSRC_CACHE_EVICT_SPLIT_FAILED_LOCK	2149
/*! cache: recent modification of a page blocked its eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_RECENTLY_MODIFIED	2150
/*! cache: reconciled pages scrubbed and added back to the cache clean */
#define	WT_STAT_DSRC_CACHE_SCRUB_RESTORE		2151
/*! cache: reverse splits performed */
#define	WT_STAT_DSRC_CACHE_REVERSE_SPLITS		2152
/*!
 * cache: reverse splits skipped because of VLCS namespace gap
 * restrictions
 */
#define	WT_STAT_DSRC_CACHE_REVERSE_SPLITS_SKIPPED_VLCS	2153
/*! cache: shared history store cursor not cached during eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_HS_SHARED_CURSOR_NOT_CACHED	2154
/*! cache: size of delta updates reconstructed on the base page */
#define	WT_STAT_DSRC_CACHE_READ_DELTA_UPDATES		2155
/*! cache: size of tombstones restored when reading a page */
#define	WT_STAT_DSRC_CACHE_READ_RESTORED_TOMBSTONE_BYTES	2156
/*! cache: the number of times full update inserted to history store */
#define	WT_STAT_DSRC_CACHE_HS_INSERT_FULL_UPDATE	2157
/*! cache: the number of times reverse modify inserted to history store */
#define	WT_STAT_DSRC_CACHE_HS_INSERT_REVERSE_MODIFY	2158
/*! cache: tracked dirty bytes in the cache */
#define	WT_STAT_DSRC_CACHE_BYTES_DIRTY			2159
/*! cache: tracked dirty internal page bytes in the cache */
#define	WT_STAT_DSRC_CACHE_BYTES_DIRTY_INTERNAL		2160
/*! cache: tracked dirty leaf page bytes in the cache */
#define	WT_STAT_DSRC_CACHE_BYTES_DIRTY_LEAF		2161
/*! cache: uncommitted truncate blocked page eviction */
#define	WT_STAT_DSRC_CACHE_EVICTION_BLOCKED_UNCOMMITTED_TRUNCATE	2162
/*! cache: unmodified pages evicted */
#define	WT_STAT_DSRC_CACHE_EVICTION_CLEAN		2163
/*!
 * cache_walk: Average difference between current eviction generation
 * when the page was last considered, only reported if cache_walk or all
 * statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_GEN_AVG_GAP		2164
/*!
 * cache_walk: Average on-disk page image size seen, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_AVG_WRITTEN_SIZE	2165
/*!
 * cache_walk: Average time in cache for pages that have been visited by
 * the eviction server, only reported if cache_walk or all statistics are
 * enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_AVG_VISITED_AGE	2166
/*!
 * cache_walk: Average time in cache for pages that have not been visited
 * by the eviction server, only reported if cache_walk or all statistics
 * are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_AVG_UNVISITED_AGE	2167
/*!
 * cache_walk: Clean pages currently in cache, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_PAGES_CLEAN		2168
/*!
 * cache_walk: Current eviction generation, only reported if cache_walk
 * or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_GEN_CURRENT		2169
/*!
 * cache_walk: Dirty pages currently in cache, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_PAGES_DIRTY		2170
/*!
 * cache_walk: Entries in the root page, only reported if cache_walk or
 * all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_ROOT_ENTRIES		2171
/*!
 * cache_walk: Internal pages currently in cache, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_PAGES_INTERNAL		2172
/*!
 * cache_walk: Leaf pages currently in cache, only reported if cache_walk
 * or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_PAGES_LEAF		2173
/*!
 * cache_walk: Maximum difference between current eviction generation
 * when the page was last considered, only reported if cache_walk or all
 * statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_GEN_MAX_GAP		2174
/*!
 * cache_walk: Maximum page size seen, only reported if cache_walk or all
 * statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_MAX_PAGESIZE		2175
/*!
 * cache_walk: Minimum on-disk page image size seen, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_MIN_WRITTEN_SIZE	2176
/*!
 * cache_walk: Number of pages never visited by eviction server, only
 * reported if cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_UNVISITED_COUNT	2177
/*!
 * cache_walk: On-disk page image sizes smaller than a single allocation
 * unit, only reported if cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_SMALLER_ALLOC_SIZE	2178
/*!
 * cache_walk: Pages created in memory and never written, only reported
 * if cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_MEMORY			2179
/*!
 * cache_walk: Pages currently queued for eviction, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_QUEUED			2180
/*!
 * cache_walk: Pages that could not be queued for eviction, only reported
 * if cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_NOT_QUEUEABLE		2181
/*!
 * cache_walk: Refs skipped during cache traversal, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_REFS_SKIPPED		2182
/*!
 * cache_walk: Size of the root page, only reported if cache_walk or all
 * statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_ROOT_SIZE		2183
/*!
 * cache_walk: Total number of pages currently in cache, only reported if
 * cache_walk or all statistics are enabled
 */
#define	WT_STAT_DSRC_CACHE_STATE_PAGES			2184
/*! checkpoint-cleanup: pages added for eviction */
#define	WT_STAT_DSRC_CHECKPOINT_CLEANUP_PAGES_EVICT	2185
/*! checkpoint-cleanup: pages dirtied due to obsolete time window */
#define	WT_STAT_DSRC_CHECKPOINT_CLEANUP_PAGES_OBSOLETE_TW	2186
/*! checkpoint-cleanup: pages read into cache (reclaim_space) */
#define	WT_STAT_DSRC_CHECKPOINT_CLEANUP_PAGES_READ_RECLAIM_SPACE	2187
/*! checkpoint-cleanup: pages read into cache due to obsolete time window */
#define	WT_STAT_DSRC_CHECKPOINT_CLEANUP_PAGES_READ_OBSOLETE_TW	2188
/*! checkpoint-cleanup: pages removed */
#define	WT_STAT_DSRC_CHECKPOINT_CLEANUP_PAGES_REMOVED	2189
/*! checkpoint-cleanup: pages skipped during tree walk */
#define	WT_STAT_DSRC_CHECKPOINT_CLEANUP_PAGES_WALK_SKIPPED	2190
/*! checkpoint-cleanup: pages visited */
#define	WT_STAT_DSRC_CHECKPOINT_CLEANUP_PAGES_VISITED	2191
/*! checkpoint: checkpoint has acquired a snapshot for its transaction */
#define	WT_STAT_DSRC_CHECKPOINT_SNAPSHOT_ACQUIRED	2192
/*!
 * compression: compressed page maximum internal page size prior to
 * compression
 */
#define	WT_STAT_DSRC_COMPRESS_PRECOMP_INTL_MAX_PAGE_SIZE	2193
/*!
 * compression: compressed page maximum leaf page size prior to
 * compression
 */
#define	WT_STAT_DSRC_COMPRESS_PRECOMP_LEAF_MAX_PAGE_SIZE	2194
/*! compression: page written to disk failed to compress */
#define	WT_STAT_DSRC_COMPRESS_WRITE_FAIL		2195
/*! compression: page written to disk was too small to compress */
#define	WT_STAT_DSRC_COMPRESS_WRITE_TOO_SMALL		2196
/*! compression: pages read from disk */
#define	WT_STAT_DSRC_COMPRESS_READ			2197
/*!
 * compression: pages read from disk with compression ratio greater than
 * 64
 */
#define	WT_STAT_DSRC_COMPRESS_READ_RATIO_HIST_MAX	2198
/*!
 * compression: pages read from disk with compression ratio smaller than
 * 2
 */
#define	WT_STAT_DSRC_COMPRESS_READ_RATIO_HIST_2		2199
/*!
 * compression: pages read from disk with compression ratio smaller than
 * 4
 */
#define	WT_STAT_DSRC_COMPRESS_READ_RATIO_HIST_4		2200
/*!
 * compression: pages read from disk with compression ratio smaller than
 * 8
 */
#define	WT_STAT_DSRC_COMPRESS_READ_RATIO_HIST_8		2201
/*!
 * compression: pages read from disk with compression ratio smaller than
 * 16
 */
#define	WT_STAT_DSRC_COMPRESS_READ_RATIO_HIST_16	2202
/*!
 * compression: pages read from disk with compression ratio smaller than
 * 32
 */
#define	WT_STAT_DSRC_COMPRESS_READ_RATIO_HIST_32	2203
/*!
 * compression: pages read from disk with compression ratio smaller than
 * 64
 */
#define	WT_STAT_DSRC_COMPRESS_READ_RATIO_HIST_64	2204
/*! compression: pages written to disk */
#define	WT_STAT_DSRC_COMPRESS_WRITE			2205
/*!
 * compression: pages written to disk with compression ratio greater than
 * 64
 */
#define	WT_STAT_DSRC_COMPRESS_WRITE_RATIO_HIST_MAX	2206
/*!
 * compression: pages written to disk with compression ratio smaller than
 * 2
 */
#define	WT_STAT_DSRC_COMPRESS_WRITE_RATIO_HIST_2	2207
/*!
 * compression: pages written to disk with compression ratio smaller than
 * 4
 */
#define	WT_STAT_DSRC_COMPRESS_WRITE_RATIO_HIST_4	2208
/*!
 * compression: pages written to disk with compression ratio smaller than
 * 8
 */
#define	WT_STAT_DSRC_COMPRESS_WRITE_RATIO_HIST_8	2209
/*!
 * compression: pages written to disk with compression ratio smaller than
 * 16
 */
#define	WT_STAT_DSRC_COMPRESS_WRITE_RATIO_HIST_16	2210
/*!
 * compression: pages written to disk with compression ratio smaller than
 * 32
 */
#define	WT_STAT_DSRC_COMPRESS_WRITE_RATIO_HIST_32	2211
/*!
 * compression: pages written to disk with compression ratio smaller than
 * 64
 */
#define	WT_STAT_DSRC_COMPRESS_WRITE_RATIO_HIST_64	2212
/*! cursor: Total number of deleted pages skipped during tree walk */
#define	WT_STAT_DSRC_CURSOR_TREE_WALK_DEL_PAGE_SKIP	2213
/*! cursor: Total number of entries skipped by cursor next calls */
#define	WT_STAT_DSRC_CURSOR_NEXT_SKIP_TOTAL		2214
/*! cursor: Total number of entries skipped by cursor prev calls */
#define	WT_STAT_DSRC_CURSOR_PREV_SKIP_TOTAL		2215
/*!
 * cursor: Total number of entries skipped to position the history store
 * cursor
 */
#define	WT_STAT_DSRC_CURSOR_SKIP_HS_CUR_POSITION	2216
/*!
 * cursor: Total number of in-memory deleted pages skipped during tree
 * walk
 */
#define	WT_STAT_DSRC_CURSOR_TREE_WALK_INMEM_DEL_PAGE_SKIP	2217
/*! cursor: Total number of on-disk deleted pages skipped during tree walk */
#define	WT_STAT_DSRC_CURSOR_TREE_WALK_ONDISK_DEL_PAGE_SKIP	2218
/*!
 * cursor: Total number of times a search near has exited due to prefix
 * config
 */
#define	WT_STAT_DSRC_CURSOR_SEARCH_NEAR_PREFIX_FAST_PATHS	2219
/*!
 * cursor: Total number of times cursor fails to temporarily release
 * pinned page to encourage eviction of hot or large page
 */
#define	WT_STAT_DSRC_CURSOR_REPOSITION_FAILED		2220
/*!
 * cursor: Total number of times cursor temporarily releases pinned page
 * to encourage eviction of hot or large page
 */
#define	WT_STAT_DSRC_CURSOR_REPOSITION			2221
/*! cursor: bulk loaded cursor insert calls */
#define	WT_STAT_DSRC_CURSOR_INSERT_BULK			2222
/*! cursor: cache cursors reuse count */
#define	WT_STAT_DSRC_CURSOR_REOPEN			2223
/*! cursor: close calls that result in cache */
#define	WT_STAT_DSRC_CURSOR_CACHE			2224
/*! cursor: create calls */
#define	WT_STAT_DSRC_CURSOR_CREATE			2225
/*! cursor: cursor bound calls that return an error */
#define	WT_STAT_DSRC_CURSOR_BOUND_ERROR			2226
/*! cursor: cursor bounds cleared from reset */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_RESET		2227
/*! cursor: cursor bounds comparisons performed */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_COMPARISONS		2228
/*! cursor: cursor bounds next called on an unpositioned cursor */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_NEXT_UNPOSITIONED	2229
/*! cursor: cursor bounds next early exit */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_NEXT_EARLY_EXIT	2230
/*! cursor: cursor bounds prev called on an unpositioned cursor */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_PREV_UNPOSITIONED	2231
/*! cursor: cursor bounds prev early exit */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_PREV_EARLY_EXIT	2232
/*! cursor: cursor bounds search early exit */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_SEARCH_EARLY_EXIT	2233
/*! cursor: cursor bounds search near call repositioned cursor */
#define	WT_STAT_DSRC_CURSOR_BOUNDS_SEARCH_NEAR_REPOSITIONED_CURSOR	2234
/*! cursor: cursor cache calls that return an error */
#define	WT_STAT_DSRC_CURSOR_CACHE_ERROR			2235
/*! cursor: cursor close calls that return an error */
#define	WT_STAT_DSRC_CURSOR_CLOSE_ERROR			2236
/*! cursor: cursor compare calls that return an error */
#define	WT_STAT_DSRC_CURSOR_COMPARE_ERROR		2237
/*! cursor: cursor equals calls that return an error */
#define	WT_STAT_DSRC_CURSOR_EQUALS_ERROR		2238
/*! cursor: cursor get key calls that return an error */
#define	WT_STAT_DSRC_CURSOR_GET_KEY_ERROR		2239
/*! cursor: cursor get value calls that return an error */
#define	WT_STAT_DSRC_CURSOR_GET_VALUE_ERROR		2240
/*! cursor: cursor insert calls that return an error */
#define	WT_STAT_DSRC_CURSOR_INSERT_ERROR		2241
/*! cursor: cursor insert check calls that return an error */
#define	WT_STAT_DSRC_CURSOR_INSERT_CHECK_ERROR		2242
/*! cursor: cursor largest key calls that return an error */
#define	WT_STAT_DSRC_CURSOR_LARGEST_KEY_ERROR		2243
/*! cursor: cursor modify calls that return an error */
#define	WT_STAT_DSRC_CURSOR_MODIFY_ERROR		2244
/*! cursor: cursor next calls that return an error */
#define	WT_STAT_DSRC_CURSOR_NEXT_ERROR			2245
/*!
 * cursor: cursor next calls that skip due to a globally visible history
 * store tombstone
 */
#define	WT_STAT_DSRC_CURSOR_NEXT_HS_TOMBSTONE		2246
/*!
 * cursor: cursor next calls that skip greater than 1 and fewer than 100
 * entries
 */
#define	WT_STAT_DSRC_CURSOR_NEXT_SKIP_LT_100		2247
/*!
 * cursor: cursor next calls that skip greater than or equal to 100
 * entries
 */
#define	WT_STAT_DSRC_CURSOR_NEXT_SKIP_GE_100		2248
/*! cursor: cursor next random calls that return an error */
#define	WT_STAT_DSRC_CURSOR_NEXT_RANDOM_ERROR		2249
/*! cursor: cursor prev calls that return an error */
#define	WT_STAT_DSRC_CURSOR_PREV_ERROR			2250
/*!
 * cursor: cursor prev calls that skip due to a globally visible history
 * store tombstone
 */
#define	WT_STAT_DSRC_CURSOR_PREV_HS_TOMBSTONE		2251
/*!
 * cursor: cursor prev calls that skip greater than or equal to 100
 * entries
 */
#define	WT_STAT_DSRC_CURSOR_PREV_SKIP_GE_100		2252
/*! cursor: cursor prev calls that skip less than 100 entries */
#define	WT_STAT_DSRC_CURSOR_PREV_SKIP_LT_100		2253
/*! cursor: cursor reconfigure calls that return an error */
#define	WT_STAT_DSRC_CURSOR_RECONFIGURE_ERROR		2254
/*! cursor: cursor remove calls that return an error */
#define	WT_STAT_DSRC_CURSOR_REMOVE_ERROR		2255
/*! cursor: cursor reopen calls that return an error */
#define	WT_STAT_DSRC_CURSOR_REOPEN_ERROR		2256
/*! cursor: cursor reserve calls that return an error */
#define	WT_STAT_DSRC_CURSOR_RESERVE_ERROR		2257
/*! cursor: cursor reset calls that return an error */
#define	WT_STAT_DSRC_CURSOR_RESET_ERROR			2258
/*! cursor: cursor search calls that return an error */
#define	WT_STAT_DSRC_CURSOR_SEARCH_ERROR		2259
/*! cursor: cursor search near calls that return an error */
#define	WT_STAT_DSRC_CURSOR_SEARCH_NEAR_ERROR		2260
/*! cursor: cursor update calls that return an error */
#define	WT_STAT_DSRC_CURSOR_UPDATE_ERROR		2261
/*! cursor: insert calls */
#define	WT_STAT_DSRC_CURSOR_INSERT			2262
/*! cursor: insert key and value bytes */
#define	WT_STAT_DSRC_CURSOR_INSERT_BYTES		2263
/*! cursor: modify */
#define	WT_STAT_DSRC_CURSOR_MODIFY			2264
/*! cursor: modify key and value bytes affected */
#define	WT_STAT_DSRC_CURSOR_MODIFY_BYTES		2265
/*! cursor: modify value bytes modified */
#define	WT_STAT_DSRC_CURSOR_MODIFY_BYTES_TOUCH		2266
/*! cursor: next calls */
#define	WT_STAT_DSRC_CURSOR_NEXT			2267
/*! cursor: open cursor count */
#define	WT_STAT_DSRC_CURSOR_OPEN_COUNT			2268
/*! cursor: open cursor time application (usecs) */
#define	WT_STAT_DSRC_CURSOR_OPEN_TIME_USER_USECS	2269
/*! cursor: open cursor time internal (usecs) */
#define	WT_STAT_DSRC_CURSOR_OPEN_TIME_INTERNAL_USECS	2270
/*! cursor: operation restarted */
#define	WT_STAT_DSRC_CURSOR_RESTART			2271
/*! cursor: prev calls */
#define	WT_STAT_DSRC_CURSOR_PREV			2272
/*! cursor: remove calls */
#define	WT_STAT_DSRC_CURSOR_REMOVE			2273
/*! cursor: remove key bytes removed */
#define	WT_STAT_DSRC_CURSOR_REMOVE_BYTES		2274
/*! cursor: reserve calls */
#define	WT_STAT_DSRC_CURSOR_RESERVE			2275
/*! cursor: reset calls */
#define	WT_STAT_DSRC_CURSOR_RESET			2276
/*! cursor: search calls */
#define	WT_STAT_DSRC_CURSOR_SEARCH			2277
/*! cursor: search history store calls */
#define	WT_STAT_DSRC_CURSOR_SEARCH_HS			2278
/*! cursor: search near calls */
#define	WT_STAT_DSRC_CURSOR_SEARCH_NEAR			2279
/*! cursor: truncate calls */
#define	WT_STAT_DSRC_CURSOR_TRUNCATE			2280
/*! cursor: update calls */
#define	WT_STAT_DSRC_CURSOR_UPDATE			2281
/*! cursor: update key and value bytes */
#define	WT_STAT_DSRC_CURSOR_UPDATE_BYTES		2282
/*! cursor: update value size change */
#define	WT_STAT_DSRC_CURSOR_UPDATE_BYTES_CHANGED	2283
/*! layered: Layered table cursor insert operations */
#define	WT_STAT_DSRC_LAYERED_CURS_INSERT		2284
/*! layered: Layered table cursor modify operations */
#define	WT_STAT_DSRC_LAYERED_CURS_MODIFY		2285
/*! layered: Layered table cursor next operations */
#define	WT_STAT_DSRC_LAYERED_CURS_NEXT			2286
/*! layered: Layered table cursor next operations from the ingest btrees */
#define	WT_STAT_DSRC_LAYERED_CURS_NEXT_INGEST		2287
/*! layered: Layered table cursor next operations from the stable btrees */
#define	WT_STAT_DSRC_LAYERED_CURS_NEXT_STABLE		2288
/*!
 * layered: Layered table cursor opens the stable btree for the first
 * time
 */
#define	WT_STAT_DSRC_LAYERED_CURS_OPEN_STABLE		2289
/*! layered: Layered table cursor prev operations */
#define	WT_STAT_DSRC_LAYERED_CURS_PREV			2290
/*! layered: Layered table cursor prev operations from the ingest btrees */
#define	WT_STAT_DSRC_LAYERED_CURS_PREV_INGEST		2291
/*! layered: Layered table cursor prev operations from the stable btrees */
#define	WT_STAT_DSRC_LAYERED_CURS_PREV_STABLE		2292
/*! layered: Layered table cursor remove operations */
#define	WT_STAT_DSRC_LAYERED_CURS_REMOVE		2293
/*!
 * layered: Layered table cursor reopens the stable btree (role change or
 * checkpoint advance)
 */
#define	WT_STAT_DSRC_LAYERED_CURS_REOPEN_STABLE		2294
/*! layered: Layered table cursor search near operations */
#define	WT_STAT_DSRC_LAYERED_CURS_SEARCH_NEAR		2295
/*!
 * layered: Layered table cursor search near operations from the ingest
 * btrees
 */
#define	WT_STAT_DSRC_LAYERED_CURS_SEARCH_NEAR_INGEST	2296
/*!
 * layered: Layered table cursor search near operations from the stable
 * btrees
 */
#define	WT_STAT_DSRC_LAYERED_CURS_SEARCH_NEAR_STABLE	2297
/*! layered: Layered table cursor search operations */
#define	WT_STAT_DSRC_LAYERED_CURS_SEARCH		2298
/*! layered: Layered table cursor search operations from the ingest btrees */
#define	WT_STAT_DSRC_LAYERED_CURS_SEARCH_INGEST		2299
/*! layered: Layered table cursor search operations from the stable btrees */
#define	WT_STAT_DSRC_LAYERED_CURS_SEARCH_STABLE		2300
/*! layered: Layered table cursor update operations */
#define	WT_STAT_DSRC_LAYERED_CURS_UPDATE		2301
/*!
 * layered: checkpoints performed on this table by the layered table
 * manager
 */
#define	WT_STAT_DSRC_LAYERED_TABLE_MANAGER_CHECKPOINTS	2302
/*! layered: disagg pick up checkpoints failed */
#define	WT_STAT_DSRC_LAYERED_TABLE_MANAGER_CHECKPOINTS_DISAGG_PICK_UP_FAILED	2303
/*! layered: disagg pick up checkpoints succeeded */
#define	WT_STAT_DSRC_LAYERED_TABLE_MANAGER_CHECKPOINTS_DISAGG_PICK_UP_SUCCEED	2304
/*!
 * layered: how many log applications the layered table manager applied
 * on this tree
 */
#define	WT_STAT_DSRC_LAYERED_TABLE_MANAGER_LOGOPS_APPLIED	2305
/*!
 * layered: how many log applications the layered table manager skipped
 * on this tree
 */
#define	WT_STAT_DSRC_LAYERED_TABLE_MANAGER_LOGOPS_SKIPPED	2306
/*!
 * layered: how many previously-applied LSNs the layered table manager
 * skipped on this tree
 */
#define	WT_STAT_DSRC_LAYERED_TABLE_MANAGER_SKIP_LSN	2307
/*! reconciliation: VLCS pages explicitly reconciled as empty */
#define	WT_STAT_DSRC_REC_VLCS_EMPTIED_PAGES		2308
/*! reconciliation: approximate byte size of timestamps in pages written */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_BYTES_TS		2309
/*!
 * reconciliation: approximate byte size of transaction IDs in pages
 * written
 */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_BYTES_TXN		2310
/*!
 * reconciliation: average length of delta chain on internal page with
 * deltas
 */
#define	WT_STAT_DSRC_REC_AVERAGE_INTERNAL_PAGE_DELTA_CHAIN_LENGTH	2311
/*! reconciliation: average length of delta chain on leaf page with deltas */
#define	WT_STAT_DSRC_REC_AVERAGE_LEAF_PAGE_DELTA_CHAIN_LENGTH	2312
/*!
 * reconciliation: changes since prior reconciliation (bucket 1) between
 * 1 and 5
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_LE5			2313
/*!
 * reconciliation: changes since prior reconciliation (bucket 2) between
 * 6 and 10
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_LE10			2314
/*!
 * reconciliation: changes since prior reconciliation (bucket 3) between
 * 11 and 20
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_LE20			2315
/*!
 * reconciliation: changes since prior reconciliation (bucket 4) between
 * 21 and 50
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_LE50			2316
/*!
 * reconciliation: changes since prior reconciliation (bucket 5) between
 * 51 and 100
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_LE100		2317
/*!
 * reconciliation: changes since prior reconciliation (bucket 6) between
 * 101 and 200
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_LE200		2318
/*!
 * reconciliation: changes since prior reconciliation (bucket 7) between
 * 201 and 500
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_LE500		2319
/*!
 * reconciliation: changes since prior reconciliation (bucket 8) greater
 * than 500
 */
#define	WT_STAT_DSRC_REC_PAGE_MODS_GT500		2320
/*! reconciliation: cursor next/prev calls during HS wrapup search_near */
#define	WT_STAT_DSRC_REC_HS_WRAPUP_NEXT_PREV_CALLS	2321
/*! reconciliation: dictionary matches */
#define	WT_STAT_DSRC_REC_DICTIONARY			2322
/*! reconciliation: fast-path pages deleted */
#define	WT_STAT_DSRC_REC_PAGE_DELETE_FAST		2323
/*!
 * reconciliation: free page ID due to failed page replacement
 * reconciliation in disagg
 */
#define	WT_STAT_DSRC_REC_FREE_PAGE_ID_DUE_TO_FAILED_REPLACEMENT_RECONCILIATION	2324
/*! reconciliation: full internal pages written instead of a page delta */
#define	WT_STAT_DSRC_REC_PAGE_FULL_IMAGE_INTERNAL	2325
/*! reconciliation: full leaf pages written instead of a page delta */
#define	WT_STAT_DSRC_REC_PAGE_FULL_IMAGE_LEAF		2326
/*!
 * reconciliation: ingest btree reconciliation keeps the aborted prepare
 * updates
 */
#define	WT_STAT_DSRC_REC_INGEST_KEEP_PREPARE_ROLLBACK	2327
/*! reconciliation: internal page delta keys deleted */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_INTERNAL_KEY_DELETED	2328
/*! reconciliation: internal page delta keys updated/inserted */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_INTERNAL_KEY_UPDATED	2329
/*! reconciliation: internal page deltas written */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_INTERNAL		2330
/*!
 * reconciliation: internal page key bytes discarded using suffix
 * compression
 */
#define	WT_STAT_DSRC_REC_SUFFIX_COMPRESSION		2331
/*! reconciliation: internal page multi-block writes */
#define	WT_STAT_DSRC_REC_MULTIBLOCK_INTERNAL		2332
/*!
 * reconciliation: leaf delta page key bytes discarded using prefix
 * compression
 */
#define	WT_STAT_DSRC_REC_PREFIX_COMPRESSION_DELTA	2333
/*!
 * reconciliation: leaf full page key bytes discarded using prefix
 * compression
 */
#define	WT_STAT_DSRC_REC_PREFIX_COMPRESSION_FULL	2334
/*! reconciliation: leaf page deltas written */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_LEAF		2335
/*! reconciliation: leaf page multi-block writes */
#define	WT_STAT_DSRC_REC_MULTIBLOCK_LEAF		2336
/*! reconciliation: leaf-page overflow keys */
#define	WT_STAT_DSRC_REC_OVERFLOW_KEY_LEAF		2337
/*! reconciliation: max deltas seen on internal page during reconciliation */
#define	WT_STAT_DSRC_REC_MAX_INTERNAL_PAGE_DELTAS	2338
/*! reconciliation: max deltas seen on leaf page during reconciliation */
#define	WT_STAT_DSRC_REC_MAX_LEAF_PAGE_DELTAS		2339
/*! reconciliation: maximum blocks required for a page */
#define	WT_STAT_DSRC_REC_MULTIBLOCK_MAX			2340
/*!
 * reconciliation: number of keys that are garbage collected from the
 * disk images in the ingest btrees for disaggregated storage
 */
#define	WT_STAT_DSRC_REC_INGEST_GARBAGE_COLLECTION_KEYS_DISK_IMAGE	2341
/*!
 * reconciliation: number of keys that are garbage collected from the
 * update chains in the ingest btrees for disaggregated storage
 */
#define	WT_STAT_DSRC_REC_INGEST_GARBAGE_COLLECTION_KEYS_UPDATE_CHAIN	2342
/*! reconciliation: overflow values written */
#define	WT_STAT_DSRC_REC_OVERFLOW_VALUE			2343
/*! reconciliation: page deltas rejected due to invalid page ID */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_INVALID_PAGE_ID	2344
/*! reconciliation: page deltas rejected due to max consecutive limit */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_MAX_CONSECUTIVE_EXCEEDED	2345
/*! reconciliation: page deltas rejected due to multiblock reconciliation */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_MULTIBLOCK	2346
/*!
 * reconciliation: page deltas rejected due to non-single page in
 * previous reconciliation
 */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_NON_SINGLE_PAGE	2347
/*! reconciliation: page deltas rejected due to size threshold */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_SIZE_THRESHOLD	2348
/*!
 * reconciliation: page deltas rejected due to too many keys removed from
 * the disk image
 */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_DELETE_THRESHOLD	2349
/*! reconciliation: page deltas rejected due to zero entries */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_ZERO_ENTRIES	2350
/*!
 * reconciliation: page deltas rejected: build function returned false
 * (disabled, in-memory split, or internal page constraints not met)
 */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_REJECTED_BUILD_FAILED	2351
/*! reconciliation: page reconciliation calls */
#define	WT_STAT_DSRC_REC_PAGES				2352
/*! reconciliation: page reconciliation calls for eviction */
#define	WT_STAT_DSRC_REC_PAGES_EVICTION			2353
/*! reconciliation: page reconciliation calls for pages between 1 and 10MB */
#define	WT_STAT_DSRC_REC_PAGES_SIZE_1MB_TO_10MB		2354
/*!
 * reconciliation: page reconciliation calls for pages between 10 and
 * 100MB
 */
#define	WT_STAT_DSRC_REC_PAGES_SIZE_10MB_TO_100MB	2355
/*!
 * reconciliation: page reconciliation calls for pages between 100MB and
 * 1GB
 */
#define	WT_STAT_DSRC_REC_PAGES_SIZE_100MB_TO_1GB	2356
/*! reconciliation: page reconciliation calls for pages over 1GB */
#define	WT_STAT_DSRC_REC_PAGES_SIZE_1GB_PLUS		2357
/*! reconciliation: pages deleted */
#define	WT_STAT_DSRC_REC_PAGE_DELETE			2358
/*! reconciliation: pages eligible for delta generation */
#define	WT_STAT_DSRC_REC_PAGE_DELTA_ELIGIBLE		2359
/*!
 * reconciliation: pages written including an aggregated newest start
 * durable timestamp
 */
#define	WT_STAT_DSRC_REC_TIME_AGGR_NEWEST_START_DURABLE_TS	2360
/*!
 * reconciliation: pages written including an aggregated newest stop
 * durable timestamp
 */
#define	WT_STAT_DSRC_REC_TIME_AGGR_NEWEST_STOP_DURABLE_TS	2361
/*!
 * reconciliation: pages written including an aggregated newest stop
 * timestamp
 */
#define	WT_STAT_DSRC_REC_TIME_AGGR_NEWEST_STOP_TS	2362
/*!
 * reconciliation: pages written including an aggregated newest stop
 * transaction ID
 */
#define	WT_STAT_DSRC_REC_TIME_AGGR_NEWEST_STOP_TXN	2363
/*!
 * reconciliation: pages written including an aggregated newest
 * transaction ID
 */
#define	WT_STAT_DSRC_REC_TIME_AGGR_NEWEST_TXN		2364
/*!
 * reconciliation: pages written including an aggregated oldest start
 * timestamp
 */
#define	WT_STAT_DSRC_REC_TIME_AGGR_OLDEST_START_TS	2365
/*! reconciliation: pages written including an aggregated prepare */
#define	WT_STAT_DSRC_REC_TIME_AGGR_PREPARED		2366
/*! reconciliation: pages written including at least one prepare state */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PAGES_PREPARED	2367
/*!
 * reconciliation: pages written including at least one start durable
 * timestamp
 */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PAGES_DURABLE_START_TS	2368
/*! reconciliation: pages written including at least one start timestamp */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PAGES_START_TS	2369
/*!
 * reconciliation: pages written including at least one start transaction
 * ID
 */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PAGES_START_TXN	2370
/*!
 * reconciliation: pages written including at least one stop durable
 * timestamp
 */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PAGES_DURABLE_STOP_TS	2371
/*! reconciliation: pages written including at least one stop timestamp */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PAGES_STOP_TS	2372
/*!
 * reconciliation: pages written including at least one stop transaction
 * ID
 */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PAGES_STOP_TXN	2373
/*! reconciliation: pages written with at least one internal page delta */
#define	WT_STAT_DSRC_REC_PAGES_WITH_INTERNAL_DELTAS	2374
/*! reconciliation: pages written with at least one leaf page delta */
#define	WT_STAT_DSRC_REC_PAGES_WITH_LEAF_DELTAS		2375
/*! reconciliation: records written including a prepare state */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_PREPARED		2376
/*! reconciliation: records written including a start durable timestamp */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_DURABLE_START_TS	2377
/*! reconciliation: records written including a start timestamp */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_START_TS		2378
/*! reconciliation: records written including a start transaction ID */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_START_TXN		2379
/*! reconciliation: records written including a stop durable timestamp */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_DURABLE_STOP_TS	2380
/*! reconciliation: records written including a stop timestamp */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_STOP_TS		2381
/*! reconciliation: records written including a stop transaction ID */
#define	WT_STAT_DSRC_REC_TIME_WINDOW_STOP_TXN		2382
/*! reconciliation: writes skipped in disaggregated storage */
#define	WT_STAT_DSRC_REC_SKIP_WRITE			2383
/*! session: object compaction */
#define	WT_STAT_DSRC_SESSION_COMPACT			2384
/*!
 * transaction: a reader raced with a prepared transaction commit and
 * skipped an update or updates
 */
#define	WT_STAT_DSRC_TXN_READ_RACE_PREPARE_COMMIT	2385
/*! transaction: number of times overflow removed value is read */
#define	WT_STAT_DSRC_TXN_READ_OVERFLOW_REMOVE		2386
/*! transaction: race to read prepared update retry */
#define	WT_STAT_DSRC_TXN_READ_RACE_PREPARE_UPDATE	2387
/*! transaction: rollback to stable applied btrees */
#define	WT_STAT_DSRC_TXN_RTS_BTREES_APPLIED		2388
/*!
 * transaction: rollback to stable history store keys that would have
 * been swept in non-dryrun mode
 */
#define	WT_STAT_DSRC_TXN_RTS_SWEEP_HS_KEYS_DRYRUN	2389
/*!
 * transaction: rollback to stable history store records with stop
 * timestamps older than newer records
 */
#define	WT_STAT_DSRC_TXN_RTS_HS_STOP_OLDER_THAN_NEWER_START	2390
/*! transaction: rollback to stable inconsistent checkpoint */
#define	WT_STAT_DSRC_TXN_RTS_INCONSISTENT_CKPT		2391
/*! transaction: rollback to stable keys removed */
#define	WT_STAT_DSRC_TXN_RTS_KEYS_REMOVED		2392
/*! transaction: rollback to stable keys restored */
#define	WT_STAT_DSRC_TXN_RTS_KEYS_RESTORED		2393
/*!
 * transaction: rollback to stable keys that would have been removed in
 * non-dryrun mode
 */
#define	WT_STAT_DSRC_TXN_RTS_KEYS_REMOVED_DRYRUN	2394
/*!
 * transaction: rollback to stable keys that would have been restored in
 * non-dryrun mode
 */
#define	WT_STAT_DSRC_TXN_RTS_KEYS_RESTORED_DRYRUN	2395
/*!
 * transaction: rollback to stable prepared fast truncations that would
 * have been rolled back in non-dryrun mode
 */
#define	WT_STAT_DSRC_TXN_RTS_PREPARED_FAST_TRUNCATE_DRYRUN	2396
/*! transaction: rollback to stable restored tombstones from history store */
#define	WT_STAT_DSRC_TXN_RTS_HS_RESTORE_TOMBSTONES	2397
/*! transaction: rollback to stable restored updates from history store */
#define	WT_STAT_DSRC_TXN_RTS_HS_RESTORE_UPDATES		2398
/*! transaction: rollback to stable rolled back prepared fast truncations */
#define	WT_STAT_DSRC_TXN_RTS_PREPARED_FAST_TRUNCATE	2399
/*! transaction: rollback to stable skipped btrees */
#define	WT_STAT_DSRC_TXN_RTS_BTREES_SKIPPED		2400
/*! transaction: rollback to stable skipping delete rle */
#define	WT_STAT_DSRC_TXN_RTS_DELETE_RLE_SKIPPED		2401
/*! transaction: rollback to stable skipping stable rle */
#define	WT_STAT_DSRC_TXN_RTS_STABLE_RLE_SKIPPED		2402
/*! transaction: rollback to stable sweeping history store keys */
#define	WT_STAT_DSRC_TXN_RTS_SWEEP_HS_KEYS		2403
/*!
 * transaction: rollback to stable tombstones from history store that
 * would have been restored in non-dryrun mode
 */
#define	WT_STAT_DSRC_TXN_RTS_HS_RESTORE_TOMBSTONES_DRYRUN	2404
/*!
 * transaction: rollback to stable updates from history store that would
 * have been restored in non-dryrun mode
 */
#define	WT_STAT_DSRC_TXN_RTS_HS_RESTORE_UPDATES_DRYRUN	2405
/*! transaction: rollback to stable updates removed from history store */
#define	WT_STAT_DSRC_TXN_RTS_HS_REMOVED			2406
/*!
 * transaction: rollback to stable updates that would have been removed
 * from history store in non-dryrun mode
 */
#define	WT_STAT_DSRC_TXN_RTS_HS_REMOVED_DRYRUN		2407
/*! transaction: update conflicts */
#define	WT_STAT_DSRC_TXN_UPDATE_CONFLICT		2408
