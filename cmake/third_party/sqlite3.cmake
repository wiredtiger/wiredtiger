...
if(${SQLITE3_VERSION} VERSION_LESS ${SQLITE3_REQUIRED_VERSION})
    message(
        FATAL_ERROR
            "Could NOT find SQLite3: Found unsuitable version"
...
add_library(sqlite3_lib STATIC ${SQLITE3_DIR}/sqlite3.c ${SQLITE3_DIR}/sqlite3.h)

target_include_directories(sqlite3_lib PUBLIC ${SQLITE3_DIR})

target_compile_definitions(
    sqlite3_lib
    PRIVATE # Omitting the possibility of using shared cache allows many conditionals in
...
if(CMAKE_BUILD_TYPE_UPPER MATCHES "^(ASAN|UBSAN|MSAN|TSAN)$")
    target_compile_options(
        sqlite3_lib PRIVATE $<$<NOT:$<BOOL:${MSVC_C_COMPILER}>>:-fno-sanitize=all>
    )
endif()

# Needed for SQLite3 on some platforms
target_link_libraries(
    sqlite3_lib PUBLIC $<$<BOOL:${WT_LINUX}>:Threads::Threads>
                       $<$<BOOL:${WT_LINUX}>:${CMAKE_DL_LIBS}> $<$<BOOL:${WT_LINUX}>:m>
)

add_library(wt::sqlite3 ALIAS sqlite3_lib)

add_executable(sqlite3 ${SQLITE3_DIR}/shell.c)
