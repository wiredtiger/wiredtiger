...
macro(define_wiredtiger_library target type)
    ...
    if(HAVE_LIBVCDENC)
        target_link_libraries(${target} PUBLIC ${HAVE_LIBVCDENC})
        target_link_libraries(${target} PUBLIC ${HAVE_LIBVCDDEC})
        target_link_libraries(${target} PUBLIC ${HAVE_LIBVCDCOM})
        target_link_libraries(${target} PUBLIC ${HAVE_LIBCXX})
        if(HAVE_LIBVCDENC_INCLUDES)
            target_include_directories(${target} PUBLIC ${HAVE_LIBVCDENC_INCLUDES})
        endif()
        if(HAVE_LIBVCDDEC_INCLUDES)
            target_include_directories(${target} PUBLIC ${HAVE_LIBVCDDEC_INCLUDES})
        endif()
    endif()
