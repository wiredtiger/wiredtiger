...
TRACY_API uint64_t ___tracy_alloc_srcloc(uint32_t line, const char *source, size_t sourceSz,
  const char *function, size_t functionSz, uint32_t color);
TRACY_API uint64_t ___tracy_alloc_srcloc_name(uint32_t line, const char *source, size_t sourceSz,
  const char *function, size_t functionSz, const char *name, size_t nameSz, uint32_t color);

TRACY_API TracyCZoneCtx ___tracy_emit_zone_begin(
  const struct ___tracy_source_location_data *srcloc, int32_t active);
TRACY_API TracyCZoneCtx ___tracy_emit_zone_begin_callstack(
  const struct ___tracy_source_location_data *srcloc, int32_t depth, int32_t active);
TRACY_API TracyCZoneCtx ___tracy_emit_zone_begin_alloc(uint64_t srcloc, int32_t active);
TRACY_API TracyCZoneCtx ___tracy_emit_zone_begin_alloc_callstack(
  uint64_t srcloc, int32_t depth, int32_t active);
...
TRACY_API void ___tracy_emit_gpu_zone_begin_callstack(
...
TRACY_API void ___tracy_emit_gpu_zone_begin_callstack_serial(
  const struct ___tracy_gpu_zone_begin_callstack_data);
...
#define TracyCZone(ctx, active)                                                                  \
    static const struct ___tracy_source_location_data TracyConcat(                               \
      __tracy_source_location, TracyLine) = {NULL, __func__, TracyFile, (uint32_t)TracyLine, 0}; \
...
#define TracyCZoneNC(ctx, name, color, active)                                             \
    static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location, \
      TracyLine) = {name, __func__, TracyFile, (uint32_t)TracyLine, color};                \
    TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack(                                \
      &TracyConcat(__tracy_source_location, TracyLine), TRACY_CALLSTACK, active);
...
TRACY_API void ___tracy_emit_memory_alloc_callstack(
  const void *ptr, size_t size, int32_t depth, int32_t secure);
...
TRACY_API void ___tracy_emit_memory_alloc_named(
  const void *ptr, size_t size, int32_t secure, const char *name);
TRACY_API void ___tracy_emit_memory_alloc_callstack_named(
  const void *ptr, size_t size, int32_t depth, int32_t secure, const char *name);
TRACY_API void ___tracy_emit_memory_free_named(const void *ptr, int32_t secure, const char *name);
TRACY_API void ___tracy_emit_memory_free_callstack_named(
  const void *ptr, int32_t depth, int32_t secure, const char *name);
TRACY_API void ___tracy_emit_memory_discard(const char *name, int32_t secure);
TRACY_API void ___tracy_emit_memory_discard_callstack(
  const char *name, int32_t secure, int32_t depth);
...
TRACY_API void ___tracy_emit_messageC(
  const char *txt, size_t size, uint32_t color, int32_t callstack_depth);
...
#define TracyCSecureAlloc(ptr, size) \
    ___tracy_emit_memory_alloc_callstack(ptr, size, TRACY_CALLSTACK, 1)
#define TracyCSecureFree(ptr) ___tracy_emit_memory_free_callstack(ptr, TRACY_CALLSTACK, 1)
#define TracyCSecureMemoryDiscard(name) \
    ___tracy_emit_memory_discard_callstack(name, 1, TRACY_CALLSTACK);

#define TracyCAllocN(ptr, size, name) \
    ___tracy_emit_memory_alloc_callstack_named(ptr, size, TRACY_CALLSTACK, 0, name)
#define TracyCFreeN(ptr, name) \
    ___tracy_emit_memory_free_callstack_named(ptr, TRACY_CALLSTACK, 0, name)
#define TracyCSecureAllocN(ptr, size, name) \
    ___tracy_emit_memory_alloc_callstack_named(ptr, size, TRACY_CALLSTACK, 1, name)
#define TracyCSecureFreeN(ptr, name) \
    ___tracy_emit_memory_free_callstack_named(ptr, TRACY_CALLSTACK, 1, name)
...
TRACY_API void ___tracy_emit_frame_image(
  const void *image, uint16_t w, uint16_t h, uint8_t offset, int32_t flip);
...
#define TracyCFrameImage(image, width, height, offset, flip) \
    ___tracy_emit_frame_image(image, width, height, offset, flip);
...
TRACY_API void ___tracy_emit_plot_config(
  const char *name, int32_t type, int32_t step, int32_t fill, uint32_t color);
...
#define TracyCPlotConfig(name, type, step, fill, color) \
...
#define TracyCSecureAllocS(ptr, size, depth) \
    ___tracy_emit_memory_alloc_callstack(ptr, size, depth, 1)
#define TracyCSecureFreeS(ptr, depth) ___tracy_emit_memory_free_callstack(ptr, depth, 1)
#define TracyCSecureMemoryDiscardS(name, depth) \
    ___tracy_emit_memory_discard_callstack(name, 1, depth)

#define TracyCAllocNS(ptr, size, depth, name) \
    ___tracy_emit_memory_alloc_callstack_named(ptr, size, depth, 0, name)
#define TracyCFreeNS(ptr, depth, name) \
    ___tracy_emit_memory_free_callstack_named(ptr, depth, 0, name)
#define TracyCSecureAllocNS(ptr, size, depth, name) \
    ___tracy_emit_memory_alloc_callstack_named(ptr, size, depth, 1, name)
#define TracyCSecureFreeNS(ptr, depth, name) \
    ___tracy_emit_memory_free_callstack_named(ptr, depth, 1, name)
...
TRACY_API struct __tracy_lockable_context_data *___tracy_announce_lockable_ctx(
...
TRACY_API void ___tracy_after_try_lock_lockable_ctx(
  struct __tracy_lockable_context_data *lockdata, int32_t acquired);
TRACY_API void ___tracy_mark_lockable_ctx(struct __tracy_lockable_context_data *lockdata,
  const struct ___tracy_source_location_data *srcloc);
TRACY_API void ___tracy_custom_name_lockable_ctx(
  struct __tracy_lockable_context_data *lockdata, const char *name, size_t nameSz);

#define TracyCLockAnnounce(lock)                                                                 \
    static const struct ___tracy_source_location_data TracyConcat(                               \
      __tracy_source_location, TracyLine) = {NULL, __func__, TracyFile, (uint32_t)TracyLine, 0}; \
    lock = ___tracy_announce_lockable_ctx(&TracyConcat(__tracy_source_location, TracyLine));
...
#define TracyCLockMark(lock)                                                                     \
    static const struct ___tracy_source_location_data TracyConcat(                               \
      __tracy_source_location, TracyLine) = {NULL, __func__, TracyFile, (uint32_t)TracyLine, 0}; \
    ___tracy_mark_lockable_ctx(lock, &TracyConcat(__tracy_source_location, TracyLine));
#define TracyCLockCustomName(lock, name, nameSz) \
...
// 2. Define the new, scoped macro by modifying the original
// We copy the body of TracyCZone and add our attribute on the correct line.
