 ...
 *     another thread might cache the same chunk -- bad idea. We insert the new chunk into
 *     the cache in the invalid state. Once we insert the chunk, we can release the lock. As long as
 *     the chunk is marked invalid, no other thread will try to re-cache it or to read from it. As a
 *     result, we can read data from the remote storage into this chunk without holding the lock and
 *     this is what the current code does. We can even allocate the space for that chunk outside the
 *     critical section, although the current code does not do that. Once we read the data into the
 *     chunk, we atomically set the valid flag, so other threads can use it.
 *
