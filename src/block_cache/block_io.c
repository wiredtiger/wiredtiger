 ...
 * compression is not performed (not configured, block too small, or compression didn't help), the
 * output parameter is set to NULL. The caller is responsible for freeing the output buffer.
 */
int
  ...
  WT_ITEM **compressed_bufp, size_t *compressed_sizep, bool *compressedp)
{
    ...
    *compressedp = already_compressed;
     ...
     * memory because they don't stop just because there's no more memory into which to compress.
     */
