...
static inline void
__blkcache_verbose(
  WT_SESSION_IMPL *session, WT_VERBOSE_LEVEL level, const char *tag, uint64_t hash, const uint8_t *addr, size_t addr_size)
{
    WT_DECL_ITEM(tmp);
    ...
    __wt_verbose_level(session, WT_VERB_BLKCACHE, level, "%s: %s, hash=%" PRIu64, tag, addr_string, hash);
    ...
    *blkcache_retp = NULL;
    ...
    if (blkcache_item != NULL) {
        ...
        *foundp = *skip_cache_putp = true;
        ...
        __blkcache_verbose(session, WT_VERBOSE_DEBUG_2, "block found in cache", hash, addr, addr_size);
    } else {
        WT_STAT_CONN_INCR(session, block_cache_misses);
        __blkcache_verbose(session, WT_VERBOSE_DEBUG_2, "block not found in cache", hash, addr, addr_size);
    }
}
  ...
  WT_SESSION_IMPL *session, WT_ITEM *data, const uint8_t *addr, size_t addr_size, bool write)
{
        ...
        TAILQ_FOREACH (blkcache_item, &blkcache->hash[bucket], hashq)
            if (blkcache_item->addr_size == addr_size && blkcache_item->fid == S2BT(session)->id &&
              memcmp(blkcache_item->addr, addr, addr_size) == 0) {
                ...
                __blkcache_verbose(session, WT_VERBOSE_DEBUG_2, "block already in cache", hash, addr, addr_size);
                goto err;
            }
    ...
    __blkcache_verbose(session, WT_VERBOSE_DEBUG_1, "block inserted in cache", hash, addr, addr_size);
    return (0);
    ...
    TAILQ_FOREACH (blkcache_item, &blkcache->hash[bucket], hashq) {
        if (blkcache_item->addr_size == addr_size && blkcache_item->fid == S2BT(session)->id &&
          memcmp(blkcache_item->addr, addr, addr_size) == 0) {
            ...
            __blkcache_verbose(session, WT_VERBOSE_DEBUG_1, "block removed from cache", hash, addr, addr_size);
            return;
        }
    }
    __wt_spin_unlock(session, &blkcache->hash_locks[bucket]);
