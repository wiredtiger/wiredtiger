...
__posix_file_truncate(WT_FILE_HANDLE *file_handle, WT_SESSION *wt_session, wt_off_t len)
{
    ...
    if (remap && release) {
        if (ret == 0)
            ...
            WT_RET_MSG(
              session, ret, "%s: handle-truncate: ftruncate ret %d", file_handle->name, ret);
        }
    }
    return (0);
}
...
#define WT_REMAP_SKIP 10
    if (pfh->mmap_buf != NULL && !pfh->mmap_resizing && pfh->mmap_size < offset + (wt_off_t)len)
        ...
        if ((remap_opportunities++) % WT_REMAP_SKIP == 0) {
            release = false;
            __wt_prepare_remap_resize_file(file_handle, wt_session, &release);
            WT_ASSERT(session, release == true);
            __wt_remap_resize_file(file_handle, wt_session);
...
void
__wt_prepare_remap_resize_file(
  WT_FILE_HANDLE *file_handle, WT_SESSION *wt_session, bool *need_release)
{
    WT_FILE_HANDLE_POSIX *pfh;
    WT_SESSION_IMPL *session;
