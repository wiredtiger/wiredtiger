  ...
  WT_FILE_HANDLE *file_handle, WT_SESSION *wt_session, wt_off_t offset, size_t len, void *buf)
{
    ...
    /*
     * If the I/O falls outside of the mapped buffer, or the buffer is being resized, we defer to
     * the regular system call.
     */
    if (pfh->mmap_buf != 0 && pfh->mmap_size >= (size_t)offset + len && !pfh->mmap_resizing) {
...
__wt_drain_mmap_users(WT_FILE_HANDLE *file_handle, WT_SESSION *wt_session)
{
    ...
    /*
     * Here is the synchronization protocol to prevent race conditions when a session is remapping
     * the file while others might be reading or writing it:
     *
 ...
 * memory-mapped and the client writes to that block via mmap directly into storage (via DAX), the
 * file system may not know that the data was written, so it may not flush the metadata prior to
 * data being written. Therefore, the block may be reallocated or lost upon crash.
 *
