...
__cache_pool_config(WT_SESSION_IMPL *session, const char **cfg)
{
    ...
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_CACHE_POOL))
        updating = true;
    else {
    ...
    F_SET_ATOMIC_64(conn, WT_CONN_CACHE_POOL);
err:
    __wt_spin_unlock(session, &__wt_process.spinlock);
...
__wt_cache_pool_create(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    was_shared = F_ISSET_ATOMIC_64(conn, WT_CONN_CACHE_POOL);

    if (now_shared) {
        WT_RET(__cache_pool_config(session, cfg));
        WT_ASSERT(session, F_ISSET_ATOMIC_64(conn, WT_CONN_CACHE_POOL));
        if (!was_shared)
...
__wt_cache_pool_destroy(WT_SESSION_IMPL *session)
{
    ...
    if (!F_ISSET_ATOMIC_64(conn, WT_CONN_CACHE_POOL))
        return (0);
    F_CLR_ATOMIC_64(conn, WT_CONN_CACHE_POOL);
