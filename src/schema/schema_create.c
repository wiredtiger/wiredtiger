    ...
    WT_SESSION_IMPL *session, WT_TABLE *table, const char *cgname, const char *config, WT_ITEM *buf)
{
	...
	if (cgname == NULL)
		WT_RET(
		    __wt_buf_fmt(session, buf, "%.*s:%s%s", (int)len, prefix, tablename, suffix));
	else
		WT_RET(__wt_buf_fmt(
...
__create_colgroup(WT_SESSION_IMPL *session, const char *name, bool exclusive, const char *config)
{
	...
	if (cgname != NULL &&
	    (ret = __wt_config_subgets(session, &table->cgconf, cgname, &cval)) != 0)
 ...
 */
int
__wt_schema_index_source(WT_SESSION_IMPL *session, WT_TABLE *table, const char *idxname,
    const char *config, WT_ITEM *buf)
{
	WT_CONFIG_ITEM cval;
	WT_DECL_RET;
	size_t len;
	...
	WT_RET(__wt_buf_fmt(
	    session, buf, "%.*s:%s_%s%s", (int)len, prefix, tablename, idxname, suffix));
...
__fill_index(WT_SESSION_IMPL *session, WT_TABLE *table, WT_INDEX *idx)
{
	...
	while ((ret = tcur->next(tcur)) == 0)
		WT_ERR(__wt_apply_single_idx(
		    session, idx, icur, (WT_CURSOR_TABLE *)tcur, icur->insert));
...
__create_index(WT_SESSION_IMPL *session, const char *name, bool exclusive, const char *config)
{
	...
	if ((ret = __wt_schema_get_table(
	         session, tablename, tlen, true, WT_DHANDLE_EXCLUSIVE, &table)) != 0)
		WT_RET_MSG(
		    session, ret, "Can't create an index for table: %.*s", (int)tlen, tablename);
	WT_RET(__wt_schema_release_table(session, &table));

	if ((ret = __wt_schema_get_table(session, tablename, tlen, true, 0, &table)) != 0)
		WT_RET_MSG(session, ret, "Can't create an index for a non-existent table: %.*s",
		    (int)tlen, tablename);
	...
	if (__wt_config_getones_none(session, config, "extractor", &cval) == 0 && cval.len != 0) {
		...
		if ((ret = __wt_config_getones(session, config, "key_format", &kval)) != 0)
			WT_ERR_MSG(
			    session, EINVAL, "%s: custom extractors require a key_format", name);
	}
...
__create_table(WT_SESSION_IMPL *session, const char *uri, bool exclusive, const char *config)
{
	...
	if (WT_META_TRACKING(session)) {
		WT_WITH_DHANDLE(
		    session, &table->iface, ret = __wt_meta_track_handle_lock(session, true));
		WT_ERR(ret);
		table = NULL;
	}
    ...
    WT_SESSION_IMPL *session, const char *uri, const char *config, WT_DATA_SOURCE *dsrc)
{
	...
	if (__wt_config_getones_none(session, config, "collator", &cval) != WT_NOTFOUND &&
	    cval.len != 0)
		WT_RET_MSG(session, EINVAL, "WT_DATA_SOURCE objects do not support WT_COLLATOR "
		                            "ordering");
