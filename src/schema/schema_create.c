...
__create_layered(WT_SESSION_IMPL *session, const char *uri, bool exclusive, const char *config)
{
    ...
    /*
     * A table created while a step-down is armed is marked step_down_created, which sends every
     ...
     * missing constituent. The schema lock held here serializes the arm, making the relaxed load
     * safe.
     */
    if (__wt_atomic_load_bool_relaxed(&conn->layered_table_manager.leader)) {
        if (!__wt_atomic_load_bool_relaxed(&conn->disaggregated_storage.step_down_armed)) {
            ...
            __wt_verbose_info(session, WT_VERB_LAYERED,
              "%s: created without a stable constituent, a step-down is armed", uri);
        }
    }
