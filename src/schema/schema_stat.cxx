  ...
  WT_SESSION_IMPL *session, const char *uri, const char *cfg[], WT_CURSOR_STAT *cst)
{
    ...
    ret = __wt_curstat_init(session, static_cast<const char*>(buf->data), NULL, cfg, cst);
...
__curstat_size_only(WT_SESSION_IMPL *session, const char *uri, bool *was_fast, WT_CURSOR_STAT *cst)
{
    ...
    *was_fast = false;
     ...
     * handle.
     */
    WT_ERR(__wt_fs_exist(session, static_cast<const char*>(namebuf.data), &exist));
    if (exist) {
	    WT_ERR(__wt_fs_size(session, static_cast<const char*>(namebuf.data), &filesize));
  ...
  WT_SESSION_IMPL *session, const char *uri, const char *cfg[], WT_CURSOR_STAT *cst)
{
    ...
    for (i = 0; i < WT_COLGROUPS(table); i++) {
        ...
        WT_ERR(__wt_curstat_open(session, static_cast<const char*>(buf->data), NULL, cfg, &stat_cursor));
