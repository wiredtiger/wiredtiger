...
__drop_issue_trim(WT_SESSION_IMPL *session, const char *uri)
{
    ...
    /*
     * A table awaiting publication has never been checkpointed and trims its stable data before
     * closing the handle, so committed data is gone before the close could fail. Refuse the drop
     * until a checkpoint has persisted the data, so this table behaves like a regular table, which
     * returns EBUSY when it holds uncheckpointed data.
     */
