 ...
 */
int
__wt_schema_range_truncate(WT_SESSION_IMPL *session, WT_CURSOR *start, WT_CURSOR *stop, WT_CURFILE_STATE_TMP *start_orig, WT_CURFILE_STATE_TMP *stop_orig)
{
    WT_DATA_SOURCE *dsrc;
    WT_DECL_RET;
    const char *uri;
    ...
    if (WT_STREQ(uri, WT_HS_URI))
        ...
        WT_WITH_BTREE(session, CUR2BT(start),
            ret = __wt_btcur_range_truncate((WT_CURSOR_BTREE *)start, (WT_CURSOR_BTREE *)stop, start_orig, stop_orig));
