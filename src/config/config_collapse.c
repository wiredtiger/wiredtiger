...
/*
 * __wt_config_overlay --
 *     Walk a complete base configuration once and substitute values from the overlay strings. Keys
 *     not in the base are discarded. Nested structures are replaced wholesale, matching
 *     __wt_config_collapse. Unlike collapse, each key is looked up only in the overlay: that
 *     matters when the base is the file.meta default string and the overlay is small.
 */
int
__wt_config_overlay(
  WT_SESSION_IMPL *session, const char *base, const char **overlay, char **config_ret)
{
    WT_CONFIG cparser;
    WT_CONFIG_ITEM k, overlay_v, v;
    WT_DECL_ITEM(tmp);
    WT_DECL_RET;
    bool have_overlay;

    *config_ret = NULL;
    have_overlay = overlay != NULL && overlay[0] != NULL;
    ...
    while ((ret = __wt_config_next(&cparser, &k, &v)) == 0) {
        ...
        if (have_overlay) {
            ret = __wti_config_get(session, overlay, &k, &overlay_v);
            if (ret == 0)
                v = overlay_v;
            else
