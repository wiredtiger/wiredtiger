...
__wt_turtle_update(WT_SESSION_IMPL *session, const char *key, const char *value)
{
    ...
    if (conn->key_provider != NULL && conn->disaggregated_storage.last_key_provider_page_lsn[WT_DISAGG_KEY_PROVIDER_MAIN_PAGE_ID] != 0) {
        WT_ERR(__wt_scr_alloc(session, 0, &key_provider_str));
        WT_ERR(__wt_buf_fmt(
            session, key_provider_str, "key_provider=(pages=(page_id=%d,lsn=%" PRIu64 "))", WT_DISAGG_KEY_PROVIDER_MAIN_PAGE_ID,  conn->disaggregated_storage.last_key_provider_page_lsn[WT_DISAGG_KEY_PROVIDER_MAIN_PAGE_ID]));
