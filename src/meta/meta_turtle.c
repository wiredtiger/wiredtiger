...
__metadata_load_hot_backup(WT_SESSION_IMPL *session, WT_BACKUPHASH *backuphash)
{
    ...
    F_SET_ATOMIC_64(conn, WT_CONN_WAS_BACKUP);
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_BACKUP_PARTIAL_RESTORE) &&
  ...
  WT_SESSION_IMPL *session, bool exist_backup, const char *cfg[], WT_BACKUPHASH *backuphash)
{
    ...
    if (cval.len != 0) {
        ...
        F_SET_ATOMIC_64(S2C(session), WT_CONN_BACKUP_PARTIAL_RESTORE);
...
err:
    ...
    if (ret == 0 || strcmp(key, WT_METADATA_COMPAT) == 0 ||
      ...
      F_ISSET_ATOMIC_64(S2C(session), WT_CONN_SALVAGE))
        return (ret);
...
__wt_turtle_update(WT_SESSION_IMPL *session, const char *key, const char *value)
{
     ...
     */
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_COMPATIBILITY))
