  ...
  uint32_t new_max, uint32_t flags)
{
    ...
    for (i = group->max; i < new_max; i++) {
        ...
        /*
         * Threads get their own session and history_store table cursor
         * (if the history_store table is open).
         */
        ...
        if (LF_ISSET(WT_THREAD_HISTORY_STORE) && F_ISSET(conn, WT_CONN_HISTORY_STORE_OPEN))
            WT_ERR(__wt_history_store_cursor_open(thread->session));
