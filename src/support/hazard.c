    ...
    )
{
	...
	*busyp = false;
	 ...
	 */
	if (ref->state != WT_REF_AMNESIA && ref->state != WT_REF_MEM) {
		*busyp = true;
		return (0);
	}
	 ...
	 * of WT_REF_AMNESIA or WT_REF_MEM, which can be set to WT_REF_LOCKED
	 ...
	 * state of WT_REF_AMNESIA or WT_REF_MEM.
	 */
	if (ref->state == WT_REF_AMNESIA || ref->state == WT_REF_MEM) {
		++session->nhazard;
