...
__wt_try_readlock(WT_SESSION_IMPL *session, WT_RWLOCK *l)
{
     ...
     */
    new_.u.v = old.u.v;
    if (++new_.u.s.readers_active == 0)
        return (__wt_set_return(session, EBUSY));

    /* We rely on this atomic operation to provide a barrier. */
    return (__wt_atomic_casv64(&l->u.v, old.u.v, new_.u.v) ? 0 : EBUSY);
}
...
__wt_readlock(WT_SESSION_IMPL *session, WT_RWLOCK *l)
{
    ...
    for (;;) {
        ...
        for (old.u.v = __wt_atomic_loadv64(&l->u.v); old.u.s.current == old.u.s.next;
            ...
            if (++new_.u.s.readers_active == 0)
                goto stall;
            if (__wt_atomic_casv64(&l->u.v, old.u.v, new_.u.v))
                return;
            WT_PAUSE();
        }
...
stall:
         ...
         */
        new_.u.v = old.u.v;
        if (new_.u.s.readers_queued++ == 0)
            new_.u.s.reader = new_.u.s.next;
        ticket = new_.u.s.reader;
        WT_ASSERT(session, new_.u.s.readers_queued != 0);
        if (__wt_atomic_casv64(&l->u.v, old.u.v, new_.u.v))
            break;
    }
...
__wt_readunlock(WT_SESSION_IMPL *session, WT_RWLOCK *l)
{
    ...
    do {
         ...
         */
        new_.u.v = old.u.v;
        --new_.u.s.readers_active;
    } while (!__wt_atomic_casv64(&l->u.v, old.u.v, new_.u.v));

    if (new_.u.s.readers_active == 0 && new_.u.s.current != new_.u.s.next)
...
__wt_writelock(WT_SESSION_IMPL *session, WT_RWLOCK *l)
{
    ...
    for (;;) {
         ...
         */
        if (new_.u.s.current == new_.u.s.next) {
...
__wt_writeunlock(WT_SESSION_IMPL *session, WT_RWLOCK *l)
{
    ...
    do {
         ...
         */
        new_.u.v = old.u.v;
        if (++new_.u.s.current == new_.u.s.reader) {
            new_.u.s.readers_active = new_.u.s.readers_queued;
            new_.u.s.readers_queued = 0;
        }
    } while (!__wt_atomic_casv64(&l->u.v, old.u.v, new_.u.v));

    if (new_.u.s.readers_active != 0)
        __wt_cond_signal(session, l->cond_readers);
    else if (new_.u.s.current != new_.u.s.next)
        __wt_cond_signal(session, l->cond_writers);
