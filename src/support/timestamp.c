  ...
  WT_SESSION_IMPL *session, WT_TIME_AGGREGATE *ta, WT_TIME_AGGREGATE *parent, bool silent)
{
     ...
     * durable timestamp may be larger than newest page stop durable timestamp. Check whether start
     * and stop are equal first and then check the newest durable timestamp against newest page stop
    ...
    if (ta->newest_page_stop_durable_ts != WT_TS_NONE &&
      ta->newest_durable_ts != ta->newest_page_stop_durable_ts && ta->newest_stop_ts != WT_TS_MAX &&
