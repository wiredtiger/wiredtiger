...
static const char *const __stats_connection_desc[] = {
  ...
  "transaction: prepared transactions rolled back and left the history store entry with checkpoint "
  "next transaction id",
...
__wt_stat_connection_clear_single(WT_CONNECTION_STATS *stats)
{
    ...
    stats->txn_prepare_rollback_left_hs_update_with_ckpt_next_txnid = 0;
...
__wt_stat_connection_aggregate(WT_CONNECTION_STATS **from, WT_CONNECTION_STATS *to)
{
    ...
    to->txn_prepare_rollback_left_hs_update_with_ckpt_next_txnid +=
      WT_STAT_READ(from, txn_prepare_rollback_left_hs_update_with_ckpt_next_txnid);
