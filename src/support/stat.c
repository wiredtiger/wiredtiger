...
static const char *const __stats_checkpoint_desc[] = {
  "checkpoint: byte count of a file",
  "checkpoint: row count of a file",
};

int
__wt_stat_checkpoint_desc(WT_CURSOR_STAT *cst, int slot, const char **p)
{
    WT_UNUSED(cst);
    *p = __stats_checkpoint_desc[slot];
    return (0);
}

void
__wt_stat_checkpoint_init_single(WT_CHECKPOINT_STATS *stats)
{
    memset(stats, 0, sizeof(*stats));
}
