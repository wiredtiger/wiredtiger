...
static const char *const __stats_connection_desc[] = {
  ...
  "block-cache: pre-fetch not triggered as ref on the queue does not have pre-fetch flag set",
...
__wt_stat_connection_clear_single(WT_CONNECTION_STATS *stats)
{
    ...
    stats->block_prefetch_skipped_no_leaf_page = 0;
...
__wt_stat_connection_aggregate(WT_CONNECTION_STATS **from, WT_CONNECTION_STATS *to)
{
    ...
    to->block_prefetch_skipped_no_leaf_page +=
      WT_STAT_READ(from, block_prefetch_skipped_no_leaf_page);
