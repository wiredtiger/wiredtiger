  ...
  size_t size, const uint8_t *data)
{
    ...
    /*
     * Fast-path the expected case, where we're overwriting a set of bytes that already exist in the
     * buffer.
     */
...
__wt_modify_apply(WT_SESSION_IMPL *session, WT_CURSOR *cursor, const void *modify)
{
    ...
    /*
     * Get the number of entries, and set a second pointer to reference the change data. The modify
     * string isn't necessarily aligned for size_t access, copy to be sure.
     */
    p = modify;
