...
extern __thread int wt_call_nest_level;
extern __thread char __wt_indent_str[4096];
extern __thread char __wt_session_info_str[4096];

#define __WT_SET_INDENT() do { \
    memset(__wt_indent_str, ' ', wt_call_nest_level*2);    \
    __wt_indent_str[wt_call_nest_level*2] = 0;    \
} while(0)
...
#define __WT_SET_SESSION_INFO(__ts__) do { \
    if (__session__) { \
        /* See __eventv() */    \
        \
        char *__p__ = __wt_session_info_str;    \
        size_t __remain__ = sizeof(__wt_session_info_str);    \
        \
