...
extern void __wt_chunkcache_check(WT_SESSION_IMPL *session, WT_BLOCK *block, uint32_t objectid,
  ...
  void *dst);
extern void __wt_chunkcache_complete_read(WT_SESSION_IMPL *session, WT_BLOCK *block,
  WT_CHUNKCACHE_CHUNK *chunk, wt_off_t offset, uint32_t size, void *dst, bool *chunkcache_has_data);
