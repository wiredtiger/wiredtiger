...
extern int __wt_hazard_weak_set(WT_SESSION_IMPL *session, WT_REF *ref, WT_HAZARD_WEAK **whpp)
  WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
extern int __wt_hazard_weak_try_upgrade(WT_SESSION_IMPL *session, WT_HAZARD_WEAK *whp,
  WT_REF **refp) WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
...
extern int __wt_page_inmem_readonly(WT_SESSION_IMPL *session, WT_REF *ref, const void *image,
  uint32_t flags, WT_PAGE **pagep) WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
...
static inline void __wt_txn_op_set_timestamp(
  WT_SESSION_IMPL *session, WT_TXN_OP *op, WT_UPDATE *upd);
