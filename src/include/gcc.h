 ...
 * for details.
 */
#define WT_ATOMIC_CAS(ptr, old, new) (__sync_val_compare_and_swap(ptr, old, new) == (old))
#else
#define WT_ATOMIC_CAS(ptr, old, new) __sync_bool_compare_and_swap(ptr, old, new)
#endif
...
WT_ATOMIC_CAS_FUNC(v32, volatile uint32_t *vp, volatile uint32_t old, volatile uint32_t new)
WT_ATOMIC_CAS_FUNC(i32, int32_t *vp, int32_t old, int32_t new)
WT_ATOMIC_CAS_FUNC(iv32, volatile int32_t *vp, volatile int32_t old, volatile int32_t new)
WT_ATOMIC_CAS_FUNC(64, uint64_t *vp, uint64_t old, uint64_t new)
WT_ATOMIC_CAS_FUNC(v64, volatile uint64_t *vp, volatile uint64_t old, volatile uint64_t new)
WT_ATOMIC_CAS_FUNC(i64, int64_t *vp, int64_t old, int64_t new)
WT_ATOMIC_CAS_FUNC(iv64, volatile int64_t *vp, volatile int64_t old, volatile int64_t new)
...
#define WT_ATOMIC_FUNC(name, ret, vp_arg, v_arg)                 \
    ...
    static inline ret __wt_atomic_store##name(vp_arg, v_arg)     \
    {                                                            \
