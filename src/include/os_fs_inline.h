 ...
 */
static WT_INLINE int
__wt_fs_free_space(WT_SESSION_IMPL *session, WT_FH *fh, wt_off_t *freep)
{
#ifdef _WIN32
    return (__wt_win_fs_free_space(session, fh, freep));
#else
    return (__wt_posix_fs_free_space(session, fh, freep));
#endif
}
