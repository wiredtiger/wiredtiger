...
__txn_apply_prepare_state_update(WT_SESSION_IMPL *session, WT_UPDATE *upd, bool commit)
{
    ...
    if (commit) {
        ...
        WT_RELEASE_BARRIER();
        /* do we need to reset prepare_ts and prepared_id here? */
...
__wt_txn_upd_visible_type(WT_SESSION_IMPL *session, WT_UPDATE *upd)
{
    ...
    for (;; __wt_yield()) {
        ...
        upd_visible = __wt_txn_visible(session, upd->txnid,
          prepare_state == WT_PREPARE_INPROGRESS ? upd->prepare_ts : upd->upd_start_ts,
          prepare_state == WT_PREPARE_INPROGRESS ? upd->prepare_ts : upd->upd_durable_ts);
