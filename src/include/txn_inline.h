...
__wt_txn_begin(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    if (txn->isolation == WT_ISO_SNAPSHOT &&
      !(F_ISSET(txn, WT_TXN_AUTOCOMMIT) && F_ISSET(txn, WT_TXN_HAS_SNAPSHOT))) {
        ...
        /*
         * Stall here if the cache is completely full. We have not allocated a transaction ID yet.
         * Transaction IDs make it possible for eviction to decide we're contributing to the problem
         * and return WT_ROLLBACK. The WT_SESSION.begin_transaction API can't return rollback,
         * continue on.
         */
