...
#define ref_ikey key.ikey
     ...
     * WT_REF.ft_info.del field is allocated and initialized.
     *
     * If the page must be instantiated before the truncate commits: (1) WT_UPDATE structures are
     * created for the page entries, (2) the transaction information from WT_REF.ft_info.del is
     * migrated to those WT_UPDATE structures (making them a match for the truncate operation), and
     * (3) the WT_REF.ft_info.del field is freed and replaced by WT_REF.ft_info.update, an array of
     * references to the WT_UPDATE structures (needed for subsequent transaction commit/abort).
     * Additionally, the WT_REF.state switches to WT_REF_MEM. If the page must be instantiated
     ...
     * does), requires the WT_REF be locked. If the locked WT_REF's previous state was
     * WT_REF_DELETED, WT_REF.del is valid, if the WT_REF's previous state was an in-memory state,
     * then WT_REF.update may be valid.
     */
    union {
