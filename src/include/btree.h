 ...
 * block manager. The btree engine limits the size of a single object to (4GB - 1KB); that gives us
 * additional bytes if we ever want to store a structure length plus the object size in 4B, or if
 * we need additional flag values. Attempts to store large key/value items in the tree trigger an
 * immediate check to the block manager, to make sure it can write the item. Storing 4GB objects
 * in a btree borders on clinical insanity, anyway.
 *
