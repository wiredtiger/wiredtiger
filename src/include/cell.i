...
/*
 * __cell_unpack_init_time_window --
 *     Setup a time window in a cell.
 */
static inline void
__cell_unpack_init_time_window(WT_CELL_UNPACK *unpack)
{
    ...
    /*
     * It's a bit odd that flags initialization is here - but it's only used when timestamps are
     * also used.
     */
    unpack->flags = 0;
}
  ...
  wt_timestamp_t stop_ts, uint64_t stop_txn, bool prepare)
{
    ...
    /*
     * We are running in version 4.2, so should never be writing out timestamps. The remainder of
     * this function is kept in sync with the 4.4 version, to make it easy to remember what the data
     * format generated by 4.4 is.
     */
    WT_ASSERT(session, !__wt_process.page_version_ts);
...
static inline int
__wt_cell_unpack_safe(
  const WT_PAGE_HEADER *dsk, WT_CELL *cell, WT_CELL_UNPACK *unpack, const void *end)
{
    struct {
        uint64_t v;
...
restart:
    ...
    unpack->ovfl = 0;
    __cell_unpack_init_time_window(unpack);
 ...
 */
    ...
    /*
     * We keep the cell unpack code consistent between 4.2 and 4.4 releases of MongoDB, since 4.2
     * needs to understand the 4.4 data format, but 4.2 never needs to know about time window
     * information, so clear it here before it leaks out of the cell unpack code.
     */
    __cell_unpack_init_time_window(unpack);
  ...
  WT_SESSION_IMPL *session, const WT_PAGE_HEADER *dsk, WT_CELL *cell, WT_CELL_UNPACK *unpack)
{
    ...
    WT_IGNORE_RET(__wt_cell_unpack_safe(dsk, cell, unpack, NULL));
