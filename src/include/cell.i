...
__cell_unpack_init_time_window(WT_CELL_UNPACK *unpack) {
    ...
    /*
     * It's a bit odd that flags initialization is here - but it's only used when timestamps
     * are also used.
     */
    unpack->flags = 0;
}
  ...
  wt_timestamp_t stop_ts, uint64_t stop_txn, bool prepare)
{
     ...
     * this function is kept in sync with the 4.4 version, to make it easy to remember what the
     * data format generated by 4.4 is.
     */
 ...
 */
static inline int
__wt_cell_unpack_safe(const WT_PAGE_HEADER *dsk, WT_CELL *cell,
