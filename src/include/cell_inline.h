...
__wt_cell_total_len(void *unpack_arg)
{
    ...
    unpack = static_cast<WT_CELL_UNPACK_COMMON *>(unpack_arg);
  ...
  WT_CELL_UNPACK_COMMON *unpack, WT_ITEM *store)
{
    ...
    return (huffman == NULL || store->size == 0 ?
        0 :
            __wt_huffman_decode(session, huffman, static_cast<const uint8_t *>(store->data), store->size, store));
}
...
{
	return (__cell_data_ref(session, page, page->type, static_cast<WT_CELL_UNPACK_COMMON *>(unpack_arg), store));
}
...
#define WT_CELL_FOREACH_ADDR(session, dsk, unpack)                                              \
    do {                                                                                        \
        uint32_t __i;                                                                           \
        uint8_t *__cell;                                                                        \
        for (__cell = static_cast<uint8_t *>(WT_PAGE_HEADER_BYTE(S2BT(session), dsk)), __i = (dsk)->u.entries; __i > 0; \
