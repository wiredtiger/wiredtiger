  ...
  WT_PAGE_DELETED *page_del, WT_TIME_AGGREGATE *ta, size_t size)
{
    ...
    *p = '\0';
    ...
    if (page_del != NULL && __wt_process.fast_truncate_2022) {
        ...
        printf("7.0 packed fast-truncate page_del->txnid %lu\n", page_del->txnid);
        printf("7.0 packed fast-truncate page_del->timestamp %lu\n", page_del->timestamp);
        printf("7.0 packed fast-truncate page_del->durable_timestamp %lu\n", page_del->durable_timestamp);
    }
