...
#define WT_LRU_TRACEEF(STR, elm, field) \
    WT_LRU_TRACEF(STR, "ref=%p %s CACHE %s %p %p %p(%p) ELEMENT %p %p %p(%p)", (void*)elm, __wt_ref_is_root(elm) ? "is_root" : "is_not_root", #field, \
...
#define __WT_LRU_LOCK(field) \
    __wt_spin_lock(session, &S2C(session)->cache->field##_lock); \
    printf("LOCK %s %s %s %s:%d\n", session->name, #field, __FUNCTION__, __FILE__, __LINE__); fflush(stdout)
#define __WT_LRU_UNLOCK(field) \
    printf("UNLOCK %s %s %s %s:%d\n", session->name, #field, __FUNCTION__, __FILE__, __LINE__); fflush(stdout); \
    __wt_spin_unlock(session, &S2C(session)->cache->field##_lock)
...
#define WT_LRU_POP(elm, field) \
    do { \
        if (__wt_atomic_load_pointer(&S2C(session)->cache->field.tqh_first)) { \
            ...
            WT_ASSERT(session, S2C(session)->cache->field.tqh_last != NULL); \
            elm = TAILQ_FIRST(&S2C(session)->cache->field); \
            WT_LRU_REMOVE_AND_CLEAR_NOLOCK(elm, field); \
            __wt_atomic_store64(&elm->field##_t, 0); \
            /* WT_RELEASE_BARRIER(); */ \
