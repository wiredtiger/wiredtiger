...
/*
 * __wt_evict_busy_clr --
 *     Clear the eviction busy field atomically. Restore the saved cursor if needed.
 */
static inline void
__wt_evict_busy_clr(WT_SESSION_IMPL *session, WT_BTREE *btree, WT_CURSOR *save)
{
    (void)__wt_atomic_subv32(&btree->evict_busy, 1);
    if (save != NULL)
        /* Restore the caller's history store cursor. */
        session->hs_cursor = save;
}

/*
 * __wt_evict_busy_set --
 *     Set the eviction busy field atomically. Before doing so make sure we have cached a history
 *     store cursor. Save the session's original history store cursor if needed.
 */
static inline int
__wt_evict_busy_set(WT_SESSION_IMPL *session, WT_BTREE *btree, WT_CURSOR **save)
{
    WT_CONNECTION_IMPL *conn;
     ...
     */
    if (save != NULL) {
        *save = session->hs_cursor;
        session->hs_cursor = NULL;
    }

    /*
     * Before we mark the btree busy for eviction, make sure this session has a cached history store
    ...
    if ((session->dhandle == NULL || !WT_IS_METADATA(S2BT(session)->dhandle)) &&
      session->hs_cursor == NULL && !F_ISSET(conn, WT_CONN_IN_MEMORY) &&
