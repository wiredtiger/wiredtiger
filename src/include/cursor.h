 ...
 */
typedef enum { WT_CLAYERED_ROLE_FOLLOWER, WT_CLAYERED_ROLE_LEADER } WT_CLAYERED_ROLE;
...
/*
 * WT_CLAYERED_OP_MODE --
 *	The kind of layered-table operation; maps 1:1 onto the API methods.
 */
 ...
 */
typedef enum {
    WT_CLAYERED_RESULT_NONE,        /* unpositioned: insert success, NOTFOUND, error, reset */
...
struct __wt_clayered_op {
    ...
    WT_CURSOR *current_cursor;       /* snapshot of clayered->current_cursor; propagated back by wrapper */
    ...
    bool alternate_positioned; /* the alternate constituent is seated for this op's walk direction */
