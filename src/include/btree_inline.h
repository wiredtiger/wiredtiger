...
__wt_page_evict_retry(WT_SESSION_IMPL *session, WT_PAGE *page)
{
    ...
    if (__wt_cache_aggressive(session) ||
      mod->wti_last_evict_pass_gen + 5 < __wt_atomic_load64(&S2C(session)->cache->evict_pass_gen))
        return (true);
    ...
    if (__wt_atomic_loadv64(&txn_global->current) == __wt_atomic_loadv64(&txn_global->oldest_id) ||
      mod->wti_last_eviction_id != __wt_txn_oldest_id(session))
        return (true);
     ...
     */
    if (mod->wti_last_eviction_timestamp != WT_TS_NONE) {
        __wt_txn_pinned_timestamp(session, &pinned_ts);
        if (pinned_ts > mod->wti_last_eviction_timestamp)
            return (true);
    }
