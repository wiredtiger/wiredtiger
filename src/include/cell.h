...
struct __wt_cell {
     ...
     * 25: data length		(address, max 5 bytes, 2 lengths, max 1 byte, 2 counts, max 9 bytes)
     *
...
struct __wt_cell_unpack_kv {
    ...
    WT_PAGE_STAT ovfl_ps; /* Page information including row and byte counts */
};
