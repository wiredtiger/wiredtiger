 ...
 */
#define __wt_verbose_level(session, id, category, level, fmt, ...)               \
    do {                                                                         \
        if (WT_VERBOSE_LEVEL_ISSET(session, category, level))                    \
            __wt_verbose_worker(session, id, category, level, fmt, __VA_ARGS__); \
    } while (0)
 ...
 */
#define __wt_verbose_error(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_ERROR, fmt, __VA_ARGS__)

/*
 * __wt_verbose_warning --
 *     Wrapper to __wt_verbose_level defaulting the verbosity level to WT_VERBOSE_WARNING.
 */
#define __wt_verbose_warning(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_WARNING, fmt, __VA_ARGS__)

/*
 * __wt_verbose_notice --
 *     Wrapper to __wt_verbose_level defaulting the verbosity level to WT_VERBOSE_NOTICE.
 */
#define __wt_verbose_notice(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_NOTICE, fmt, __VA_ARGS__)

/*
 * __wt_verbose_info --
 *     Wrapper to __wt_verbose_level defaulting the verbosity level to WT_VERBOSE_INFO.
 */
#define __wt_verbose_info(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_INFO, fmt, __VA_ARGS__)

/*
 * __wt_verbose_debug1 --
 *     Wrapper to __wt_verbose_level using the default (DEBUG_1) verbosity level.
 */
#define __wt_verbose_debug1(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_DEBUG_1, fmt, __VA_ARGS__)

/*
 * __wt_verbose_debug2 --
 *     Wrapper to __wt_verbose_level using the DEBUG_2 level.
 */
#define __wt_verbose_debug2(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_DEBUG_2, fmt, __VA_ARGS__)

/*
 * __wt_verbose_debug3 --
 *     Wrapper to __wt_verbose_level using the DEBUG_3 level.
 */
#define __wt_verbose_debug3(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_DEBUG_3, fmt, __VA_ARGS__)
 ...
 */
#define __wt_verbose(session, id, category, fmt, ...) \
    __wt_verbose_level(session, id, category, WT_VERBOSE_LEVEL_DEFAULT, fmt, __VA_ARGS__)
 ...
 */
#define __wt_verbose_level_multi(session, id, multi_category, level, fmt, ...)                    \
    do {                                                                                          \
        uint32_t __v_idx;                                                                         \
        /*                                                                                        \
        ...
        for (__v_idx = 0; __v_idx < __multi_category.cnt; __v_idx++) {                            \
            if (WT_VERBOSE_LEVEL_ISSET(session, __multi_category.categories[__v_idx], level)) {   \
                __wt_verbose_worker(                                                              \
                  session, id, __multi_category.categories[__v_idx], level, fmt, __VA_ARGS__);    \
                break;                                                                            \
            }                                                                                     \
        }                                                                                         \
    } while (0)
 ...
 *     verbosity level.
 */
#define __wt_verbose_multi(session, id, multi_category, fmt, ...)                                 \
    do {                                                                                          \
        uint32_t __v_idx;                                                                         \
        /*                                                                                        \
         * multi_category can be a ternary expression that returns one of two structs. If we call \
        ...
        for (__v_idx = 0; __v_idx < __multi_category.cnt; __v_idx++) {                            \
            if (WT_VERBOSE_ISSET(session, __multi_category.categories[__v_idx])) {                \
                __wt_verbose_worker(session, id, __multi_category.categories[__v_idx],            \
