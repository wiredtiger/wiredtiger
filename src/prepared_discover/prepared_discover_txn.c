...
static int
__prepared_discover_find_or_create_transaction(
  WT_SESSION_IMPL *session, uint64_t prepare_transaction_id, WT_SESSION_IMPL **prep_sessionp)
{
    WT_SESSION_IMPL *new_session;
    ...
    if (txn_global->pending_prepared_sessions != NULL) {
        for (prepared_session_cnt = 0;
             (next_sess = txn_global->pending_prepared_sessions[prepared_session_cnt]) != NULL;
             prepared_session_cnt++) {
            if (next_sess->txn->prepare_transaction_id == prepare_transaction_id) {
                *prep_sessionp = next_sess;
                return (0);
            }
        }
    }
    ...
    if (txn_global->pending_prepared_sessions_allocated == 0 ||
      txn_global->pending_prepared_session_allocated - 1 == prepared_session_cnt) {
        txn_global->pending_prepared_sessiions_allocated += 10;
        WT_RET(__wt_realloc_def(session, txn_global->pending_prepared_sessions_allocated,
          &txn_global->pending_prepared_sessions));
    }
    ...
    new_session->txn->prepared_transaction_id = prepared_transaction_id;
    /* Add it to the discovered set of sessions. */
    txn_global->pending_prepared_sessions[prepared_session_cnt];
  ...
  WT_SESSION_IMPL *session, uint64_t prepare_transaction_id, WT_ITEM *key, WT_UPDATE *upd)
{
    ...
    WT_ERR(__prepared_discover_find_or_create_transaction(
  ...
  WT_SESSION_IMPL *session, uint64_t prepare_transaction_id, WT_TIME_WINDOW *tw, WT_ITEM *key)
{
     ...
     */
    WT_RET(__wt_upd_alloc(session, full_value, WT_UPDATE_STANDARD, &upd, NULL));
...
err:
     ...
     */
    __wt_free_upd_list(session, &upd);
    return (ret);
}
