...
__log_openfile(WT_SESSION_IMPL *session, uint32_t id, uint32_t flags, WT_FH **fhp)
{
    ...
    WT_ERR(__wt_open(
      session, static_cast<const char *>(buf->data), WT_FS_OPEN_FILE_TYPE_LOG, wtopen_flags, fhp));
err:
    __wt_scr_free(session, &buf);
    return (ret);
}
...
__log_alloc_prealloc(WT_SESSION_IMPL *session, uint32_t to_num)
{
     ...
     * it.
     */
    WT_ERR(__wt_fs_rename(session, static_cast<const char *>(from_path->data),
      static_cast<const char *>(to_path->data), false));
