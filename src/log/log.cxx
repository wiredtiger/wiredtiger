...
__log_checksum_match(WT_ITEM *buf, uint32_t reclen)
{
    ...
    logrec = static_cast<WT_LOG_RECORD*>(buf->mem);
...
__log_openfile(WT_SESSION_IMPL *session, uint32_t id, uint32_t flags, WT_FH **fhp)
{
    ...
    WT_ERR(__wt_open(session, static_cast<const char*>(buf->data), WT_FS_OPEN_FILE_TYPE_LOG, wtopen_flags, fhp));
err:
    __wt_scr_free(session, &buf);
    return (ret);
}
...
__log_alloc_prealloc(WT_SESSION_IMPL *session, uint32_t to_num)
{
     ...
     * it.
     */
    WT_ERR(__wt_fs_rename(session, static_cast<const char*>(from_path->data), static_cast<const char*>(to_path->data), false));
...
__wt_log_remove(WT_SESSION_IMPL *session, const char *file_prefix, uint32_t lognum)
{
    ...
    WT_ERR(__wt_fs_remove(session, static_cast<const char*>(path->data), false));
err:
...
__log_check_partial_write(WT_SESSION_IMPL *session, WT_ITEM *buf, uint32_t reclen)
{
     ...
     */
    rec = static_cast<uint8_t*>(buf->mem);
