...
__wt_log_reset(WT_SESSION_IMPL *session, uint32_t lognum)
{
    ...
    WT_ASSERT(session, F_ISSET_ATOMIC_64(conn, WT_CONN_RECOVERING));
    WT_ASSERT(session, !F_ISSET_ATOMIC_64(conn, WT_CONN_READONLY));
    /*
  ...
  uint16_t *versionp, bool *need_salvagep)
{
    ...
    need_salvage = false;
    WT_RET(__wt_scr_alloc(session, 0, &buf));
    salvage_mode = (need_salvagep != NULL && F_ISSET_ATOMIC_64(conn, WT_CONN_SALVAGE));
    ...
    *error_offset = 0;
    ...
    for (off = offset; remainder > 0; remainder -= (wt_off_t)rdlen, off += (wt_off_t)rdlen) {
        ...
        if (memcmp(buf, zerobuf, rdlen) != 0) {
             ...
             * record.
             */
            if (!F_ISSET_ATOMIC_64(conn, WT_CONN_WAS_BACKUP)) {
...
__wti_log_release(WT_SESSION_IMPL *session, WTI_LOGSLOT *slot, bool *freep)
{
     ...
     */
    if (WT_CKPT_LOGSIZE(conn) && !F_ISSET_ATOMIC_64(conn, WT_CONN_CLOSING)) {
...
advance:
        ...
        need_salvage = F_ISSET_ATOMIC_64(conn, WT_CONN_SALVAGE);
        WT_ERR(
...
__wt_log_write(WT_SESSION_IMPL *session, WT_ITEM *record, WT_LSN *lsnp, uint32_t flags)
{
     ...
     */
    if (!F_ISSET(log, WTI_LOG_OPENED) || F_ISSET_ATOMIC_64(conn, WT_CONN_READONLY))
        return (0);
    ip = record;
