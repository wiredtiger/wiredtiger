...
/*
 * __wt_log_compute_min_lognum --
 *     Determine the number of the earliest log file we must keep.
 */
uint32_t
__wt_log_compute_min_lognum(WT_SESSION_IMPL *session, WT_LOG *log, uint32_t backup_file)
{
 ...
 */
int
__wt_log_remove_once(WT_SESSION_IMPL *session, uint32_t backup_file)
{
    WT_CONNECTION_IMPL *conn;
    WT_DECL_RET;
    WT_LOG *log;
    ...
    min_lognum = __wt_log_compute_min_lognum(session, log, backup_file);
...
__wt_log_truncate_files(WT_SESSION_IMPL *session, WT_CURSOR *cursor, bool force)
{
    ...
    ret = __wt_log_remove_once(session, backup_file);
