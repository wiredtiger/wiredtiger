  ...
  uint16_t *versionp, bool *need_salvagep)
{
     ...
     * maximum.
     */
    if (__wt_version_ne(conn->req_max_version, WT_NO_VERSION) && desc->version > conn->log_req_max)
        WT_ERR_MSG(session, WT_ERROR,
    ...
    if (__wt_version_ne(conn->req_max_version, WT_NO_VERSION) && desc->version < conn->log_req_min)
        WT_ERR_MSG(session, WT_ERROR,
          WT_COMPAT_MSG_PREFIX
