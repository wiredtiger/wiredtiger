...
__log_zero(WT_SESSION_IMPL *session, WT_FH *fh, wt_off_t start_off, wt_off_t len)
{
    ...
    /*
     * Read in a chunk starting at the end of the file. Keep going until we reach the beginning or
     * we find a chunk that contains any non-zero bytes. Compare against a known zero byte chunk.
     */
    off = (uint32_t)start_off;
    while (off < (uint32_t)len) {
        /*
         * Typically we start to zero the file after the log header and the bufsz is a
         * sector-aligned size. So we want to align our writes when we can.
         */
        partial = off % bufsz;
        if (partial != 0)
            wrlen = bufsz - partial;
