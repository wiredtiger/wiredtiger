...
__wti_log_slot_join(WT_SESSION_IMPL *session, uint64_t mysize, uint32_t flags, WT_MYSLOT *myslot)
{
    ...
    bool closed, diag_yield, raced, slept, unbuffered, yielded, force_unbuffered;
    ...
    force_unbuffered = (log->write_calls % WT_THOUSAND) == 0 || mysize > WT_LOG_SLOT_BUF_MAX;
#else
    diag_yield = false;
    force_unbuffered = mysize > WT_LOG_SLOT_BUF_MAX;
#endif
    if (force_unbuffered) {
        unbuffered = true;
