...
__bulk_col_keycmp_err(WT_CURSOR_BULK *cbulk)
{
    ...
    WT_RET_MSG(session, EINVAL, "bulk-load presented with out-of-order keys: %" PRIu64 " is less "
...
__bulk_row_keycmp_err(WT_CURSOR_BULK *cbulk)
{
    ...
    WT_ERR_MSG(session, EINVAL, "bulk-load presented with out-of-order keys: %s compares smaller "
