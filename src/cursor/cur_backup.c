...
__wt_verbose_dump_backup(WT_SESSION_IMPL *session)
{
    ...
    if (!F_ISSET_ATOMIC_64(conn, WT_CONN_INCR_BACKUP)) {
...
__curbackup_next(WT_CURSOR *cursor)
{
     ...
     */
    if (F_ISSET_ATOMIC_64(S2C(session), WT_CONN_INCR_BACKUP) && cb->cfg_list != NULL)
  ...
  WT_CURSOR **cursorp)
{
    ...
    *cursor = iface;
    ...
    if (WT_STRING_LIT_MATCH("backup:query_id", uri, uri_len)) {
        ...
        if (!F_ISSET_ATOMIC_64(S2C(session), WT_CONN_INCR_BACKUP))
