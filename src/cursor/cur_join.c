...
__curjoin_iter_set_entry(WT_CURSOR_JOIN_ITER *iter, u_int entry_pos)
{
	...
	size_t size;
	const char *raw_cfg[] = {
	    WT_CONFIG_BASE(iter->session, WT_SESSION_open_cursor), "raw", NULL};
	...
	iter->end_skip =
	    (entry->ends_next > 0 && WT_CURJOIN_END_RANGE(&entry->ends[0]) == WT_CURJOIN_END_GE) ?
	    1 :
	    0;
	...
	if (entry->subjoin == NULL) {
		...
		if ((c = iter->cursor) == NULL || strcmp(c->uri, uri) != 0) {
			...
			WT_ERR(__wt_open_cursor(
			    session, uri, (WT_CURSOR *)topjoin, config, &iter->cursor));
		}
	...
	if ((cursor = endpoint->cursor) != NULL) {
		if (entry->index != NULL) {
			...
			WT_RET(__wt_struct_repack(session, cindex->child->key_format,
			    (entry->repack_format != NULL ? entry->repack_format :
			...
			if (WT_CURSOR_RECNO(cursor)) {
				r = *(uint64_t *)k->data;
				WT_RET(__curjoin_pack_recno(session, r, endpoint->recno_buf,
				    sizeof(endpoint->recno_buf), &endpoint->key));
			} else
				endpoint->key = *k;
		}
	}
 ...
 */
static int
__curjoin_entry_in_range(WT_SESSION_IMPL *session, WT_CURSOR_JOIN_ENTRY *entry, WT_ITEM *curkey,
    WT_CURSOR_JOIN_ITER *iter)
{
...
__curjoin_init_next(WT_SESSION_IMPL *session, WT_CURSOR_JOIN *cjoin, bool iterable)
{
	...
	if (cjoin->entries_next == 0)
		WT_RET_MSG(session, EINVAL, "join cursor has not yet been joined with any other "
		                            "cursors");
	...
	for (je = cjoin->entries; je < jeend; je++) {
		...
		if (iterable && je->ends_next == 1 && F_ISSET(&je->ends[0], WT_CURJOIN_END_LT)) {
			...
			WT_ERR(__wt_open_cursor(session, origcur->uri, (WT_CURSOR *)cjoin,
			    ...
			    WT_CURJOIN_END_GT | WT_CURJOIN_END_EQ | WT_CURJOIN_END_OWN_CURSOR;
		...
		if (!iterable && F_ISSET(je, WT_CURJOIN_ENTRY_BLOOM)) {
			...
			if (je->bloom == NULL) {
				...
				for (je2 = je + 1; je2 < jeend; je2++)
					if (F_ISSET(je2, WT_CURJOIN_ENTRY_BLOOM) &&
					    je2->count == je->count) {
				...
				WT_ERR(__wt_bloom_create(
				    session, NULL, NULL, je->count, f, k, &je->bloom));
				 ...
				 */
				WT_ERR(__wt_bloom_create(session, NULL, NULL, je->count,
				    je->bloom_bit_count, je->bloom_hash_count, &bloom));
    ...
    WT_CURSOR_JOIN_ENDPOINT **newendp)
{
	...
	WT_RET(
	    __wt_realloc_def(session, &entry->ends_allocated, entry->ends_next + 1, &entry->ends));
...
__curjoin_next(WT_CURSOR *cursor)
{
	...
	while ((ret = __curjoin_iter_next(iter, cursor)) == 0) {
		if ((ret = __curjoin_entries_in_range(session, cjoin, iter->curkey, iter)) !=
		    WT_NOTFOUND)
			break;
	}
    ...
    WT_CURSOR *tocur, WT_CURSOR *fromcur, const char *repack_fmt, bool isindex)
{
	...
	if (isindex) {
		 ...
		 */
		WT_RET(__wt_struct_repack(session, cindex->child->key_format,
		    repack_fmt != NULL ? repack_fmt : cindex->iface.key_format, &cindex->child->key,
		    idxkey));
		...
		if (WT_CURSOR_RECNO(tocur)) {
			...
			WT_RET(__curjoin_pack_recno(session, tocur->recno, cjoin->recno_buf,
			    sizeof(cjoin->recno_buf), &tocur->key));
		} else {
    ...
    WT_CURSOR **cursorp)
{
	...
	if (owner != NULL)
		WT_RET_MSG(
		    session, EINVAL, "unable to initialize a join cursor with existing owner");
    ...
    uint32_t bloom_hash_count)
{
	...
	if (entry == NULL) {
		...
		if (LF_ISSET(WT_CURJOIN_ENTRY_BLOOM) && needbloom) {
			...
			memmove(entry + 1, entry,
			    (cjoin->entries_next - nonbloom) * sizeof(WT_CURSOR_JOIN_ENTRY));
		...
		if (count != 0 && entry->count != 0 && entry->count != count)
			WT_RET_MSG(session, EINVAL, "count=%" PRIu64 " does not match "
			                            "previous count=%" PRIu64 " for this index",
			    count, entry->count);
		if (LF_MASK(WT_CURJOIN_ENTRY_BLOOM) != F_MASK(entry, WT_CURJOIN_ENTRY_BLOOM))
			WT_RET_MSG(session, EINVAL, "join has incompatible strategy "
		...
		for (i = 0; i < entry->ends_next; i++) {
			...
			if (range == WT_CURJOIN_END_EQ && endrange == WT_CURJOIN_END_EQ &&
				...
				WT_RET_MSG(session, EINVAL, "compare=eq can only be combined "
				                            "using operation=or");
	...
	if (nested) {
		...
		WT_RET(__curjoin_insert_endpoint(
		    session, entry, hasins ? ins : entry->ends_next, &end));
		...
		if (entry->main == NULL && idx != NULL) {
			...
			WT_RET(__wt_snprintf(
			    entry->repack_format, len, "%s0x", cindex->iface.key_format));
		}
	}
	return (0);
}
