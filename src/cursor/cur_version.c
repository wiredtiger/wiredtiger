 ...
 * value.
 *
 * Note: in WT_UPDATE, durable_ts/start_ts share memory with rollback_ts/saved_txnid via a union.
 * For aborted (rolled-back) updates we read through the rollback-side accessors (upd_rollback_ts,
 * upd_saved_txnid) so the emitted start_ts and start_durable_ts fields carry the rollback timestamp
 * and saved transaction ID respectively. The consumer detects this case via the
 * WT_UPDATE_PREPARE_ROLLBACK flag or by checking txnid == WT_TXN_ABORTED.
 */
    ...
    if (!F_ISSET(version_cursor, WT_CURVERSION_UPDATE_EXHAUSTED)) {
        ...
        if (upd == NULL) {
            ...
            if (upd->type == WT_UPDATE_TOMBSTONE) {
                ...
                /*
                 * For aborted updates the WT_UPDATE union stores rollback_ts/saved_txnid instead of
                 * durable_ts/start_ts. Use the correct accessors based on the transaction state.
                 */
                if (upd->txnid == WT_TXN_ABORTED) {
                    version_cursor->upd_durable_stop_ts = upd->upd_rollback_ts;
                    version_cursor->upd_stop_ts = upd->upd_saved_txnid;
                } else {
            ...
            if (upd == NULL) {
                ...
                if (upd->txnid == WT_TXN_ABORTED)
                    WT_ERR(__curversion_set_value_with_format(cursor, WT_CURVERSION_METADATA_FORMAT,
                      upd->txnid, upd->upd_rollback_ts, upd->upd_saved_txnid,
                      (uint64_t)upd->prepare_ts, (uint64_t)upd->prepared_id,
