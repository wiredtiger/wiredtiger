...
__curhs_insert(WT_CURSOR *cursor)
{
     ...
     */
    WT_ERR(__wt_upd_alloc(session, cbt->ref ? cbt->ref->page : NULL, &file_cursor->value,
      WT_UPDATE_STANDARD, &hs_upd, NULL));
    ...
    if (WT_TIME_WINDOW_HAS_STOP(&hs_cursor->time_window)) {
         ...
         */
        WT_ERR(
          __wt_upd_alloc_tombstone(session, cbt->ref ? cbt->ref->page : NULL, &hs_tombstone, NULL));
