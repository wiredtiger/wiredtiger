...
__curhs_next(WT_CURSOR *cursor)
{
    ...
    CURSOR_API_CALL_PREPARE_ALLOWED(
      cursor, session, next, ((WT_CURSOR_BTREE *)file_cursor)->dhandle);
...
__curhs_prev(WT_CURSOR *cursor)
{
    ...
    file_cursor = hs_cursor->file_cursor;
    CURSOR_API_CALL_PREPARE_ALLOWED(
      cursor, session, prev, ((WT_CURSOR_BTREE *)file_cursor)->dhandle);
...
__curhs_close(WT_CURSOR *cursor)
{
    ...
    CURSOR_API_CALL_PREPARE_ALLOWED(cursor, session, close,
      file_cursor == NULL ? NULL : ((WT_CURSOR_BTREE *)file_cursor)->dhandle);
err:
...
__curhs_search_near(WT_CURSOR *cursor, int *exactp)
{
    ...
    *exactp = 0;
    ...
    CURSOR_API_CALL_PREPARE_ALLOWED(
      cursor, session, search_near, ((WT_CURSOR_BTREE *)file_cursor)->dhandle);
...
__curhs_insert(WT_CURSOR *cursor)
{
    ...
    hs_upd->upd_commit_ts = hs_cursor->time_window.start_commit_ts;
    hs_upd->upd_durable_ts = hs_cursor->time_window.start_durable_ts;
    ...
    if (WT_TIME_WINDOW_HAS_STOP(&hs_cursor->time_window)) {
        ...
        hs_tombstone->upd_commit_ts = hs_cursor->time_window.stop_commit_ts;
        hs_tombstone->upd_durable_ts = hs_cursor->time_window.stop_durable_ts;
        hs_tombstone->txnid = hs_cursor->time_window.stop_txn;

        WT_ASSERT(session,
          hs_tombstone->upd_commit_ts >= hs_upd->upd_commit_ts &&
...
__curhs_remove_int(WT_CURSOR_BTREE *cbt, const WT_ITEM *value, u_int modify_type)
{
    ...
    hs_tombstone->upd_commit_ts = hs_tombstone->upd_durable_ts = WT_TS_NONE;
