...
{
    if (hexonly)
	    WT_RET(__wt_raw_to_hex(session, static_cast<const uint8_t*>(from->data), from->size, to));
    else
	    WT_RET(__wt_raw_to_esc_hex(session, static_cast<const uint8_t*>(from->data), from->size, to));
...
__curdump_get_key(WT_CURSOR *cursor, ...)
{
    ...
    if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON)) {
        ...
        if (F_ISSET(cursor, WT_CURSTD_RAW)) {
	        ...
	        *va_arg(ap, const char **) = static_cast<const char*>(cursor->key.data);
        va_end(ap);
    }
...
__curdump_set_keyv(WT_CURSOR *cursor, va_list ap)
{
    ...
    if (F_ISSET(cursor, WT_CURSTD_RAW))
	    p = static_cast<const char*>(va_arg(ap, WT_ITEM *)->data);
    else
...
__curdump_get_value(WT_CURSOR *cursor, ...)
{
    ...
    if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON)) {
        ...
        if (F_ISSET(cursor, WT_CURSTD_RAW)) {
	        ...
	        *va_arg(ap, const char **) = static_cast<const char*>(cursor->value.data);
        va_end(ap);
    }
