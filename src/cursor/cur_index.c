...
{
    # TBD - Pending colgroup deletion. Otherwise have to maintain internal structures of projection. 
}
  ...
  WT_CURSOR **cursorp)
{
    ...
    *cursor = iface;
    ...
    if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON))
        WT_ERR(
          __wt_json_column_init(cursor, table->key_format, &idx->colconf, &table->colconf));
