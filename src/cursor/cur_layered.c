...
__clayered_cursor_enter(WT_CURSOR_LAYERED *clayered)
{
    ...
    if (!F_ISSET(clayered, WT_CLAYERED_ACTIVE)) {
        /*
         * A layered cursor keeps its constituents active for its lifetime, so session->ncursors is
         * effectively never zero during layered use and the file-cursor "first cursor" eviction
         * check (gated on ncursors == 0 in __cursor_enter) never fires. The layered API entry is
         * the outermost cursor boundary for the operation, so run the check here unconditionally:
         * otherwise cache-overflow back-pressure (the WT_ROLLBACK that enforces cache_max_wait_ms)
         * never reaches the application on layered tables.
         *
         * The eviction worker assumes the current dhandle is a btree (it walks into the history
         * store via S2BT), but a layered operation's dhandle is the layered table, so run the check
         * with no dhandle as the transaction-level checks do.
         */
