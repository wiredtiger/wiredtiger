  ...
  WT_SESSION_IMPL *session, WT_CURSOR_LAYERED *clayered, const WT_ITEM *key, bool positioned)
{
    ...
    WT_CURSOR *const c_current = clayered->current_cursor;
    WT_DECL_RET;
    WT_ITEM value;

    WT_CLEAR(value);

    if (!positioned || (c_current != NULL && !F_ISSET(c_current, WT_CURSTD_VALUE_INT))) {
