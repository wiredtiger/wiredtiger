  ...
  WTI_CLAYERED_OP *op, WT_MODIFY *entries, int nentries, WT_ITEM *value, bool *need_full_updatep)
{
    ...
    *need_full_updatep = false;
    ...
    if (ret == WT_NOTFOUND) {
        /*
         * The stable open may have been skipped at enter time, but eviction from ingest implies the
         * key is in a picked-up stable checkpoint, so open stable now. Read the connection's LSN
         * before the open so the recorded LSN never claims a newer checkpoint than the cursor
         * holds.
         */
        if (op->stable == NULL) {
            if (clayered->stable_cursor == NULL) {
                conn_lsn = __wt_atomic_load_uint64_acquire(
                  &S2C(session)->disaggregated_storage.last_checkpoint_meta_lsn);
                F_CLR(clayered, WTI_CLAYERED_ITERATE_NEXT | WTI_CLAYERED_ITERATE_PREV);
                WT_RET(__clayered_open_stable(clayered, false, WTI_CLAYERED_ROLE_FOLLOWER));
                WT_RET(__clayered_copy_bounds(clayered));
                clayered->stable_checkpoint_meta_lsn = conn_lsn;
                WT_STAT_CONN_DSRC_INCR(session, layered_curs_open_stable);
            }
