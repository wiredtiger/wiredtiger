...
static int
__clayered_open_stable(
  WT_CURSOR_LAYERED *clayered, bool leader, const char **cfg, u_int cfg_len, u_int cfg_pos)
{
    WT_DECL_RET;
    ...
    if (!leader) {
        ...
        if (checkpoint_name == NULL) {
             ...
             * keeps the code easy.
             */
            cfg[cfg_pos++] = "readonly,checkpoint_use_history=false";
            ...
            cfg[cfg_pos++] = "readonly";
        }
    }
    WT_ASSERT_ALWAYS(session, cfg_pos < cfg_len, "configuration array too small");
    cfg[cfg_pos] = NULL;
...
__clayered_open_cursors(WT_CURSOR_LAYERED *clayered, bool update)
{
    ...
    if (clayered->stable_cursor == NULL) {
        ...
        WT_RET(__clayered_open_stable(clayered, leader, ckpt_cfg, WT_ELEMENTS(ckpt_cfg), cfg_pos));
    }
