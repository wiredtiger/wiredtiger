...
static int __clayered_op_finish(WT_CURSOR_LAYERED *, WT_CLAYERED_OP *, int);
...
__clayered_op_init(WT_CURSOR_LAYERED *clayered, WT_CLAYERED_OP_MODE mode, WT_CLAYERED_OP *op)
{
    ...
    /*
     * Default the position and walk state; the real values are taken in __clayered_op_finalize
     * once the constituents have settled. These defaults keep the wrapper's finalize safe if
     * __clayered_enter fails before finalize runs.
     */
    op->current_cursor = NULL;
    op->alternate_positioned = false;
    op->result = WT_CLAYERED_RESULT_NONE;
...
__clayered_iterate_int(WT_CLAYERED_OP *op, uint32_t iter_flag)
{
    ...
    do {
        WT_RET(__clayered_iterate_constituents(op, iter_flag));
        WT_RET_NOLOG(__clayered_get_current(op, iter_flag == WT_CLAYERED_ITERATE_NEXT));
...
err:
    WT_TRET(__clayered_op_finish(clayered, &op, ret));
 ...
 *     because a prepare conflict clears KEY_SET while leaving the btree cursor positioned
 *     (ref != NULL). A closed (leader) ingest cursor is NULL and skipped.
 */
static int
...
/*
 * __clayered_op_finish --
 *     The single finalize/cleanup point for a layered operation: propagate the op's result onto the
 *     public cursor on success and clean up the position on failure. Ordering matters and is the
 *     reason this is one function:
 *       1. Reset the constituents whenever no live position survives (an unpositioned or external
 *          result), so the body never has to. The one exception is a prepared conflict, whose
 *          half-finished walk (iteration) the retry depends on -- there we preserve.
 *       2. Reflect the result disposition onto iface (the only writer of the layered position). A
 *          positioned result paired with an error (duplicate key) is detached from the constituent,
 *          which is about to be unwound; a prepare conflict on search keeps the input key.
 *       3. Publish the resolved current_cursor (NULL after a reset).
 *       4. For next/prev, record the surviving walk direction. This runs after the reset so an
 *          exhausted walk (reset -> alternate_positioned cleared) does not leave the flag set.
 */
static int
__clayered_op_finish(WT_CURSOR_LAYERED *clayered, WT_CLAYERED_OP *op, int op_ret)
{
    WT_CURSOR *current = op->current_cursor;
    WT_CURSOR *iface = &clayered->iface;
    WT_DECL_RET;

    if ((op->result == WT_CLAYERED_RESULT_NONE || op->result == WT_CLAYERED_RESULT_EXTERNAL_KEY) &&
      op_ret != WT_PREPARE_CONFLICT)
        WT_TRET(__clayered_reset_cursors_op(op, false));

    switch (op->result) {
    case WT_CLAYERED_RESULT_NONE:
        /*
         * A prepare conflict on search/search_near leaves the application's input key in place so
         * it can retry the same key; every other unpositioned result clears it.
         */
        if (op_ret != WT_PREPARE_CONFLICT || op->mode != WT_CLAYERED_MODE_SEARCH)
    ...
    case WT_CLAYERED_RESULT_POSITIONED:
        ...
        /*
         * A duplicate-key insert is the one error that leaves the cursor positioned: it exposes the
         * existing record so the application can read it. That position outlives the constituent it
         * came from -- the failing insert's transaction unwinds and releases the in-tree reference.
         * Detach the key/value into the cursor's own buffers and drop the constituent position.
         */
        if (op_ret == WT_DUPLICATE_KEY) {
    ...
    if (op->mode == WT_CLAYERED_MODE_ITERATE_NEXT && op->alternate_positioned)
        F_SET(clayered, WT_CLAYERED_ITERATE_NEXT);
    else if (op->mode == WT_CLAYERED_MODE_ITERATE_PREV && op->alternate_positioned)
...
__clayered_modify_check(WT_CLAYERED_OP *op, const WT_ITEM *key)
{
    ...
    WT_RET(__clayered_constituent_check(op, op->ingest, key));
    WT_RET(__clayered_constituent_check(op, op->stable, key));
...
{
    WT_RET(op->role == WT_CLAYERED_ROLE_LEADER ? __clayered_remove_leader(op, key, positioned) :
                                                 __clayered_remove_follower(op, key, positioned));

    /*
     * A positioned remove keeps the key (no value); an unpositioned one leaves no position.
     * R_NONE makes op_finish reset the constituents, so a later iteration restarts cleanly.
     */
...
__clayered_insert(WT_CURSOR *cursor)
{
     ...
     * the ingest constituent positioned on it; expose that record (R_POSITIONED) and report the
     * duplicate.
     */
...
__clayered_largest_key(WT_CURSOR *cursor)
{
    ...
    /* Copy the larger key onto the cursor as an external key; op_finish resets the constituents. */
...
__clayered_insert() {
    CLAYERED_OP op;
    __clayered_enter(op)
    __clayered_insert_int(op);
    __clayered_op_finish(op, clayered); // propagates the result of the op to clayered
