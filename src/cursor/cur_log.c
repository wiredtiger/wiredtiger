...
__curlog_next(WT_CURSOR *cursor)
{
    ...
    if (cl->stepp == NULL || cl->stepp >= cl->stepp_end || !*cl->stepp) {
        cl->txnid = 0;
        ret = __wt_log_scan(session, cl->next_lsn, WT_LOGSCAN_ONE, __curlog_logrec, cl, NULL, NULL);
        if (ret == ENOENT)
            ret = WT_NOTFOUND;
        WT_ERR(ret);
    }
...
__curlog_search(WT_CURSOR *cursor)
{
    ...
    ret = __wt_log_scan(session, &key, WT_LOGSCAN_ONE, __curlog_logrec, cl, NULL, NULL);
    if (ret == ENOENT)
        ret = WT_NOTFOUND;
    WT_ERR(ret);
    WT_ERR(__curlog_kv(session, cursor));
