...
__clsm_close_bulk(WT_CURSOR *cursor)
{
	...
	bulk_cursor = clsm->units[0]->cursor;
	WT_RET(bulk_cursor->close(bulk_cursor));
	__wt_free(session, clsm->units[0]);
...
__wt_clsm_open_bulk(WT_CURSOR_LSM *clsm, const char *cfg[])
{
	 ...
	 */
	WT_RET(__wt_realloc_def(session, &clsm->units_alloc, 1, &clsm->units));
	WT_RET(__wt_calloc_one(session, &clsm->units[0]));
	clsm->nchunks = 1;
	...
	clsm->units[0]->cursor = bulk_cursor;
