...
__lsm_meta_read_v0(WT_SESSION_IMPL *session, WT_LSM_TREE *lsm_tree, const char *lsmconf)
{
	...
	while ((ret = __wt_config_next(&cparser, &ck, &cv)) == 0) {
		if (WT_STRING_MATCH("key_format", ck.str, ck.len)) {
			...
			WT_CLEAR(metadata);
			WT_RET_NOTFOUND_OK(
			    __wt_config_subgets(session, &fileconf, "app_metadata", &metadata));
			...
			WT_RET(
			    __wt_strndup(session, cv.str + 1, cv.len - 2, &lsm_tree->bloom_config));
		} else if (WT_STRING_MATCH("file_config", ck.str, ck.len)) {
			__wt_free(session, lsm_tree->file_config);
			/* Don't include the brackets. */
			WT_RET(
			    __wt_strndup(session, cv.str + 1, cv.len - 2, &lsm_tree->file_config));
			...
			for (nchunks = 0; (ret = __wt_config_next(&lparser, &lk, &lv)) == 0;) {
				if (WT_STRING_MATCH("id", lk.str, lk.len)) {
					WT_RET(__wt_realloc_def(session, &lsm_tree->chunk_alloc,
					    nchunks + 1, &lsm_tree->chunk));
					WT_RET(__wt_calloc_one(session, &chunk));
					lsm_tree->chunk[nchunks++] = chunk;
					chunk->id = (uint32_t)lv.val;
					WT_RET(__wt_lsm_tree_chunk_name(session, lsm_tree,
			...
			for (nchunks = 0; (ret = __wt_config_next(&lparser, &lk, &lv)) == 0;) {
				if (WT_STRING_MATCH("bloom", lk.str, lk.len)) {
					WT_RET(__wt_strndup(
					    session, lv.str, lv.len, &chunk->bloom_uri));
					F_SET(chunk, WT_LSM_CHUNK_BLOOM);
					continue;
				}
				WT_RET(__wt_realloc_def(session, &lsm_tree->old_alloc, nchunks + 1,
				    &lsm_tree->old_chunks));
				WT_RET(__wt_calloc_one(session, &chunk));
...
__lsm_meta_read_v1(WT_SESSION_IMPL *session, WT_LSM_TREE *lsm_tree, const char *lsmconf)
{
	...
	if (cv.len != 0 && !WT_STRING_MATCH("none", cv.str, cv.len)) {
		...
		WT_CLEAR(metadata);
		WT_ERR_NOTFOUND_OK(
		    __wt_config_getones(session, lsmconf, "app_metadata", &metadata));
		WT_ERR(__wt_collator_config(session, lsm_tree->name, &cv, &metadata,
		    &lsm_tree->collator, &lsm_tree->collator_owned));
	...
	WT_ERR(
	    __wt_config_check(session, WT_CONFIG_REF(session, WT_SESSION_create), cv.str, cv.len));
	...
	if (lsm_tree->merge_min > lsm_tree->merge_max)
		WT_ERR_MSG(
		    session, EINVAL, "LSM merge_min must be less than or equal to merge_max");
...
__lsm_meta_upgrade_v1(WT_SESSION_IMPL *session, WT_LSM_TREE *lsm_tree)
{
	...
	WT_ERR(__wt_scr_alloc(session, 0, &buf));
	WT_ERR(__wt_buf_fmt(session, buf, "key_format=%s,value_format=%s", lsm_tree->key_format,
	...
	WT_ERR(__wt_buf_catfmt(
	    session, buf, "auto_throttle=%d", F_ISSET(lsm_tree, WT_LSM_TREE_THROTTLE)));
	...
	WT_ERR(
	    __wt_buf_catfmt(session, buf, ",bloom_bit_count=%" PRIu32, lsm_tree->bloom_bit_count));
	...
	WT_ERR(__wt_buf_catfmt(
	    session, buf, ",bloom_hash_count=%" PRIu32, lsm_tree->bloom_hash_count));

	WT_ERR(__wt_buf_catfmt(
	    session, buf, ",chunk_count_limit=%" PRIu32, lsm_tree->chunk_count_limit));
