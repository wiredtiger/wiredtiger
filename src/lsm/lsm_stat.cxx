...
__curstat_lsm_init(WT_SESSION_IMPL *session, const char *uri, WT_CURSOR_STAT *cst)
{
    ...
    for (bloom_count = 0, i = 0; i < lsm_tree->nchunks; i++) {
        ...
        ret = __wt_curstat_open(session, static_cast<const char*>(uribuf->data), NULL,
          F_ISSET(chunk, WT_LSM_CHUNK_ONDISK) ? disk_cfg : cfg, &stat_cursor);
        if (ret == WT_NOTFOUND && F_ISSET(chunk, WT_LSM_CHUNK_ONDISK))
	        ret = __wt_curstat_open(session, static_cast<const char*>(uribuf->data), NULL, cfg, &stat_cursor);
        WT_ERR(ret);
         ...
         */
        newv = (WT_DSRC_STATS *)WT_CURSOR_STATS(stat_cursor);
        WT_STAT_WRITE(session, newv, lsm_generation_max, chunk->generation);

        /* Aggregate statistics from each new chunk. */
        __wt_stat_dsrc_aggregate_single(newv, stats);
        ...
        WT_ERR(__wt_curstat_open(session, static_cast<const char*>(uribuf->data), NULL, cfg, &stat_cursor));
        ...
        newv = (WT_DSRC_STATS *)WT_CURSOR_STATS(stat_cursor);
        WT_STAT_WRITE(
          session, newv, bloom_size, (int64_t)((chunk->count * lsm_tree->bloom_bit_count) / 8));
        WT_STAT_WRITE(
          session, newv, bloom_page_evict, newv->cache_eviction_clean + newv->cache_eviction_dirty);
        WT_STAT_WRITE(session, newv, bloom_page_read, newv->cache_read);
