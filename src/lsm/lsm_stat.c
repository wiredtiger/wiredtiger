...
__curstat_lsm_init(WT_SESSION_IMPL *session, const char *uri, WT_CURSOR_STAT *cst)
{
    ...
    for (bloom_count = 0, i = 0; i < lsm_tree->nchunks; i++) {
        ...
        WT_STAT_WRITE(session, new_, lsm_generation_max, chunk->generation);
        ...
        new_ = (WT_DSRC_STATS *)WT_CURSOR_STATS(stat_cursor);
        WT_STAT_WRITE(
          session, new_, bloom_size, (int64_t)((chunk->count * lsm_tree->bloom_bit_count) / 8));
        WT_STAT_WRITE(
          session, new_, bloom_page_evict, new_->cache_eviction_clean + new_->cache_eviction_dirty);
        WT_STAT_WRITE(session, new_, bloom_page_read, new_->cache_read);
