    ...
    for (newpage = false;; newpage = true, restart = false) {
        ...
        if (F_ISSET(cbt, WT_CBT_ITERATE_APPEND)) {
            ...
            case WT_PAGE_ROW_LEAF:
                ...
                total_skipped += skipped;
                /*
                 * We need to check against the prefix here as well as inside the call to cursor row
                 * next as we cannot differentiate between exiting that function as a result of a
                 * prefix mismatch or walking off the end of the page. If we have walked off the end
                 * of the page we compare again to make sure that the key range is no longer
                 * relevant before returning not found to the caller. This relies on the assumption
                 * that the cursor key is always set to the last entry on the page before walking
                 * off it.
                 */
