...
__cursor_fix_append_next(WT_CURSOR_BTREE *cbt, bool newpage, bool restart)
{
    ...
    if (newpage) {
        if ((cbt->ins = WT_SKIP_FIRST_ATOMIC(cbt->ins_head)) == NULL)
            return (WT_NOTFOUND);
    } else if (cbt->recno >= WT_INSERT_RECNO(cbt->ins) &&
      (cbt->ins = WT_SKIP_NEXT_ATOMIC(cbt->ins)) == NULL)
        return (WT_NOTFOUND);
  ...
  WT_CURSOR_BTREE *cbt, bool newpage, bool restart, size_t *skippedp, bool *key_out_of_boundsp)
{
    ...
    *skippedp = 0;
    ...
    if (newpage) {
        cbt->ins = WT_SKIP_FIRST_ATOMIC(cbt->ins_head);
        goto new_page;
    }

    for (;;) {
        cbt->ins = WT_SKIP_NEXT_ATOMIC(cbt->ins);
new_page:
        if (cbt->ins == NULL)
            return (WT_NOTFOUND);
