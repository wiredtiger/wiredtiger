 ...
 */
static inline int
__cursor_fix_append_next(WT_CURSOR_BTREE *cbt, bool newpage, bool retry)
{
 ...
 */
static inline int
__cursor_fix_next(WT_CURSOR_BTREE *cbt, bool newpage, bool retry)
{
	WT_BTREE *btree;
	WT_PAGE *page;
	WT_SESSION_IMPL *session;
 ...
 */
static inline int
__cursor_var_append_next(WT_CURSOR_BTREE *cbt, bool newpage, bool retry)
{
	WT_SESSION_IMPL *session;
	WT_UPDATE *upd;
 ...
 */
static inline int
__cursor_var_next(WT_CURSOR_BTREE *cbt, bool newpage, bool retry)
{
	WT_CELL *cell;
 ...
 */
static inline int
__cursor_row_next(WT_CURSOR_BTREE *cbt, bool newpage, bool retry)
{
	WT_INSERT *ins;
	WT_ITEM *key;
	WT_PAGE *page;
...
__wt_btcur_next(WT_CURSOR_BTREE *cbt, bool truncating)
{
	 ...
	 * file.
	 */
	retry = F_ISSET(cbt, WT_CBT_ITERATE_RETRY_NEXT);
	...
	for (newpage = false;; newpage = true, retry = false) {
		...
		if (F_ISSET(cbt, WT_CBT_ITERATE_APPEND)) {
			...
			switch (page->type) {
			case WT_PAGE_COL_FIX:
				ret = __cursor_fix_next(cbt, newpage, retry);
				break;
			case WT_PAGE_COL_VAR:
				ret = __cursor_var_next(cbt, newpage, retry);
				break;
			case WT_PAGE_ROW_LEAF:
				ret = __cursor_row_next(cbt, newpage, retry);
				break;
			WT_ILLEGAL_VALUE_ERR(session, page->type);
			}
			if (ret != WT_NOTFOUND)
