  ...
  u_int modify_type, bool exclusive, bool restore)
{
    ...
    if (cbt->compare == 0 && cbt->ins != NULL) {
        ...
        if (upd_arg == NULL) {
            ...
            WT_ASSERT_ALWAYS(session,
              !restore ||
                (old_upd == NULL ||
                  (old_upd->type == WT_UPDATE_TOMBSTONE &&
                    F_ISSET(old_upd, WT_UPDATE_RESTORED_FROM_DS))),
    ...
    if (added_to_txn && modify_type != WT_UPDATE_RESERVE) {
        if (__wt_layered_table_log_op(session))
            WT_ERR(__wt_txn_layered_table_log_op(session, cbt));
