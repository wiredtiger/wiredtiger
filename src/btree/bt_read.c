...
read:
            /*
             * The page isn't in memory, read it. If this thread respects the cache size, check 
             * if the cache needs help evicting clean pages (don't force a read to do dirty 
             * eviction).
             */
            if (!LF_ISSET(WT_READ_IGNORE_CACHE_SIZE))
