    ...
    case WT_CELL_ADDR_DEL:
        ...
        if (F_ISSET(dsk, WT_PAGE_FT_UPDATE)) {
            ...
            WT_RET(ds->f(ds, " | page_del : %s",
              __wt_time_point_to_string(get_page_del_start_ts(page_del),
                page_del->pg_del_durable_ts, page_del->txnid, time_string)));
        }
    /* FALLTHROUGH */
...
__debug_update(WT_DBG *ds, WT_UPDATE *upd, bool hexbyte)
{
    ...
    for (; upd != NULL; upd = upd->next) {
        ...
        WT_RET(
          ds->f(ds, ", start_ts %s", __wt_timestamp_to_string(get_upd_start_ts(upd), ts_string)));
