...
/*
 * __wt_debug_cursor_history_store --
 *     Dump the LAS tree given a user cursor.
 */
int
__wt_debug_cursor_history_store(void *cursor_arg, const char *ofile)
  WT_GCC_FUNC_ATTRIBUTE((visibility("default")))
{
    ...
    history_store_session = conn->cache->history_store_session[0];
    if (history_store_session == NULL)
        return (0);
    cbt = (WT_CURSOR_BTREE *)history_store_session->history_store_cursor;
    return (__wt_debug_tree_all(history_store_session, cbt->btree, NULL, ofile));
}
