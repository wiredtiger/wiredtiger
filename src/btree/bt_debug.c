...
__wt_debug_offset_blind(WT_SESSION_IMPL *session, wt_off_t offset, const char *ofile)
{
	 ...
	 */
	WT_RET(
	    __wt_block_read_off_blind(session, S2BT(session)->bm->block, offset, &size, &checksum));
...
__debug_tree_shape_info(WT_REF *ref, char *buf, size_t len)
{
	...
	(void)__wt_snprintf(buf, len,
	    "(%p, %" PRIu64 "%s, evict gen %" PRIu64 ", create gen %" PRIu64 ")", (void *)ref, v,
	    unit, page->evict_pass_gen, page->cache_create_gen);
	return (buf);
}
...
__debug_modify(WT_DBG *ds, WT_UPDATE *upd)
{
	...
	for (; nentries-- > 0; data += data_size) {
		...
		WT_RET(ds->f(ds, "{%" WT_SIZET_FMT ", %" WT_SIZET_FMT ", %" WT_SIZET_FMT ", ",
...
__debug_update(WT_DBG *ds, WT_UPDATE *upd, bool hexbyte)
{
	...
	for (; upd != NULL; upd = upd->next) {
		...
		WT_RET(
		    ds->f(ds, ", start_ts %s", __wt_timestamp_to_string(upd->start_ts, ts_string)));
