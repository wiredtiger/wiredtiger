  ...
  size_t parent_incr, bool exclusive, bool discard)
{
    ...
    if (!WT_BTREE_SYNCING(btree) || WT_SESSION_BTREE_SYNC(session))
        for (i = 0; i < parent_entries; ++i) {
            ...
            WT_ASSERT(session, __wt_ref_get_state(next_ref) != WT_REF_SPLIT);
             ...
             */
            if (next_ref != ref && __wt_ref_get_state(next_ref) == WT_REF_DELETED &&
     ...
     */
    if (discard) {
        WT_ASSERT(session, exclusive || __wt_ref_get_state(ref) == WT_REF_LOCKED);
        WT_TRET(
    ...
    for (i = 0; i < deleted_entries; ++i) {
        ...
        WT_ASSERT(session, __wt_ref_get_state(next_ref) == WT_REF_LOCKED);
...
__split_insert(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    __wt_atomic_storev8(&child->__state, WT_REF_MEM); /* Visible as soon as the split completes. */
    child->addr = ref->addr;
    if (type == WT_PAGE_ROW_LEAF) {
