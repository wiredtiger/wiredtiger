...
/*
 * __wt_split_col_prepend --
 *     Prepend an empty page at the beginning of an internal page. Column-store only. Called when we
 *     need to visit the gap between the internal page's start recno and the first child's start
 *     recno, if such a gap exists.
 */
int
__wt_split_col_prepend(WT_SESSION_IMPL *session, WT_REF *intl)
{
    WT_DECL_RET;
    WT_PAGE *intlpage;
    WT_PAGE_INDEX *pindex;
    WT_REF *child, *insertion[2], *newref;

    intlpage = intl->page;

    /* Only used by VLCS. No real reason FLCS couldn't use it if a need arose. */
    WT_ASSERT(session, S2BT(session)->type == BTREE_COL_VAR);
    WT_ASSERT(session, intlpage->type == WT_PAGE_COL_INT);

    /* We need a modify structure to lock the page for the split call. */
    WT_RET(__wt_page_modify_init(session, intlpage));

    /*
     * Allocate the new page: a deleted ref with no on-disk address, which will be instantiated
     * empty when visited. Use the barrier in WT_REF_SET_STATE to make sure the initialization is
     * visible before we try to attach the page to the tree.
     */
    WT_RET(__wt_calloc_one(session, &newref));
    newref->home = intlpage;
    newref->pindex_hint = 0;
    newref->flags = WT_REF_FLAG_LEAF;
    newref->ref_recno = intl->ref_recno;
    WT_REF_SET_STATE(newref, WT_REF_DELETED);

    /* Lock the internal page for the split. */
    WT_PAGE_LOCK(session, intlpage);

    WT_INTL_INDEX_GET(session, intlpage, pindex);

    child = pindex->index[0];
    if (child->ref_recno == intl->ref_recno) {
        /* Someone else got in ahead of us and did the split already. */
        goto err;
    }
    WT_ASSERT(session, child->ref_recno > intl->ref_recno);

    /* Split the child into the new page followed by the child. */
    insertion[0] = newref;
    insertion[1] = child;
    WT_ERR(__split_parent(session, child, insertion, 2, sizeof(WT_REF), false, false));

    WT_PAGE_UNLOCK(session, intlpage);
    return (0);

err:
    WT_PAGE_UNLOCK(session, intlpage);
    __wt_free(session, newref);
    return (ret);
}
