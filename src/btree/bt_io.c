...
__wt_bt_read(WT_SESSION_IMPL *session, WT_ITEM *buf, const uint8_t *addr, size_t addr_size)
{
	...
	if (F_ISSET(dsk, WT_PAGE_ENCRYPTED)) {
		...
		if ((ret = __wt_decrypt(session, encryptor, WT_BLOCK_ENCRYPT_SKIP, ip, etmp)) !=
		    0) {
	...
	corrupt:
		...
		if (!F_ISSET(btree, WT_BTREE_VERIFY) &&
			...
			WT_PANIC_ERR(session, ret, "%s: fatal read error: %s", btree->dhandle->name,
			    fail_msg);
		}
	}
    ...
    size_t *compressed_sizep, bool checkpoint, bool checkpoint_io, bool compressed)
{
	...
	if (compressed) {
		...
		WT_ERR(btree->compressor->decompress(btree->compressor, &session->iface,
		    ...
		    ctmp->memsize - WT_BLOCK_COMPRESS_SKIP, &result_len));
	...
	if (btree->compressor == NULL || btree->compressor->compress == NULL || compressed)
		...
		WT_ERR(btree->compressor->compress(btree->compressor, &session->iface, src, src_len,
		    dst, dst_len, &result_len, &compression_failed));
		...
		if (compression_failed ||
		    buf->size / btree->allocsize <= result_len / btree->allocsize) {
			ip = buf;
