...
delete_b:
     ...
     */
    WT_RET(__wt_calloc_one(session, &new_));
     ...
     */
    new_->col_start = b_trk->col_stop + 1;
    new_->col_stop = a_trk->col_stop;
...
delete_b:
     ...
     */
    WT_RET(__slvg_key_copy(session, &new_->row_stop, A_TRK_STOP));
     ...
     */
    WT_RET(__slvg_key_copy(session, A_TRK_STOP, B_TRK_START));
    F_SET(new_, WT_TRACK_CHECK_START);
