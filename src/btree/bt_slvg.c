...
#define	trk_oldest_start_txn	shared->addr.oldest_start_txn
#define	trk_newest_stop_txn	shared->addr.newest_stop_txn
    ...
    uint8_t *addr, size_t addr_size, WT_STUFF *ss, WT_TRACK **retp)
{
	...
	trk->trk_oldest_start_txn = WT_TXN_MAX;
	trk->trk_newest_stop_txn = WT_TXN_NONE;
	if (!__wt_process.page_version_ts || dsk->type == WT_PAGE_COL_FIX) {
		...
		trk->trk_oldest_start_txn = WT_TXN_NONE;
		trk->trk_newest_stop_txn = WT_TXN_MAX;
	}
...
__slvg_trk_leaf_ts(WT_TRACK *trk, WT_CELL_UNPACK *unpack)
{
	...
	trk->trk_oldest_start_txn =
	    WT_MIN(unpack->start_txn, trk->trk_oldest_start_txn);
	trk->trk_newest_stop_txn =
	    WT_MAX(unpack->stop_txn, trk->trk_newest_stop_txn);
}
...
merge:
	...
	a_trk->trk_oldest_start_txn = b_trk->trk_oldest_start_txn =
	    WT_MIN(a_trk->trk_oldest_start_txn, b_trk->trk_oldest_start_txn);
	a_trk->trk_newest_stop_txn = b_trk->trk_newest_stop_txn =
	    WT_MAX(a_trk->trk_newest_stop_txn, b_trk->trk_newest_stop_txn);
    ...
    WT_SESSION_IMPL *session, uint32_t leaf_cnt, WT_STUFF *ss)
{
	...
	for (refp = pindex->index, i = 0; i < ss->pages_next; ++i) {
		...
		addr->oldest_start_txn = trk->trk_oldest_start_txn;
		addr->newest_stop_txn = trk->trk_newest_stop_txn;
		WT_ERR(__wt_memdup(
