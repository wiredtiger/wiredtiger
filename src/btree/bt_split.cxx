...
__split_ovfl_key_cleanup(WT_SESSION_IMPL *session, WT_PAGE *page, WT_REF *ref)
{
    ...
    cell = static_cast<WT_CELL *>(WT_PAGE_REF_OFFSET(page, cell_offset));
...
__wt_split_rewrite(WT_SESSION_IMPL *session, WT_REF *ref, WT_MULTI *multi)
{
     ...
     */
    WT_RET(__wt_calloc_one(session, &newv));
    newv->ref_recno = ref->ref_recno;

    WT_ERR(__split_multi_inmem(session, page, multi, newv));
    ...
    __wt_free(session, newv);
    return (0);

err:
    __split_multi_inmem_fail(session, page, multi, newv);
    return (ret);
}
