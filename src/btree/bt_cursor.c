...
/*
 * Touch search support.
 *
 * When ``cbt->touch_enabled`` is set, WT_CURSOR::search descends the btree using only internal
 * pages and forwards a fire-and-forget hint to the page log layer (PALI) for the would-be leaf
 * page. The caller always sees WT_NOTFOUND; no leaf is materialized into the WiredTiger cache.
 *
 * The intent is to let storage layers below WT (e.g. SLS via PALI) promote pages to a faster tier
 * ahead of the real read. Worst-case latency is identical to the non-touch path: a hint that the
 * page log ignores costs at most one extra round-trip, and the caller's subsequent real search()
 * still pays full cold-tier cost.
 *
 * Public surface:
...
/*
 * __btcur_touch --
 *     Touch-mode implementation of WT_CURSOR::search. Descends through internal pages only, issues
 *     a warmup hint via PALI for the would-be leaf page, and returns WT_NOTFOUND.
 */
static int
__btcur_touch(WT_CURSOR_BTREE *cbt)
{
    WT_BTREE *btree;
    WT_CURSOR *cursor;
    WT_DECL_RET;
     ...
     * check keeps __btcur_touch usable as a stand-alone helper.
     */
    ...
    /*
     * The custom descent reads ref->addr from an unloaded leaf via __wt_ref_addr_copy. That is only
     * safe inside WT_GEN_SPLIT; __wt_page_swap holds it internally for the in-cache path but we
     * bypass page_swap for the final descent, so we take the generation ourselves here.
     */
...
__wt_btcur_search(WT_CURSOR_BTREE *cbt)
{
    ...
    /*
     * Touch search is a fire-and-forget mode of the normal search verb: descend the internal pages
     * and forward a warmup hint to the page log layer instead of materializing the leaf. It
     * accounts to its own cursor_touch_* stats and always returns WT_NOTFOUND.
     */
    if (cbt->touch_enabled)
        return (__btcur_touch(cbt));
