  ...
  WT_ITEM *orig_stop_key, bool local_start)
{
     ...
     * standalone builds (MongoDB). MongoDB do perform truncate operation only on a table that do
     * not have historical versions to avoid the problem. Remove this standalone build specific code
     * when MongoDB switched to perform truncate operation with timestamps.
     */
