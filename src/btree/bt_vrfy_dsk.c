  ...
  WT_ADDR *addr, const char *tag)
{
    ...
    if (unpack->page_del.pg_del_durable_ts < unpack->page_del.pg_del_commit_ts)
        WT_RET_VRFY(session,
          "fast-delete cell %" PRIu32 " on page at %s has durable timestamp %" PRIu64
          " before its commit timestamp %" PRIu64,
          cell_num - 1, tag, unpack->page_del.pg_del_durable_ts, unpack->page_del.pg_del_commit_ts);
    ...
    if (unpack->ta.newest_stop_ts != WT_TS_MAX &&
      unpack->ta.newest_stop_ts > unpack->page_del.pg_del_commit_ts)
        WT_RET_VRFY(session,
          ...
          "; time aggregate %s",
          cell_num - 1, tag, unpack->page_del.pg_del_commit_ts,
          __wt_time_aggregate_to_string(&unpack->ta, time_string));
