  ...
  uint32_t row_count, uint32_t byte_count, WT_ADDR *addr)
{
    ...
    if (addr->row_count != row_count)
        WT_RET_VRFY(session,
          "%s page at %s has an stored key count of %" PRIu64
          ", the addressed page has a key count count of %" PRIu32,
          __wt_page_type_string(dsk->type), tag, addr->row_count, row_count);
    if (addr->byte_count != byte_count)
        WT_RET_VRFY(session,
          "%s page at %s has an stored page byte count of %" PRIu64
          ", the addressed page has a page byte count of %" PRIu32,
          __wt_page_type_string(dsk->type), tag, addr->byte_count, byte_count);
