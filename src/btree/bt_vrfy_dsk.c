  ...
  WT_ADDR *addr, const char *tag)
{
    ...
    if (unpack->page_del.pg_del_durable_ts < unpack->page_del.u.commit.start_ts)
        WT_RET_VRFY(session,
    ...
    if (unpack->ta.newest_stop_ts != WT_TS_MAX &&
      unpack->ta.newest_stop_ts > unpack->page_del.u.commit.start_ts)
        WT_RET_VRFY(session,
          "fast-delete cell %" PRIu32
          " on page at %s has invalid newest stop time; should be <= %" PRIu64
          "; time aggregate %s",
          cell_num - 1, tag, unpack->page_del.u.commit.start_ts,
          __wt_time_aggregate_to_string(&unpack->ta, time_string));
    ...
    ta_with_delete.newest_stop_ts = unpack->page_del.u.commit.start_ts;
