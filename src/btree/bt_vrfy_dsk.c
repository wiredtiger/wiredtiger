  ...
  WT_ADDR *addr, const char *tag, const WT_PAGE_HEADER *dsk)
{
    ...
    WT_RET(__verify_dsk_ts_addr_cmp(session, cell_num - 1, "start durable",
      ta->newest_start_durable_ts, "start durable", addr->ta.newest_start_durable_ts, false, tag));
  ...
  WT_ADDR *addr, const char *tag, const WT_PAGE_HEADER *dsk)
{
    ...
    WT_RET(__verify_dsk_ts_addr_cmp(session, cell_num - 1, "start durable", tw->durable_start_ts,
