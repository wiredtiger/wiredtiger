    ...
    wt_timestamp_t ts1, const char *ts2_name, wt_timestamp_t ts2, bool gt, const char *tag)
{
	...
	case WT_TS_NONE:
		...
		break;
	default:
		WT_RET(__wt_snprintf(ts1_buf, sizeof(ts1_buf), "%" PRIu64, ts1));
		ts1_bp = ts1_buf;
		break;
	}
	switch (ts2) {
	...
	case WT_TS_NONE:
		...
		break;
	default:
		WT_RET(__wt_snprintf(ts2_buf, sizeof(ts2_buf), "%" PRIu64, ts2));
		ts2_bp = ts2_buf;
		break;
	}
 ...
 */
static int
__verify_dsk_ts(WT_SESSION_IMPL *session, WT_CELL_UNPACK *unpack, uint32_t cell_num, WT_ADDR *addr,
    const char *tag)
{
	/*
	...
	case WT_CELL_ADDR_LEAF_NO:
		...
		WT_RET(__verify_dsk_ts_addr_cmp(session, cell_num - 1, "newest start",
		    unpack->newest_start_ts, "newest start", addr->newest_start_ts, false, tag));
	...
	case WT_CELL_VALUE_SHORT:
		...
		WT_RET(__verify_dsk_ts_addr_cmp(session, cell_num - 1, "start", unpack->start_ts,
		    "newest start", addr->newest_start_ts, false, tag));
    ...
    WT_SESSION_IMPL *session, const char *tag, const WT_PAGE_HEADER *dsk, WT_ADDR *addr)
{
	...
	WT_CELL_FOREACH_VRFY (btree, dsk, cell, unpack, i) {
		 ...
		 */
		if (unpack->start_ts != last.start_ts || unpack->stop_ts != last.stop_ts)
			;
