...
#define MAX_PAGE_SIZE_FOR_COMPRESSION 4 * 1024
        if (btree->maxintlpage >= MAX_PAGE_SIZE_FOR_COMPRESSION && btree->maxmempage_image > btree->maxintlpage) {
            btree->intlpage_compadjust = true;
            btree->maxintlpage_precomp = btree->maxmempage_image;
        }
        if (btree->maxleafpage >= MAX_PAGE_SIZE_FOR_COMPRESSION && btree->maxmempage_image > btree->maxleafpage) {
