...
__wt_btree_close(WT_SESSION_IMPL *session)
{
     ...
     */
    WT_ASSERT(session,
      !F_ISSET_ATOMIC_64(S2C(session), WT_CONN_HS_OPEN) || !btree->hs_entries ||
...
__btree_conf(WT_SESSION_IMPL *session, WT_CKPT *ckpt, bool is_ckpt)
{
     ...
     * checkpoint.
     */
    if ((!F_ISSET_ATOMIC_64(conn, WT_CONN_RECOVERING) || F_ISSET(btree, WT_BTREE_LOGGED) ||
...
#define WT_MIN_PAGES 10
    if (!F_ISSET_ATOMIC_64(conn, WT_CONN_CACHE_POOL) && (cache_size = conn->cache_size) > 0)
