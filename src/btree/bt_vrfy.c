...
__wt_verify(WT_SESSION_IMPL *session, const char *cfg[])
{
	...
	WT_CKPT_FOREACH (ckptbase, ckpt) {
		...
		if (WT_VRFY_DUMP(vs)) {
			...
			WT_ERR(__wt_msg(
			    session, "%s: checkpoint %s", btree->dhandle->name, ckpt->name));
		}
	...
	WT_CELL_FOREACH_BEGIN (session, btree, dsk, unpack, false) {
		...
		case WT_CELL_ADDR_LEAF_NO:
			...
			WT_RET(__verify_ts_addr_cmp(session, ref, cell_num - 1, "oldest start",
			    ...
			    true, vs));
			WT_RET(__verify_ts_addr_cmp(session, ref, cell_num - 1, "newest start",
			    unpack.newest_start_ts, "newest start", addr_unpack->newest_start_ts,
			    false, vs));
		...
		case WT_CELL_VALUE_SHORT:
			...
			WT_RET(__verify_ts_addr_cmp(session, ref, cell_num - 1, "start",
			    ...
			    vs));
			WT_RET(__verify_ts_addr_cmp(session, ref, cell_num - 1, "start",
			    unpack.start_ts, "newest start", addr_unpack->newest_start_ts, false,
			    vs));
