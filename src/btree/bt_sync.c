...
__sync_checkpoint_can_skip(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    if (F_ISSET(session, WT_SESSION_ROLLBACK_TO_STABLE))
        return (false);
    if (F_ISSET_ATOMIC_64(S2C(session), WT_CONN_RECOVERING | WT_CONN_CLOSING_CHECKPOINT))
        return (false);
...
__wt_sync_file(WT_SESSION_IMPL *session, WT_CACHE_OP syncop)
{
    ...
    case WT_SYNC_CHECKPOINT:
         ...
         */
        if (!btree->modified &&
          !F_ISSET_ATOMIC_64(conn, WT_CONN_RECOVERING | WT_CONN_CLOSING_CHECKPOINT) &&
...
err:
     ...
     */
    if (ret == 0 && syncop == WT_SYNC_WRITE_LEAVES && F_ISSET_ATOMIC_64(conn, WT_CONN_CKPT_SYNC))
