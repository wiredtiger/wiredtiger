...
__wt_sync_file(WT_SESSION_IMPL *session, WT_CACHE_OP syncop)
{
     ...
     * visible to the checkpoint, the on-disk version is correct. If the truncate is globally
     * visible, we ignore the page. If the truncate is visible to checkpoint, we write a proxy cell
     * to its parent. We check whether a truncate is visible in the checkpoint as part of
     * reconciling internal pages (specifically in __rec_child_deleted).
     */
