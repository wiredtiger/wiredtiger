...
/*
 * __sync_cleanup_deleted_children --
 *     Mark a clean internal page dirty if it has a globally visible deleted child.
 */
static int
__sync_cleanup_deleted_children(WT_SESSION_IMPL *session, WT_PAGE *page, bool *dirtiedp)
{
        ...
        for (;;) {
            ...
            if (!dirty && is_internal && F_ISSET(btree, WT_BTREE_DISAGGREGATED) &&
              !F_ISSET_ATOMIC_32(btree, WT_BTREE_READONLY)) {
                WT_WITH_PAGE_INDEX(
                  session, ret = __sync_cleanup_deleted_children(session, page, &dirty));
                WT_ERR(ret);
            }
