...
__wt_txn_stats_update(WT_SESSION_IMPL *session)
{
    ...
    checkpoint_pinned_ts_lag = oldest_reader_lag = durable_oldest_lag = WT_TS_NONE;
    ...
    /* Represents the lag of the oldest timestamp with respect to the durable timestamp. */
    if (durable_timestamp != WT_TS_NONE && durable_timestamp > oldest_timestamp)
