...
__wt_txn_global_shutdown(WT_SESSION_IMPL *session, const char **cfg)
{
    ...
    if (!F_ISSET(conn, WT_CONN_IN_MEMORY | WT_CONN_READONLY | WT_CONN_PANIC)) {
        ...
        if (use_timestamp) {
            ...
            __wt_timer_evaluate(session, &timer, &conn->shutdown_timeline.rts);
            if (ret != 0)
        ...
        if (s != NULL) {
            ...
            __wt_timer_evaluate(session, &timer, &conn->shutdown_timeline.checkpoint);
            __wt_verbose(session, WT_VERB_RECOVERY_PROGRESS,
