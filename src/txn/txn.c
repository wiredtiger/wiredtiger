  ...
  WT_SESSION_IMPL *session, WT_TXN_OP *op, WT_CURSOR **cursorp, WT_UPDATE **updp)
{
    ...
    *updp = NULL;
    ...
    case WT_TXN_OP_TRUNCATE_ROW:
        WT_RET_PANIC_ASSERT(
          session, WT_DIAG_PREPARED, false, WT_PANIC, "invalid prepared operation update type");
        break;
    }
    ...
    F_CLR(txn, WT_TXN_PREPARE_IGNORE_API_CHECK);
    WT_RET(ret);
    WT_RET_ASSERT(session, WT_DIAG_PREPARED, *updp != NULL, WT_NOTFOUND,
...
prepare_verify:
    if (EXTRA_DIAGNOSTICS_ENABLED(session, WT_DIAG_PREPARED)) {
