...
__wt_txn_get_snapshot(WT_SESSION_IMPL *session)
{
	...
	for (i = 0, s = txn_global->states; i < session_cnt; i++, s++) {
		 ...
		 */
		while (s != txn_state && (id = s->id) != WT_TXN_NONE &&
...
__txn_commit_timestamps_assert(WT_SESSION_IMPL *session)
{
	...
	for (i = 0, op = txn->mod; i < txn->mod_count; i++, op++)
		if (op->type == WT_TXN_OP_BASIC_COL || op->type == WT_TXN_OP_BASIC_ROW) {
			 ...
			 */
			if (F_ISSET(txn, WT_TXN_PREPARE)) {
				WT_RET(__wt_open_cursor(session, op->btree->dhandle->name, NULL,
				...
				if (ret != 0)
					WT_RET_MSG(
					    session, EINVAL, "prepared update restore failed");
				op->u.op_upd = upd;
			} else
			 ...
			 */
			while (
			    upd != NULL && (upd->txnid == WT_TXN_ABORTED || upd->txnid == txn->id))
				upd = upd->next;
			...
			if (op_zero_ts != upd_zero_ts)
				WT_RET_MSG(
				    session, EINVAL, "per-key timestamps used inconsistently");
			/*
...
__wt_txn_commit(WT_SESSION_IMPL *session, const char *cfg[])
{
	...
	if (cval.len != 0) {
		 ...
		 */
		WT_ERR(__wt_txn_commit_timestamp_validate(session, "commit", ts, &cval, !prepare));
	...
	if (prepare && !F_ISSET(txn, WT_TXN_HAS_TS_COMMIT))
		WT_ERR_MSG(
		    session, EINVAL, "commit_timestamp is required for a prepared transaction");
	 ...
	 * transactions.
	 */
	WT_ERR(__wt_config_gets_def(session, cfg, "durable_timestamp", 0, &cval));
	if (cval.len != 0) {
		if (!prepare)
		...
		WT_ERR(__wt_txn_parse_timestamp(session, "durable", &ts, &cval));
		/* Durable timestamp should be later than stable timestamp. */
		F_SET(txn, WT_TXN_HAS_TS_DURABLE);
		txn->durable_timestamp = ts;
		WT_ERR(__wt_txn_commit_timestamp_validate(session, "durable", ts, &cval, true));
	}
	...
	if (update_timestamp)
		while (txn->commit_timestamp > prev_commit_timestamp) {
			if (__wt_atomic_cas64(&txn_global->commit_timestamp, prev_commit_timestamp,
	...
	for (i = 0, op = txn->mod; i < txn->mod_count; i++, op++) {
		...
		case WT_TXN_OP_INMEM_ROW:
			...
			if (F_ISSET(txn, WT_TXN_PREPARE))
				...
				WT_ASSERT(
				    session, upd->txnid == txn->id || upd->txnid == WT_TXN_ABORTED);
				upd->txnid = WT_TXN_ABORTED;
			}
			break;
		case WT_TXN_OP_REF_DELETE:
...
__wt_verbose_dump_txn_one(WT_SESSION_IMPL *session, WT_TXN *txn)
{
	...
	WT_RET(__wt_msg(session,
	    ...
	    txn->mod_count, txn->snap_min, txn->snap_max, ts_string[0], ts_string[1], ts_string[2],
	    ts_string[3], txn->flags, iso_tag));
	return (0);
}
...
__wt_verbose_dump_txn(WT_SESSION_IMPL *session)
{
	...
	WT_RET(
	    __wt_msg(session, "oldest_is_pinned: %s", txn_global->oldest_is_pinned ? "yes" : "no"));
	WT_RET(
	    __wt_msg(session, "stable_is_pinned: %s", txn_global->stable_is_pinned ? "yes" : "no"));

	WT_RET(__wt_msg(
	    session, "checkpoint running: %s", txn_global->checkpoint_running ? "yes" : "no"));
	WT_RET(__wt_msg(
	    session, "checkpoint generation: %" PRIu64, __wt_gen(session, WT_GEN_CHECKPOINT)));
	WT_RET(__wt_msg(
	...
	WT_RET(
	    __wt_msg(session, "oldest named snapshot ID: %" PRIu64, txn_global->nsnap_oldest_id));
