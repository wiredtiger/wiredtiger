 ...
 */
static int
__txn_conf_operation_timeout(WT_SESSION_IMPL *session, const WT_CONF_LIST *cf, bool start_timer)
{
    WT_CONFIG_ITEM cval;
    WT_TXN *txn;
    ...
    /* Retrieve the maximum operation time, defaulting to the database-wide configuration. */
    WT_RET(__wt_conf_gets_def(session, cf, operation_timeout_ms, 0, &cval));
 ...
 */
int
__wt_txn_config(WT_SESSION_IMPL *session, WT_CONF_LIST *cf)
{
    WT_CONFIG_ITEM cval;
    WT_DECL_RET;
    WT_TXN *txn;
    ...
    WT_ERR(__wt_conf_gets_def(session, cf, isolation, 0, &cval));
    if (cval.len != 0)
    ...
    WT_ERR(__txn_conf_operation_timeout(session, cf, false));
    ...
    WT_ERR(__wt_conf_gets_def(session, cf, sync, (int)UINT_MAX, &cval));
    ...
    WT_ERR(__wt_conf_gets_def(session, cf, ignore_prepare, 0, &cval));
    ...
    WT_ERR(__wt_conf_gets_def(session, cf, no_timestamp, 0, &cval));
    if (cval.val)
    ...
    WT_ERR(__wt_conf_gets_def(session, cf, read_timestamp, 0, &cval));
    if (cval.len != 0) {
