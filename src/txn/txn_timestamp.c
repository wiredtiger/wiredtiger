 ...
 */
int
__wt_txn_commit_timestamp_validate_and_roundup(WT_SESSION_IMPL *session)
{
	...
	if (!F_ISSET(txn, WT_TXN_HAS_TS_PREPARE)) {
		 ...
		 */
		if (has_oldest_ts && txn->commit_timestamp < oldest_ts) {
			__wt_timestamp_to_string(
			    txn->commit_timestamp, ts_string[0]);
			__wt_timestamp_to_string(
		...
		if (has_stable_ts && txn->commit_timestamp < stable_ts) {
			__wt_timestamp_to_string(
			    txn->commit_timestamp, ts_string[0]);
			__wt_timestamp_to_string(
			    oldest_ts, ts_string[1]);
		 ...
		 */
		if (txn->prepare_timestamp > txn->commit_timestamp) {
...
__wt_txn_set_timestamp(WT_SESSION_IMPL *session, const char *cfg[])
{
	...
	if (ret == 0 && cval.len != 0) {
		...
		WT_RET(__wt_txn_commit_timestamp_validate_and_roundup(session));
