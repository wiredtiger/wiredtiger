...
__wt_txn_global_set_timestamp(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    if (has_commit && (has_oldest || txn_global->has_oldest_timestamp) && oldest_ts > commit_ts) {
        __wt_readunlock(session, &txn_global->rwlock);
        WT_RET_MSG(session, EINVAL, "set_timestamp: oldest timestamp %s must not be later than "
                                    "commit timestamp %s",
    ...
    if (has_commit && (has_stable || txn_global->has_stable_timestamp) && stable_ts > commit_ts) {
        __wt_readunlock(session, &txn_global->rwlock);
        WT_RET_MSG(session, EINVAL, "set_timestamp: stable timestamp %s must not be later than "
                                    "commit timestamp %s",
...
__wt_txn_set_commit_timestamp(WT_SESSION_IMPL *session, wt_timestamp_t commit_ts)
{
    ...
    if (!F_ISSET(txn, WT_TXN_HAS_TS_PREPARE)) {
        ...
        if (txn->prepare_timestamp > commit_ts) {
            if (!F_ISSET(txn, WT_TXN_TS_ROUND_PREPARED))
                WT_RET_MSG(session, EINVAL, "commit timestamp %s is less than the "
    ...
    WT_ASSERT(session,
      txn->durable_timestamp == WT_TS_NONE || txn->durable_timestamp == txn->commit_timestamp);
...
__wt_txn_set_durable_timestamp(WT_SESSION_IMPL *session, wt_timestamp_t durable_ts)
{
    ...
    if (durable_ts < txn->commit_timestamp)
        WT_RET_MSG(session, EINVAL, "durable timestamp %s is less than the commit timestamp %s "
...
__wt_txn_set_prepare_timestamp(WT_SESSION_IMPL *session, wt_timestamp_t prepare_ts)
{
    ...
    if (prepare_ts < oldest_ts) {
        ...
        if (F_ISSET(txn, WT_TXN_TS_ROUND_PREPARED)) {
            ...
            __wt_verbose(session, WT_VERB_TIMESTAMP, "prepare timestamp %s rounded to oldest "
                                                     "timestamp %s",
