...
__recovery_set_checkpoint_snapshot(WT_SESSION_IMPL *session)
{
     ...
     * in some versions of WiredTiger that are tagged with the 10.0.0 release, which saved the
     * wrong checkpoint snapshot (see WT-8395), so we ignore the snapshot when it was created with
     * one of those versions. Versions of WiredTiger prior to 10.0.0 never saved a checkpoint
     * snapshot.
