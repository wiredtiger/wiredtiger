...
__wt_checkpoint_get_handles(WT_SESSION_IMPL *session, const char *cfg[])
{
     ...
     */
    btree->evict.evict_walk_saved = __wt_atomic_load32(&btree->evict.evict_walk_period);
...
__checkpoint_presync(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    __wt_atomic_store32(&btree->evict.evict_walk_period, btree->evict.evict_walk_saved);
    return (0);
}
