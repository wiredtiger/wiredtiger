    ...
    for (upd = first_upd; upd != NULL; upd = upd->next) {
        ...
        if (upd->txnid == WT_TXN_ABORTED) {
          ...
          upd->prepare_state == WT_PREPARE_INPROGRESS) {
            ...
            __wt_verbose(session, verb_flags,
  ...
  wt_timestamp_t rollback_timestamp, bool replace)
{
    ...
    if (replace) {
        ...
        if (valid_update_found) {
            ...
            __wt_verbose(session, verb_flags, "%p: key removed", (void *)key);
        }
  ...
  WT_SESSION_IMPL *session, WT_REF *ref, wt_timestamp_t rollback_timestamp)
{
    ...
    if (ref->page_del != NULL && rollback_timestamp < ref->page_del->durable_timestamp) {
        __wt_verbose(session, verb_flags, "%p: deleted page rolled back", (void *)ref);
    ...
    if (!__wt_page_is_modified(page) &&
      !__rollback_page_needs_abort(session, ref, rollback_timestamp)) {
        __wt_verbose(session, verb_flags, "%p: page skipped", (void *)ref);
        return (0);
    }

    WT_STAT_CONN_INCR(session, txn_rts_pages_visited);
    __wt_verbose(session, verb_flags, "%p: page rolled back when page is modified: %s", (void *)ref,
...
__wt_rts_page_skip(WT_SESSION_IMPL *session, WT_REF *ref, void *context, bool *skipp)
{
    ...
    *skipp = false; /* Default to reading */
    ...
    if (!__rollback_page_needs_abort(session, ref, rollback_timestamp)) {
        *skipp = true;
        __wt_verbose(session, verb_flags, "%p: page walk skipped", (void *)ref);
...
__rollback_to_stable_hs_final_pass(WT_SESSION_IMPL *session, wt_timestamp_t rollback_timestamp)
{
    ...
    if (max_durable_ts > rollback_timestamp) {
        __wt_verbose(session, verb_flags, "tree rolled back with durable timestamp: %s",
    ...
    while ((ret = cursor->next(cursor)) == 0) {
        ...
        if ((F_ISSET(S2C(session), WT_CONN_RECOVERING) ||
          ...
          (addr_size == 0 ||
            ...
            __wt_verbose(session, verb_flags, "Skip rollback to stable on file %s because %s", uri,
...
__wt_rollback_to_stable(WT_SESSION_IMPL *session, const char *cfg[], bool no_ckpt)
{
    ...
    verb_flags = WT_VERB_RTS;
    if (F_ISSET(S2C(session), WT_CONN_RECOVERING))
        verb_flags |= WT_VERB_RECOVERY;
