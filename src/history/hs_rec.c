...
static int __hs_delete_key_from_pos(
  WT_SESSION_IMPL *session, WT_CURSOR *hs_cursor, WT_HS_KEY *hs_key, bool reinsert);
static int __hs_fixup_out_of_order_from_pos(WT_SESSION_IMPL *session, WT_CURSOR *hs_cursor,
  WT_HS_KEY *hs_key, wt_timestamp_t ts, uint64_t *hs_counter, const WT_ITEM *srch_key);
 ...
 */
static int
__hs_insert_record_with_btree_int(WT_SESSION_IMPL *session, WT_CURSOR *cursor, WT_HS_KEY *hs_key,
  WT_HS_VALUE *hs_value, WT_TIME_WINDOW *tw, uint64_t counter)
{
    WT_CURSOR_BTREE *cbt;
    WT_DECL_RET;
     ...
     */
    cursor->set_key(cursor, hs_key->btree_id, hs_key->ds_key, tw->start_ts, counter);
    cursor->set_value(cursor, tw->durable_stop_ts, tw->durable_start_ts, (uint64_t)hs_value->type,
      hs_value->ds_value);
 ...
 */
static int
__hs_insert_record_with_btree(WT_SESSION_IMPL *session, WT_CURSOR *cursor, WT_HS_KEY *hs_key,
  WT_HS_VALUE *hs_value, WT_TIME_WINDOW *tw)
{
    ...
    WT_ERR(__wt_scr_alloc(session, 0, &retrieved_key.ds_key));
     ...
     */
    WT_ASSERT(session, hs_value->type == WT_UPDATE_STANDARD || hs_value->type == WT_UPDATE_MODIFY);
    ...
    WT_ERR_NOTFOUND_OK(__wt_hs_cursor_position(
                         session, cursor, hs_key->btree_id, hs_key->ds_key, tw->start_ts, srch_key),
      true);
    if (ret == 0) {
        WT_ERR(cursor->get_key(
          cursor, &retrieved_key.btree_id, retrieved_key.ds_key, &hs_start_ts, &hs_counter));
        /*
         ...
         */
        if (retrieved_key.btree_id == hs_key->btree_id && tw->start_ts == hs_start_ts) {
            WT_ERR(__wt_compare(session, NULL, hs_key->ds_key, retrieved_key.ds_key, &cmp));

#ifdef HAVE_DIAGNOSTIC
            if (cmp == 0) {
                WT_ERR(cursor->get_value(cursor, &hs_stop_durable_ts_diag, &durable_timestamp_diag,
                  &upd_type_full_diag, existing_val));
                WT_ERR(__wt_compare(session, NULL, existing_val, hs_value->ds_value, &cmp));
                if (cmp == 0)
                    WT_ASSERT(session,
    ...
    if (tw->start_ts != WT_TS_NONE) {
        ...
        if (ret == 0)
            WT_ERR(__hs_fixup_out_of_order_from_pos(
              session, cursor, hs_key, tw->start_ts, &counter, srch_key));
    }
    ...
    while ((ret = __hs_insert_record_with_btree_int(
              session, cursor, hs_key, hs_value, tw, counter)) == WT_RESTART) {
...
#endif
    __wt_scr_free(session, &retrieved_key.ds_key);
 ...
 */
static int
__hs_insert_record(WT_SESSION_IMPL *session, WT_CURSOR *cursor, WT_HS_KEY *hs_key,
  WT_HS_VALUE *hs_value, WT_TIME_WINDOW *tw)
{
    WT_CURSOR_BTREE *cbt;
    WT_DECL_RET;
    ...
    WT_WITH_BTREE(session, CUR2BT(cbt),
      ret = __hs_insert_record_with_btree(session, cursor, hs_key, hs_value, tw));
    return (ret);
}
...
#define MAX_REVERSE_MODIFY_NUM 16
    ...
    WT_ERR(__wt_scr_alloc(session, WT_INTPACK64_MAXSIZE, &hs_key.ds_key));
    ...
    for (i = 0, list = multi->supd; i < multi->supd_entries; ++i, ++list) {
        ...
        case WT_PAGE_COL_VAR:
            ...
            hs_key.ds_key->size = WT_PTRDIFF(p, hs_key.ds_key->data);
            break;
        case WT_PAGE_ROW_LEAF:
            if (list->ins == NULL) {
                WT_WITH_BTREE(session, btree,
                  ret = __wt_row_leaf_key(session, page, list->ripcip, hs_key.ds_key, false));
                WT_ERR(ret);
            } else {
                hs_key.ds_key->data = WT_INSERT_KEY(list->ins);
                hs_key.ds_key->size = WT_INSERT_KEY_SIZE(list->ins);
            }
            break;
        default:
        ...
        if (oldest_upd->type == WT_UPDATE_TOMBSTONE && oldest_upd == first_non_ts_upd &&
          !F_ISSET(first_non_ts_upd, WT_UPDATE_CLEARED_HS)) {
            ...
            WT_ERR(__wt_hs_delete_key_from_ts(session, hs_key.btree_id, hs_key.ds_key, 1, true));
...
err:
    ...
    __wt_scr_free(session, &hs_key.ds_key);
...
static int
__hs_delete_key_from_ts_int(
  WT_SESSION_IMPL *session, WT_HS_KEY *hs_key, wt_timestamp_t ts, bool reinsert)
{
    WT_CURSOR *hs_cursor;
    WT_DECL_ITEM(srch_key);
    WT_DECL_RET;
    ...
    hs_cursor->set_key(hs_cursor, hs_key->btree_id, hs_key->ds_key, ts, 0);
    ...
    WT_ERR(hs_cursor->get_key(
      hs_cursor, &retrieved_key.btree_id, retrieved_key.ds_key, &hs_start_ts, &hs_counter));
    if (retrieved_key.btree_id != hs_key->btree_id)
        goto done;
    WT_ERR(__wt_compare(session, NULL, hs_key->ds_key, retrieved_key.ds_key, &cmp));
    if (cmp != 0)
        goto done;
    ...
    WT_ERR(__hs_delete_key_from_pos(session, hs_cursor, hs_key, reinsert));
done:
    ret = 0;
err:
    __wt_scr_free(session, &retrieved_key.ds_key);
...
int
__wt_hs_delete_key_from_ts(
  WT_SESSION_IMPL *session, uint32_t btree_id, WT_ITEM *key, wt_timestamp_t ts, bool reinsert)
{
    WT_DECL_RET;
    WT_HS_KEY hs_key;

    hs_key.btree_id = btree_id;
    hs_key.ds_key = key;
    ...
    do {
        WT_WITH_BTREE(session, CUR2BT(session->hs_cursor),
          (ret = __hs_delete_key_from_ts_int(session, &hs_key, ts, reinsert)));
        if (ret == WT_RESTART) {
            WT_STAT_CONN_INCR(session, cache_hs_insert_restart);
            WT_STAT_DATA_INCR(session, cache_hs_insert_restart);
        }
 ...
 */
static int
__hs_fixup_out_of_order_from_pos(WT_SESSION_IMPL *session, WT_CURSOR *hs_cursor, WT_HS_KEY *hs_key,
  wt_timestamp_t ts, uint64_t *counter, const WT_ITEM *srch_key)
{
    ...
    WT_ERR(__wt_scr_alloc(session, 0, &retrieved_key.ds_key));
    WT_ERR(__wt_scr_alloc(session, 0, &retrieved_value.ds_value));
    ...
    for (; ret == 0; ret = __wt_hs_cursor_next(session, hs_cursor)) {
         ...
         */
        WT_ERR(hs_cursor->get_key(
          hs_cursor, &retrieved_key.btree_id, retrieved_key.ds_key, &hs_ts, &hs_counter));
        WT_ERR(__wt_compare(session, NULL, &hs_cursor->key, srch_key, &cmp));
        if (cmp > 0)
            break;
    }
    ...
    for (; ret == 0; ret = __wt_hs_cursor_next(session, hs_cursor)) {
        ...
        WT_ERR(__wt_compare(session, NULL, retrieved_key.ds_key, hs_key->ds_key, &cmp));
        if (cmp != 0)
            break;
        /*
        ...
        while ((ret = __hs_insert_record_with_btree_int(
                  session, insert_cursor, hs_key, &retrieved_value, &tw, *counter)) == WT_RESTART)
            ;
        WT_ERR(ret);
        ++(*counter);
...
err:
    ...
    __wt_scr_free(session, &retrieved_key.ds_key);
    __wt_scr_free(session, &retrieved_value.ds_value);
    if (insert_cursor != NULL)
  ...
  WT_SESSION_IMPL *session, WT_CURSOR *hs_cursor, WT_HS_KEY *hs_key, bool reinsert)
{
    ...
    WT_TIME_WINDOW tw;
    WT_UPDATE *upd;
    wt_timestamp_t start_durable_ts, start_ts, stop_durable_ts;
    uint64_t hs_counter, hs_insert_counter;
    int cmp;
    const char *open_cursor_cfg[] = {WT_CONFIG_BASE(session, WT_SESSION_open_cursor), NULL};
    ...
    if (reinsert) {
        ...
        WT_ERR_NOTFOUND_OK(__wt_hs_cursor_position(session, insert_cursor, hs_key->btree_id,
                             hs_key->ds_key, WT_TS_NONE, NULL),
          true);
        ...
        if (ret == WT_NOTFOUND) {
            ...
            WT_ERR(insert_cursor->get_key(insert_cursor, &retrieved_key.btree_id,
              retrieved_key.ds_key, &start_ts, &hs_insert_counter));
            /*
    ...
    for (; ret == 0; ret = __wt_hs_cursor_next(session, hs_cursor)) {
        WT_ERR(hs_cursor->get_key(
          hs_cursor, &retrieved_key.btree_id, retrieved_key.ds_key, &start_ts, &hs_counter));
        /*
        ...
        if (reinsert) {
            WT_ERR(hs_cursor->get_value(hs_cursor, &stop_durable_ts, &start_durable_ts,
              &retrieved_value.type, retrieved_value.ds_value));
            ...
            while ((ret = __hs_insert_record_with_btree_int(session, insert_cursor, &retrieved_key,
                      &retrieved_value, &tw, hs_insert_counter)) == WT_RESTART)
                ;
            hs_insert_counter++;
            WT_ERR(ret);
        }
