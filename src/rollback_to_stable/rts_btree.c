  ...
  wt_timestamp_t rollback_timestamp, bool *stable_update_found)
{
    ...
    if (stable_upd != NULL) {
         ...
         * updates are from a prepared transaction or from a reinstantiated fast deleted page or
         * from a reinstantiated tombstone on-disk. Reset the transaction ID of the stable update
         * that was restored. Ignore the history store as we cannot have a prepared transaction on
         * it and a fast deleted page in the history store should never be reinstantiated as it is
         * globally visible.
         */
