...
{
    return (F_ISSET_ATOMIC_64(S2C(session), WT_CONN_RTS_THREAD_RUN));
}
...
__rts_thread_run(WT_SESSION_IMPL *session, WT_THREAD *thread)
{
    ...
    /* Wait here. */
    if (F_ISSET_ATOMIC_64(S2C(session), WT_CONN_RTS_THREAD_RUN))
...
__rts_thread_create(WT_SESSION_IMPL *session)
{
    ...
    F_SET_ATOMIC_64(conn, WT_CONN_RTS_THREAD_RUN);
...
__rts_thread_destroy(WT_SESSION_IMPL *session)
{
    ...
    F_CLR_ATOMIC_64(conn, WT_CONN_RTS_THREAD_RUN);
