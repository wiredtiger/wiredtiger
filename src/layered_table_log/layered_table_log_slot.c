...
/*
 * __layered_table_log_slot_dump --
 *     Dump the entire slot state.
 */
static void
__layered_table_log_slot_dump(WT_SESSION_IMPL *session)
{
    ...
    ret = __wt_verbose_dump_layered_table_log(session);
    WT_ASSERT_ALWAYS(session, ret == 0, "Failed to dump log system information");
    earliest = 0;
    for (i = 0; i < WT_SLOT_POOL; i++) {
        slot = &log->slot_pool[i];
        if (__wt_layered_table_log_cmp(
        ...
        __wt_errx(session, "    Start LSN: %" PRIu32 "/%" PRIu32, slot->slot_start_lsn.l.file,
          __wt_layered_table_lsn_offset(&slot->slot_start_lsn));
        __wt_errx(session, "    End  LSN: %" PRIu32 "/%" PRIu32, slot->slot_end_lsn.l.file,
          __wt_layered_table_lsn_offset(&slot->slot_end_lsn));
        __wt_errx(session, "    Release LSN: %" PRIu32 "/%" PRIu32, slot->slot_release_lsn.l.file,
          __wt_layered_table_lsn_offset(&slot->slot_release_lsn));
...
/*
 * __wt_layered_table_log_slot_activate --
 *     Initialize a slot to become active.
 */
void
__wt_layered_table_log_slot_activate(WT_SESSION_IMPL *session, WT_LOGSLOT *slot)
{
    WT_CONNECTION_IMPL *conn;
    WT_LOG *log;
    ...
    __wt_atomic_storei64(&slot->slot_start_offset, __wt_layered_table_lsn_offset(&log->alloc_lsn));
    __wt_atomic_storei64(&slot->slot_last_offset, __wt_layered_table_lsn_offset(&log->alloc_lsn));
...
/*
 * __layered_table_log_slot_close --
            ...
            if (EXTRA_DIAGNOSTICS_ENABLED(session, WT_DIAGNOSTIC_SLOW_OPERATION)) {
                ...
                if (count > WT_MILLION) {
                    ...
                    if (WT_CLOCKDIFF_SEC(time_stop, time_start) > 10) {
                        ...
                        __layered_table_log_slot_dump(session);
                        __wt_abort(session);
                    }
                    count = 0;
                }
...
/*
 * __layered_table_log_slot_dirty_max_check --
 ...
 */
static void
__layered_table_log_slot_dirty_max_check(WT_SESSION_IMPL *session, WT_LOGSLOT *slot)
{
    WT_CONNECTION_IMPL *conn;
    WT_LOG *log;
    ...
    if (S2C(session)->layered_table_log_info.log_dirty_max == 0)
        return;
    ...
    if (__wt_layered_table_log_cmp(&log->dirty_lsn, &log->sync_lsn) < 0)
        last_sync = &log->sync_lsn;
    else
        last_sync = &log->dirty_lsn;
    current_offset = __wt_layered_table_lsn_offset(current);
    last_sync_offset = __wt_layered_table_lsn_offset(last_sync);
    if (current->l.file == last_sync->l.file && current_offset > last_sync_offset &&
      current_offset - last_sync_offset > conn->layered_table_log_info.log_dirty_max) {
...
__layered_table_log_slot_new(WT_SESSION_IMPL *session)
{
    ...
    for (;;) {
        ...
        if ((slot = log->active_slot) != NULL &&
          WT_LOG_SLOT_OPEN(__wt_atomic_loadiv64(&slot->slot_state)))
            return (0);
        /*
        ...
        for (i = 0, pool_i = log->pool_index; i < WT_SLOT_POOL; i++, pool_i++) {
            ...
            if (__wt_atomic_loadiv64(&slot->slot_state) == WT_LOG_SLOT_FREE) {
                /*
                 * Acquire our starting position in the log file. Assume the full buffer size.
                 */
                WT_RET(__wt_layered_table_log_acquire(session, log->slot_buf_size, slot));
                /*
                ...
                __layered_table_log_slot_dirty_max_check(session, slot);
                return (0);
            }
        }
        /*
...
/*
 * __layered_log_slot_switch_internal --
  ...
  WT_SESSION_IMPL *session, WT_MYSLOT *myslot, bool forced, bool *did_work)
{
    ...
    if (!F_ISSET(myslot, WT_MYSLOT_CLOSE)) {
        ret = __layered_table_log_slot_close(session, slot, &release, forced);
        /*
     ...
     */
    WT_RET(__layered_table_log_slot_new(session));
    ...
    if (F_ISSET(myslot, WT_MYSLOT_NEEDS_RELEASE)) {
         ...
         */
        WT_RET(__wt_layered_table_log_release(session, slot, &free_slot));
...
/*
 * __wt_layered_table_log_slot_switch --
 *     Switch out the current slot and set up a new one.
 */
int
__wt_layered_table_log_slot_switch(
  WT_SESSION_IMPL *session, WT_MYSLOT *myslot, bool retry, bool forced, bool *did_work)
{
    ...
    do {
        WT_WITH_LAYERED_TABLE_SLOT_LOCK(session, log,
          ret = __layered_log_slot_switch_internal(session, myslot, forced, did_work));
        /*
...
/*
 * __wt_layered_table_log_slot_init --
 *     Initialize the slot array.
 */
int
__wt_layered_table_log_slot_init(WT_SESSION_IMPL *session, bool alloc)
{
    WT_CONNECTION_IMPL *conn;
    WT_DECL_RET;
    WT_LOG *log;
    ...
    if (alloc) {
        log->slot_buf_size = (uint32_t)WT_MIN(
          (size_t)conn->layered_table_log_info.log_file_max / 10, WT_LOG_SLOT_BUF_SIZE);
        for (i = 0; i < WT_SLOT_POOL; i++) {
...
/*
 * __wt_layered_table_log_slot_destroy --
 *     Clean up the slot array on shutdown.
 */
int
__wt_layered_table_log_slot_destroy(WT_SESSION_IMPL *session)
{
    WT_CONNECTION_IMPL *conn;
    WT_LOG *log;
    WT_LOGSLOT *slot;
...
/*
 * __wt_layered_table_log_slot_join --
...
/*
 * __wt_layered_table_log_slot_release --
 ...
 */
int64_t
__wt_layered_table_log_slot_release(WT_MYSLOT *myslot, int64_t size)
{
    WT_LOGSLOT *slot;
...
/*
 * __wt_layered_table_log_slot_free --
 *     Free a slot back into the pool.
 */
void
__wt_layered_table_log_slot_free(WT_SESSION_IMPL *session, WT_LOGSLOT *slot)
{
    /*
    ...
    __wt_atomic_storei32(&slot->slot_error, 0);
    __wt_atomic_storeiv64(&slot->slot_state, WT_LOG_SLOT_FREE);
}
