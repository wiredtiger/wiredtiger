...
__wt_evict_config(WT_SESSION_IMPL *session, const char *cfg[], bool reconfig)
{
    ...
    conn->evict_threads.threads_max = evict_threads_max;
    conn->evict_threads.threads_min = evict_threads_min;

    WT_RET(__wt_config_gets(session, cfg, "eviction.evict_sample_inmem", &cval));
    conn->evict_threads.sample_inmem = cval.val != 0;

    WT_RET(__wt_config_gets(session, cfg, "eviction.evict_use_softptr", &cval));
    __wt_atomic_store_bool_relaxed(&conn->evict_threads.use_npos, cval.val != 0);

    WT_RET(__wt_config_gets(session, cfg, "eviction.legacy_page_visit_strategy", &cval));
    conn->evict_threads.legacy_page_visit_strategy = cval.val != 0;
     ...
     */
    if (reconfig)
        WT_RET(__wt_thread_group_resize(session, &conn->evict_threads.threads,
          conn->evict_threads.threads_min, conn->evict_threads.threads_max,
