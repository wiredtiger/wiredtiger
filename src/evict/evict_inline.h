...
__wt_evict_page_cache_bytes_decr(WT_SESSION_IMPL *session, WT_PAGE *page)
{
    ...
    if (WT_PAGE_IS_INTERNAL(page)) {
        ...
        if (F_ISSET(btree, WT_BTREE_GARBAGE_COLLECT))
            __wt_cache_decr_check_uint64(session, &cache->bytes_internal_ingest, memory_footprint,
              "WT_CACHE.bytes_internal_ingest");
        else if (F_ISSET(btree, WT_BTREE_DISAGGREGATED))
            __wt_cache_decr_check_uint64(session, &cache->bytes_internal_stable, memory_footprint,
              "WT_CACHE.bytes_internal_stable");
    }
