...
__wt_page_release_evict(WT_SESSION_IMPL *session, WT_REF *ref, uint32_t flags)
{
    ...
    /*
     * Track how long the call to evict took. If eviction is successful then we have one of two
     * pairs of stats to increment.
     */
    ...
    if (ret == 0) {
        if (too_big) {
            ...
            WT_STAT_CONN_INCRV(
              session, cache_eviction_force_time, WT_CLOCKDIFF_US(time_stop, time_start));
        } else {
            /*
             * If the page isn't too big, we are evicting it because it had a chain of deleted
             * entries that make traversal expensive.
             */
            WT_STAT_CONN_INCR(session, cache_eviction_force_delete);
            WT_STAT_CONN_INCRV(
              session, cache_eviction_force_delete_time, WT_CLOCKDIFF_US(time_stop, time_start));
        }
    } else {
