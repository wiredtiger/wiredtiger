...
__evict_page_victim_cache(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    if (!__wt_disagg_cache_configured(S2C(session)))
        return;
    ...
    WT_PAGE_LOG_PUT_ARGS args = {
      ...
      .encryption = page->disagg_info->block_meta.encryption,
      .flags = compressed ? WT_PAGE_LOG_COMPRESSED : 0,
      .lsn = page->disagg_info->block_meta.disagg_lsn,
    };

    WT_IGNORE_RET(
      __wt_disagg_cache_put(session, block_disagg, page->disagg_info->block_meta.page_id, &args,
        cache_buf));
...
__wt_evict(WT_SESSION_IMPL *session, WT_REF *ref, WT_REF_STATE previous_state, uint32_t flags)
{
    ...
    if (__wt_ref_is_root(ref))
      ...
      tree_dead) {
        bool instantiated_deleted = page->modify != NULL && page->modify->instantiated;
        if (!instantiated_deleted && !tree_dead &&
          (clean_page && !F_ISSET(conn, WT_CONN_IN_MEMORY) &&
            !F_ISSET(S2BT(session), WT_BTREE_IN_MEMORY)) &&
          !closing)
