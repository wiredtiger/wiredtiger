...
__evict_stat_walk(WT_SESSION_IMPL *session)
{
    ...
    while (__wt_tree_walk_count(session, &next_walk, &walk_count,
      ...
      next_walk != NULL) {
        ++seen_count;
        page = next_walk->page;
        if (__wt_atomic_load_size_relaxed(&page->memory_footprint) > max_pagesize)
            max_pagesize = __wt_atomic_load_size_relaxed(&page->memory_footprint);
        ...
        uint64_t evict_pass_gen = __wt_atomic_load_uint64_relaxed(&evict->evict_pass_gen);
        if (page->evict_pass_gen == 0) {
            unvisited_age_gap_sum += (evict_pass_gen - page->cache_create_gen);
            ++unvisited_count;
        } else {
            visited_age_gap_sum += (evict_pass_gen - page->cache_create_gen);
            gen_gap = evict_pass_gen - page->evict_pass_gen;
...
__wt_evict_cache_stat_walk(WT_SESSION_IMPL *session)
{
    ...
    WT_STAT_DSRC_SET(session, cache_state_root_size,
      __wt_atomic_load_size_relaxed(&btree->root.page->memory_footprint));
