 ...
 *     This function gathers tree-level eviction statistics. It helps in understanding how eviction
 *     is functioning for a specific tree, providing insights into eviction activity that may assist
 *     in diagnosing issues. This function is not for connection-level statistics, and requires a
 *     special `cache_walk` configuration defined in `api_data.py` to enable eviction statistics for
 *     a specific tree.
 */
void
