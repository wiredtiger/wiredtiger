...
__wt_evict_list_clear_page(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    WT_ASSERT(session, __wt_ref_is_root(ref) || __wt_ref_state(ref) == WT_REF_LOCKED);
    ...
    if (ref != NULL) {
        if (__wt_ref_is_root(ref) || evict == start || give_up ||
          ref->page->memory_footprint >= btree->splitmempage) {
            ...
            while (ref != NULL &&
              (__wt_ref_state(ref) != WT_REF_MEM || __wt_readgen_evict_soon(&ref->page->read_gen)))
  ...
  uint8_t *previous_statep)
{
    ...
    *refp = NULL;
    ...
    for (evict = queue->evict_current;
         ...
         */
        if ((previous_state = __wt_ref_state(evict->ref)) != WT_REF_MEM ||
