...
__evict_lock_handle_list(WT_SESSION_IMPL *session)
{
    ...
    for (spins = 0; (ret = __wt_try_readlock(session, dh_lock)) == EBUSY &&
         __wt_atomic_load_uint32_v_relaxed(&evict->impl.randlru.pass_intr) == 0;
         spins++) {
...
__evict_entry_priority(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    read_gen += btree->evict_impl.randlru.evict_priority;
...
__evict_list_clear_page_locked(WT_SESSION_IMPL *session, WT_REF *ref, bool exclude_urgent)
{
    ...
    WT_ASSERT_SPINLOCK_OWNED(session, &evict->impl.randlru.evict_queue_lock);

    for (q = 0; q < last_queue_idx && !found; q++) {
        __wt_spin_lock(session, &evict->impl.randlru.evict_queues[q].evict_lock);
        elem = evict->impl.randlru.evict_queues[q].evict_max;
        for (i = 0, evict_entry = evict->impl.randlru.evict_queues[q].evict_queue; i < elem; i++, evict_entry++)
            if (evict_entry->ref == ref) {
                found = true;
                __evict_list_clear(session, evict_entry);
                break;
            }
        __wt_spin_unlock(session, &evict->impl.randlru.evict_queues[q].evict_lock);
    }
...
__wti_evict_list_clear_page(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    __wt_spin_lock(session, &evict->impl.randlru.evict_queue_lock);

    /* Remove the reference from the eviction queues. */
    __evict_list_clear_page_locked(session, ref, false);

    __wt_spin_unlock(session, &evict->impl.randlru.evict_queue_lock);
}
...
__wt_evict_randlru_server_wake(WT_EVICT *evict, WT_SESSION_IMPL *session)
{
    ...
    __wt_cond_signal(session, evict->impl.randlru.evict_cond);
}
...
__evict_thread_run(WT_SESSION_IMPL *session, WT_THREAD *thread)
{
    ...
    if (__wt_atomic_load_bool_relaxed(&conn->evict_server_running) &&
      __wt_spin_trylock(session, &evict->impl.randlru.evict_pass_lock) == 0) {
        /*
        ...
        FLD_SET(evict->impl.randlru.walk_session->lock_flags, WT_SESSION_LOCKED_PASS);
        ret = __evict_server(session, &did_work);
        FLD_CLR(evict->impl.randlru.walk_session->lock_flags, WT_SESSION_LOCKED_PASS);
        FLD_CLR(session->lock_flags, WT_SESSION_LOCKED_PASS);
        was_intr = __wt_atomic_load_uint32_v_relaxed(&evict->impl.randlru.pass_intr) != 0;
        __wt_spin_unlock(session, &evict->impl.randlru.evict_pass_lock);
        WT_ERR(ret);
         ...
         */
        if (was_intr)
            while (__wt_atomic_load_uint32_v_relaxed(&evict->impl.randlru.pass_intr) != 0 &&
            ...
            __wt_cond_auto_wait(session, evict->impl.randlru.evict_cond, did_work, NULL);
...
__evict_set_saved_walk_tree(WT_SESSION_IMPL *session, WT_DATA_HANDLE *new_dhandle)
{
    ...
    evict = S2C(session)->evict;
    old_dhandle = evict->impl.randlru.walk_tree;
    ...
    evict->impl.randlru.walk_tree = new_dhandle;
...
__evict_server(WT_SESSION_IMPL *session, bool *did_work)
{
    ...
    *did_work = false;
    ...
    WT_ASSERT_SPINLOCK_OWNED(session, &evict->impl.randlru.evict_pass_lock);
     ...
     * running.
     */
    evict->impl.randlru.use_npos_in_pass = __wt_atomic_load_bool_relaxed(&conn->evict_use_npos);
    ...
    if (!__wt_evict_cache_stuck(session)) {
        if (evict->impl.randlru.use_npos_in_pass)
        ...
        evict->impl.randlru.last_eviction_progress = 0;
        return (0);
    }

    /* Track if work was done. */
    *did_work =
      __wt_atomic_load_uint64_v_relaxed(&evict->impl.randlru.eviction_progress) != evict->impl.randlru.last_eviction_progress;
    evict->impl.randlru.last_eviction_progress = __wt_atomic_load_uint64_v_relaxed(&evict->impl.randlru.eviction_progress);
...
__evict_update_work(WT_SESSION_IMPL *session, bool *eviction_needed)
{
    ...
    if (!__evict_queue_empty(evict->impl.randlru.evict_urgent_queue, false))
    ...
    if (LF_ISSET(WT_EVICT_CACHE_CLEAN_HARD) &&
      (__wt_evict_aggressive(session) || evict->impl.randlru.evict_empty_score > WT_EVICT_SCORE_CUTOFF))
        LF_SET(WT_EVICT_CACHE_DIRTY);
...
__evict_pass(WT_SESSION_IMPL *session)
{
    ...
    eviction_progress = __wt_atomic_load_uint64_v_relaxed(&evict->impl.randlru.eviction_progress);
    prev_oldest_id = __wt_atomic_load_uint64_v_relaxed(&txn_global->oldest_id);

    /* Evict pages from the cache. */
    for (loop = 0; __wt_atomic_load_uint32_v_relaxed(&evict->impl.randlru.pass_intr) == 0; loop++) {
         ...
         */
        __wt_atomic_add_uint64(&evict->impl.randlru.read_gen, 1);
        __wt_atomic_add_uint64(&evict->impl.randlru.evict_pass_gen, 1);
        ...
        if (!WT_EVICT_HAS_WORKERS(session) &&
          (evict->impl.randlru.evict_empty_score < WT_EVICT_SCORE_CUTOFF ||
            !__evict_queue_empty(evict->impl.randlru.evict_urgent_queue, false)))
            WT_RET(__evict_randlru_pages(session, true));

        if (__wt_atomic_load_uint32_v_relaxed(&evict->impl.randlru.pass_intr) != 0)
            break;
         ...
         */
        if (eviction_progress == __wt_atomic_load_uint64_v_relaxed(&evict->impl.randlru.eviction_progress)) {
            if (WT_CLOCKDIFF_MS(time_now, time_prev) >= 20 && F_ISSET(evict, WT_EVICT_CACHE_HARD)) {
                if (__wt_atomic_load_uint32_relaxed(&evict->impl.randlru.evict_aggressive_score) <
                  WT_EVICT_SCORE_MAX)
                    (void)__wt_atomic_add_uint32_v(&evict->impl.randlru.evict_aggressive_score, 1);
            ...
            if (loop < 100 ||
              __wt_atomic_load_uint32_relaxed(&evict->impl.randlru.evict_aggressive_score) <
                ...
                __wt_cond_wait(session, evict->impl.randlru.evict_cond, WT_THOUSAND, NULL);
                continue;
            }
        ...
        __wt_atomic_decrement_if_positive(&evict->impl.randlru.evict_aggressive_score);
        loop = 0;
        eviction_progress = __wt_atomic_load_uint64_v_relaxed(&evict->impl.randlru.eviction_progress);
    }
    return (0);
...
#define PATH_STR_MAX 1024
    ...
    if ((ref = __wt_atomic_load_ptr_relaxed(&btree->evict_impl.randlru.evict_ref)) == NULL)
        return (0);

    if (!evict->impl.randlru.use_npos_in_pass || clear_pos)
     ...
     */
    __wt_atomic_store_ptr_relaxed(&btree->evict_impl.randlru.evict_ref, NULL);
    ...
    if (evict->impl.randlru.use_npos_in_pass) {
        ...
        if (clear_pos)
             ...
             */
            btree->evict_impl.randlru.evict_saved_ref_check = (uint64_t)ref;
            ...
            if (F_ISSET(ref, WT_REF_FLAG_LEAF)) {
                 ...
                 * of walk.
                 */
                if (btree->evict_impl.randlru.evict_start_type == WT_EVICT_WALK_NEXT ||
                  btree->evict_impl.randlru.evict_start_type == WT_EVICT_WALK_RAND_NEXT) {
                    pos = WT_NPOS_RIGHT;
                    where = "RIGHT";
                } else {
                    pos = WT_NPOS_LEFT;
            ...
            if (!WT_VERBOSE_LEVEL_ISSET(session, WT_VERB_EVICTION, WT_VERBOSE_DEBUG_1))
                btree->evict_impl.randlru.evict_pos = __wt_page_npos(session, ref, pos, NULL, NULL, 0);
            else {
                btree->evict_impl.randlru.evict_pos =
                ...
                __wt_verbose_debug1(session, WT_VERB_EVICTION,
                  ...
                  btree->evict_impl.randlru.evict_pos, where, F_ISSET(ref, WT_REF_FLAG_INTERNAL) ? "INTERNAL" : "LEAF",
                  path_str, (void *)ref);
            }
        }
    }

    WT_WITH_DHANDLE(evict->impl.randlru.walk_session, session->dhandle,
      (ret = __wt_page_release(evict->impl.randlru.walk_session, ref, WT_READ_NO_EVICT)));
    return (ret);
#undef PATH_STR_MAX
}
...
{
    WT_ASSERT_SPINLOCK_OWNED(session, &S2C(session)->evict->impl.randlru.evict_pass_lock);
    if (session->dhandle == S2C(session)->evict->impl.randlru.walk_tree)
        __evict_set_saved_walk_tree(session, NULL);
...
__wt_evict_randlru_file_exclusive_on(WT_EVICT *evict, WT_SESSION_IMPL *session)
{
    ...
    __wt_spin_lock(session, &evict->impl.randlru.evict_walk_lock);
    if (++btree->evict_impl.randlru.evict_disabled > 1) {
        __wt_spin_unlock(session, &evict->impl.randlru.evict_walk_lock);
        return (0);
    }
     ...
     */
    (void)__wt_atomic_add_uint32_v(&evict->impl.randlru.pass_intr, 1);
    WTI_WITH_PASS_LOCK(session, ret = __evict_clear_walk_and_saved_tree_if_current_locked(session));
    (void)__wt_atomic_sub_uint32_v(&evict->impl.randlru.pass_intr, 1);
    WT_ERR(ret);
     ...
     */
    while (__wt_tsan_suppress_load_uint32_v(&btree->evict_impl.randlru.evict_busy) > 0)
        __wt_yield();

    if (0) {
err:
        --btree->evict_impl.randlru.evict_disabled;
    }
    __wt_spin_unlock(session, &evict->impl.randlru.evict_walk_lock);
    return (ret);
}
 ...
 */
#if defined(HAVE_DIAGNOSTIC)
    {
        ...
        WT_ASSERT(session, __wt_atomic_load_ptr_relaxed(&btree->evict_impl.randlru.evict_ref) == NULL);
        v = __wt_atomic_sub_int32(&btree->evict_impl.randlru.evict_disabled, 1);
        WT_ASSERT(session, v >= 0);
    }
#else
    (void)__wt_atomic_sub_int32(&btree->evict_impl.randlru.evict_disabled, 1);
#endif
...
__evict_tune_workers(WT_SESSION_IMPL *session)
{
    ...
    time_diff = WT_TIMEDIFF_MS(current_time, evict->impl.randlru.evict_tune_last_time);

    /*
     * If we have reached the stable state and have not run long enough to surpass the forced
     * re-tuning threshold, return.
     */
    if (evict->impl.randlru.evict_tune_stable) {
         ...
         */
        evict->impl.randlru.evict_tune_stable = false;
        evict->impl.randlru.evict_tune_last_action_time.tv_sec = 0;
        evict->impl.randlru.evict_tune_progress_last = 0;
        evict->impl.randlru.evict_tune_num_points = 0;
        evict->impl.randlru.evict_tune_progress_rate_max = 0;
     ...
     * return.
     */
    if (evict->impl.randlru.evict_tune_progress_last == 0)
        goto done;

    delta_msec = WT_TIMEDIFF_MS(current_time, evict->impl.randlru.evict_tune_last_time);
    delta_pages = eviction_progress - evict->impl.randlru.evict_tune_progress_last;
    eviction_progress_rate = (delta_pages * WT_THOUSAND) / delta_msec;
    evict->impl.randlru.evict_tune_num_points++;

    /*
     * Keep track of the maximum eviction throughput seen and the number of workers corresponding to
     * that throughput.
     */
    if (eviction_progress_rate > evict->impl.randlru.evict_tune_progress_rate_max) {
        evict->impl.randlru.evict_tune_progress_rate_max = eviction_progress_rate;
        current_threads = __wt_atomic_load_uint32_relaxed(&conn->evict_threads.current_threads);
        __wt_atomic_store_uint32_relaxed(&evict->impl.randlru.evict_tune_workers_best, current_threads);
    }
     ...
     */
    if (evict->impl.randlru.evict_tune_num_points >= evict->impl.randlru.evict_tune_datapts_needed) {
        current_threads = __wt_atomic_load_uint32_relaxed(&conn->evict_threads.current_threads);
        if (evict->impl.randlru.evict_tune_workers_best == current_threads &&
          current_threads < conn->evict_threads_max) {
            /*
             * Keep adding workers. We will check again at the next check point.
             */
            evict->impl.randlru.evict_tune_datapts_needed += WT_MIN(EVICT_TUNE_DATAPT_MIN,
            ...
            thread_surplus =
              (int32_t)__wt_atomic_load_uint32_relaxed(&conn->evict_threads.current_threads) -
              (int32_t)evict->impl.randlru.evict_tune_workers_best;

            for (i = 0; i < thread_surplus; i++)
                __wt_thread_group_stop_one(session, &conn->evict_threads);

            evict->impl.randlru.evict_tune_stable = true;
            goto done;
        }
    }
     ...
     */
    if (evict->impl.randlru.evict_tune_last_action_time.tv_sec == 0)
        evict->impl.randlru.evict_tune_datapts_needed = EVICT_TUNE_DATAPT_MIN;
    ...
    if (F_ISSET(evict, WT_EVICT_CACHE_ALL)) {
        ...
        evict->impl.randlru.evict_tune_last_action_time = current_time;
    }

done:
    evict->impl.randlru.evict_tune_last_time = current_time;
    evict->impl.randlru.evict_tune_progress_last = eviction_progress;
}
...
__evict_randlru_walk(WT_SESSION_IMPL *session)
{
    ...
    if (evict->impl.randlru.evict_empty_score > 0)
        --evict->impl.randlru.evict_empty_score;

    /* Fill the next queue (that isn't the urgent queue). */
    queue = evict->impl.randlru.evict_fill_queue;
    other_queue = evict->impl.randlru.evict_queues + (1 - (queue - evict->impl.randlru.evict_queues));
    evict->impl.randlru.evict_fill_queue = other_queue;
    ...
    if (__evict_queue_full(queue) && evict->impl.randlru.evict_empty_score < WT_EVICT_SCORE_CUTOFF) {
    ...
    if (__evict_queue_empty(queue, false)) {
        if (F_ISSET(evict, WT_EVICT_CACHE_HARD))
            evict->impl.randlru.evict_empty_score =
              WT_MIN(evict->impl.randlru.evict_empty_score + WT_EVICT_SCORE_BUMP, WT_EVICT_SCORE_MAX);
     ...
     */
    if ((ret = __evict_walk(evict->impl.randlru.walk_session, queue)) == EBUSY)
        ret = 0;
     ...
     */
    if (queue == evict->impl.randlru.evict_current_queue)
    ...
    if (__wt_evict_aggressive(session))
        ...
        if (__wti_evict_readgen_is_soon_or_wont_need(&read_gen_oldest))
            ...
            evict->impl.randlru.read_gen_oldest = read_gen_oldest;
        }
    }
...
__evict_walk(WT_SESSION_IMPL *session, WTI_EVICT_QUEUE *queue)
{
    ...
    max_entries = WT_MIN(slot + WTI_EVICT_WALK_INCR, evict->impl.randlru.evict_slots);
...
retry:
    ...
    while (slot < max_entries && loop_count++ < conn->dhandle_count) {
        ...
        if (dhandle == NULL) {
             ...
             */
            if ((dhandle = evict->impl.randlru.walk_tree) != NULL)
                __evict_set_saved_walk_tree(session, NULL);
            else
        ...
        if (btree->evict_impl.randlru.evict_disabled > 0) {
         ...
         * its pages.
         */
        if (btree->evict_impl.randlru.evict_priority != 0 && !__wt_evict_aggressive(session) &&
        ...
        if (!evict->impl.randlru.use_npos_in_pass) {
             ...
             * walk session.
             */
            if (__wt_atomic_load_ptr_relaxed(&btree->evict_impl.randlru.evict_ref) == NULL &&
         ...
         */
        if (__wt_conn_is_disagg(session) && btree->evict_impl.randlru.last_evict_walk_flags != evict->flags) {
            __wt_atomic_store_uint32_relaxed(&btree->evict_impl.randlru.evict_walk_period, 0);
            btree->evict_impl.randlru.last_evict_walk_flags = evict->flags;
        }

        /*
         * If we are filling the queue, skip files that haven't been useful in the past.
         */
        evict_walk_period = __wt_atomic_load_uint32_relaxed(&btree->evict_impl.randlru.evict_walk_period);
        if (evict_walk_period != 0 && btree->evict_impl.randlru.evict_walk_skips++ < evict_walk_period) {
        ...
        btree->evict_impl.randlru.evict_walk_skips = 0;
         ...
         */
        if (btree->evict_impl.randlru.evict_disabled == 0 && !__wt_spin_trylock(session, &evict->impl.randlru.evict_walk_lock)) {
            if (btree->evict_impl.randlru.evict_disabled == 0 && btree->root.page != NULL) {
                WT_WITH_DHANDLE(
...
err:
     ...
     */
    if (queue->evict_entries == slot && __wt_atomic_load_uint32_v_relaxed(&evict->impl.randlru.pass_intr) == 0)
        ret = WT_NOTFOUND;
...
#define MIN_PAGES_PER_TREE 10
    ...
    if (F_ISSET(evict, WT_EVICT_CACHE_CLEAN)) {
        ...
        bytes_per_slot = 1 + cache_inuse / evict->impl.randlru.evict_slots;
...
__evict_get_target_pages(WT_SESSION_IMPL *session, u_int max_entries, uint32_t slot)
{
    ...
    if ((target_pages == 0) || btree->evict_impl.randlru.evict_walk_progress >= btree->evict_impl.randlru.evict_walk_target) {
        btree->evict_impl.randlru.evict_walk_target = target_pages;
        btree->evict_impl.randlru.evict_walk_progress = 0;
    }
    target_pages = btree->evict_impl.randlru.evict_walk_target - btree->evict_impl.randlru.evict_walk_progress;
...
#define PATH_STR_MAX 1024
    ...
    if ((evict_ref = __wt_atomic_load_ptr_relaxed(&btree->evict_impl.randlru.evict_ref)) != NULL)
        return (0); /* We've got a pointer already */
    if (WT_NPOS_IS_INVALID(btree->evict_impl.randlru.evict_pos))
        return (0); /* No restore point */
    WT_RET_ONLY(
      __wt_page_from_npos_for_eviction(session, &evict_ref, btree->evict_impl.randlru.evict_pos, 0, walk_flags),
      WT_PANIC);
    __wt_atomic_store_ptr_relaxed(&btree->evict_impl.randlru.evict_ref, evict_ref);
    ...
    if (evict_ref != NULL &&
      WT_VERBOSE_LEVEL_ISSET(session, WT_VERB_EVICTION, WT_VERBOSE_DEBUG_1)) {
        ...
        __wt_verbose_debug1(session, WT_VERB_EVICTION,
          "Evict walk point recalled from position %lf %s page %s ref %p", btree->evict_impl.randlru.evict_pos,
          F_ISSET(evict_ref, WT_REF_FLAG_INTERNAL) ? "INTERNAL" : "LEAF", path_str,
          (void *)evict_ref);
    }

    WT_STAT_CONN_INCR(session, eviction_restored_pos);
    if (btree->evict_impl.randlru.evict_saved_ref_check != 0 && btree->evict_impl.randlru.evict_saved_ref_check != (uint64_t)evict_ref)
...
__evict_walk_prepare(WT_SESSION_IMPL *session, uint32_t *walk_flagsp)
{
    ...
    *walk_flagsp = WT_READ_EVICT_WALK_FLAGS;
    ...
    evict_ref = __wt_atomic_load_ptr_relaxed(&btree->evict_impl.randlru.evict_ref);
    if (evict_ref != NULL)
     ...
     * in-cache content.
     */
    switch (btree->evict_impl.randlru.evict_start_type) {
    case WT_EVICT_WALK_NEXT:
        ...
        if (!S2C(session)->evict_legacy_page_visit_strategy && evict_ref == NULL &&
          (++btree->evict_impl.randlru.linear_walk_restarts) & 1) {
            if (S2C(session)->evict->impl.randlru.use_npos_in_pass)
  ...
  uint64_t min_pages, uint32_t target_pages)
{
    ...
    if (give_up) {
        ...
        switch (btree->evict_impl.randlru.evict_start_type) {
        case WT_EVICT_WALK_NEXT:
            btree->evict_impl.randlru.evict_start_type = WT_EVICT_WALK_PREV;
            break;
        case WT_EVICT_WALK_PREV:
            btree->evict_impl.randlru.evict_start_type = WT_EVICT_WALK_RAND_PREV;
            break;
        case WT_EVICT_WALK_RAND_PREV:
            btree->evict_impl.randlru.evict_start_type = WT_EVICT_WALK_RAND_NEXT;
            break;
        case WT_EVICT_WALK_RAND_NEXT:
            btree->evict_impl.randlru.evict_start_type = WT_EVICT_WALK_NEXT;
            break;
        }
    ...
    *queuedp = false;
    ...
    if (!FLD_ISSET(conn->debug_flags, WT_CONN_DEBUG_EVICT_AGGRESSIVE_MODE) &&
      F_ISSET(ref, WT_REF_FLAG_INTERNAL)) {
        ...
        if (__wt_atomic_load_uint32_relaxed(&btree->evict_impl.randlru.evict_walk_period) == 0 &&
...
__evict_walk_tree(WT_SESSION_IMPL *session, WTI_EVICT_QUEUE *queue, u_int max_entries, u_int *slotp)
{
    ...
    WT_ASSERT_SPINLOCK_OWNED(session, &evict->impl.randlru.evict_walk_lock);
     ...
     */
    ref = __wt_atomic_load_ptr_relaxed(&btree->evict_impl.randlru.evict_ref);
    __wt_atomic_store_ptr_relaxed(&btree->evict_impl.randlru.evict_ref, NULL);
    ...
    for (evict_entry = start, pages_already_queued = pages_queued = pages_seen = refs_walked = 0;
        ...
        if (page->evict_impl.randlru.evict_pass_gen == 0) {
            ...
            __wt_atomic_stats_max_uint64(&evict->impl.randlru.evict_max_unvisited_gen_gap, gen_gap);
            __wt_atomic_stats_max_uint64(
              &evict->impl.randlru.evict_max_unvisited_gen_gap_per_checkpoint, gen_gap);
        } else {
            const uint64_t gen_gap =
              __wt_atomic_load_uint64_relaxed(&evict->impl.randlru.evict_pass_gen) - page->evict_impl.randlru.evict_pass_gen;
            __wt_atomic_stats_max_uint64(&evict->impl.randlru.evict_max_visited_gen_gap, gen_gap);
            __wt_atomic_stats_max_uint64(&evict->impl.randlru.evict_max_visited_gen_gap_per_checkpoint, gen_gap);
        }

        page->evict_impl.randlru.evict_pass_gen = __wt_atomic_load_uint64_relaxed(&evict->impl.randlru.evict_pass_gen);
        ...
        __wt_atomic_stats_max_uint16(
          &evict->impl.randlru.evict_max_eviction_queue_attempts, page->evict_queue_attempts);
        ...
        if (queued) {
            ++evict_entry;
            ++pages_queued;
            ++btree->evict_impl.randlru.evict_walk_progress;
    ...
    *slotp += (u_int)(evict_entry - start);
    ...
    if (pages_queued < target_pages / 2 && !urgent_queued)
        __wt_atomic_store_uint32_relaxed(
          &btree->evict_impl.randlru.evict_walk_period, WT_MIN(WT_MAX(1, 2 * evict_walk_period), 100));
        ...
        __wt_atomic_store_uint32_relaxed(&btree->evict_impl.randlru.evict_walk_period, evict_walk_period / 2);
    ...
    if (ref != NULL) {
        if (__wt_ref_is_root(ref) || evict_entry == start || give_up ||
          __wt_atomic_load_size_relaxed(&ref->page->memory_footprint) >= btree->splitmempage) {
            if (restarts == 0)
                WT_STAT_CONN_INCR(session, eviction_walks_abandoned);
            WT_RET(__wt_page_release(evict->impl.randlru.walk_session, ref, walk_flags));
            ref = NULL;
        } else {
            while (ref != NULL &&
         ...
         * Using atomic store here is for code consistency with other accesses to btree->evict_impl.randlru.evict_ref.
         */
        __wt_atomic_store_ptr_relaxed(&btree->evict_impl.randlru.evict_ref, ref);
        if (evict->impl.randlru.use_npos_in_pass)
  ...
  WT_REF_STATE *previous_statep)
{
    ...
    *refp = NULL;
    ...
    urgent_queue = evict->impl.randlru.evict_urgent_queue;

    /* Avoid the LRU lock if no pages are available. */
    if (__evict_queue_empty(evict->impl.randlru.evict_current_queue, is_server) &&
      __evict_queue_empty(evict->impl.randlru.evict_other_queue, is_server) &&
    ...
    if (is_server && (!urgent_ok || __evict_queue_empty(urgent_queue, false)) &&
      !__evict_queue_full(evict->impl.randlru.evict_current_queue) &&
      !__evict_queue_full(evict->impl.randlru.evict_fill_queue) &&
      (evict->impl.randlru.evict_empty_score > WT_EVICT_SCORE_CUTOFF ||
        __evict_queue_empty(evict->impl.randlru.evict_fill_queue, false)))
        return (WT_NOTFOUND);
    ...
    if (urgent_ok && !__evict_queue_empty(urgent_queue, false))
         ...
         */
        queue = evict->impl.randlru.evict_current_queue;
        other_queue = evict->impl.randlru.evict_other_queue;
        if (__evict_queue_empty(queue, server_only) &&
          !__evict_queue_empty(other_queue, server_only)) {
            evict->impl.randlru.evict_current_queue = other_queue;
            evict->impl.randlru.evict_other_queue = queue;
        }
    }
    ...
    for (evict_entry = queue->evict_current;
         ...
         */
        (void)__wt_atomic_add_uint32_v(&evict_entry->btree->evict_impl.randlru.evict_busy, 1);
...
__evict_page(WT_SESSION_IMPL *session, bool is_server)
{
    ...
    (void)__wt_atomic_sub_uint32_v(&btree->evict_impl.randlru.evict_busy, 1);
  ...
  WT_SESSION_IMPL *session, bool busy, bool readonly, bool interruptible)
{
     ...
     */
    for (uint64_t initial_progress = __wt_atomic_load_uint64_v_relaxed(&evict->impl.randlru.eviction_progress);;
         ret = 0) {
        /*
        ...
        if (!__wt_evict_needed(session, busy, readonly, true, &pct_full) ||
          (pct_full < 100.0 &&
            (__wt_atomic_load_uint64_v_relaxed(&evict->impl.randlru.eviction_progress) >
...
__wt_evict_randlru_page_urgent(WT_EVICT *evict, WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    page = ref->page;
    if (S2BT(session)->evict_impl.randlru.evict_disabled > 0 || F_ISSET_ATOMIC_16(page, WT_PAGE_EVICT_LRU_URGENT)) {
    ...
    urgent_queue = &evict->impl.randlru.evict_queues[WTI_EVICT_URGENT_QUEUE];
    queued = false;
    ...
    if (evict_entry < urgent_queue->evict_queue + evict->impl.randlru.evict_slots &&
...
{
    WT_UNUSED(evict);
    S2BT(session)->evict_impl.randlru.evict_priority = v;
}
...
{
    WT_UNUSED(evict);
    S2BT(session)->evict_impl.randlru.evict_priority = 0;
}
  ...
  uint64_t *total_dirty_bytesp, uint64_t *total_updates_bytesp)
{
    ...
    WT_RET(__wt_msg(session, "%s(%s%s)%s%s:", dhandle->name,
      ...
      btree->evict_impl.randlru.evict_disabled != 0 ? " eviction disabled" : "",
      btree->evict_impl.randlru.evict_disabled_open ? " at open" : ""));
    ...
    if (F_ISSET_ATOMIC_32(
      ...
      !__wt_evict_randlru_clean_needed(evict, session, &pct_full)) {
        if (pct_full <= evict->eviction_target &&
          ((__wt_atomic_load_uint32_relaxed(&evict->impl.randlru.evict_aggressive_score)) <= (session->id % 20)))
            return (0);
    }
...
{
    WT_UNUSED(session);
    return (evict->impl.randlru.walk_tree);
}
...
{
    WT_UNUSED(session);
    return (__wt_atomic_load_uint64_relaxed(&evict->impl.randlru.evict_pass_gen));
}
...
__wt_evict_randlru_get_page_evict_pass_gen(WT_EVICT *evict, WT_SESSION_IMPL *session, WT_PAGE *page)
{
    WT_UNUSED(evict);
    WT_UNUSED(session);
    return (page->evict_impl.randlru.evict_pass_gen);
}
...
{
    mod->evict_impl.randlru.last_eviction_id = __wt_txn_oldest_id(session);
    __wt_txn_pinned_timestamp(session, &mod->evict_impl.randlru.last_eviction_timestamp);
    mod->evict_impl.randlru.last_evict_pass_gen =
  ...
  WT_EVICT *evict, WT_SESSION_IMPL *session, WT_PAGE_MODIFY *dst, WT_PAGE_MODIFY *src)
{
    WT_UNUSED(evict);
    WT_UNUSED(session);
    dst->evict_impl.randlru.last_evict_pass_gen = src->evict_impl.randlru.last_evict_pass_gen;
    dst->evict_impl.randlru.last_eviction_id = src->evict_impl.randlru.last_eviction_id;
    dst->evict_impl.randlru.last_eviction_timestamp =
      src->evict_impl.randlru.last_eviction_timestamp;
}
...
__wt_evict_randlru_page_evict_retry(WT_EVICT *evict, WT_SESSION_IMPL *session, WT_PAGE *page)
{
    ...
    if (__wt_evict_randlru_aggressive(evict, session) ||
      mod->evict_impl.randlru.last_evict_pass_gen + 5 <
        __wt_atomic_load_uint64_relaxed(&evict->impl.randlru.evict_pass_gen))
        return (true);
     ...
     */
    if (mod->evict_impl.randlru.last_eviction_timestamp != WT_TS_NONE) {
        __wt_txn_pinned_timestamp(session, &pinned_ts);
        if (pinned_ts > mod->evict_impl.randlru.last_eviction_timestamp)
            return (true);
    }
...
{
    WT_UNUSED(session);
    page->evict_impl.randlru.cache_create_gen =
      __wt_atomic_load_uint64_relaxed(&evict->impl.randlru.evict_pass_gen);
}
  ...
  WT_EVICT *evict, WT_SESSION_IMPL *session, WT_PAGE *page)
{
    WT_UNUSED(evict);
    WT_UNUSED(session);
    return (page->evict_impl.randlru.cache_create_gen);
}
...
{
    WT_UNUSED(evict);
    return (S2BT(session)->evict_impl.randlru.evict_priority);
}
...
__wt_evict_randlru_btree_save_walk_period(WT_EVICT *evict, WT_SESSION_IMPL *session)
{
    ...
    WT_UNUSED(evict);
    btree = S2BT(session);
    btree->evict_impl.randlru.evict_walk_saved =
      __wt_atomic_load_uint32_relaxed(&btree->evict_impl.randlru.evict_walk_period);
}
...
__wt_evict_randlru_btree_restore_walk_period(WT_EVICT *evict, WT_SESSION_IMPL *session)
{
    ...
    btree = S2BT(session);
    __wt_atomic_store_uint32_relaxed(
      &btree->evict_impl.randlru.evict_walk_period, btree->evict_impl.randlru.evict_walk_saved);
}
...
{
    WT_UNUSED(evict);
    return (S2BT(session)->evict_impl.randlru.evict_disabled > 0);
}
...
{
    WT_UNUSED(evict);
    S2BT(session)->evict_impl.randlru.evict_disabled_open = true;
}
...
{
    WT_UNUSED(evict);
    return (S2BT(session)->evict_impl.randlru.evict_disabled_open);
}
...
{
    WT_UNUSED(evict);
    S2BT(session)->evict_impl.randlru.evict_disabled_open = false;
}
...
__wt_evict_randlru_btree_busy_inc(WT_EVICT *evict, WT_SESSION_IMPL *session, WT_BTREE *btree)
{
    WT_UNUSED(evict);
    WT_UNUSED(session);
    (void)__wt_atomic_add_uint32_v(&btree->evict_impl.randlru.evict_busy, 1);
}
  ...
  WT_EVICT *evict, WT_SESSION_IMPL *session, WT_BTREE *btree)
{
    WT_UNUSED(evict);
    WT_UNUSED(session);
    (void)__wt_atomic_add_uint32_v(&btree->evict_impl.randlru.prefetch_busy, 1);
}
  ...
  WT_EVICT *evict, WT_SESSION_IMPL *session, WT_BTREE *btree)
{
    WT_UNUSED(evict);
    WT_UNUSED(session);
    (void)__wt_atomic_sub_uint32_v(&btree->evict_impl.randlru.prefetch_busy, 1);
}
  ...
  WT_EVICT *evict, WT_SESSION_IMPL *session, WT_BTREE *btree)
{
    WT_UNUSED(evict);
    WT_UNUSED(session);
    while (__wt_tsan_suppress_load_uint32_v(&btree->evict_impl.randlru.prefetch_busy) > 0)
        __wt_yield();
}
...
{
    WT_UNUSED(evict);
    return (__wt_atomic_load_ptr_relaxed(&S2BT(session)->evict_impl.randlru.evict_ref));
}
