...
__block_disagg_checkpoint_resolve(WT_BM *bm, WT_SESSION_IMPL *session, bool failed)
{
    ...
    __wt_verbose_debug2(session, WT_VERB_DISAGGREGATED_STORAGE,
      "Resolving disaggregated checkpoint for block manager: %s", block_disagg->name);

    /* Allocate a buffer for metadata keys and table names. */
    ...
    if (strcmp(block_disagg->name, WT_DISAGG_METADATA_FILE) == 0) {
        ...
        /* XXX This table name determination is a bit of a hack. */
        if (WT_SUFFIX_MATCH(block_disagg->name, ".wt")) {
            ...
            WT_SAVE_DHANDLE(
              session, ret = __wt_disagg_copy_metadata_later(session, md_key, table_name));
            WT_ERR(ret);
            ...
            WT_SAVE_DHANDLE(
              session, ret = __wt_disagg_copy_metadata_later(session, md_key, block_disagg->name));
            WT_ERR(ret);
        }
    }
