  ...
  bool checkpoint_io)
{
    ...
    *checksump = 0;  /* -Werror=maybe-uninitialized */
     ...
     */
    disagg_id = __wt_atomic_add64(&block_disagg->next_disagg_id, 1);
 ...
 */
int
__wt_block_disagg_write(WT_SESSION_IMPL *session, WT_BLOCK *block, WT_ITEM *buf, uint8_t *addr,
  size_t *addr_sizep, bool data_checksum, bool checkpoint_io)
{
    ...
    WT_RET(__wt_block_disagg_write_internal(
      session, block_disagg, buf, &disagg_id, &size, &checksum, data_checksum, checkpoint_io));
    __wt_page_header_byteswap(buf->mem);
