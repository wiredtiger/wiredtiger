  ...
  WT_GCC_FUNC_ATTRIBUTE((visibility("default")))
{
    ...
    WT_ERR(__wt_remove_if_exists(session, static_cast<const char*>(tmp->data), false));

    /* Open the from and temporary file handles. */
    WT_ERR(__wt_open(session, from, WT_FS_OPEN_FILE_TYPE_REGULAR, 0, &ffh));
    WT_ERR(__wt_open(session, static_cast<const char*>(tmp->data), WT_FS_OPEN_FILE_TYPE_REGULAR,
...
#define WT_BACKUP_COPY_SIZE (128 * 1024)
    ...
    ret = __wt_fs_rename(session, static_cast<const char*>(tmp->data), to, true);
