...
__disagg_parse_meta(WT_SESSION_IMPL *session, const WT_ITEM *meta_buf, WT_DISAGG_METADATA *metadata)
{
    ...
    while ((ret = __wt_config_next(&meta_cfg, &cfg_key, &cfg_value)) == 0) {
        ...
        } else if (WT_CONFIG_LIT_MATCH("base_write_gen", cfg_key)) {
            WT_ASSERT_ALWAYS(session, metadata->base_write_gen == 0,
              "Duplicate base write generation entry in disaggregated storage metadata: "
              "metadata->base_write_gen=%" PRIu64,
              metadata->base_write_gen);

            if (cfg_value.len > 0)
                metadata->base_write_gen = (uint64_t)cfg_value.val;
