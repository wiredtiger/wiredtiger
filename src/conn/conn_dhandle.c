  ...
  int (*name_func)(WT_SESSION_IMPL *, const char *, bool *), const char *cfg[])
{
    ...
    if (time_start != 0) {
        ...
        if (F_ISSET(S2BT(session), WT_BTREE_SKIP_CKPT)) {
            ++conn->ckpt.handle.skip;
            conn->ckpt.handle.skip_time += time_diff;
        } else {
            ++conn->ckpt.handle.apply;
            conn->ckpt.handle.apply_time += time_diff;
        }
    }
  ...
  int (*name_func)(WT_SESSION_IMPL *, const char *, bool *), const char *cfg[])
{
    ...
    if (uri != NULL) {
        ...
        if (WT_SESSION_IS_CHECKPOINT(session)) {
            ...
            conn->ckpt.handle.apply = conn->ckpt.handle.drop = conn->ckpt.handle.lock =
              conn->ckpt.handle.meta_check = conn->ckpt.handle.skip = 0;
            conn->ckpt.handle.apply_time = conn->ckpt.handle.drop_time =
              conn->ckpt.handle.lock_time = conn->ckpt.handle.meta_check_time =
                conn->ckpt.handle.skip_time = 0;
        ...
        if (time_start != 0) {
            ...
            time_stop = __wt_clock(session);
            time_diff = WT_CLOCKDIFF_US(time_stop, time_start);
            WT_STAT_CONN_SET(session, checkpoint_handle_applied, conn->ckpt.handle.apply);
            WT_STAT_CONN_SET(
              session, checkpoint_handle_apply_duration, conn->ckpt.handle.apply_time);
            WT_STAT_CONN_SET(session, checkpoint_handle_dropped, conn->ckpt.handle.drop);
            WT_STAT_CONN_SET(session, checkpoint_handle_drop_duration, conn->ckpt.handle.drop_time);
            WT_STAT_CONN_SET(session, checkpoint_handle_duration, time_diff);
            WT_STAT_CONN_SET(session, checkpoint_handle_locked, conn->ckpt.handle.lock);
            WT_STAT_CONN_SET(session, checkpoint_handle_lock_duration, conn->ckpt.handle.lock_time);
            WT_STAT_CONN_SET(session, checkpoint_handle_meta_checked, conn->ckpt.handle.meta_check);
            WT_STAT_CONN_SET(
              session, checkpoint_handle_meta_check_duration, conn->ckpt.handle.meta_check_time);
            WT_STAT_CONN_SET(session, checkpoint_handle_skipped, conn->ckpt.handle.skip);
            WT_STAT_CONN_SET(session, checkpoint_handle_skip_duration, conn->ckpt.handle.skip_time);
