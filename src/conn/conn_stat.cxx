    ...
    while ((ret = cursor->next(cursor)) == 0) {
        ...
        if (FLD_ISSET(conn->stat_flags, WT_STAT_JSON)) {
            ...
            if (first || tmp->size != prefixlen ||
              strncmp(desc, static_cast<const char *>(tmp->data), tmp->size) != 0) {
...
__statlog_log_one(WT_SESSION_IMPL *session, WT_ITEM *path, WT_ITEM *tmp)
{
    ...
    if (conn->stat_fs == NULL || path == NULL ||
      strcmp(static_cast<const char *>(tmp->mem), static_cast<const char *>(path->mem)) != 0) {
        WT_RET(__wt_fclose(session, &conn->stat_fs));
        WT_RET(__wt_fopen(session, static_cast<const char *>(tmp->mem),
          WT_FS_OPEN_CREATE | WT_FS_OPEN_FIXED, WT_STREAM_APPEND, &conn->stat_fs));
