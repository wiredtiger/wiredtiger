...
__statlog_config(WT_SESSION_IMPL *session, const char **cfg, bool *runp)
{
    ...
    *runp = cval.val != 0;
    ...
    WT_ERR(__wt_filename(session, static_cast<const char*>(tmp->data), &conn->stat_path));
...
__statlog_dump(WT_SESSION_IMPL *session, const char *name, bool conn_stats)
{
    ...
    while ((ret = cursor->next(cursor)) == 0) {
        ...
        if (FLD_ISSET(conn->stat_flags, WT_STAT_JSON)) {
            ...
            if (first || tmp->size != prefixlen || strncmp(desc, static_cast<const char*>(tmp->data), tmp->size) != 0) {
...
__statlog_log_one(WT_SESSION_IMPL *session, WT_ITEM *path, WT_ITEM *tmp)
{
    ...
    if (strftime(static_cast<char*>(tmp->mem), tmp->memsize, conn->stat_path, &localt) == 0)
        WT_RET_MSG(session, ENOMEM, "strftime path conversion");

    /* If the path has changed, cycle the log file. */
    if (conn->stat_fs == NULL || path == NULL || strcmp(static_cast<const char*>(tmp->mem), static_cast<const char*>(path->mem)) != 0) {
        WT_RET(__wt_fclose(session, &conn->stat_fs));
        WT_RET(__wt_fopen(session, static_cast<const char*>(tmp->mem), WT_FS_OPEN_CREATE | WT_FS_OPEN_FIXED, WT_STREAM_APPEND,
          &conn->stat_fs));

        if (path != NULL)
	        WT_RET(__wt_buf_setstr(session, path, static_cast<const char*>(tmp->mem)));
    }

    /* Create the entry prefix for this time of day. */
    if (strftime(static_cast<char*>(tmp->mem), tmp->memsize, conn->stat_format, &localt) == 0)
        WT_RET_MSG(session, ENOMEM, "strftime timestamp conversion");
    conn->stat_stamp = static_cast<const char*>(tmp->mem);
