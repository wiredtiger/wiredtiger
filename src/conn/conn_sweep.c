...
__sweep_expire(WT_SESSION_IMPL *session, time_t now)
{
    ...
    TAILQ_FOREACH (dhandle, &conn->dhqh, q) {
        ...
        if (WT_IS_METADATA(dhandle) || !F_ISSET(dhandle, WT_DHANDLE_OPEN) ||
          ...
          difftime(now, dhandle->timeofdeath) <= conn->sweep_idle_time)
            continue;
...
__sweep_server(void *arg)
{
    ...
    for (;;) {
        ...
        if (FLD_ISSET(conn->timing_stress_flags, WT_TIMING_STRESS_AGGRESSIVE_SWEEP))
            __wt_cond_wait(
              session, conn->sweep_cond, min_sleep * 100 * WT_THOUSAND, __sweep_server_run_chk);
        else
            __wt_cond_wait(
              session, conn->sweep_cond, min_sleep * WT_MILLION, __sweep_server_run_chk);
