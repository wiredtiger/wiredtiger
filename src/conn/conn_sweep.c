...
__sweep_discard_trees(WT_SESSION_IMPL *session, u_int *dead_handlesp)
{
	...
	*dead_handlesp = 0;
	...
	TAILQ_FOREACH (dhandle, &conn->dhqh, q) {
		...
		WT_WITH_DHANDLE(
		    session, dhandle, ret = __wt_conn_dhandle_close(session, false, false));
...
__sweep_remove_one(WT_SESSION_IMPL *session, WT_DATA_HANDLE *dhandle)
{
	...
	WT_WITH_DHANDLE(
	    session, dhandle, ret = __wt_conn_dhandle_discard_single(session, false, true));
...
__sweep_server(void *arg)
{
	...
	for (;;) {
		...
		if (FLD_ISSET(conn->timing_stress_flags, WT_TIMING_STRESS_AGGRESSIVE_SWEEP))
			__wt_cond_wait(session, conn->sweep_cond, min_sleep * 100 * WT_THOUSAND,
			    __sweep_server_run_chk);
		else
			__wt_cond_wait(session, conn->sweep_cond, min_sleep * WT_MILLION,
			    __sweep_server_run_chk);
