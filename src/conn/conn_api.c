...
__wti_debug_mode_config(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    WT_RET(__wt_config_gets(session, cfg, "debug_mode.conn_close_stress_log_printf", &cval));
    if (cval.val)
        FLD_SET(conn->debug_flags, WT_CONN_DEBUG_CLOSE_STRESS_LOG);
    else
        FLD_CLR(conn->debug_flags, WT_CONN_DEBUG_CLOSE_STRESS_LOG);
    return (0);
}
