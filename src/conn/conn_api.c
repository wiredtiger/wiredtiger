...
err:
    ...
    if (conn->default_session->event_handler->handle_general != NULL &&
      F_ISSET_ATOMIC_64(conn, WT_CONN_MINIMAL | WT_CONN_READY))
    ...
    F_CLR_ATOMIC_64(conn, WT_CONN_MINIMAL | WT_CONN_READY);
     ...
     */
    F_SET_ATOMIC_64(conn, WT_CONN_MINIMAL);
    if (conn->default_session->event_handler->handle_general != NULL)
        WT_TRET(conn->default_session->event_handler->handle_general(
    ...
    F_CLR_ATOMIC_64(conn, WT_CONN_MINIMAL);
    ...
    if (ret != 0) {
        ...
        F_SET_ATOMIC_64(conn, WT_CONN_PANIC);
    }
    ...
    if (cval.val != 0)
        F_SET_ATOMIC_64(conn, WT_CONN_LEAK_MEMORY);
...
__conn_single(WT_SESSION_IMPL *session, const char *cfg[])
{
     ...
     */
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_READONLY) && (ret == EACCES || ret == ENOENT)) {
        bytelock = false;
        ret = 0;
    }
...
#define WT_EXTRA_INTERNAL_SESSIONS 25
    ...
    if (F_ISSET_ATOMIC_64(S2C(session), WT_CONN_LIVE_RESTORE_FS)) {
...
__conn_config_file_system(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    return (__conn_chk_file_system(session, F_ISSET_ATOMIC_64(conn, WT_CONN_READONLY)));
}
  ...
  WT_CONNECTION **connectionp)
{
    ...
    *connectionp = NULL;
    ...
    if (cval.val != 0)
        F_SET_ATOMIC_64(conn, WT_CONN_IN_MEMORY);
    WT_ERR(__wt_config_gets(session, cfg, "readonly", &cval));
    if (cval.val)
        F_SET_ATOMIC_64(conn, WT_CONN_READONLY);
    ...
    if (cval.val)
        F_SET_ATOMIC_64(conn, WT_CONN_CACHE_CURSORS);

    WT_ERR(__wt_config_gets(session, cfg, "checkpoint_sync", &cval));
    if (cval.val)
        F_SET_ATOMIC_64(conn, WT_CONN_CKPT_SYNC);
    ...
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_IN_MEMORY) && conn->prefetch_available)
        WT_ERR_MSG(
    ...
    F_SET_ATOMIC_64(conn, WT_CONN_READY);
    F_CLR_ATOMIC_64(conn, WT_CONN_MINIMAL);
...
err:
     ...
     */
    F_CLR_ATOMIC_64(conn, WT_CONN_BACKUP_PARTIAL_RESTORE);
    ...
    if (ret != 0) {
         ...
         * the connection.
         */
        if (F_ISSET_ATOMIC_64(conn, WT_CONN_DATA_CORRUPTION) &&
