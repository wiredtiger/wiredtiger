...
{
    return (F_ISSET_ATOMIC_64(S2C(session), WT_CONN_PREFETCH_RUN));
}
...
__prefetch_thread_run(WT_SESSION_IMPL *session, WT_THREAD *thread)
{
    ...
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_PREFETCH_RUN))
    ...
    while (!TAILQ_EMPTY(&conn->pfqh)) {
         ...
         */
        if (!F_ISSET_ATOMIC_64(conn, WT_CONN_DATA_CORRUPTION) && pe->ref->page_del == NULL)
...
__wti_prefetch_create(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    F_SET_ATOMIC_64(conn, WT_CONN_PREFETCH_RUN);
...
__wti_prefetch_destroy(WT_SESSION_IMPL *session)
{
    ...
    F_CLR_ATOMIC_64(conn, WT_CONN_PREFETCH_RUN);
