...
__wti_conn_compat_config(WT_SESSION_IMPL *session, const char **cfg, bool reconfig)
{
    ...
    if (cval.len == 0) {
        ...
        F_CLR_ATOMIC_64(conn, WT_CONN_COMPATIBILITY);
    } else {
        ...
        F_SET_ATOMIC_64(conn, WT_CONN_COMPATIBILITY);
    }
    /*
     * If we're a reconfigure and the user did not set any compatibility or did not change the
     * setting, we're done.
     */
    if (reconfig && (!F_ISSET_ATOMIC_64(conn, WT_CONN_COMPATIBILITY) || unchg))
        goto done;
...
__wti_conn_optrack_setup(WT_SESSION_IMPL *session, const char *cfg[], bool reconfig)
{
    ...
    if (cval.val == 0) {
        if (F_ISSET_ATOMIC_64(conn, WT_CONN_OPTRACK)) {
            WT_RET(__wti_conn_optrack_teardown(session, reconfig));
            F_CLR_ATOMIC_64(conn, WT_CONN_OPTRACK);
        }
        return (0);
    }
    ...
    F_SET_ATOMIC_64(conn, WT_CONN_OPTRACK);
...
__wti_conn_reconfig(WT_SESSION_IMPL *session, const char **cfg)
{
    ...
    F_SET_ATOMIC_64(conn, WT_CONN_RECONFIGURING);
...
err:
    F_CLR_ATOMIC_64(conn, WT_CONN_RECONFIGURING);
