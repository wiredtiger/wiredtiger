 ...
 *     Process the metadata entries stored in the shared metadata table for a new checkpoint. On
 *     the first pickup for this process (last_checkpoint_meta_lsn is still 0), creation of ingest
 *     tables is deferred to first cursor open. On subsequent pickups, ingest tables are created
 *     eagerly as part of the pickup.
 */
static int
__disagg_apply_checkpoint_meta(WT_SESSION_IMPL *session, WT_CURSOR *md_cursor)
{
    WT_CONFIG_ITEM cval;
    WT_CURSOR *cursor;
    WT_DECL_ITEM(metadata_cfg);
    ...
    const char *cfg[3], *checkpoint_name, *current_value, *metadata_checkpoint_name, *metadata_key,
      *metadata_value;
    bool discard, eager_ingest_create;
    ...
    /*
     * Treat the first pickup for this process as startup: defer ingest-table creation to first
     * cursor open. Any subsequent pickup (last_checkpoint_meta_lsn already advanced) uses the
     * eager path. This detection works regardless of whether the caller drove the first pickup
     * via wiredtiger_open or via reconfigure.
     */
    eager_ingest_create = __wt_atomic_load_uint64_acquire(
                            &S2C(session)->disaggregated_storage.last_checkpoint_meta_lsn) != 0;
    ...
    while ((ret = cursor->next(cursor)) == 0) {
        ...
        if (ret == 0 && WT_PREFIX_MATCH(metadata_key, "file:")) {
            ...
            /*
             * Create the corresponding ingest table, if it does not exist. On startup pickup
             * (eager_ingest_create == false) we skip this entirely; the ingest table will be
             * created lazily when a cursor is first opened on the layered table.
             */
            if (WT_PREFIX_MATCH(metadata_key, "layered:")) {
                if (eager_ingest_create) {
...
__disagg_pick_up_checkpoint(WT_SESSION_IMPL *session, const WT_DISAGG_CHECKPOINT_META *ckpt_meta)
{
    ...
    __wt_timer_start(session, &timer_step);
    WT_WITH_SCHEMA_LOCK(
      session, ret = __disagg_apply_checkpoint_meta(session, md_cursor));
    ...
    if (__wt_conn_is_disagg(session)) {
        ...
        if (ret == 0 && cval.len > 0) {
            WT_ERR_MSG_CHK(session,
              __disagg_pick_up_checkpoint_meta(session, cval.str, cval.len),
              "Failed to pick up a new checkpoint with config: %.*s", (int)cval.len, cval.str);
              printf("Picked up checkpoint from config at startup\n");
            picked_up = true;
        }
