...
__wt_disagg_put_crypt_helper(WT_SESSION_IMPL *session)
{
    ...
    if (session->ckpt.key_provider_crash_state == KEY_PROVIDER_CRASH_BEFORE_KEY_ROTATION)
    ...
    if (session->ckpt.key_provider_crash_state == KEY_PROVIDER_CRASH_DURING_KEY_ROTATION)
        __wt_debug_crash(session);
    ...
    if (session->ckpt.key_provider_crash_state == KEY_PROVIDER_CRASH_AFTER_KEY_ROTATION)
        __wt_debug_crash(session);
done:
err:
