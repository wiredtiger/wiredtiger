        ...
        if (md_has[WT_DISAGG_CURSOR_LAYERED] && sh_has[WT_DISAGG_CURSOR_LAYERED]) {
            ...
            if (md_has[WT_DISAGG_CURSOR_FILE])
                ...
                WT_ERR(__disagg_insert_meta(
                  session, sh_cursors[WT_DISAGG_CURSOR_FILE], md_write_cursor, &stable_btree_ids));
            ++existing_tables;
            ...
            WT_ERR(__disagg_insert_meta(
              session, sh_cursors[WT_DISAGG_CURSOR_LAYERED], md_write_cursor, &stable_btree_ids));
            WT_ERR(__disagg_insert_meta(
              session, sh_cursors[WT_DISAGG_CURSOR_FILE], md_write_cursor, &stable_btree_ids));
            if (sh_has[WT_DISAGG_CURSOR_COLGROUP])
                WT_ERR(__disagg_insert_meta(session, sh_cursors[WT_DISAGG_CURSOR_COLGROUP],
                  md_write_cursor, &stable_btree_ids));
            if (sh_has[WT_DISAGG_CURSOR_TABLE])
                WT_ERR(__disagg_insert_meta(
                  session, sh_cursors[WT_DISAGG_CURSOR_TABLE], md_write_cursor, &stable_btree_ids));
            ++new_tables;
        } else if (md_has[WT_DISAGG_CURSOR_LAYERED] && !sh_has[WT_DISAGG_CURSOR_LAYERED]) {
            /*
