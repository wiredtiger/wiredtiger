 ...
 */
static bool
__obsolete_cleanup_tw_check(WT_SESSION_IMPL *session, WT_TIME_AGGREGATE ta)
{
    ...
    if (__obsolete_cleanup_limit_reached(session))
        return (false);
...
/*
 * __obsolete_cleanup_inmem_evict_or_mark_dirty --
 *     This function checks whether the in-memory ref contains obsolete information and takes
 *     necessary action.
 */
static int
__obsolete_cleanup_inmem_evict_or_mark_dirty(WT_SESSION_IMPL *session, WT_REF *ref)
{
    WT_ADDR_COPY addr;
    WT_BTREE *btree;
    WT_CONNECTION_IMPL *conn;
    ...
    } else if (__obsolete_cleanup_tw_check(session, newest_ta)) {
...
/*
 * __obsolete_cleanup_deleted_cleanup --
 ...
 */
static int
__obsolete_cleanup_deleted_cleanup(WT_SESSION_IMPL *session, WT_REF *ref)
{
...
/*
 * __obsolete_cleanup_disk_cleanup --
 ...
 */
static int
__obsolete_cleanup_disk_cleanup(WT_SESSION_IMPL *session, WT_REF *ref, bool *ref_deleted)
{
    WT_ADDR_COPY addr;
    WT_DECL_RET;
    WT_TIME_AGGREGATE newest_ta;
...
__obsolete_cleanup_one(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    if (ref_state == WT_REF_DELETED || ref_state == WT_REF_DISK) {
        ...
        if (previous_state == WT_REF_DELETED)
            ret = __obsolete_cleanup_deleted_cleanup(session, ref);
        else if (previous_state == WT_REF_DISK) {
            ret = __obsolete_cleanup_disk_cleanup(session, ref, &ref_deleted);
            if (ref_deleted)
...
/*
 * __obsolete_cleanup_obsolete_cleanup --
 *     Traverse an internal page and identify the leaf pages that are obsolete and mark them as
 *     deleted.
 */
static int
__obsolete_cleanup_obsolete_cleanup(WT_SESSION_IMPL *session, WT_REF *parent)
{
 ...
 *     Check to decide if obsolete cleanup should continue running.
 */
static bool
 ...
 *     Return if the obsolete cleanup should read this page.
 */
static int
__obsolete_cleanup_page_skip(
  WT_SESSION_IMPL *session, WT_REF *ref, void *context, bool visible_all, bool *skipp)
{
    ...
    *skipp = false; /* Default to reading */
     ...
     * checks to verify the cache can handle it. Obsolete cleanup reads clean pages into the cache
     ...
     * AND
     *  - obsolete cleanup is not configured with the reclaim space method OR
     ...
     * cleaned up.
     */
    if (*skipp && __obsolete_cleanup_tw_check(session, addr.ta)) {
...
__obsolete_cleanup_walk_btree(WT_SESSION_IMPL *session, WT_ITEM *uri)
{
    ...
    while ((ret = __wt_tree_walk_custom_skip(
      ...
      ref != NULL) {
        if (F_ISSET(ref, WT_REF_FLAG_INTERNAL)) {
            WT_WITH_PAGE_INDEX(session, ret = __obsolete_cleanup_obsolete_cleanup(session, ref));
        } else {
            WT_ENTER_GENERATION(session, WT_GEN_SPLIT);
            ret = __obsolete_cleanup_inmem_evict_or_mark_dirty(session, ref);
...
__obsolete_cleanup_eligibility(WT_SESSION_IMPL *session, const char *uri, const char *config)
{
    ...
    /* Obsolete cleanup cannot remove obsolete pages from tiered tables. */
...
__obsolete_cleanup(void *arg)
{
    ...
    for (;;) {
        ...
        /*
         * See if it is time to perform obsolete cleanup. Obsolete cleanup is an operation that
...
__wt_obsolete_cleanup_create(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    /*
     * Obsolete cleanup does enough I/O it may be called upon to perform slow operations for the
     * block manager.
     */
    ...
    WT_RET(__wt_cond_alloc(session, "obsolete cleanup", &conn->obsolete_cleanup.cond));
