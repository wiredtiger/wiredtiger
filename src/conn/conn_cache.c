...
__cache_config_abs_to_pct(WT_SESSION_IMPL *session,
    double *param, const char *param_name)
{
	...
	if (*param > 100.0) {
		...
		/*
		 * Avoid division by zero if the cache size has not yet been set
		 * in a shared cache.
		 */
		*param = (*param * 100.0) / (conn->cache_size + 1);
	}
