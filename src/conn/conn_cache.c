...
__cache_config_local(WT_SESSION_IMPL *session, bool shared, const char *cfg[])
{
	...
	if (evict_threads_min > evict_threads_max)
		WT_RET_MSG(session, EINVAL, "eviction=(threads_min) cannot be greater than "
...
__wt_cache_config(WT_SESSION_IMPL *session, bool reconfigure, const char *cfg[])
{
	 ...
	 */
	if (reconfigure)
		WT_RET(__wt_thread_group_resize(session, &conn->evict_threads,
...
__wt_cache_create(WT_SESSION_IMPL *session, const char *cfg[])
{
	...
	if (cache->eviction_target >= cache->eviction_trigger)
		WT_RET_MSG(
		    session, EINVAL, "eviction target must be lower than the eviction trigger");
	...
	for (i = 0; i < WT_EVICT_QUEUE_MAX; ++i) {
		WT_RET(__wt_calloc_def(
		    session, cache->evict_slots, &cache->evict_queues[i].evict_queue));
		WT_RET(
		    __wt_spin_init(session, &cache->evict_queues[i].evict_lock, "cache eviction"));
	}
...
__wt_cache_stats_update(WT_SESSION_IMPL *session)
{
	...
	WT_STAT_SET(
	    session, stats, cache_eviction_active_workers, conn->evict_threads.current_threads);
	WT_STAT_SET(
	...
	if (conn->evict_server_running)
		WT_STAT_SET(
		    session, stats, cache_eviction_walks_active, cache->walk_session->nhazard);
}
...
__wt_cache_destroy(WT_SESSION_IMPL *session)
{
	...
	if (cache->pages_inmem != cache->pages_evicted)
		__wt_errx(session, "cache server: exiting with %" PRIu64 " pages in "
		                   "memory and %" PRIu64 " pages evicted",
		    cache->pages_inmem, cache->pages_evicted);
	if (cache->bytes_image != 0)
		__wt_errx(session, "cache server: exiting with %" PRIu64 " image bytes in "
		                   "memory",
		    cache->bytes_image);
	...
	if (cache->bytes_dirty_intl + cache->bytes_dirty_leaf != 0 ||
		...
		__wt_errx(session, "cache server: exiting with %" PRIu64 " bytes dirty and %" PRIu64
		                   " pages dirty",
