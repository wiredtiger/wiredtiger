  ...
  uint32_t *sizep, uint32_t *checksump, bool data_checksum, bool checkpoint_io, bool caller_locked)
{
    ...
    WT_FH *fh;
    wt_off_t offset;
    size_t align_size, padding_size;
    uint32_t checksum;
    uint8_t *file_sizep;
    bool local_locked;
    ...
    *checksump = 0; /* -Werror=maybe-uninitialized */
    ...
    padding_size = align_size - buf->size;
    ...
    memset((uint8_t *)buf->mem + buf->size, 0, padding_size);
    ...
    WT_STAT_DATA_INCRV(session, block_total_padding_size, padding_size);
    WT_STAT_CONN_INCR(session, block_write);
    WT_STAT_CONN_INCRV(session, block_byte_write, align_size);
    WT_STAT_CONN_INCRV(session, block_byte_write_unused, padding_size);
    if (checkpoint_io)
    ...
    __wt_verbose_debug2(session, WT_VERB_WRITE,
      ...
      (uintmax_t)offset, (uintmax_t)align_size, (uintmax_t)padding_size, checksum);
