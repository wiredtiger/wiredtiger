  ...
  size_t addr_size, uint8_t *root_addr, size_t *root_addr_sizep, bool checkpoint)
{
    ...
    *root_addr_sizep = 0;
    ...
    if (checkpoint) {
        ...
        WT_ERR_ASSERT(session, WT_DIAG_CHECKPOINT_VALIDATION, live_open == false, EBUSY,
...
__ckpt_process(WT_SESSION_IMPL *session, WT_BLOCK *block, WT_CKPT *ckptbase)
{
    ...
    if (EXTRA_DIAGNOSTICS_ENABLED(session, WT_DIAG_CHECKPOINT_VALIDATION))
    ...
    WT_CKPT_FOREACH (ckptbase, ckpt) {
        ...
        if (next_ckpt->bpriv == NULL && !F_ISSET(next_ckpt, WT_CKPT_ADD)) {
            ...
            WT_ERR_ASSERT(session, WT_DIAG_CHECKPOINT_VALIDATION, local == true, WT_PANIC,
