    ...
    WT_SESSION_IMPL *session, WT_BLOCK *block, const uint8_t *addr, size_t addr_size, bool live)
{
	...
	WT_RET(__wt_block_misplaced(
	    session, block, "addr-valid", offset, size, live, __func__, __LINE__));
#endif
    ...
    WT_SESSION_IMPL *session, WT_BLOCK *block, uint8_t **pp, WT_BLOCK_CKPT *ci, bool skip_avail)
{
	...
	WT_RET(__wt_block_addr_to_buffer(
	    block, pp, ci->root_offset, ci->root_size, ci->root_checksum));
	WT_RET(__wt_block_addr_to_buffer(
	    block, pp, ci->alloc.offset, ci->alloc.size, ci->alloc.checksum));
	if (skip_avail)
    ...
    const uint8_t *ckpt_string)
{
	...
	if (ci->root_offset == WT_BLOCK_INVALID_OFFSET)
		...
		WT_ERR(__wt_buf_catfmt(session, tmp,
		    ...
		    (uintmax_t)ci->root_offset, (uintmax_t)(ci->root_offset + ci->root_size),
	...
	if (ci->discard.offset == WT_BLOCK_INVALID_OFFSET)
		...
		WT_ERR(__wt_buf_catfmt(session, tmp,
		    ...
		    (uintmax_t)(ci->discard.offset + ci->discard.size), ci->discard.size,
		    ci->discard.checksum));
