 ...
 */
static int
__oligarch_log_get_files(WT_SESSION_IMPL *session, const char *file_prefix, char ***filesp, u_int *countp)
{
...
__oligarch_log_prealloc_remove(WT_SESSION_IMPL *session)
{
     ...
     */
    WT_ERR(__oligarch_log_get_files(session, WT_LOG_TMPNAME, &logfiles, &logcount));
    for (i = 0; i < logcount; i++) {
        WT_ERR(__wt_oligarch_log_extract_lognum(session, logfiles[i], &lognum));
        WT_ERR(__wt_oligarch_log_remove(session, WT_LOG_TMPNAME, lognum));
    }
    WT_ERR(__wt_fs_directory_list_free(session, &logfiles, logcount));
    WT_ERR(__oligarch_log_get_files(session, WT_LOG_PREPNAME, &logfiles, &logcount));
    for (i = 0; i < logcount; i++) {
        WT_ERR(__wt_oligarch_log_extract_lognum(session, logfiles[i], &lognum));
        WT_ERR(__wt_oligarch_log_remove(session, WT_LOG_PREPNAME, lognum));
    }
err:
 ...
 */
static int
__oligarch_log_fsync_file(WT_SESSION_IMPL *session, WT_LSN *min_lsn, const char *method, bool use_own_fh)
{
    WT_DECL_RET;
    WT_FH *log_fh;
    WT_LOG *log;
  ...
  WT_SESSION_IMPL *session, char ***filesp, u_int *countp, uint32_t *maxid, bool active_only)
{
    ...
    *maxid = 0;
    ...
    WT_RET(__wt_oligarch_log_force_write(session, true, NULL));
    WT_RET(__oligarch_log_get_files(session, WT_LOG_FILENAME, &files, &count));
 ...
 */
int
__wt_oligarch_log_filename(WT_SESSION_IMPL *session, uint32_t id, const char *file_prefix, WT_ITEM *buf)
{
...
__wt_oligarch_log_reset(WT_SESSION_IMPL *session, uint32_t lognum)
{
    ...
    WT_RET(__oligarch_log_get_files(session, WT_LOG_FILENAME, &logfiles, &logcount));
    for (i = 0; i < logcount; i++) {
        WT_ERR(__wt_oligarch_log_extract_lognum(session, logfiles[i], &old_lognum));
        WT_ASSERT(session, old_lognum < lognum || lognum == 1);
        WT_ERR(__wt_oligarch_log_remove(session, WT_LOG_FILENAME, old_lognum));
    }
    log->fileid = lognum;
...
__oligarch_log_size_fit(WT_SESSION_IMPL *session, WT_LSN *lsn, uint64_t recsize)
{
    ...
    return (
      offset == log->first_record || offset + (wt_off_t)recsize < conn->oligarch_log_info.log_file_max);
}
...
__oligarch_log_file_header(WT_SESSION_IMPL *session, WT_FH *fh, WT_LSN *end_lsn, bool prealloc)
{
    ...
    __wt_oligarch_log_desc_byteswap(desc);
    ...
    /*
     * We may recursively call __wt_oligarch_log_acquire to allocate log space for the log descriptor record.
     * Call __wt_oligarch_log_fill to write it, but we do not need to call __wt_oligarch_log_release because we're not
...
__oligarch_log_openfile(WT_SESSION_IMPL *session, uint32_t id, uint32_t flags, WT_FH **fhp)
{
    ...
    if (LF_ISSET(WT_LOG_OPEN_CREATE_OK)) {
        ...
        WT_ERR(__wt_oligarch_log_filename(session, id, WT_LOG_TMPNAME, buf));
    } else {
        wtopen_flags = 0;
        WT_ERR(__wt_oligarch_log_filename(session, id, WT_LOG_FILENAME, buf));
    }
  ...
  uint16_t *versionp, bool *need_salvagep)
{
     ...
     * maximum.
     */
    if (__wt_version_defined(conn->compat_req_max) && desc->version > conn->oligarch_log_info.log_req_max)
        WT_ERR_MSG(session, WT_ERROR,
    ...
    if (__wt_version_defined(conn->compat_req_min) && desc->version < conn->oligarch_log_info.log_req_min)
        WT_ERR_MSG(session, WT_ERROR,
          WT_COMPAT_MSG_PREFIX
...
__oligarch_log_alloc_prealloc(WT_SESSION_IMPL *session, uint32_t to_num)
{
     ...
     */
    WT_RET(__oligarch_log_get_files_single(session, WT_LOG_PREPNAME, &logfiles, &logcount));
    if (logcount == 0)
        return (WT_NOTFOUND);
    ...
    WT_ERR(__wt_oligarch_log_filename(session, from_num, WT_LOG_PREPNAME, from_path));
    WT_ERR(__wt_oligarch_log_filename(session, to_num, WT_LOG_FILENAME, to_path));
    __wt_spin_lock(session, &log->log_fs_lock);
...
__oligarch_log_newfile(WT_SESSION_IMPL *session, bool conn_open, bool *created)
{
    ...
    if (create_log) {
        ...
        if (conn->hot_backup_start == 0 && !conn_open)
            log->prep_missed++;
        WT_RET(__wt_oligarch_log_allocfile(session, log->fileid, WT_LOG_FILENAME));
    }
    /*
 ...
 */
int
__wt_oligarch_log_set_version(WT_SESSION_IMPL *session, uint16_t version, uint32_t first_rec, bool downgrade,
  bool live_chg, uint32_t *lognump)
{
    ...
    WT_WITH_OLIGARCH_SLOT_LOCK(
      session, log, ret = __oligarch_log_set_version(session, version, first_rec, live_chg, downgrade));
    if (!live_chg)
        return (ret);
    WT_ERR(ret);
    /*
     ...
     * printf record.
     */
    WT_ERR(__wt_oligarch_log_printf(session, "COMPATIBILITY: Version now %" PRIu16, log->log_version));
    if (lognump != NULL)
 ...
 *     Called serially when switching slots. Can be called recursively from __oligarch_log_newfile when we
 *     change log files.
 */
int
__wt_oligarch_log_acquire(WT_SESSION_IMPL *session, uint64_t recsize, WT_LOGSLOT *slot)
{
     ...
     */
    if (F_ISSET(log, WT_LOG_FORCE_NEWFILE) || !__oligarch_log_size_fit(session, &log->alloc_lsn, recsize)) {
...
__oligarch_log_truncate_file(WT_SESSION_IMPL *session, WT_FH *log_fh, wt_off_t offset)
{
    ...
    WT_STAT_CONN_INCR(session, log_zero_fills);
    return (__wt_file_zero(session, log_fh, offset, conn->oligarch_log_info.log_file_max, WT_THROTTLE_LOG));
}
...
__oligarch_log_truncate(WT_SESSION_IMPL *session, WT_LSN *lsn, bool this_log, bool salvage_mode)
{
    ...
    WT_ERR(__oligarch_log_truncate_file(session, log_fh, __wt_lsn_offset(lsn)));
    WT_ERR(__wt_fsync(session, log_fh, true));
    ...
    if (this_log)
        goto err;
    WT_ERR(__oligarch_log_get_files(session, WT_LOG_FILENAME, &logfiles, &logcount));
    for (i = 0; i < logcount; i++) {
        WT_ERR(__wt_oligarch_log_extract_lognum(session, logfiles[i], &lognum));
        if (lognum > lsn->l.file && lognum < log->trunc_lsn.l.file) {
            ...
            if (salvage_mode) {
                 ...
                 */
                need_salvage = false;
                WT_ERR(__oligarch_log_open_verify(session, lognum, &log_fh, NULL, NULL, &need_salvage));
                if (need_salvage) {
                    WT_ASSERT(session, log_fh == NULL);
                    WT_ERR(__wt_oligarch_log_remove(session, WT_LOG_FILENAME, lognum));
                    WT_ERR(__wt_oligarch_log_allocfile(session, lognum, WT_LOG_FILENAME));
                } else
                    opened = true;
...
__wt_oligarch_log_allocfile(WT_SESSION_IMPL *session, uint32_t lognum, const char *dest)
{
    ...
    WT_ERR(__wt_oligarch_log_filename(session, tmp_id, WT_LOG_TMPNAME, from_path));
...
__wt_oligarch_log_open(WT_SESSION_IMPL *session)
{
    ...
    if (log->log_dir_fh == NULL) {
        __wt_verbose(
          session, WT_VERB_LOG, "log_open: open fh to directory %s", conn->oligarch_log_info.log_path);
        WT_RET(__wt_open(
          session, conn->oligarch_log_info.log_path, WT_FS_OPEN_FILE_TYPE_DIRECTORY, 0, &log->log_dir_fh));
    }
...
again:
    ...
    if (firstlog == UINT32_MAX) {
         ...
         */
        if (need_salvage) {
            WT_ERR(__wt_oligarch_log_remove(session, WT_LOG_FILENAME, lastlog));
            __wt_verbose_notice(
        ...
        if (memcmp(buf, zerobuf, rdlen) != 0) {
            ...
            if (!F_ISSET(conn, WT_CONN_WAS_BACKUP)) {
                ...
                if (buf_left >= sizeof(WT_LOG_RECORD)) {
                    off += p - buf;
                    WT_ERR(__oligarch_log_record_verify(session, fh, (uint32_t)off, logrec, &corrupt));
                    if (corrupt)
                        *error_offset = off;
                }
            }
 ...
 */
int
__wt_oligarch_log_scan(WT_SESSION_IMPL *session, WT_LSN *start_lsnp, WT_LSN *end_lsnp, uint32_t flags,
  ...
  void *cookie)
{
    ...
    if (start_lsnp != NULL) {
        ...
        if (__wt_lsn_offset(start_lsnp) % allocsize != 0) {
            if (LF_ISSET(WT_LOGSCAN_RECOVER | WT_LOGSCAN_RECOVER_METADATA))
                WT_ERR_MSG(session, WT_NOTFOUND, "__wt_oligarch_log_scan unaligned LSN %" PRIu32 "/%" PRIu32,
        ...
        if (start_lsnp->l.file > lastlog) {
            if (LF_ISSET(WT_LOGSCAN_RECOVER | WT_LOGSCAN_RECOVER_METADATA))
                WT_ERR_MSG(session, WT_NOTFOUND,
                  "__wt_oligarch_log_scan LSN %" PRIu32 "/%" PRIu32 " larger than biggest log file %" PRIu32,
    ...
    WT_ERR(__oligarch_log_open_verify(session, start_lsn.l.file, &log_fh, &prev_lsn, NULL, &need_salvage));
    if (need_salvage)
...
advance:
            if (__wt_lsn_offset(&rd_lsn) == log_size)
                 ...
                 */
                WT_ERR(__oligarch_log_has_hole(session, log_fh, log_size, __wt_lsn_offset(&rd_lsn),
        ...
        WT_ERR(
          __oligarch_log_fs_read(session, log_fh, __wt_lsn_offset(&rd_lsn), (size_t)allocsize, buf->mem));
        need_salvage = false;
        /*
        ...
        if (reclen > allocsize) {
            ...
            WT_ERR(
              __oligarch_log_fs_read(session, log_fh, __wt_lsn_offset(&rd_lsn), (size_t)rdup_len, buf->mem));
        ...
        if (!__oligarch_log_checksum_match(buf, reclen)) {
            ...
            if (!__oligarch_log_check_partial_write(session, buf, reclen)) {
                 ...
                 */
                WT_TRET(
                  __oligarch_log_record_verify(session, log_fh, __wt_lsn_offset(&rd_lsn), logrec, &corrupt));
                if (corrupt) {
                    need_salvage = true;
                    WT_TRET(
                      __oligarch_log_salvage_message(session, log_fh->name, "", __wt_lsn_offset(&rd_lsn)));
                }
            }
            break;
        }
...
__wt_oligarch_log_write(WT_SESSION_IMPL *session, WT_ITEM *record, WT_LSN *lsnp, uint32_t flags)
{
    ...
    if (!F_ISSET(log, WT_LOG_OPENED) || F_ISSET(conn, WT_CONN_READONLY))
        return (0);
    ip = record;
    if ((compressor = conn->oligarch_log_info.log_compressor) != NULL && record->size < log->allocsize) {
 ...
 */
static int
__oligarch_log_write_internal(WT_SESSION_IMPL *session, WT_ITEM *record, WT_LSN *lsnp, uint32_t flags)
{
    WT_CONNECTION_IMPL *conn;
    WT_DECL_RET;
    WT_LOG *log;
...
err:
     ...
     */
    WT_ASSERT(
      session, ret != 0 || !LF_ISSET(WT_LOG_FLUSH) || __wt_oligarch_log_cmp(&log->write_lsn, &lsn) >= 0);
    WT_ASSERT(
      session, ret != 0 || !LF_ISSET(WT_LOG_FSYNC) || __wt_oligarch_log_cmp(&log->sync_lsn, &lsn) >= 0);
    return (ret);
}
