...
__wti_live_restore_validate_directories(WT_SESSION_IMPL *session, WTI_LIVE_RESTORE_FS *lr_fs)
{
     ...
     * of nbits=-1. If we don't live restore could see this config, think it applies to the file in
     * the destination, and never copy across the file causing data loss.
     */
