...
static int __checkpoint_log_flush(WT_SESSION_IMPL *);
static int __checkpoint_log_prepare(WT_SESSION_IMPL *);
static int __checkpoint_log_start(WT_SESSION_IMPL *);
 ...
 *     Reset checkpoint-related statistics that are tracked across the entire checkpoint.
 */
static void
 ...
 */
static int
__checkpoint_can_skip(WT_SESSION_IMPL *session, WT_CHECKPOINT_DB_CFG *ckpt_cfg)
{
    WT_CONNECTION_IMPL *conn;
    WT_TXN_GLOBAL *txn_global;
    WT_CONFIG_ITEM cval;
...
static int
__checkpoint_parse_config(
  WT_SESSION_IMPL *session, const char *cfg[], WT_CHECKPOINT_DB_CFG *ckpt_cfg)
{
    WT_CONFIG_ITEM cval;
...
__checkpoint_db_internal(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    if (logging)
        WT_ERR(__checkpoint_log_prepare(session));
    ...
    if (logging)
        WT_ERR(__checkpoint_log_start(session));

    /* Add a ten second wait to simulate checkpoint slowness. */
    tsp.tv_sec = 10;
    tsp.tv_nsec = 0;
    WT_ERR(__checkpoint_trees(session, cfg, tsp));
...
/*
 * __checkpoint_log_prepare --
 *     Log that a checkpoint is about to start.
 */
static int
__checkpoint_log_prepare(WT_SESSION_IMPL *session)
{
    WT_DECL_RET;
...
/*
 * __checkpoint_log_start --
 *     Log the start of a checkpoint.
 */
static int
__checkpoint_log_start(WT_SESSION_IMPL *session)
{
    WT_DECL_RET;
...
/*
 * __checkpoint_log_flush --
 *     Flush the logs for a checkpoint.
 */
static int
__checkpoint_log_flush(WT_SESSION_IMPL *session)
{
    WT_DECL_RET;
...
/*
 * __checkpoint_log_end --
 *     Log the end of a checkpoint.
 */
static int
__checkpoint_log_end(WT_SESSION_IMPL *session)
{
    WT_DECL_RET;
    bool idle;
 ...
 */
static int
__checkpoint_trees(WT_SESSION_IMPL *session, const char *cfg[], struct timespec tsp)
{
  ...
  WT_TXN_GLOBAL *txn_global, struct timespec *tsp, bool logging)
{
     ...
     */
    if (logging) {
        WT_ERR(__checkpoint_log_flush(session));
    }
