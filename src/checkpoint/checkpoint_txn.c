...
__checkpoint_flush_tier(WT_SESSION_IMPL *session, bool force)
{
    ...
    F_CLR_ATOMIC_64(conn, WT_CONN_TIERED_FIRST_FLUSH);
    return (0);
...
__checkpoint_prepare(WT_SESSION_IMPL *session, bool *trackingp, const char *cfg[])
{
    ...
    *trackingp = true;
    ...
    if (use_timestamp) {
        ...
        } else if (!F_ISSET_ATOMIC_64(conn, WT_CONN_RECOVERING))
  ...
  WT_BTREE *btree, WT_CKPT *ckpt, WT_CKPT *ckptbase)
{
    ...
    WT_CKPT_FOREACH (ckptbase, ckpt) {
         ...
         */
        if (F_ISSET_ATOMIC_64(conn, WT_CONN_RECOVERING) &&
          F_ISSET_ATOMIC_64(conn, WT_CONN_WAS_BACKUP)) {
...
__wt_checkpoint_tree_reconcile_update(WT_SESSION_IMPL *session, WT_TIME_AGGREGATE *ta)
{
    ...
    if (F_ISSET(session, WT_SESSION_ROLLBACK_TO_STABLE) ||
      F_ISSET_ATOMIC_64(S2C(session), WT_CONN_CLOSING_CHECKPOINT | WT_CONN_RECOVERING))
...
err:
    ...
    if (ret != 0 || WT_IS_METADATA(session->dhandle) || F_ISSET_ATOMIC_64(conn, WT_CONN_CLOSING))
        __wt_ckptlist_saved_free(session);
    else {
...
__wt_checkpoint_sync(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    if (!F_ISSET_ATOMIC_64(S2C(session), WT_CONN_CKPT_SYNC))
        return (0);
