 ...
 *
 * WARNING: Do not change the order of existing states. External code and dependencies may rely on
 * these hardcoded values. Inactive must always be 0. Other states are roughly ordered by appearance
 * in the checkpoint life cycle.
 *
 * When adding new states, always append them at the end to avoid breaking dependencies.
 */
typedef enum {
    ...
    WTI_CHECKPOINT_STATE_FLUSH_DATASOURCES,
    WTI_CHECKPOINT_STATE_SCRUB,
    WTI_CHECKPOINT_STATE_PREPARE,
    WTI_CHECKPOINT_STATE_UPDATE_METADATA,
    WTI_CHECKPOINT_STATE_PRESYNC,
    WTI_CHECKPOINT_STATE_HS_SHARED_SYNC,
    WTI_CHECKPOINT_STATE_CLEANUP
