...
__rec_row_leaf_insert(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_INSERT *ins)
{
    ...
    for (; ins != NULL; ins = WT_SKIP_NEXT(ins)) {
        ...
        case WT_UPDATE_MODIFY:
             ...
             */
            cbt->slot = UINT32_MAX;
            WT_ERR(__wt_modify_reconstruct_from_upd_list(session, cbt, upd, cbt->upd_value, true));
    ...
    WT_ROW_FOREACH (page, rip, i) {
        ...
        if (upd == NULL) {
            ...
            case WT_UPDATE_MODIFY:
                ...
                WT_ERR(
                  __wt_modify_reconstruct_from_upd_list(session, cbt, upd, cbt->upd_value, true));
                __wt_value_return(cbt, cbt->upd_value);
                WT_ERR(__wt_rec_cell_build_val(
