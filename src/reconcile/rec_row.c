...
__rec_row_leaf_insert(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_INSERT *ins)
{
    ...
    for (; ins != NULL; ins = WT_SKIP_NEXT_ATOMIC(ins)) {
  ...
  WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_REF *pageref, WT_SALVAGE_COOKIE *salvage)
{
     ...
     */
    if ((ins = WT_SKIP_FIRST_ATOMIC(WT_ROW_INSERT_SMALLEST(page))) != NULL)
...
leaf_insert:
        ...
        if ((ins = WT_SKIP_FIRST_ATOMIC(WT_ROW_INSERT(page, rip))) != NULL)
