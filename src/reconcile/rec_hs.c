...
#define MAX_REVERSE_MODIFY_NUM 16
    ...
    for (i = 0, list = multi->supd; i < multi->supd_entries; ++i, ++list) {
        ...
        for (upd = list->onpage_upd, prev_upd = NULL; upd != NULL; upd = upd->next) {
            ...
            if (prev_upd != NULL && get_upd_start_ts(prev_upd) < get_upd_start_ts(upd)) {
                WT_ASSERT_ALWAYS(session, get_upd_start_ts(prev_upd) == WT_TS_NONE,
            ...
            if (prev_upd != NULL && prev_upd->txnid == upd->txnid &&
              get_upd_start_ts(prev_upd) == get_upd_start_ts(upd))
                enable_reverse_modify = false;
             ...
             */
            if (no_ts_upd == NULL && get_upd_start_ts(upd) == WT_TS_NONE) {
        ...
        if (!hs_flag_set && oldest_upd->type == WT_UPDATE_TOMBSTONE &&
          get_upd_start_ts(oldest_upd) == WT_TS_NONE) {
        ...
        for (;; tmp = full_value, full_value = prev_full_value, prev_full_value = tmp,
            ...
            if (no_ts_upd != NULL) {
                ...
                tw.start_ts = get_upd_start_ts(upd);
            }
            ...
            if (prev_upd->prepare_state == WT_PREPARE_INPROGRESS) {
                WT_ASSERT(session,
                  list->onpage_upd->txnid == prev_upd->txnid &&
                    get_upd_start_ts(list->onpage_upd) == get_upd_start_ts(prev_upd));
                 ...
                 */
                WT_ASSERT(session, get_upd_start_ts(prev_upd) <= prev_upd->upd_durable_ts);
                ...
                if (no_ts_upd != NULL) {
                    ...
                    tw.stop_ts = get_upd_start_ts(prev_upd);
                }
  ...
  WT_SESSION_IMPL *session, WTI_RECONCILE *r, WT_ITEM *key, WT_UPDATE *upd, WT_UPDATE *tombstone)
{
    ...
    if (ret == WT_NOTFOUND) {
        ...
        if (hs_tw->stop_ts <= get_upd_start_ts(upd))
            goto done;
        ...
        if (EXTRA_DIAGNOSTICS_ENABLED(session, WT_DIAGNOSTIC_HS_VALIDATE)) {
            ...
            WT_ASSERT_ALWAYS(session,
              hs_tw->start_ts == WT_TS_NONE || hs_tw->start_ts == get_upd_start_ts(upd),
            ...
            if (tombstone != NULL) {
                ...
                WT_ASSERT_ALWAYS(session, hs_tw->stop_ts == get_upd_start_ts(tombstone),
