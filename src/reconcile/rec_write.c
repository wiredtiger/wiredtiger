...
static bool
__rec_split_write_reuse(
  WT_SESSION_IMPL *session, WTI_RECONCILE *r, WT_MULTI *multi, WT_ITEM *image, bool last_block)
{
    ...
    WT_STAT_CONN_DSRC_INCR(session, rec_page_match);
    return (true);
}
...
__rec_split_discard(WT_SESSION_IMPL *session, WTI_RECONCILE *r, WT_PAGE *page)
{
    ...
    for (multi = mod->mod_multi, i = 0; i < mod->mod_multi_entries; ++multi, ++i) {
         ...
         */
        if (multi->addr.block_cookie != NULL && !multi->addr.reuse) {
...
__rec_write_err(WT_SESSION_IMPL *session, WTI_RECONCILE *r, WT_PAGE *page)
{
    ...
    for (multi = r->multi, i = 0; i < r->multi_next; ++multi, ++i) {
        if (multi->addr.block_cookie != NULL) {
             ...
             */
            else if (page->disagg_info == NULL || F_ISSET(r, WT_REC_DISAGG_NEW_PAGE)) {
