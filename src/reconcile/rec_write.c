...
__rec_should_save_disk_image(WT_SESSION_IMPL *session, WTI_RECONCILE *r)
{
     ...
     * accumulated image bytes (no overhead factor) against the same fraction of cache_size, so the
     * bound matches the config knob directly.
     */
    WT_CONNECTION_IMPL *conn = S2C(session);
    uint8_t max_pct =
      __wt_atomic_load_uint8_relaxed(&conn->cache->cache_eviction_controls.clean_scrub_max_pct);
    if (max_pct > 0) {
        uint64_t saved = __wt_atomic_load_uint64_relaxed(&conn->cache->bytes_clean_scrub_image);
