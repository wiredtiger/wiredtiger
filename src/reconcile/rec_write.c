  ...
  void *reconcilep)
{
    ...
    if (LF_ISSET(WT_REC_VISIBLE_NO_SNAPSHOT)) {
        ...
        /*
         * History store table eviction is configured when eviction gets aggressive, adjust the
         * flags for cases we don't support.
         */
        ...
        /*
         * Dictionary compression only writes repeated values once. We grow the dictionary as
         * necessary, always using the largest size we've seen.
         *
        ...
        if (btree->dictionary != 0 && btree->dictionary > r->dictionary_slots)
            WT_ERR(__wti_rec_dictionary_init(
              session, r, btree->dictionary < 100 ? 100 : btree->dictionary));
         ...
         * necessary for tree navigation. We don't do suffix compression if there is a custom
         * collator because we don't know what bytes a custom collator might use. Some custom
         * collators (for example, a collator implementing reverse ordering of strings), won't have
         * any problem with suffix compression: if there's ever a reason to implement suffix
         * compression for custom collators, we can add a setting to the collator, configured when
         * the collator is added, that turns on suffix compression.
         */
        ...
        r->cache_write_hs = r->cache_write_restore_invisible = r->cache_upd_chain_all_aborted =
          false;

        /*
         * The fake cursor used to figure out modified update values points to the enclosing WT_REF
         * as a way to access the page, and also needs to set the format.
         */
        ...
        /*
         * When removing a key due to a tombstone with a durable timestamp of "none", also remove
         * the history store contents associated with that key. It's safe to do even if we fail
         * reconciliation after the removal, the history store content must be obsolete in order for
         * us to consider removing the key.
         *
         * Ignore if this is metadata, as metadata doesn't have any history.
         *
         * Some code paths, such as schema removal, involve deleting keys in metadata and assert
         * that they shouldn't open new dhandles. In those cases we won't ever need to blow away
         * history store content, so we can skip this.
         */
...
err:
     ...
     */
    static int __rec_cleanup(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_BTREE *btree;
        WT_MULTI *multi;
        uint32_t i;
     ...
     *     Clean up the reconciliation structure.
     */
    static int __rec_destroy(WT_SESSION_IMPL * session, void *reconcilep)
    {
        WTI_RECONCILE *r;
     ...
     */
    static int __rec_write(WT_SESSION_IMPL * session, WT_ITEM * buf,
      WT_PAGE_BLOCK_META * block_meta, uint8_t * addr, size_t * addr_sizep,
      size_t * compressed_sizep, bool checkpoint, bool checkpoint_io, bool compressed)
    {
        ...
        if (EXTRA_DIAGNOSTICS_ENABLED(session, WT_DIAGNOSTIC_DISK_VALIDATE)) {
            ...
            /*
             * We're passed a table's disk image. Decompress if necessary and verify the image.
             * Always check the in-memory length for accuracy.
             */
            if (compressed) {
                ...
                /*
                 * Return an error rather than assert because the test suite tests that the error
                 * hits.
                 */
     ...
     */
    static WT_INLINE uint32_t __rec_leaf_page_max_slvg(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_BTREE *btree;
        WT_PAGE *page;
        uint32_t page_size;
        ...
        case WT_PAGE_COL_FIX:
            /*
             * Column-store pages can grow if there are missing records (that is, we lost a chunk of
             * the range, and have to write deleted records). Fixed-length objects are a problem, if
             * there's a big missing range, we could theoretically have to write large numbers of
             * missing objects.
             *
             * The code in rec_col.c already figured this out for us, including both space for
             * missing chunks of the namespace and space for time windows, so we will take what it
             * says. Thus, we shouldn't come here.
             */
        ...
        case WT_PAGE_COL_VAR:
            /*
             * Column-store pages can grow if there are missing records (that is, we lost a chunk of
             * the range, and have to write deleted records). Variable-length objects aren't usually
             * a problem because we can write any number of deleted records in a single page entry
             * because of the RLE, we just need to ensure that additional entry fits.
             */
            break;
        case WT_PAGE_ROW_LEAF:
        default:
            /*
             * Row-store pages can't grow, salvage never does anything other than reduce the size of
        ...
        /*
         * The page we read from the disk should be smaller than the page size we just calculated,
    ...
    uint32_t __wt_split_page_size(int split_pct, uint32_t maxpagesize, uint32_t allocsize)
    {
        ...
        /*
         * Respect the configured split percentage if the calculated split size is either zero or a
         * full page. The user has either configured an allocation size that matches the page size,
         * or a split percentage that is close to zero or one hundred. Rounding is going to provide
         * a worse outcome than having a split point that doesn't fall on an allocation size
     ...
     */
    static int __rec_split_chunk_init(
      WT_SESSION_IMPL * session, WTI_RECONCILE * r, WTI_REC_CHUNK * chunk)
    {
...
#ifdef HAVE_DIAGNOSTIC
        /*
         * For fixed-length column-store, poison the rest of the buffer. This helps verify ensure
         * that all the bytes in the buffer are explicitly set and not left uninitialized.
         */
     ...
     */
    int __wti_rec_split_init(WT_SESSION_IMPL * session, WTI_RECONCILE * r, WT_PAGE * page,
      uint64_t recno, uint64_t primary_size, uint32_t auxiliary_size)
    {
        ...
        /*
         * The maximum leaf page size governs when an in-memory leaf page splits into multiple
         * on-disk pages; however, salvage can't be allowed to split, there's no parent page yet. If
         * we're doing salvage, override the caller's selection of a maximum page size, choosing a
         * page size that ensures we won't split.
         *
         ...
         * placed. r->page_size is the complete page size; we'll use r->space_avail to track how
         * much more primary space is remaining, and r->aux_space_avail to track how much more
         * auxiliary space there is.
         *
         * Because (for FLCS) we need to start writing time windows into the auxiliary space before
         * we know for sure how much bitmap data there is, we always start the time window data at a
         * fixed offset from the page start: the place where it goes naturally if the page is full.
         * If the page is not full (and there was at least one timestamp to write), we waste the
         * intervening unused space. Odd-sized pages are supposed to be rare (ideally only the last
         * page in the tree, though currently there are some other ways they can appear) so only a
         * few KB is wasted and not enough to be particularly concerned about.
         *
         * For FLCS, primary_size will always be the tree's configured maximum leaf page size,
         * except for pages created or rewritten during salvage, which might be larger. (This is not
         * ideal, because once created larger they cannot be split again later, but for the moment
         * at least it isn't readily avoided.)
         */
         ...
         * pick the minimum size either, because that penalizes an application that did a bulk load
         * and subsequently inserted a few items into packed pages. Currently defaulted to 75%, but
         * I have no empirical evidence that's "correct".
         *
         * The maximum page size may be a multiple of the split page size (for example, there's a
         * maximum page size of 128KB, but because the table is active and we don't want to split a
         * lot, the split size is 20KB). The maximum page size may NOT be an exact multiple of the
         * split page size.
         *
         * It's lots of work to build these pages and don't want to start over when we reach the
         * maximum page size (it's painful to restart after creating overflow items and compacted
         * data, for example, as those items have already been written to disk). So, the loop calls
         * the helper functions when approaching a split boundary, and we save the information at
         * that point. We also save the boundary information at the minimum split size. We maintain
         * two chunks (each boundary represents a chunk that gets written as a page) in the memory,
         * writing out the older one to the disk as a page when we need to make space for a new
         * chunk. On reaching the last chunk, if it turns out to be smaller than the minimum split
         * size, we go back into the penultimate chunk and split at this minimum split size
         * boundary. This moves some data from the penultimate chunk to the last chunk, hence
         * increasing the size of the last page written without decreasing the penultimate page size
         * beyond the minimum split size.
         *
         * FLCS pages are different, because they have two pieces: bitmap data ("primary") and time
         * window data ("auxiliary"); the bitmap data is supposed to be a fixed amount per page.
         * FLCS pages therefore split based on the bitmap size, and the time window data comes along
         * for the ride no matter how large it is. If the time window data gets larger than expected
         * (it can at least in theory get rather large), we have to realloc the page image.
         *
        ...
        if (page->type == WT_PAGE_COL_FIX) {
            ...
            /* FIXME-WT-14881: Temporary hack to ensure we don't run out of space when rewriting
             * deltas.
             */
         ...
         * adjustments based on the compression), this buffer should be the greater of split_size
         * and page_size, then aligned to the next allocation size boundary. The latter shouldn't be
         * an issue, but it's a possible scenario if, for example, the compression engine is
         * expected to give us 5x compression and gives us nothing at all.
         */
     ...
     */
    static bool __rec_is_checkpoint(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_BTREE *btree;
         ...
         *
         * Any time we write the root page of the tree without splitting we are creating a
         * checkpoint (and have to tell the underlying block manager so it creates and writes the
         * additional information checkpoints require). However, checkpoints are completely
         * consistent, and so we have to resolve information about the blocks we're expecting to
         * free as part of the checkpoint, before writing the checkpoint. In short, we don't do
         * checkpoint writes here; clear the boundary information as a reminder and create the
         * checkpoint during wrapup.
         */
        return (
    ...
    static int __rec_split_row_promote(
      WT_SESSION_IMPL * session, WTI_RECONCILE * r, WT_ITEM * key, uint8_t type)
    {
        WT_BTREE *btree;
        WT_DECL_ITEM(update);
        WT_DECL_RET;
         ...
         *
         * This function is called from the split code at each split boundary, but that means we're
         * not called before the first boundary, and we will eventually have to get the first key
         * explicitly when splitting a page.
         *
         * For the current slot, take the last key we built, after doing suffix compression. The
         * "last key we built" describes some process: before calling the split code, we must place
         * the last key on the page before the boundary into the "last" key structure, and the first
         * key on the page after the boundary into the "current" key structure, we're going to
         * compare them for suffix compression.
         *
         * Suffix compression is a hack to shorten keys on internal pages. We only need enough bytes
         * in the promoted key to ensure searches go to the correct page: the promoted key has to be
         * larger than the last key on the leaf page preceding it, but we don't need any more bytes
         * than that. In other words, we can discard any suffix bytes not required to distinguish
         * between the key being promoted and the last key on the leaf page preceding it. This can
         * only be done for the first level of internal pages, you cannot repeat suffix truncation
         * as you split up the tree, it loses too much information.
         *
         * Note #1: if the last key on the previous page was an overflow key, we don't have the
         * in-memory key against which to compare, and don't try to do suffix compression. The code
         * for that case turns suffix compression off for the next key, we don't have to deal with
         * it here.
         */
        ...
        /*
         * Note #2: if we skipped updates, an update key may be larger than the last key stored in
         * the previous block (probable for append-centric workloads). If there are skipped updates
         * and we cannot evict the page, check for one larger than the last key and smaller than the
         * current key.
         */
        max = r->last;
        if (r->cache_write_restore_invisible)
            for (i = r->supd_next; i > 0; --i) {
                ...
                /*
                 * The saved updates are in key-sort order so the entry we're looking for is either
                 * the last or the next-to- last one in the list. Once we've compared an entry
        ...
        /*
         * The largest key on the last block must sort before the current key, so we'll either find
         * a larger byte value in the current key, or the current key will be a longer key, and the
...
err:
     ...
     */
    int __wti_rec_split_grow(WT_SESSION_IMPL * session, WTI_RECONCILE * r, size_t add_len)
    {
        WT_BM *bm;
        WT_BTREE *btree;
     ...
     */
    static void __rec_split_fix_shrink(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
         ...
         * auxiliary data, on the assumption that the cost of the memmove is outweighed by the cost
         * of taking checksums of, writing out, and reading back in a bunch of useless empty space.
         */
...
#define WT_PAGE_INTL_MINIMUM_ENTRIES 20
    ...
    /*
     * __wti_rec_split --
     *     Handle the page reconciliation bookkeeping. (Did you know "bookkeeper" has 3 doubled
     *     letters in a row? Sweet-tooth does, too.)
     */
    int __wti_rec_split(WT_SESSION_IMPL * session, WTI_RECONCILE * r, size_t next_len)
    {
        WT_BTREE *btree;
        WTI_REC_CHUNK *tmp;
        size_t inuse;
        ...
        /*
         * We should never split during salvage, and we're about to drop core because there's no
         * parent page.
         */
        if (r->salvage != NULL)
        ...
        /*
         * We can get here if the first key/value pair won't fit. Grow the buffer to contain the
         * current item if we haven't already consumed a reasonable portion of a split chunk. This
         * logic should not trigger for FLCS, because FLCS splits happen at very definite places;
         * and if it does, the interaction between here and there will corrupt the database, so
         * assert otherwise.
         *
         * If we're promoting huge keys into an internal page, we might be about to write an
         * internal page with too few items, which isn't good for tree depth or search. Grow the
         * buffer to contain the current item if we don't have enough items to split an internal
         * page.
         */
        ...
        if (r->page->type == WT_PAGE_COL_FIX) {
            /*
             * In the first chunk, we use the passed-in primary size, whatever it is, as the size
             * for the bitmap data; the auxiliary space follows it. It might be larger than the
             * configured maximum leaf page size if we're in salvage. For the second and subsequent
             * chunks, we aren't in salvage so always use the maximum leaf page size; that will
             * produce the fixed size pages we want.
             */
        ...
        if (r->page->type == WT_PAGE_COL_FIX) {
            r->aux_space_avail =
              r->page_size - btree->maxleafpage - WT_COL_FIX_AUXHEADER_RESERVATION;
        } else
            r->min_space_avail = r->min_split_size - WT_PAGE_HEADER_BYTE_SIZE(btree);

done:
        /*
         * We may have declined the split as described above, in which case grow the buffer based on
         * the next key/value pair's length. In the internal page minimum-key case, we could grow
         * more than a single key/value pair's length to avoid repeatedly calling this function, but
         * we'd prefer not to have internal pages that are larger than they need to be, and
         * repeatedly trying to split means we will split as soon as we can.
         *
         * Also, overflow values can be larger than the maximum page size but still be "on-page". If
         * the next key/value pair is larger than space available after a split has happened (in
         * other words, larger than the maximum page size), create a page sized to hold that one
         * key/value pair. This generally splits the page into key/value pairs before a large
         * object, the object, and key/value pairs after the object. It's possible other key/value
         * pairs will also be aggregated onto the bigger page before or after, if the page happens
         * to hold them, but it won't necessarily happen that way.
         */
     ...
     */
    int __wti_rec_split_crossing_bnd(WT_SESSION_IMPL * session, WTI_RECONCILE * r, size_t next_len)
    {
        /*
         * If crossing the minimum split size boundary, store the boundary details at the current
         * location in the buffer. If we are crossing the split boundary at the same time, possible
         * when the next record is large enough, just split at this point.
         */
        if (WTI_CROSSING_MIN_BND(r, next_len) && !WTI_CROSSING_SPLIT_BND(r, next_len) &&
          !__wti_rec_need_split(r, 0)) {
            /*
             * If the first record doesn't fit into the minimum split size, we end up here. Write
             * the record without setting a boundary here. We will get the opportunity to setup a
             * boundary before writing out the next record.
             */
            if (r->entries == 0)
                return (0);
            ...
            WT_ASSERT_ALWAYS(session, r->cur_ptr->min_offset == 0,
              "Trying to re-enter __wti_rec_split_crossing_bnd");
     ...
     *     If the two split chunks together fit in a single page, merge them into one. If they do
     *     not fit in a single page but the last is smaller than the minimum desired, move some data
     *     from the penultimate chunk to the last chunk and write out the previous/penultimate.
     *     Finally, update the pointer to the current image buffer. After this function exits, we
     *     will have one (last) buffer in memory, pointed to by the current image pointer.
     */
    static int __rec_split_finish_process_prev(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_BTREE *btree;
        WT_PAGE_HEADER *dsk;
        ...
        WT_ASSERT_ALWAYS(
          session, r->prev_ptr != NULL, "Attempting to merge with non-existing chunk");
        ...
        /*
         * The sizes in the chunk include the header, so when calculating the combined size, be sure
         * not to include the header twice.
         */
        combined_size =
          prev_ptr->image.size + (cur_ptr->image.size - WT_PAGE_HEADER_BYTE_SIZE(btree));
        ...
        if (combined_size <= r->page_size) {
            ...
            /*
             * At this point, there is only one disk image in the memory, the previous chunk. Update
             * the current chunk to that chunk, discard the unused chunk.
             */
            tmp = r->prev_ptr;
            r->prev_ptr = r->cur_ptr;
            r->cur_ptr = tmp;
        ...
        if (prev_ptr->min_offset != 0 && cur_ptr->image.size < r->min_split_size) {
            ...
            /*
             * The last chunk, pointed to by the current image pointer, has less than the minimum
             * data. Let's move any data more than the minimum from the previous image into the
             * current.
             *
             ...
             * prepended into the current buffer. Copy the data from the previous buffer to the
     ...
     */
    int __wti_rec_split_finish(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        /*
         * We're done reconciling, write the final page. We may arrive here with no entries to write
         * if the page was entirely empty or if nothing on the page was visible to us.
         *
         * Pages with skipped or not-yet-globally visible updates aren't really empty; otherwise,
         * the page is truly empty and we will merge it into its parent during the parent's
         * reconciliation.
         *
         * Checkpoint never writes uncommitted changes to disk and only saves the updates to move
         * older updates to the history store. Thus it can consider the reconciliation done if there
         * are no more entries left to write. This will also remove its reference entry from its
         * parent.
         */
         ...
         *
         * Skip for FLCS because (a) pages can be combined only if the combined bitmap data size is
         * in range, not the overall page size (which requires entirely different logic) and (b)
         * this cannot happen because we only split when we've fully filled the previous page. This
         * is true even when in-memory splits give us odd page sizes to work with -- some of those
         * might be mergeable (though more likely not) but we can't see them on this code path. So
         * instead just write the previous chunk out.
         */
        if (r->prev_ptr != NULL) {
            if (r->page->type != WT_PAGE_COL_FIX)
     ...
     *     structure.
     */
    static int __rec_split_write_supd(WT_SESSION_IMPL * session, WTI_RECONCILE * r,
      WTI_REC_CHUNK * chunk, WT_MULTI * multi, bool last_block)
    {
        WT_BTREE *btree;
        WT_DECL_ITEM(key);
        WT_DECL_RET;
         ...
         * structure.
         *
         * This code requires a key be filled in for the next block (or the last block flag be set,
         * if there's no next block).
         *
         ...
         * list belongs with the block we're about to write, move it to the per-block memory. Check
         * only to the first update that doesn't go with the block, they must be in sorted order.
         *
        ...
        if (i != 0) {
            ...
            /*
             * If there are updates that weren't moved to the block, shuffle them to the beginning
             * of the cached list (we maintain the saved updates in sorted order, new saved updates
...
err:
     ...
     */
    static void __rec_set_page_write_gen(WT_BTREE * btree, WT_PAGE_HEADER * dsk)
    {
        /*
         * We increment the block's write generation so it's easy to identify newer versions of
         * blocks during salvage. (It's common in WiredTiger, at least for the default block
         * manager, for multiple blocks to be internally consistent with identical first and last
         * keys, so we need a way to know the most recent state of the block. We could check which
         * leaf is referenced by a valid internal page, but that implies salvaging internal pages,
         * which I don't want to do, and it's not as good anyway, because the internal page may not
         * have been written after the leaf page was updated. So, write generations it is.
         *
         * The write generation number should be increased atomically to prevent it from moving
         * backward when it is updated simultaneously.
         *
         * Other than salvage, the write generation number is used to reset the stale transaction
         * id's present on the page upon server restart.
         */
     ...
     */
    static void __rec_split_write_header(WT_SESSION_IMPL * session, WTI_RECONCILE * r,
      WTI_REC_CHUNK * chunk, WT_MULTI * multi, WT_PAGE_HEADER * dsk)
    {
        WT_BTREE *btree;
        WT_PAGE *page;
     ...
     */
    static WT_INLINE void __rec_compression_adjust(WT_SESSION_IMPL * session, uint32_t max,
      size_t compressed_size, bool last_block, uint64_t *adjustp)
    {
        WT_BTREE *btree;
        ...
        if (compressed_size > max) {
            /*
             * The compressed size is GT the page maximum. Check if the pre-compression size is
             * larger than the maximum. If 10% of the page size larger than the maximum, decrease it
             * by that amount. Else if it's not already at the page maximum, set it there.
             *
             * Note we're using 10% of the maximum page size as our test for when to adjust the
             * pre-compression size as well as the amount by which we adjust it. Not updating the
             * value when it's close to the page size keeps us from constantly updating a shared
             * memory location, and 10% of the page size is an OK step value as well, so we use it
             * in both cases.
             */
            adjust = current - max;
             ...
             *
             * Don't increase the pre-compressed size on the last block, the last block might be
             * tiny.
             *
             * If the compressed size is less than the page maximum by 10%, increase the
             * pre-compression size by 10% of the page, or up to the maximum in-memory image size.
             *
     ...
     *     Build delta init.
     */
    int __wti_rec_build_delta_init(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
     ...
     *     Pack the delta key
     */
    static WT_INLINE int __rec_delta_pack_key(WT_SESSION_IMPL * session, WT_BTREE * btree,
     ...
     */
    int __wti_rec_pack_delta_internal(
     ...
     */
    static int __rec_pack_delta_leaf(
      WT_SESSION_IMPL * session, WTI_RECONCILE * r, WT_SAVE_UPD * supd)
    {
        WT_CURSOR_BTREE *cbt;
        WT_DECL_RET;
        WT_ITEM *key, value;
        ...
        if (supd->onpage_upd == NULL) {
             ...
             * write the full value and in this reconciliation, it is deleted by a tombstone. Should
             * we still include the full value in the delta? We can omit it but it will make the
             * rest of the system more complicated. Include it for now to simplify the prototype.
             */
...
err:
     ...
     */
    static int __rec_build_delta_leaf(
      WT_SESSION_IMPL * session, WT_PAGE_HEADER * full_image, WTI_RECONCILE * r)
    {
        WT_MULTI *multi;
        WT_PAGE_HEADER *header;
        WT_SAVE_UPD *supd;
     ...
     */
    static void __rec_set_updates_durable(WT_SESSION_IMPL * session, WT_MULTI * multi)
    {
        WT_SAVE_UPD *supd;
        uint32_t i;
        ...
        /*
         * FIXME-WT-14882: we should rethink where we should call this. Is this safe to call this
         * right after we have called the write function of PALI? What will happen if we fail after
         * the write and before we call this function or if we fail after calling this function.
         *
         * Instead of thinking all this failure cases, we may be better off to always write a full
         * page in the next reconciliation if this reconciliation fail.
         */
        for (i = 0, supd = multi->supd; i < multi->supd_entries; ++i, ++supd) {
            if (supd->onpage_upd == NULL && supd->onpage_tombstone == NULL)
                continue;
     ...
     */
    static int __rec_write_delta(WT_SESSION_IMPL * session, WTI_RECONCILE * r,
      WTI_REC_CHUNK * chunk, uint8_t * addr, size_t * addr_sizep, size_t * compressed_sizep)
    {
        WT_CONNECTION_IMPL *conn;
        WT_MULTI *multi;
     ...
     */
    static int __rec_write_image(WT_SESSION_IMPL * session, WTI_RECONCILE * r,
      WTI_REC_CHUNK * chunk, uint8_t * addr, size_t * addr_sizep, size_t * compressed_sizep,
      bool last_block)
    {
        WT_MULTI *multi;
        ...
        if (page->disagg_info != NULL) {
            ...
            if (last_block && block_meta != NULL && r->multi_next == 1 &&
              block_meta->page_id != WT_BLOCK_INVALID_PAGE_ID) {
                *multi->block_meta = *block_meta;
                /*
                 * Full page's backlink is the previous full page. If the previous page is a delta,
                 * use the base as the new backlink. Otherwise, use the previous page as the
                 * backlink.
                 */
                if (multi->block_meta->delta_count > 0) {
     ...
     */
    static int __rec_copy_prev_addr(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_MULTI *multi;
        WT_PAGE *page;
        WT_PAGE_MODIFY *mod;
    ...
    static int __rec_split_write(
      WT_SESSION_IMPL * session, WTI_RECONCILE * r, WTI_REC_CHUNK * chunk, bool last_block)
    {
        WT_BTREE *btree;
        WT_CONNECTION_IMPL *conn;
        WT_MULTI *multi;
        ...
        /*
         * If reconciliation requires multiple blocks and checkpoint is running we'll eventually
         * fail, unless we're the checkpoint thread. Big pages take a lot of writes, avoid wasting
         * work.
         */
        ...
        if (btree->type == BTREE_ROW)
            WT_RET(
              __wt_row_ikey_alloc(session, 0, chunk->key.data, chunk->key.size, &multi->key.ikey));
        else
         ...
         *
         * Track the buffer with the image. (This is bad layering, but we can't write the image
         * until the wrapup code, and we don't have a code path from here to there.)
         */
        if (last_block && r->multi_next == 1 && __rec_is_checkpoint(session, r)) {
            WT_ASSERT_ALWAYS(session, r->supd_next == 0,
              "Attempting to write final block but further updates found");
         ...
         *
         * If we are rewriting a page restored from delta, no need to write it but directly
         * instantiate it into memory.
         */
        ...
        if (F_ISSET(r, WT_REC_EVICT) && multi->supd != NULL) {
             ...
             *
             * We need to write the disk image for btrees with delta enabled as a later
             * reconciliation may build a delta that is based on a page image that was never written
             * to disk.
             */
        ...
        if (page->disagg_info != NULL) {
            block_meta = &page->disagg_info->block_meta;
            if (WT_DELTA_ENABLED_FOR_PAGE(session, r->page->type) && last_block &&
              r->multi_next == 1 && block_meta->page_id != WT_BLOCK_INVALID_PAGE_ID &&
        ...
        if (WT_PAGE_IS_INTERNAL(page) && compressed_size != 0 && btree->intlpage_compadjust)
            __rec_compression_adjust(session, btree->maxintlpage, compressed_size, last_block,
              &btree->maxintlpage_precomp);
        if (!WT_PAGE_IS_INTERNAL(page) && compressed_size != 0 && btree->leafpage_compadjust)
            __rec_compression_adjust(session, btree->maxleafpage, compressed_size, last_block,
...
copy_image:
#ifdef HAVE_DIAGNOSTIC
        /*
         * The I/O routines verify all disk images we write, but there are paths in reconciliation
         * that don't do I/O. Verify those images, too.
         */
        WT_ASSERT(session,
          verify_image == false ||
         ...
         * rewrite the pages with deltas, or because we skipped updates to build the disk image),
    ...
    int __wt_bulk_init(WT_SESSION_IMPL * session, WT_CURSOR_BULK * cbulk)
    {
        ...
        /*
         * Bulk-load is only permitted on newly created files, not any empty file -- see the
         * checkpoint code for a discussion.
         */
        if (!btree->original)
            WT_RET_MSG(session, EINVAL, "bulk-load is only possible for newly created trees");

        /*
         * Get a reference to the empty leaf page; we have exclusive access so we can take a copy of
         * the page, confident the parent won't split.
         */
        ...
        return (
          __wti_rec_split_init(session, r, cbulk->leaf, recno, btree->maxleafpage_precomp, 0));
    }
...
err:
    ...
    static int __rec_split_discard(WT_SESSION_IMPL * session, WT_PAGE * page)
    {
        ...
        /*
         * A page that split is being reconciled for the second, or subsequent time; discard
         * underlying block space used in the last reconciliation that is not being reused for this
         * reconciliation.
         */
         ...
         * support the split, except for root splits. In the case of root splits, we have to cope
         * with multiple pages in a linked list, and we also have to discard overflow items written
         * for the page.
         */
     ...
     */
    static int __rec_split_dump_keys(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_BTREE *btree;
        WT_DECL_ITEM(tkey);
        WT_MULTI *multi;
     ...
     *     Any thread that is reviewing the page modify time aggregate in a WT_REF, must also be
     *     holding a split generation to ensure that the page index they are using remains valid.
     *     Use that same split generation to ensure that the page modify time aggregate inside the
     *     WT_REF remains valid while it is being reviewed.
     */
    static void __rec_page_modify_ta_safe_free(WT_SESSION_IMPL * session, WT_TIME_AGGREGATE * *ta)
    {
        WT_DECL_RET;
        uint64_t split_gen;
        void *p;
     ...
     */
    static int __rec_write_wrapup(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_BM *bm;
        WT_BTREE *btree;
        WT_DECL_RET;
         ...
         * some action to resolve the list of allocated/free/whatever blocks that are associated
         * with the checkpoint).
         */
        ...
        case 0: /*
                 ...
                 *
                 * FIXME-WT-14700: Does the root work for the same way in disagg? Do we need a
                 * separate API to tell the SLS that we are discarding a root page?
                 */
            if (__wt_ref_is_root(ref))
                break;
        ...
        case WT_PM_REC_REPLACE: /* 1-for-1 page swap */
                                 ...
                                 *
                                 * FIXME-WT-14700: Does the root work for the same way in disagg? Do
                                 * we need a separate API to tell the SLS that we are discarding a
                                 * root page?
                                 */
            if (!__wt_ref_is_root(ref)) {
                ...
                if (WT_DELTA_LEAF_ENABLED(session) && mod->mod_replace.block_cookie == NULL) {
                    /*
                     * We need to retain the block address if we skipped writing an empty delta
                     * again. Free the block address otherwise if it is available.
                     */
                    if (ref->addr != NULL) {
                        bool empty_delta =
                          r->multi_next == 1 && r->multi->addr.block_cookie == NULL;
                        if (!empty_delta)
                            WT_RET(__wt_ref_block_free(session, ref, r->multi_next == 1));
                    }
                } else
        ...
        /*
         * When the page is being reconciled as part of the checkpoint operation, the REF is not
         * locked. Concurrent access to the page can be enabled by safe-releasing the time aggregate
         * information.
         */
        __rec_page_modify_ta_safe_free(session, &mod->stop_ta);
        WT_TIME_AGGREGATE_INIT_MERGE(&stop_ta);

        __wt_verbose(session, WT_VERB_RECONCILE, "%p reconciled into %" PRIu32 " pages",
        ...
        case 0: /* Page delete */
            ...
            /*
             * If this is the root page, we need to create a sync point. For a page to be empty, it
             * has to contain nothing at all, which means it has no records of any kind and is
             * durable.
             *
             * FIXME-WT-14884: we need to check with the page service team if we need to write an
             * empty root page.
             */
            if (__wt_ref_is_root(ref)) {
            ...
            /*
             * If the page was empty, we want to discard it from the tree by discarding the parent's
             * key when evicting the parent. Mark the page as deleted, then return success, leaving
             * the page in memory. If the page is subsequently modified, that is OK, we'll just
             * reconcile it again.
             */
        ...
        case 1: /* 1-for-1 page swap */
            ...
            /*
             * We may have a root page, create a sync point. (The write code ignores root page
            ...
            if (r->wrapup_checkpoint == NULL) {
                if (r->multi->addr.block_cookie != NULL || F_ISSET(r, WT_REC_REWRITE_DELTA)) {
                    ...
                    if (page->disagg_info != NULL)
                        page->disagg_info->block_meta = *r->multi->block_meta;
                    WT_TIME_AGGREGATE_MERGE_OBSOLETE_VISIBLE(
                      session, &stop_ta, &mod->mod_replace.ta);
                } else
                    WT_ASSERT(session,
                      WT_DELTA_ENABLED_FOR_PAGE(session, page->type) && r->ref->addr != NULL);
            } else {
        ...
        default: /* Page split */
             ...
             * instead of reusing the existing page id. Building deltas on the split page is a
             * future thing.
             */
            if (page->disagg_info != NULL)
...
split:
        ...
        /*
         * If the page has post-instantiation delete information, we don't need it any more. Note:
         * this is the only place in the system that potentially touches ref->page_del without
         * locking the ref. There are two other pieces of code it can interact with: transaction
         * rollback and parent internal page reconciliation. We use __wt_free_page_del here and in
         * transaction rollback to make the deletion atomic. Reconciliation of the parent is locked
         * out for the following reasons: first, if we are evicting the leaf here, eviction has the
         * ref locked, and the parent will wait for it; and if we are checkpointing the leaf, we
         * can't simultaneously be checkpointing the parent, and we can't be evicting the parent
         * either because internal pages can't be evicted while they have in-memory children.
         */
        if (mod->instantiated) {
            /*
             * Unfortunately, it seems we need to lock the ref at this point. Ultimately the
             * page_del structure and the instantiated flag need to both be cleared simultaneously
             * (otherwise instantiated == false and page_del not NULL violates the intended
             * invariant and other code can assert) and there are several other places that can
             * still be interacting with the page_del structure at this point (even though the page
             * has been instantiated) and we need to wait for those to finish before discarding it.
             *
     ...
     */
    static int __rec_write_err(WT_SESSION_IMPL * session, WTI_RECONCILE * r, WT_PAGE * page)
    {
        WT_DECL_RET;
        WT_MULTI *multi;
        uint32_t i;
     ...
     */
    static int __rec_hs_wrapup(WT_SESSION_IMPL * session, WTI_RECONCILE * r)
    {
        WT_BTREE *btree;
        WT_DECL_RET;
        WT_MULTI *multi;
        ...
        /*
         * Sanity check: Can't insert updates into history store from the history store itself or
         * from the metadata file.
         */
        WT_ASSERT_ALWAYS(session, !WT_IS_HS(btree->dhandle) && !WT_IS_METADATA(btree->dhandle),
          "Attempting to write updates from the history store or metadata file into the history "
          "store");

        /*
         * Delete the updates left in the history store by prepared rollback first before moving
...
err:
     ...
     */
    int __wti_rec_cell_build_ovfl(WT_SESSION_IMPL * session, WTI_RECONCILE * r, WTI_REC_KV * kv,
      uint8_t type, WT_TIME_WINDOW * tw, uint64_t rle)
    {
        WT_BM *bm;
        WT_BTREE *btree;
        WT_DECL_ITEM(tmp);
        ...
        if (addr == NULL) {
            ...
            /*
             * Track the overflow record (unless it's a bulk load, which by definition won't ever
             * reuse a record.
             */
            if (!r->is_bulk_load)
...
err:
     ...
     *     When removing a key due to a tombstone with a durable timestamp of "none", also remove
     *     the history store contents associated with that key.
     */
    int __wti_rec_hs_clear_on_tombstone(
      WT_SESSION_IMPL * session, WTI_RECONCILE * r, uint64_t recno, WT_ITEM * rowkey, bool reinsert)
    {
        ...
        if (r->hs_cursor == NULL)
            ...
            WT_RET_ERROR_OK(
              ret = __wt_curhs_set_btree_id(session, r->hs_cursor, btree->id), EINVAL);
            if (ret == EINVAL) {
                WT_RET(r->hs_cursor->close(r->hs_cursor));
                r->hs_cursor = NULL;
                WT_RET(__wt_curhs_open(session, btree->id, NULL, &r->hs_cursor));
         ...
         * WT_REC_CHECKPOINT_RUNNING asks the function to fail with EBUSY if we are trying to evict
         * an mixed-mode update while a checkpoint is in progress; such eviction can race with the
         * checkpoint itself and lead to history store inconsistency. (Note:
         * WT_REC_CHECKPOINT_RUNNING is set only during evictions, and never in the checkpoint
         * thread itself.)
         */
