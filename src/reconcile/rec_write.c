  ...
  WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_PAGE *page, uint32_t flags, bool *page_lockedp)
{
    ...
    *page_lockedp = false;
    ...
    if (r->cache_write_restore_uncommitted || F_ISSET(r, WT_REC_SCRUB))
  ...
  bool *page_lockedp)
{
    ...
    if (ret == 0 && !(btree->evict_disabled > 0 || !F_ISSET(btree->dhandle, WT_DHANDLE_OPEN)) &&
      ...
      F_ISSET(r, WT_REC_CALL_URGENT) && !r->update_used && r->cache_write_restore_uncommitted) {
        /*
  ...
  void *reconcilep)
{
    ...
    r->cache_write_hs = r->cache_write_restore_uncommitted = false;
...
__rec_split_row_promote(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_ITEM *key, uint8_t type)
{
     ...
     */
    max = r->last;
    if (r->cache_write_restore_uncommitted)
