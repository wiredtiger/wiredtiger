...
split:
        for (multi = r->multi, i = 0; i < r->multi_next; ++multi, ++i) {
            ...
            mod->mod_multi_oldest_start_ts =
              WT_MAX(mod->mod_multi_oldest_start_ts, multi->addr.oldest_start_ts);
            mod->mod_multi_oldest_start_txn =
              WT_MAX(mod->mod_multi_oldest_start_txn, multi->addr.oldest_start_txn);
            ...
            mod->mod_multi_newest_durable_ts =
              WT_MAX(mod->mod_multi_newest_durable_ts, multi->addr.newest_durable_ts);
        }
