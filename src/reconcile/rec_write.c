  ...
  WT_SESSION_IMPL *session, WT_RECONCILE *r, int tret, bool *lookaside_retryp)
{
    ...
    if (tret == EBUSY && lookaside_retryp != NULL && !F_ISSET(r, WT_REC_UPDATE_RESTORE) &&
      !r->update_uncommitted && !r->update_inprogress)
     ...
     */
    if (r->update_uncommitted || r->update_inprogress || r->update_used)
        return (0);
  ...
  void *reconcilep)
{
    ...
    r->update_uncommitted = r->update_inprogress = r->update_used = false;
