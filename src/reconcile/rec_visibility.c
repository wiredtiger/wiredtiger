    ...
    for (upd = first_upd; upd != NULL; upd = upd->next) {
        ...
        *first_txn_updp = upd;
        ...
        if (!F_ISSET(upd, WT_UPDATE_SELECT_FOR_DS) && !is_hs_page &&
          (F_ISSET(r, WT_REC_VISIBLE_NO_SNAPSHOT) ?
              (r->rec_has_ckpt_snapshot ?
                  !__wt_txn_visible_id_snapshot(txnid, r->rec_ckpt_snap_min, r->rec_ckpt_snap_max,
                    r->rec_ckpt_snapshot_arr, r->rec_ckpt_snapshot_count) :
                  r->rec_start_pinned_id <= txnid) :
  ...
  bool *has_newer_updatesp, size_t *upd_memsizep)
{
    ...
    for (upd = first_upd; upd != NULL; upd = upd->next) {
        ...
        if (!F_ISSET(upd, WT_UPDATE_SELECT_FOR_DS) &&
          (F_ISSET(r, WT_REC_VISIBLE_NO_SNAPSHOT) ?
              (r->rec_has_ckpt_snapshot ?
                  !__wt_txn_visible_id_snapshot(upd->txnid, r->rec_ckpt_snap_min,
                    r->rec_ckpt_snap_max, r->rec_ckpt_snapshot_arr, r->rec_ckpt_snapshot_count) :
                  r->rec_start_pinned_id <= upd->txnid) :
