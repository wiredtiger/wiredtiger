...
__rec_col_merge(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_PAGE *page)
{
    ...
    for (multi = mod->mod_multi, i = 0; i < mod->mod_multi_entries; ++multi, ++i) {
        ...
        /* Copy the value onto the page. */
        __wt_rec_image_copy(session, r, val);
        WT_REC_CHUNK_AGGEGRATES_MERGE(session, r->cur_ptr, &addr->ta);
    }
    return (0);
}
...
__wti_rec_col_int(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_REF *pageref)
{
    ...
    WT_INTL_FOREACH_BEGIN (session, page, ref) {
        ...
        __wt_rec_image_copy(session, r, val);
        if (page_del != NULL)
            WT_REC_CHUNK_AGGEGRATES_MERGE(session, r->cur_ptr, &ft_ta);
        WT_REC_CHUNK_AGGEGRATES_MERGE(session, r->cur_ptr, &ta);
    }
    WT_INTL_FOREACH_END;
    ...
    for (ins = WT_SKIP_FIRST(WT_COL_APPEND(page));; ins = WT_SKIP_NEXT(ins)) {
        ...
        for (;;) {
            ...
            if (r->aux_entries < r->entries) {
                WT_TIME_WINDOW_INIT(&unpack.tw);
                WT_REC_CHUNK_AGGEGRATES_UPDATE(session, r->cur_ptr, &unpack.tw);
            }
