...
__wt_bulk_insert_var(WT_SESSION_IMPL *session, WT_CURSOR_BULK *cbulk, bool deleted)
{
    ...
    val = &r->v;
    if (deleted) {
        val->cell_len = __wt_cell_pack_del(session, &r->cell_stats, &val->cell, &tw, cbulk->rle);
        val->buf.data = NULL;
        val->buf.size = 0;
  ...
  WT_ITEM *value, WT_TIME_WINDOW *tw, uint64_t rle, bool deleted, bool overflow_type)
{
    ...
    if (deleted) {
        val->cell_len = __wt_cell_pack_del(session, &r->cell_stats, &val->cell, tw, rle);
        val->buf.data = NULL;
        val->buf.size = 0;
        val->len = val->cell_len;
    } else if (overflow_type) {
        val->cell_len = __wt_cell_pack_ovfl(
          session, &r->cell_stats, &val->cell, WT_CELL_VALUE_OVFL, tw, rle, value->size);
