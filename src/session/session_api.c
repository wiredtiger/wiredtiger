  ...
  const char *config, WT_CURSOR **cursorp)
{
     ...
     */
    if (!F_ISSET_ATOMIC_64(S2C(session), WT_CONN_READY) && !F_ISSET(session, WT_SESSION_INTERNAL) &&
      (!F_ISSET_ATOMIC_64(S2C(session), WT_CONN_MINIMAL) || strcmp(uri, "statistics:") != 0))
        WT_ERR_MSG(
  ...
  WT_SESSION_IMPL **sessionp)
{
    ...
    *sessionp = NULL;
     ...
     */
    WT_ASSERT(session, !F_ISSET_ATOMIC_64(conn, WT_CONN_CLOSING));
    ...
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_MINIMAL) && !F_ISSET(session, WT_SESSION_INTERNAL))
        session_ret->iface = stds_min;
    else
        session_ret->iface = F_ISSET_ATOMIC_64(conn, WT_CONN_READONLY) ? stds_readonly : stds;
    ...
    if (F_ISSET_ATOMIC_64(conn, WT_CONN_CACHE_CURSORS))
