...
__session_discard_dhandle(WT_SESSION_IMPL *session, WT_DATA_HANDLE_CACHE *dhandle_cache)
{
    ...
    DIAGNOSTIC_EXTRA_PRINTF(
      "In __session_discard_dhandle(), session %p, released dhandle %p, name '%s', flags = 0x%x, "
      "session_ref %u, session_inuse %d, \n",
      (void *)session, (void *)dhandle_cache->dhandle, dhandle_cache->dhandle->name,
      dhandle_cache->dhandle->flags, dhandle_cache->dhandle->session_ref,
      dhandle_cache->dhandle->session_inuse);
...
__wt_session_release_dhandle(WT_SESSION_IMPL *session)
{
    ...
    DIAGNOSTIC_EXTRA_PRINTF(
      "starting __wt_session_release_dhandle(): session %p, dhandle %p, name '%s', btree %p\n",
    ...
    if (btree != NULL && F_ISSET(btree, WT_BTREE_BULK)) {
      ...
      F_ISSET(dhandle, WT_DHANDLE_DISCARD | WT_DHANDLE_DISCARD_KILL)) {
        ...
        ret =
          __wt_conn_dhandle_close(session, false, F_ISSET(dhandle, WT_DHANDLE_DISCARD_KILL), true);
...
__session_find_shared_dhandle(WT_SESSION_IMPL *session, const char *uri, const char *checkpoint)
{
    ...
    WT_WITH_HANDLE_LIST_READ_LOCK(
      session, if ((ret = __wt_conn_dhandle_find(session, uri, checkpoint)) == 0) {
          WT_DHANDLE_ACQUIRE(session->dhandle);
          DIAGNOSTIC_EXTRA_PRINTF(
            "In __session_find_shared_dhandle() - (A), acquired dhandle %p, name '%s', flags = "
            "0x%x, session_inuse %d, session_ref %u\n",
    ...
    WT_WITH_HANDLE_LIST_WRITE_LOCK(
      session, if ((ret = __wt_conn_dhandle_alloc(session, uri, checkpoint)) == 0) {
          WT_DHANDLE_ACQUIRE(session->dhandle);
          DIAGNOSTIC_EXTRA_PRINTF(
            "In __session_find_shared_dhandle() - (B), acquired dhandle %p, name '%s', flags = "
            "0x%x, session_inuse %d, session_ref %u\n",
            (void *)session->dhandle, session->dhandle->name, session->dhandle->flags,
            session->dhandle->session_inuse, session->dhandle->session_ref);
      });
...
__session_get_dhandle(WT_SESSION_IMPL *session, const char *uri, const char *checkpoint)
{
    ...
    if ((ret = __session_add_dhandle(session)) != 0) {
        ...
        printf(
          "In __session_get_dhandle(), released dhandle %p, name '%s', flags = 0x%x, session_inuse "
          "%d, session_ref %u\n",
          (void *)session->dhandle, session->dhandle->name, session->dhandle->flags,
          session->dhandle->session_inuse, session->dhandle->session_ref);
        session->dhandle = NULL;
