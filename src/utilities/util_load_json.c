...
json_top_level(WT_SESSION *session, JSON_INPUT_STATE *ins, uint32_t flags)
{
	...
	while (json_peek(session, ins) == 's') {
		...
		if ((ret = __wt_snprintf(tableuri, ins->toklen, "%.*s", (int)(ins->toklen - 2),
		         ins->tokstart + 1)) != 0) {
			ret = util_err(session, ret, NULL);
			goto err;
		}
		...
		if (!hasversion) {
			if (strcmp(tableuri, DUMP_JSON_VERSION_MARKER) != 0) {
				ret = util_err(
				    session, ENOTSUP, "missing \"%s\"", DUMP_JSON_VERSION_MARKER);
				goto err;
			}
			hasversion = true;
			JSON_EXPECT(session, ins, 's');
			if ((ret = util_str2num(session, ins->tokstart + 1, false, &curversion)) !=
			    0)
				goto err;
...
json_peek(WT_SESSION *session, JSON_INPUT_STATE *ins)
{
	...
	if (!ins->peeking) {
		...
		if (ins->ateof)
			ins->toktype = 0;
		else if (__wt_json_token(
		             session, ins->p, &ins->toktype, &ins->tokstart, &ins->toklen) != 0)
			ins->toktype = -1;
		ins->peeking = true;
	}
	if (0) {
