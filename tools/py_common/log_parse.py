...
class LogParser():
    ...
    def process_log_file(self, f, opts):
        '''
        Extract the byte dump from mongo or wiredtiger logs.
        '''
        first_line = f.readline()
        f.seek(0)
        if self.is_mongo_log(first_line):
            return self.process_mongod_log(f, opts)
        else:
    ...
    def extract_mongodb_log_hex(self, f, opts):
        """
        ...
        for line_num, line in enumerate(lines):
            try:
                ...
                f.seek(0)
                return self.encode_bytes(f, opts)
