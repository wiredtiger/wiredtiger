...
from wt_decode.disagg.decoding import make_decode_opts, get_page_type_name
from wt_decode.output.text import print_page as _print_page
from wt_decode.ui.rich_page import rich_print_page, build_page_history_choices
...
class DisaggBrowser:
    ...
    def get_metadata_root(self, lsn: int) -> Optional[Dict[str, int]]:
        ...
        try:
            ...
            addr = btree.DisaggAddr.parse(bytes.fromhex(addr_hex))
    ...
    def fetch_and_decode(self, table_id: int, page_id: int, lsn: int) -> Optional[btree.WTPage]:
    ...
    def traverse_tree(self, table_id: int, root_page_id: int, root_lsn: int):
        ...
        while True:
            ...
            if self.use_rich:
                ...
                decode_opts = make_decode_opts()
                _print_page(page, split=getattr(decode_opts, 'split', False),
