...
from wt_decode.core.binary import unpack_int, d_and_h, BinaryFile
from wt_decode.disagg.decoding import get_page_type_name
...
def _format_cell_data(data: bytes, is_value: bool = False) -> str:
    ...
    if data[0] >= 0x7f:
        try:
            ...
            remaining = data
            while remaining:
                val, next_remaining = unpack_int(remaining)
...
def rich_print_page(table_id: int, page_id: int, lsn: int, log_id: int, page: btree.WTPage):
    ...
    if page.block_header:
        ...
        if isinstance(page.block_header, btree.BlockDisaggHeader):
    ...
    if page.cells:
        ...
        for i, cell in enumerate(page.cells):
            ...
            if cell.run_length is not None:
                details_parts.append(f"runlen/addr: {d_and_h(cell.run_length)}")
