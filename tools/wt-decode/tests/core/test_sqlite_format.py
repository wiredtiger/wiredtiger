...
from wt_decode.core import sqlite_reader
...
class TestSqliteFormat(unittest.TestCase):
    ...
    def test_is_sqlite3_file(self):
        self.assertTrue(sqlite_reader.is_sqlite3_file(self.db_path))
        ...
        non_sqlite.close()
        self.assertFalse(sqlite_reader.is_sqlite3_file(non_sqlite.name))

    def test_page_id_latest_lsn(self):
        with self.assertLogs('wt_decode.core.sqlite_reader', level=logging.WARNING) as logs:
            pages = sqlite_reader.load_disagg_pages(self.db_path, page_id=10)
    ...
    def test_discarded_lsn_query_returns_none(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            row = sqlite_reader._load_row_for_lsn(conn, 40)

        self.assertIsNone(row)

    def test_page_id_returns_base_page_chain(self):
        pages = sqlite_reader.load_disagg_pages(self.db_path, page_id=12)
    ...
    def test_lsn_and_page_id_mismatch(self):
        with self.assertRaises(ValueError):
            sqlite_reader.load_disagg_pages(self.db_path, page_id=11, lsn=7)
